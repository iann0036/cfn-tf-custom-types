...
TYPE_NAME = "TF::Limelight::Edgefunction"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/limelight_edgefunction/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'llnw/limelight',
                'providerTypeName': 'limelight',
                'terraformTypeName': 'limelight_edgefunction',
                'returnValues': ["Id", "RevisionId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/limelight_edgefunction/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/limelight_edgefunction/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CanDebug=None, Description=None, FunctionArchive=None, FunctionSha256=None, Handler=None, Id=None, Memory=None, Name=None, ReservedConcurrency=None, RevisionId=None, Runtime=None, Shortname=None, Timeout=None, EnvironmentVariable=None)
