...
TYPE_NAME = "TF::Alicloud::EcsDedicatedHost"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ecs_dedicated_host/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_ecs_dedicated_host',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ecs_dedicated_host/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_ecs_dedicated_host/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActionOnMaintenance=None, AutoPlacement=None, AutoReleaseTime=None, AutoRenew=None, AutoRenewPeriod=None, CpuOverCommitRatio=None, DedicatedHostClusterId=None, DedicatedHostName=None, DedicatedHostType=None, Description=None, DetailFee=None, DryRun=None, ExpiredTime=None, Id=None, MinQuantity=None, PaymentType=None, ResourceGroupId=None, SaleCycle=None, Status=None, Tags=None, ZoneId=None, NetworkAttributes=None, Timeouts=None)
