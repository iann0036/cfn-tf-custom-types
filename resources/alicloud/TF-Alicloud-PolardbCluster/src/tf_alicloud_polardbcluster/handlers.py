...
TYPE_NAME = "TF::Alicloud::PolardbCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_polardb_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_polardb_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_polardb_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenewPeriod=None, CollectorStatus=None, ConnectionString=None, DbNodeClass=None, DbNodeCount=None, DbType=None, DbVersion=None, Description=None, Id=None, MaintainTime=None, ModifyType=None, PayType=None, Period=None, RenewalStatus=None, ResourceGroupId=None, SecurityIps=None, Tags=None, TdeStatus=None, VswitchId=None, ZoneId=None, Parameters=None, Timeouts=None)
