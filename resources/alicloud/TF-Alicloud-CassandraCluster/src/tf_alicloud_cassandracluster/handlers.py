...
TYPE_NAME = "TF::Alicloud::CassandraCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cassandra_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_cassandra_cluster',
                'returnValues': ["Id", "PublicPoints", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cassandra_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cassandra_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenew=None, AutoRenewPeriod=None, ClusterName=None, DataCenterName=None, DiskSize=None, DiskType=None, EnablePublic=None, Id=None, InstanceType=None, IpWhite=None, MaintainEndTime=None, MaintainStartTime=None, MajorVersion=None, NodeCount=None, Password=None, PayType=None, Period=None, PeriodUnit=None, PublicPoints=None, SecurityGroups=None, Status=None, Tags=None, VswitchId=None, ZoneId=None, Timeouts=None)
