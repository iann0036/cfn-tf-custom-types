...
TYPE_NAME = "TF::Alicloud::NatGateway"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_nat_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["BandwidthPackageIds", "ForwardTableIds", "Id", "SnatTableIds", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_nat_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_nat_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BandwidthPackageIds=None, Description=None, DryRun=None, Force=None, ForwardTableIds=None, Id=None, InstanceChargeType=None, InternetChargeType=None, Name=None, NatGatewayName=None, NatType=None, PaymentType=None, Period=None, SnatTableIds=None, Spec=None, Specification=None, Status=None, Tags=None, VpcId=None, VswitchId=None, BandwidthPackages=None, Timeouts=None)
