...
TYPE_NAME = "TF::Alicloud::GaEndpointGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ga_endpoint_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_ga_endpoint_group',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ga_endpoint_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_ga_endpoint_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcceleratorId=None, Description=None, EndpointGroupRegion=None, EndpointGroupType=None, EndpointRequestProtocol=None, HealthCheckIntervalSeconds=None, HealthCheckPath=None, HealthCheckPort=None, HealthCheckProtocol=None, Id=None, ListenerId=None, Name=None, Status=None, ThresholdCount=None, TrafficPercentage=None, EndpointConfigurations=None, PortOverrides=None, Timeouts=None)
