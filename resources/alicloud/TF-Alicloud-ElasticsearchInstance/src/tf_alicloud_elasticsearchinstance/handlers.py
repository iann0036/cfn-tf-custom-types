...
TYPE_NAME = "TF::Alicloud::ElasticsearchInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_elasticsearch_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_elasticsearch_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_elasticsearch_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClientNodeAmount=None, ClientNodeSpec=None, DataNodeAmount=None, DataNodeDiskEncrypted=None, DataNodeDiskSize=None, DataNodeDiskType=None, DataNodeSpec=None, Description=None, Domain=None, EnableKibanaPrivateNetwork=None, EnableKibanaPublicNetwork=None, EnablePublic=None, Id=None, InstanceChargeType=None, KibanaDomain=None, KibanaPort=None, KibanaPrivateWhitelist=None, KibanaWhitelist=None, KmsEncryptedPassword=None, KmsEncryptionContext=None, MasterNodeSpec=None, Password=None, Period=None, Port=None, PrivateWhitelist=None, Protocol=None, PublicWhitelist=None, ResourceGroupId=None, Status=None, Tags=None, Version=None, VswitchId=None, ZoneCount=None, Timeouts=None)
