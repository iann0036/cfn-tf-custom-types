...
TYPE_NAME = "TF::Alicloud::EdasK8sApplication"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_edas_k8s_application/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_edas_k8s_application',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_edas_k8s_application/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_edas_k8s_application/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplicationDescriotion=None, ApplicationName=None, ClusterId=None, Command=None, CommandArgs=None, EdasContainerVersion=None, Envs=None, Id=None, ImageUrl=None, InternetSlbId=None, InternetSlbPort=None, InternetSlbProtocol=None, InternetTargetPort=None, Jdk=None, LimitMCpu=None, LimitMem=None, Liveness=None, LocalVolume=None, LogicalRegionId=None, MountDescs=None, Namespace=None, NasId=None, PackageType=None, PackageUrl=None, PackageVersion=None, PostStart=None, PreStop=None, Readiness=None, Replicas=None, RequestsMCpu=None, RequestsMem=None, WebContainer=None, Timeouts=None)
