...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_kms_key/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CreationDate", "Creator", "DeleteDate", "Id", "LastRotationDate", "MaterialExpireTime", "NextRotationDate", "PrimaryKeyVersion"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_kms_key/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_kms_key/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AutomaticRotation=None, CreationDate=None, Creator=None, DeleteDate=None, DeletionWindowInDays=None, Description=None, Id=None, IsEnabled=None, KeySpec=None, KeyState=None, KeyUsage=None, LastRotationDate=None, MaterialExpireTime=None, NextRotationDate=None, Origin=None, PendingWindowInDays=None, PrimaryKeyVersion=None, ProtectionLevel=None, RotationInterval=None, Status=None)
