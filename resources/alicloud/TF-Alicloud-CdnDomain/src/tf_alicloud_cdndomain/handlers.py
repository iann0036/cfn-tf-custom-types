...
TYPE_NAME = "TF::Alicloud::CdnDomain"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cdn_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cdn_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cdn_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BlockIps=None, CdnType=None, DomainName=None, Id=None, OptimizeEnable=None, PageCompressEnable=None, RangeEnable=None, Scope=None, SourcePort=None, SourceType=None, Sources=None, VideoSeekEnable=None, AuthConfig=None, CacheConfig=None, CertificateConfig=None, HttpHeaderConfig=None, Page404Config=None, ParameterFilterConfig=None, ReferConfig=None)
