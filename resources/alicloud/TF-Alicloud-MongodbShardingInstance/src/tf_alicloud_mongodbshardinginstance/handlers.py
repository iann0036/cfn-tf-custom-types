...
TYPE_NAME = "TF::Alicloud::MongodbShardingInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_mongodb_sharding_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_mongodb_sharding_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_mongodb_sharding_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountPassword=None, BackupPeriod=None, BackupTime=None, EngineVersion=None, Id=None, InstanceChargeType=None, KmsEncryptedPassword=None, KmsEncryptionContext=None, Name=None, Period=None, RetentionPeriod=None, SecurityGroupId=None, SecurityIpList=None, StorageEngine=None, Tags=None, TdeStatus=None, VswitchId=None, ZoneId=None, MongoList=None, ShardList=None)
