...
TYPE_NAME = "TF::Alicloud::RosChangeSet"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ros_change_set/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_ros_change_set',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ros_change_set/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_ros_change_set/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ChangeSetName=None, ChangeSetType=None, Description=None, DisableRollback=None, Id=None, NotificationUrls=None, RamRoleName=None, ReplacementOption=None, StackId=None, StackName=None, StackPolicyBody=None, StackPolicyDuringUpdateBody=None, StackPolicyDuringUpdateUrl=None, StackPolicyUrl=None, Status=None, TemplateBody=None, TemplateUrl=None, TimeoutInMinutes=None, UsePreviousParameters=None, Parameters=None, Timeouts=None)
