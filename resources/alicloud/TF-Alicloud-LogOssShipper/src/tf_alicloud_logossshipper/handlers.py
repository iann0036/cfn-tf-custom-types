...
TYPE_NAME = "TF::Alicloud::LogOssShipper"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_log_oss_shipper/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_log_oss_shipper',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_log_oss_shipper/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_log_oss_shipper/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BufferInterval=None, BufferSize=None, CompressType=None, CsvConfigColumns=None, CsvConfigDelimiter=None, CsvConfigHeader=None, CsvConfigLinefeed=None, CsvConfigNullidentifier=None, CsvConfigQuote=None, Format=None, Id=None, JsonEnableTag=None, LogstoreName=None, OssBucket=None, OssPrefix=None, PathFormat=None, ProjectName=None, RoleArn=None, ShipperName=None, ParquetConfig=None, Timeouts=None)
