...
TYPE_NAME = "TF::Alicloud::CassandraDataCenter"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cassandra_data_center/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_cassandra_data_center',
                'returnValues': ["DataCenterId", "Id", "PublicPoints", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cassandra_data_center/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cassandra_data_center/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenew=None, AutoRenewPeriod=None, ClusterId=None, DataCenterId=None, DataCenterName=None, DiskSize=None, DiskType=None, EnablePublic=None, Id=None, InstanceType=None, NodeCount=None, PayType=None, Period=None, PeriodUnit=None, PublicPoints=None, Status=None, VswitchId=None, ZoneId=None, Timeouts=None)
