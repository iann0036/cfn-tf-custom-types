...
TYPE_NAME = "TF::Alicloud::PrivatelinkVpcEndpoint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_privatelink_vpc_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_privatelink_vpc_endpoint',
                'returnValues': ["Bandwidth", "ConnectionStatus", "EndpointBusinessStatus", "EndpointDomain", "Id", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_privatelink_vpc_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_privatelink_vpc_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Bandwidth=None, ConnectionStatus=None, DryRun=None, EndpointBusinessStatus=None, EndpointDescription=None, EndpointDomain=None, Id=None, SecurityGroupIds=None, ServiceId=None, ServiceName=None, Status=None, VpcEndpointName=None, VpcId=None, Timeouts=None)
