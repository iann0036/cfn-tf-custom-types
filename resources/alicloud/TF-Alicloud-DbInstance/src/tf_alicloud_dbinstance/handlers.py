...
TYPE_NAME = "TF::Alicloud::DbInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_db_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ConnectionString", "Id", "Port", "SslStatus"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_db_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_db_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Acl=None, AutoRenew=None, AutoRenewPeriod=None, AutoUpgradeMinorVersion=None, CaType=None, ClientCaCert=None, ClientCaEnabled=None, ClientCertRevocationList=None, ClientCrlEnabled=None, ConnectionString=None, DbInstanceStorageType=None, EncryptionKey=None, Engine=None, EngineVersion=None, ForceRestart=None, Id=None, InstanceChargeType=None, InstanceName=None, InstanceStorage=None, InstanceType=None, MaintainTime=None, MonitoringPeriod=None, Period=None, Port=None, ReplicationAcl=None, ResourceGroupId=None, SecurityGroupId=None, SecurityGroupIds=None, SecurityIpMode=None, SecurityIps=None, ServerCert=None, ServerKey=None, SqlCollectorConfigValue=None, SqlCollectorStatus=None, SslAction=None, SslStatus=None, Tags=None, TdeStatus=None, VswitchId=None, ZoneId=None, ZoneIdSlaveA=None, ZoneIdSlaveB=None, Parameters=None, Timeouts=None)
