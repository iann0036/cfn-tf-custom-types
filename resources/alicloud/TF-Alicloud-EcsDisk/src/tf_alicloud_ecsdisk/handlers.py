...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ecs_disk/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_ecs_disk',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ecs_disk/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_ecs_disk/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdvancedFeatures=None, AvailabilityZone=None, Category=None, DedicatedBlockStorageClusterId=None, DeleteAutoSnapshot=None, DeleteWithInstance=None, Description=None, DiskName=None, DryRun=None, EnableAutoSnapshot=None, EncryptAlgorithm=None, Encrypted=None, Id=None, InstanceId=None, KmsKeyId=None, Name=None, PaymentType=None, PerformanceLevel=None, ResourceGroupId=None, Size=None, SnapshotId=None, Status=None, StorageSetId=None, StorageSetPartitionNumber=None, Tags=None, Type=None, ZoneId=None, Timeouts=None)
