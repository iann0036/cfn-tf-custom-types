...
TYPE_NAME = "TF::Alicloud::EssScalingConfiguration"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ess_scaling_configuration/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_ess_scaling_configuration/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_ess_scaling_configuration/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Active=None, CreditSpecification=None, Enable=None, ForceDelete=None, Id=None, ImageId=None, ImageName=None, InstanceIds=None, InstanceName=None, InstanceType=None, InstanceTypes=None, InternetChargeType=None, InternetMaxBandwidthIn=None, InternetMaxBandwidthOut=None, IoOptimized=None, IsOutdated=None, KeyName=None, KmsEncryptedPassword=None, KmsEncryptionContext=None, Override=None, Password=None, PasswordInherit=None, RoleName=None, ScalingConfigurationName=None, ScalingGroupId=None, SecurityGroupId=None, SecurityGroupIds=None, Substitute=None, SystemDiskAutoSnapshotPolicyId=None, SystemDiskCategory=None, SystemDiskDescription=None, SystemDiskName=None, SystemDiskSize=None, Tags=None, UserData=None, DataDisk=None)
