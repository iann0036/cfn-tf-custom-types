...
TYPE_NAME = "TF::Alicloud::CenRouteMap"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cen_route_map/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_cen_route_map',
                'returnValues': ["Id", "RouteMapId", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cen_route_map/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cen_route_map/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AsPathMatchMode=None, CenId=None, CenRegionId=None, CidrMatchMode=None, CommunityMatchMode=None, CommunityOperateMode=None, Description=None, DestinationChildInstanceTypes=None, DestinationCidrBlocks=None, DestinationInstanceIds=None, DestinationInstanceIdsReverseMatch=None, DestinationRouteTableIds=None, Id=None, MapResult=None, MatchAsns=None, MatchCommunitySet=None, NextPriority=None, OperateCommunitySet=None, Preference=None, PrependAsPath=None, Priority=None, RouteMapId=None, RouteTypes=None, SourceChildInstanceTypes=None, SourceInstanceIds=None, SourceInstanceIdsReverseMatch=None, SourceRegionIds=None, SourceRouteTableIds=None, Status=None, TransmitDirection=None, Timeouts=None)
