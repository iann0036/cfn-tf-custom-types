...
TYPE_NAME = "TF::Alicloud::OosExecution"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_oos_execution/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_oos_execution',
                'returnValues': ["Counters", "CreateDate", "EndDate", "ExecutedBy", "Id", "IsParent", "Outputs", "RamRole", "StartDate", "Status", "StatusMessage", "TemplateId", "UpdateDate"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_oos_execution/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_oos_execution/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Counters=None, CreateDate=None, Description=None, EndDate=None, ExecutedBy=None, Id=None, IsParent=None, LoopMode=None, Mode=None, Outputs=None, Parameters=None, ParentExecutionId=None, RamRole=None, SafetyCheck=None, StartDate=None, Status=None, StatusMessage=None, TemplateContent=None, TemplateId=None, TemplateName=None, TemplateVersion=None, UpdateDate=None, Timeouts=None)
