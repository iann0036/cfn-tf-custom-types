# DO NOT modify this file by hand, changes will be overwritten
import sys
from dataclasses import dataclass
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

from cloudformation_cli_python_lib.interface import (
    BaseModel,
    BaseResourceHandlerRequest,
)
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class ResourceHandlerRequest(BaseResourceHandlerRequest):
    # pylint: disable=invalid-name
    desiredResourceState: Optional["ResourceModel"]
    previousResourceState: Optional["ResourceModel"]


@dataclass
class ResourceModel(BaseModel):
    tfcfnid: Optional[str]
    Counters: Optional[str]
    CreateDate: Optional[str]
    Description: Optional[str]
    EndDate: Optional[str]
    ExecutedBy: Optional[str]
    Id: Optional[str]
    IsParent: Optional[bool]
    LoopMode: Optional[str]
    Mode: Optional[str]
    Outputs: Optional[str]
    Parameters: Optional[str]
    ParentExecutionId: Optional[str]
    RamRole: Optional[str]
    SafetyCheck: Optional[str]
    StartDate: Optional[str]
    Status: Optional[str]
    StatusMessage: Optional[str]
    TemplateContent: Optional[str]
    TemplateId: Optional[str]
    TemplateName: Optional[str]
    TemplateVersion: Optional[str]
    UpdateDate: Optional[str]
    Timeouts: Optional["_TimeoutsDefinition"]

    @classmethod
    def _deserialize(
        cls: Type["_ResourceModel"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_ResourceModel"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            tfcfnid=json_data.get("tfcfnid"),
            Counters=json_data.get("Counters"),
            CreateDate=json_data.get("CreateDate"),
            Description=json_data.get("Description"),
            EndDate=json_data.get("EndDate"),
            ExecutedBy=json_data.get("ExecutedBy"),
            Id=json_data.get("Id"),
            IsParent=json_data.get("IsParent"),
            LoopMode=json_data.get("LoopMode"),
            Mode=json_data.get("Mode"),
            Outputs=json_data.get("Outputs"),
            Parameters=json_data.get("Parameters"),
            ParentExecutionId=json_data.get("ParentExecutionId"),
            RamRole=json_data.get("RamRole"),
            SafetyCheck=json_data.get("SafetyCheck"),
            StartDate=json_data.get("StartDate"),
            Status=json_data.get("Status"),
            StatusMessage=json_data.get("StatusMessage"),
            TemplateContent=json_data.get("TemplateContent"),
            TemplateId=json_data.get("TemplateId"),
            TemplateName=json_data.get("TemplateName"),
            TemplateVersion=json_data.get("TemplateVersion"),
            UpdateDate=json_data.get("UpdateDate"),
            Timeouts=TimeoutsDefinition._deserialize(json_data.get("Timeouts")),
        )


# work around possible type aliasing issues when variable has same name as a model
_ResourceModel = ResourceModel


@dataclass
class TimeoutsDefinition(BaseModel):
    Create: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_TimeoutsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_TimeoutsDefinition"]:
        if not json_data:
            return None
        return cls(
            Create=json_data.get("Create"),
        )


# work around possible type aliasing issues when variable has same name as a model
_TimeoutsDefinition = TimeoutsDefinition


