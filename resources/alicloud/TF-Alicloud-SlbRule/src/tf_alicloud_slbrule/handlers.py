...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_slb_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_slb_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_slb_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Cookie=None, CookieTimeout=None, DeleteProtectionValidation=None, Domain=None, FrontendPort=None, HealthCheck=None, HealthCheckConnectPort=None, HealthCheckDomain=None, HealthCheckHttpCode=None, HealthCheckInterval=None, HealthCheckTimeout=None, HealthCheckUri=None, HealthyThreshold=None, Id=None, ListenerSync=None, LoadBalancerId=None, Name=None, Scheduler=None, ServerGroupId=None, StickySession=None, StickySessionType=None, UnhealthyThreshold=None, Url=None)
