...
TYPE_NAME = "TF::Alicloud::HbaseInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_hbase_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Id", "MasterInstanceQuantity", "SlbConnAddrs", "UiProxyConnAddrs", "ZkConnAddrs"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_hbase_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_hbase_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Account=None, AutoRenew=None, ColdStorageSize=None, CoreDiskSize=None, CoreDiskType=None, CoreInstanceQuantity=None, CoreInstanceType=None, DeletionProtection=None, Duration=None, Engine=None, EngineVersion=None, Id=None, ImmediateDeleteFlag=None, IpWhite=None, MaintainEndTime=None, MaintainStartTime=None, MasterInstanceQuantity=None, MasterInstanceType=None, Name=None, Password=None, PayType=None, SecurityGroups=None, SlbConnAddrs=None, Tags=None, UiProxyConnAddrs=None, VswitchId=None, ZkConnAddrs=None, ZoneId=None, Timeouts=None)
