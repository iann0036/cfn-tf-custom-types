...
TYPE_NAME = "TF::Alicloud::AdbDbCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_adb_db_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_adb_db_cluster',
                'returnValues': ["ConnectionString", "Id", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_adb_db_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_adb_db_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenewPeriod=None, ComputeResource=None, ConnectionString=None, DbClusterCategory=None, DbClusterClass=None, DbClusterVersion=None, DbNodeClass=None, DbNodeCount=None, DbNodeStorage=None, Description=None, ElasticIoResource=None, Id=None, MaintainTime=None, Mode=None, ModifyType=None, PayType=None, PaymentType=None, Period=None, RenewalStatus=None, ResourceGroupId=None, SecurityIps=None, Status=None, Tags=None, VswitchId=None, ZoneId=None, Timeouts=None)
