...
TYPE_NAME = "TF::Alicloud::ContainerCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_container_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_container_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_container_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AgentVersion=None, CidrBlock=None, DiskCategory=None, DiskSize=None, Id=None, ImageId=None, InstanceType=None, IsOutdated=None, Name=None, NamePrefix=None, NeedSlb=None, NodeNumber=None, Nodes=None, Password=None, ReleaseEip=None, SecurityGroupId=None, Size=None, SlbId=None, VpcId=None, VswitchId=None)
