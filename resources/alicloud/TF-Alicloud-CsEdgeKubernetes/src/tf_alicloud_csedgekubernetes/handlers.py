...
TYPE_NAME = "TF::Alicloud::CsEdgeKubernetes"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cs_edge_kubernetes/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_cs_edge_kubernetes',
                'returnValues': ["CertificateAuthority", "Connections", "Id", "NatGatewayId", "SlbInternet", "SlbIntranet", "VpcId", "WorkerNodes"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cs_edge_kubernetes/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cs_edge_kubernetes/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZone=None, CertificateAuthority=None, ClientCert=None, ClientKey=None, ClusterCaCert=None, Connections=None, DeletionProtection=None, ForceUpdate=None, Id=None, InstallCloudMonitor=None, IsEnterpriseSecurityGroup=None, KeyName=None, KubeConfig=None, Name=None, NamePrefix=None, NatGatewayId=None, NewNatGateway=None, NodeCidrMask=None, Password=None, PodCidr=None, ProxyMode=None, RdsInstances=None, ResourceGroupId=None, SecurityGroupId=None, ServiceCidr=None, SlbInternet=None, SlbInternetEnabled=None, SlbIntranet=None, Tags=None, UserData=None, Version=None, VpcId=None, WorkerDiskCategory=None, WorkerDiskPerformanceLevel=None, WorkerDiskSize=None, WorkerDiskSnapshotPolicyId=None, WorkerInstanceChargeType=None, WorkerInstanceTypes=None, WorkerNodes=None, WorkerNumber=None, WorkerVswitchIds=None, Addons=None, LogConfig=None, Timeouts=None, WorkerDataDisks=None)
