...
TYPE_NAME = "TF::Alicloud::QuotasQuotaApplication"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_quotas_quota_application/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_quotas_quota_application',
                'returnValues': ["ApproveValue", "AuditReason", "EffectiveTime", "ExpireTime", "Id", "QuotaDescription", "QuotaName", "QuotaUnit", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_quotas_quota_application/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_quotas_quota_application/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApproveValue=None, AuditMode=None, AuditReason=None, DesireValue=None, EffectiveTime=None, ExpireTime=None, Id=None, NoticeType=None, ProductCode=None, QuotaActionCode=None, QuotaCategory=None, QuotaDescription=None, QuotaName=None, QuotaUnit=None, Reason=None, Status=None, Dimensions=None)
