...
TYPE_NAME = "TF::Alicloud::CsServerlessKubernetes"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cs_serverless_kubernetes/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cs_serverless_kubernetes/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cs_serverless_kubernetes/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClientCert=None, ClientKey=None, ClusterCaCert=None, DeletionProtection=None, EndpointPublicAccessEnabled=None, ForceUpdate=None, Id=None, KubeConfig=None, LoadBalancerSpec=None, LoggingType=None, Name=None, NamePrefix=None, NewNatGateway=None, PrivateZone=None, ResourceGroupId=None, SecurityGroupId=None, ServiceCidr=None, ServiceDiscoveryTypes=None, SlsProjectName=None, Tags=None, TimeZone=None, Version=None, VpcId=None, VswitchId=None, VswitchIds=None, ZoneId=None, Addons=None, Timeouts=None)
