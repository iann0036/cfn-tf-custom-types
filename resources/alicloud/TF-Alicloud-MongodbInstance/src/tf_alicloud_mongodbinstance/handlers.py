...
TYPE_NAME = "TF::Alicloud::MongodbInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_mongodb_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Id", "ReplicaSetName", "RetentionPeriod", "SslStatus"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_mongodb_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_mongodb_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountPassword=None, BackupPeriod=None, BackupTime=None, DbInstanceClass=None, DbInstanceStorage=None, EngineVersion=None, Id=None, InstanceChargeType=None, KmsEncryptedPassword=None, KmsEncryptionContext=None, MaintainEndTime=None, MaintainStartTime=None, Name=None, Period=None, ReplicaSetName=None, ReplicationFactor=None, RetentionPeriod=None, SecurityGroupId=None, SecurityIpList=None, SslAction=None, SslStatus=None, StorageEngine=None, Tags=None, TdeStatus=None, VswitchId=None, ZoneId=None, Timeouts=None)
