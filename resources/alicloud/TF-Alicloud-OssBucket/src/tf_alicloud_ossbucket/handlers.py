...
TYPE_NAME = "TF::Alicloud::OssBucket"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_oss_bucket/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_oss_bucket/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_oss_bucket/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Acl=None, Bucket=None, CreationDate=None, ExtranetEndpoint=None, ForceDestroy=None, Id=None, IntranetEndpoint=None, Location=None, LoggingIsenable=None, Owner=None, Policy=None, RedundancyType=None, StorageClass=None, Tags=None, CorsRule=None, LifecycleRule=None, Logging=None, RefererConfig=None, ServerSideEncryptionRule=None, TransferAcceleration=None, Versioning=None, Website=None)
