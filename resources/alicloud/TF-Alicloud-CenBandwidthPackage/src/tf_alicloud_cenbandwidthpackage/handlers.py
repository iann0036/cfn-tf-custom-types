...
TYPE_NAME = "TF::Alicloud::CenBandwidthPackage"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cen_bandwidth_package/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cen_bandwidth_package/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cen_bandwidth_package/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Bandwidth=None, CenBandwidthPackageName=None, ChargeType=None, Description=None, ExpiredTime=None, GeographicRegionAId=None, GeographicRegionBId=None, GeographicRegionIds=None, Id=None, Name=None, PaymentType=None, Period=None, Status=None, Timeouts=None)
