...
TYPE_NAME = "TF::Alicloud::CsKubernetesNodePool"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cs_kubernetes_node_pool/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_cs_kubernetes_node_pool',
                'returnValues': ["Id", "ScalingGroupId", "VpcId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cs_kubernetes_node_pool/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cs_kubernetes_node_pool/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenew=None, AutoRenewPeriod=None, ClusterId=None, Id=None, ImageId=None, InstallCloudMonitor=None, InstanceChargeType=None, InstanceTypes=None, InternetChargeType=None, InternetMaxBandwidthOut=None, KeyName=None, KmsEncryptedPassword=None, Name=None, NodeCount=None, NodeNameMode=None, Password=None, Period=None, PeriodUnit=None, ResourceGroupId=None, ScalingGroupId=None, SecurityGroupId=None, SpotStrategy=None, SystemDiskCategory=None, SystemDiskPerformanceLevel=None, SystemDiskSize=None, Tags=None, Unschedulable=None, UserData=None, VpcId=None, VswitchIds=None, DataDisks=None, Labels=None, Management=None, ScalingConfig=None, SpotPriceLimit=None, Taints=None, Timeouts=None)
