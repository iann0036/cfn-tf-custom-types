...
TYPE_NAME = "TF::Alicloud::OosTemplate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_oos_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_oos_template',
                'returnValues': ["CreatedBy", "CreatedDate", "Description", "HasTrigger", "Id", "ShareType", "TemplateFormat", "TemplateId", "TemplateType", "TemplateVersion", "UpdatedBy", "UpdatedDate"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_oos_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_oos_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoDeleteExecutions=None, Content=None, CreatedBy=None, CreatedDate=None, Description=None, HasTrigger=None, Id=None, ShareType=None, Tags=None, TemplateFormat=None, TemplateId=None, TemplateName=None, TemplateType=None, TemplateVersion=None, UpdatedBy=None, UpdatedDate=None, VersionName=None)
