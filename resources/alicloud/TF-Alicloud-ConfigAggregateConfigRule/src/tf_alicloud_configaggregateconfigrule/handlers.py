...
TYPE_NAME = "TF::Alicloud::ConfigAggregateConfigRule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_config_aggregate_config_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_config_aggregate_config_rule',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_config_aggregate_config_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_config_aggregate_config_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AggregateConfigRuleName=None, AggregatorId=None, ConfigRuleTriggerTypes=None, Description=None, ExcludeResourceIdsScope=None, Id=None, InputParameters=None, MaximumExecutionFrequency=None, RegionIdsScope=None, ResourceGroupIdsScope=None, ResourceTypesScope=None, RiskLevel=None, SourceIdentifier=None, SourceOwner=None, Status=None, TagKeyScope=None, TagValueScope=None, Timeouts=None)
