...
TYPE_NAME = "TF::Alicloud::CmsAlarm"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cms_alarm/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_cms_alarm/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_cms_alarm/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ContactGroups=None, Dimensions=None, EffectiveInterval=None, Enabled=None, EndTime=None, Id=None, Metric=None, Name=None, NotifyType=None, Operator=None, Period=None, Project=None, SilenceTime=None, StartTime=None, Statistics=None, Status=None, Threshold=None, TriggeredCount=None, Webhook=None, EscalationsCritical=None, EscalationsInfo=None, EscalationsWarn=None)
