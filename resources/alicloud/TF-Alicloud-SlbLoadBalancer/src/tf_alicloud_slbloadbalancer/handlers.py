...
TYPE_NAME = "TF::Alicloud::SlbLoadBalancer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_slb_load_balancer/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_slb_load_balancer',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_slb_load_balancer/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_slb_load_balancer/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, AddressIpVersion=None, AddressType=None, Bandwidth=None, DeleteProtection=None, Id=None, InstanceChargeType=None, Internet=None, InternetChargeType=None, LoadBalancerName=None, LoadBalancerSpec=None, MasterZoneId=None, ModificationProtectionReason=None, ModificationProtectionStatus=None, Name=None, PaymentType=None, Period=None, ResourceGroupId=None, SlaveZoneId=None, Specification=None, Status=None, Tags=None, VswitchId=None, Timeouts=None)
