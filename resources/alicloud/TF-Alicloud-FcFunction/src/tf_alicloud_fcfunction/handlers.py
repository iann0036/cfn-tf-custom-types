...
TYPE_NAME = "TF::Alicloud::FcFunction"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_fc_function/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_fc_function/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_fc_function/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CaPort=None, CodeChecksum=None, Description=None, EnvironmentVariables=None, Filename=None, FunctionId=None, Handler=None, Id=None, InitializationTimeout=None, Initializer=None, InstanceConcurrency=None, InstanceType=None, LastModified=None, MemorySize=None, Name=None, NamePrefix=None, OssBucket=None, OssKey=None, Runtime=None, Service=None, Timeout=None, CustomContainerConfig=None)
