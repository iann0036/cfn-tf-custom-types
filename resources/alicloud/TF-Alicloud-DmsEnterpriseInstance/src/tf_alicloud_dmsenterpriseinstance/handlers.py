...
TYPE_NAME = "TF::Alicloud::DmsEnterpriseInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_dms_enterprise_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'alicloud_dms_enterprise_instance',
                'returnValues': ["DbaNickName", "Id", "State", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_dms_enterprise_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_dms_enterprise_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DataLinkName=None, DatabasePassword=None, DatabaseUser=None, DbaId=None, DbaNickName=None, DbaUid=None, DdlOnline=None, EcsInstanceId=None, EcsRegion=None, EnvType=None, ExportTimeout=None, Host=None, Id=None, InstanceAlias=None, InstanceId=None, InstanceName=None, InstanceSource=None, InstanceType=None, NetworkType=None, Port=None, QueryTimeout=None, SafeRule=None, SafeRuleId=None, Sid=None, SkipTest=None, State=None, Status=None, Tid=None, UseDsql=None, VpcId=None, Timeouts=None)
