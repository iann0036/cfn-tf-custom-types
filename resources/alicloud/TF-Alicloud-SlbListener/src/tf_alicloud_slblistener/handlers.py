...
TYPE_NAME = "TF::Alicloud::SlbListener"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_slb_listener/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alicloud_slb_listener/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alicloud_slb_listener/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AclId=None, AclStatus=None, AclType=None, BackendPort=None, Bandwidth=None, CaCertificateId=None, Cookie=None, CookieTimeout=None, DeleteProtectionValidation=None, Description=None, EnableHttp2=None, EstablishedTimeout=None, ForwardPort=None, FrontendPort=None, Gzip=None, HealthCheck=None, HealthCheckConnectPort=None, HealthCheckDomain=None, HealthCheckHttpCode=None, HealthCheckInterval=None, HealthCheckMethod=None, HealthCheckTimeout=None, HealthCheckType=None, HealthCheckUri=None, HealthyThreshold=None, Id=None, IdleTimeout=None, InstancePort=None, LbPort=None, LbProtocol=None, ListenerForward=None, LoadBalancerId=None, MasterSlaveServerGroupId=None, PersistenceTimeout=None, Protocol=None, RequestTimeout=None, Scheduler=None, ServerCertificateId=None, ServerGroupId=None, SslCertificateId=None, StickySession=None, StickySessionType=None, TlsCipherPolicy=None, UnhealthyThreshold=None, XForwardedFor=None)
