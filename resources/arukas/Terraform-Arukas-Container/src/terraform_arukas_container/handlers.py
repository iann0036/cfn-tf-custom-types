...
def create_handler(
    ...
    try:
        ...
        tf['resource']['arukas_container'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("arukas"))['SecretString']
            tf["provider"]["arukas"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['arukas_container'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
