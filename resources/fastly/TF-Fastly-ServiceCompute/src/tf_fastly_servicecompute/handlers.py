...
TYPE_NAME = "TF::Fastly::ServiceCompute"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fastly_service_compute/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fastly_service_compute',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fastly_service_compute/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fastly_service_compute/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Activate=None, ActiveVersion=None, ClonedVersion=None, Comment=None, ForceDestroy=None, Id=None, Name=None, VersionComment=None, Backend=None, Bigquerylogging=None, Blobstoragelogging=None, Dictionary=None, Director=None, Domain=None, Gcslogging=None, Healthcheck=None, Httpslogging=None, Logentries=None, LoggingCloudfiles=None, LoggingDatadog=None, LoggingDigitalocean=None, LoggingElasticsearch=None, LoggingFtp=None, LoggingGooglepubsub=None, LoggingHeroku=None, LoggingHoneycomb=None, LoggingKafka=None, LoggingKinesis=None, LoggingLoggly=None, LoggingLogshuttle=None, LoggingNewrelic=None, LoggingOpenstack=None, LoggingScalyr=None, LoggingSftp=None, Package=None, Papertrail=None, S3logging=None, Splunk=None, Sumologic=None, Syslog=None)
