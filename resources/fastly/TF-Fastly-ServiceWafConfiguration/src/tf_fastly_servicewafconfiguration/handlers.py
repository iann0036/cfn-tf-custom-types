...
TYPE_NAME = "TF::Fastly::ServiceWafConfiguration"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fastly_service_waf_configuration/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fastly_service_waf_configuration',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fastly_service_waf_configuration/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fastly_service_waf_configuration/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowedHttpVersions=None, AllowedMethods=None, AllowedRequestContentType=None, AllowedRequestContentTypeCharset=None, ArgLength=None, ArgNameLength=None, CombinedFileSizes=None, CriticalAnomalyScore=None, CrsValidateUtf8Encoding=None, ErrorAnomalyScore=None, HighRiskCountryCodes=None, HttpViolationScoreThreshold=None, Id=None, InboundAnomalyScoreThreshold=None, LfiScoreThreshold=None, MaxFileSize=None, MaxNumArgs=None, NoticeAnomalyScore=None, ParanoiaLevel=None, PhpInjectionScoreThreshold=None, RceScoreThreshold=None, RestrictedExtensions=None, RestrictedHeaders=None, RfiScoreThreshold=None, SessionFixationScoreThreshold=None, SqlInjectionScoreThreshold=None, TotalArgLength=None, WafId=None, WarningAnomalyScore=None, XssScoreThreshold=None, Rule=None, RuleExclusion=None)
