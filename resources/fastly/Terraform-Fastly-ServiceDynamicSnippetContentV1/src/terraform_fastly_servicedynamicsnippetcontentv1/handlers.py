...
def create_handler(
    ...
    try:
        ...
        tf['resource']['fastly_service_dynamic_snippet_content_v1'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("fastly"))['SecretString']
            tf["provider"]["fastly"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['fastly_service_dynamic_snippet_content_v1'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
