...
TYPE_NAME = "TF::Okta::AppAutoLogin"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_auto_login/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_app_auto_login',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_auto_login/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_app_auto_login/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessibilityErrorRedirectUrl=None, AccessibilitySelfService=None, AutoSubmitToolbar=None, CredentialsScheme=None, Groups=None, HideIos=None, HideWeb=None, Id=None, Label=None, Name=None, PreconfiguredApp=None, RevealPassword=None, SharedPassword=None, SharedUsername=None, SignOnMode=None, SignOnRedirectUrl=None, SignOnUrl=None, Status=None, UserNameTemplate=None, UserNameTemplateSuffix=None, UserNameTemplateType=None, Users=None)
