...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_saml/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'okta/okta',
                'providerTypeName': 'okta',
                'terraformTypeName': 'okta_app_saml',
                'returnValues': ["Certificate", "EntityKey", "EntityUrl", "HttpPostBinding", "HttpRedirectBinding", "Id", "KeyId", "Metadata", "MetadataUrl", "Name", "SignOnMode"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_saml/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_app_saml/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessibilityErrorRedirectUrl=None, AccessibilityLoginRedirectUrl=None, AccessibilitySelfService=None, AcsEndpoints=None, AppSettingsJson=None, AssertionSigned=None, Audience=None, AuthnContextClassRef=None, AutoSubmitToolbar=None, Certificate=None, DefaultRelayState=None, Destination=None, DigestAlgorithm=None, EntityKey=None, EntityUrl=None, Features=None, Groups=None, HideIos=None, HideWeb=None, HonorForceAuthn=None, HttpPostBinding=None, HttpRedirectBinding=None, Id=None, IdpIssuer=None, KeyId=None, KeyName=None, KeyYearsValid=None, Label=None, Metadata=None, MetadataUrl=None, Name=None, PreconfiguredApp=None, Recipient=None, RequestCompressed=None, ResponseSigned=None, SignOnMode=None, SignatureAlgorithm=None, SingleLogoutCertificate=None, SingleLogoutIssuer=None, SingleLogoutUrl=None, SpIssuer=None, SsoUrl=None, Status=None, SubjectNameIdFormat=None, SubjectNameIdTemplate=None, UserNameTemplate=None, UserNameTemplateSuffix=None, UserNameTemplateType=None, AttributeStatements=None, Users=None)
