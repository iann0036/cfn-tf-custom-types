...
TYPE_NAME = "TF::Okta::PolicyPasswordDefault"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_policy_password_default/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_policy_password_default',
                'returnValues': ["DefaultAuthProvider", "DefaultIncludedGroupId", "Description", "Id", "Name", "Priority", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_policy_password_default/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_policy_password_default/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CallRecovery=None, DefaultAuthProvider=None, DefaultIncludedGroupId=None, Description=None, EmailRecovery=None, Id=None, Name=None, PasswordAutoUnlockMinutes=None, PasswordDictionaryLookup=None, PasswordExcludeFirstName=None, PasswordExcludeLastName=None, PasswordExcludeUsername=None, PasswordExpireWarnDays=None, PasswordHistoryCount=None, PasswordLockoutNotificationChannels=None, PasswordMaxAgeDays=None, PasswordMaxLockoutAttempts=None, PasswordMinAgeMinutes=None, PasswordMinLength=None, PasswordMinLowercase=None, PasswordMinNumber=None, PasswordMinSymbol=None, PasswordMinUppercase=None, PasswordShowLockoutFailures=None, Priority=None, QuestionMinLength=None, QuestionRecovery=None, RecoveryEmailToken=None, SkipUnlock=None, SmsRecovery=None, Status=None)
