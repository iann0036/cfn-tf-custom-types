...
TYPE_NAME = "TF::Okta::AppThreeField"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_three_field/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_app_three_field',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_three_field/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_app_three_field/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessibilityErrorRedirectUrl=None, AccessibilitySelfService=None, AutoSubmitToolbar=None, ButtonSelector=None, ExtraFieldSelector=None, ExtraFieldValue=None, Groups=None, HideIos=None, HideWeb=None, Id=None, Label=None, Name=None, PasswordSelector=None, SignOnMode=None, Status=None, Url=None, UrlRegex=None, UserNameTemplate=None, UserNameTemplateSuffix=None, UserNameTemplateType=None, UsernameSelector=None, Users=None)
