...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_idp_saml/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_idp_saml',
                'returnValues': ["Audience", "Id", "Type"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_idp_saml/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_idp_saml/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountLinkAction=None, AccountLinkGroupInclude=None, AcsBinding=None, AcsType=None, Audience=None, DeprovisionedAction=None, GroupsAction=None, GroupsAssignment=None, GroupsAttribute=None, GroupsFilter=None, Id=None, Issuer=None, IssuerMode=None, Kid=None, MaxClockSkew=None, Name=None, NameFormat=None, ProfileMaster=None, ProvisioningAction=None, RequestSignatureAlgorithm=None, RequestSignatureScope=None, ResponseSignatureAlgorithm=None, ResponseSignatureScope=None, SsoBinding=None, SsoDestination=None, SsoUrl=None, Status=None, SubjectFilter=None, SubjectFormat=None, SubjectMatchAttribute=None, SubjectMatchType=None, SuspendedAction=None, Type=None, UsernameTemplate=None)
