...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_oauth/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_app_oauth',
                'returnValues': ["ClientSecret", "Id", "Name", "SignOnMode"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_app_oauth/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_app_oauth/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoKeyRotation=None, AutoSubmitToolbar=None, ClientBasicSecret=None, ClientId=None, ClientSecret=None, ClientUri=None, ConsentMethod=None, CustomClientId=None, GrantTypes=None, Groups=None, HideIos=None, HideWeb=None, Id=None, ImplicitAssignment=None, IssuerMode=None, Label=None, LoginMode=None, LoginScopes=None, LoginUri=None, LogoUri=None, Name=None, OmitSecret=None, PolicyUri=None, PostLogoutRedirectUris=None, Profile=None, RedirectUris=None, ResponseTypes=None, SignOnMode=None, Status=None, TokenEndpointAuthMethod=None, TosUri=None, Type=None, Jwks=None, Users=None)
