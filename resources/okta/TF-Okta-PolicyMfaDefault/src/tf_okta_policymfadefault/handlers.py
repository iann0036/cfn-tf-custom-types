...
TYPE_NAME = "TF::Okta::PolicyMfaDefault"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_policy_mfa_default/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_policy_mfa_default',
                'returnValues': ["DefaultIncludedGroupId", "Description", "Id", "Name", "Priority", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_policy_mfa_default/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_policy_mfa_default/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DefaultIncludedGroupId=None, Description=None, Duo=None, FidoU2f=None, FidoWebauthn=None, GoogleOtp=None, Hotp=None, Id=None, Name=None, OktaCall=None, OktaEmail=None, OktaOtp=None, OktaPassword=None, OktaPush=None, OktaQuestion=None, OktaSms=None, Priority=None, RsaToken=None, Status=None, SymantecVip=None, YubikeyToken=None)
