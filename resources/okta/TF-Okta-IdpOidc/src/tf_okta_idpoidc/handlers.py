...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_idp_oidc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_idp_oidc',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_idp_oidc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_idp_oidc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountLinkAction=None, AccountLinkGroupInclude=None, AuthorizationBinding=None, AuthorizationUrl=None, ClientId=None, ClientSecret=None, DeprovisionedAction=None, GroupsAction=None, GroupsAssignment=None, GroupsAttribute=None, GroupsFilter=None, Id=None, IssuerMode=None, IssuerUrl=None, JwksBinding=None, JwksUrl=None, MaxClockSkew=None, Name=None, ProfileMaster=None, ProtocolType=None, ProvisioningAction=None, RequestSignatureAlgorithm=None, RequestSignatureScope=None, ResponseSignatureAlgorithm=None, ResponseSignatureScope=None, Scopes=None, Status=None, SubjectMatchAttribute=None, SubjectMatchType=None, SuspendedAction=None, TokenBinding=None, TokenUrl=None, Type=None, UserInfoBinding=None, UserInfoUrl=None, UsernameTemplate=None)
