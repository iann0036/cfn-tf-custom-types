...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'okta_user',
                'returnValues': ["Id", "RawStatus"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/okta_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/okta_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminRoles=None, City=None, CostCenter=None, CountryCode=None, CustomProfileAttributes=None, Department=None, DisplayName=None, Division=None, Email=None, EmployeeNumber=None, FirstName=None, GroupMemberships=None, HonorificPrefix=None, HonorificSuffix=None, Id=None, LastName=None, Locale=None, Login=None, Manager=None, ManagerId=None, MiddleName=None, MobilePhone=None, NickName=None, Organization=None, Password=None, PostalAddress=None, PreferredLanguage=None, PrimaryPhone=None, ProfileUrl=None, RawStatus=None, RecoveryAnswer=None, RecoveryQuestion=None, SecondEmail=None, State=None, Status=None, StreetAddress=None, Timezone=None, Title=None, UserType=None, ZipCode=None)
