...
TYPE_NAME = "TF::TencentCloud::MongodbInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_mongodb_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CreateTime", "Id", "StandbyInstanceList", "Status", "Vip", "Vport"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_mongodb_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_mongodb_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenewFlag=None, AvailableZone=None, ChargeType=None, CreateTime=None, EngineVersion=None, Id=None, InstanceName=None, MachineType=None, Memory=None, Password=None, PrepaidPeriod=None, ProjectId=None, SecurityGroups=None, StandbyInstanceList=None, Status=None, SubnetId=None, Tags=None, Vip=None, Volume=None, VpcId=None, Vport=None)
