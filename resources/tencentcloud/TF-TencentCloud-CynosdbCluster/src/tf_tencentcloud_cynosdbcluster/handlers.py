...
TYPE_NAME = "TF::TencentCloud::CynosdbCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_cynosdb_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'tencentcloud_cynosdb_cluster',
                'returnValues': ["Charset", "ClusterStatus", "CreateTime", "Id", "InstanceId", "InstanceName", "InstanceStatus", "InstanceStorageSize", "RoGroupAddr", "RoGroupId", "RoGroupInstances", "RwGroupAddr", "RwGroupId", "RwGroupInstances", "StorageUsed"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_cynosdb_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_cynosdb_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenewFlag=None, AvailableZone=None, ChargeType=None, Charset=None, ClusterName=None, ClusterStatus=None, CreateTime=None, DbType=None, DbVersion=None, ForceDelete=None, Id=None, InstanceCpuCore=None, InstanceId=None, InstanceMaintainDuration=None, InstanceMaintainStartTime=None, InstanceMaintainWeekdays=None, InstanceMemorySize=None, InstanceName=None, InstanceStatus=None, InstanceStorageSize=None, Password=None, Port=None, PrepaidPeriod=None, ProjectId=None, RoGroupAddr=None, RoGroupId=None, RoGroupInstances=None, RoGroupSg=None, RwGroupAddr=None, RwGroupId=None, RwGroupInstances=None, RwGroupSg=None, StorageLimit=None, StorageUsed=None, SubnetId=None, Tags=None, VpcId=None)
