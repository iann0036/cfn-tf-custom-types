...
TYPE_NAME = "TF::TencentCloud::MysqlReadonlyInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_mysql_readonly_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_mysql_readonly_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_mysql_readonly_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRenewFlag=None, ChargeType=None, ForceDelete=None, Id=None, InstanceName=None, IntranetIp=None, IntranetPort=None, Locked=None, MasterInstanceId=None, MemSize=None, PayType=None, Period=None, PrepaidPeriod=None, SecurityGroups=None, Status=None, SubnetId=None, Tags=None, TaskStatus=None, VolumeSize=None, VpcId=None)
