...
TYPE_NAME = "TF::TencentCloud::ElasticsearchInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_elasticsearch_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'tencentcloud_elasticsearch_instance',
                'returnValues': ["CreateTime", "ElasticsearchDomain", "ElasticsearchPort", "ElasticsearchVip", "Id", "KibanaUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_elasticsearch_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_elasticsearch_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZone=None, BasicSecurityType=None, ChargePeriod=None, ChargeType=None, CreateTime=None, DeployMode=None, ElasticsearchDomain=None, ElasticsearchPort=None, ElasticsearchVip=None, Id=None, InstanceName=None, KibanaUrl=None, LicenseType=None, Password=None, RenewFlag=None, SubnetId=None, Tags=None, Version=None, VpcId=None, MultiZoneInfos=None, NodeInfoList=None)
