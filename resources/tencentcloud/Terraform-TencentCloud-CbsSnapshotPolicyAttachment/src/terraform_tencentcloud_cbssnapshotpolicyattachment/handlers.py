...
def create_handler(
    ...
    try:
        ...
        tf['resource']['tencentcloud_cbs_snapshot_policy_attachment'][request.logicalResourceIdentifier] = {}

        if "tencentcloud" == "aws":
            creds = session.credentials
            tf["provider"]["tencentcloud"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("tencentcloud"))['SecretString']
            tf["provider"]["tencentcloud"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['tencentcloud_cbs_snapshot_policy_attachment'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
