...
TYPE_NAME = "TF::TencentCloud::ClbListenerRule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_clb_listener_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_clb_listener_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_clb_listener_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CertificateCaId=None, CertificateId=None, CertificateSslMode=None, ClbId=None, Domain=None, ForwardType=None, HealthCheckHealthNum=None, HealthCheckHttpCode=None, HealthCheckHttpDomain=None, HealthCheckHttpMethod=None, HealthCheckHttpPath=None, HealthCheckIntervalTime=None, HealthCheckSwitch=None, HealthCheckUnhealthNum=None, Http2Switch=None, Id=None, ListenerId=None, RuleId=None, Scheduler=None, SessionExpireTime=None, TargetType=None, Url=None)
