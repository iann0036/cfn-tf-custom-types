...
TYPE_NAME = "TF::TencentCloud::ContainerClusterInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_container_cluster_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_container_cluster_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_container_cluster_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AbnormalReason=None, Bandwidth=None, BandwidthType=None, ClusterId=None, Cpu=None, CvmType=None, DockerGraphPath=None, Id=None, InstanceId=None, InstanceName=None, InstanceType=None, IsNormal=None, IsVpcGateway=None, KeyId=None, LanIp=None, Mem=None, MountTarget=None, Password=None, Period=None, RequireWanIp=None, RootSize=None, RootType=None, SgId=None, StorageSize=None, StorageType=None, SubnetId=None, Unschedulable=None, UserScript=None, WanIp=None, ZoneId=None)
