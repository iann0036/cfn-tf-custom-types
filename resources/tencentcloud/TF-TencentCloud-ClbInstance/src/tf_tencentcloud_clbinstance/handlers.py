...
TYPE_NAME = "TF::TencentCloud::ClbInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_clb_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ClbVips", "Id", "VipIsp"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_clb_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_clb_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddressIpVersion=None, ClbName=None, ClbVips=None, Id=None, InternetBandwidthMaxOut=None, InternetChargeType=None, LoadBalancerPassToTarget=None, MasterZoneId=None, NetworkType=None, ProjectId=None, SecurityGroups=None, SlaveZoneId=None, SubnetId=None, Tags=None, TargetRegionInfoRegion=None, TargetRegionInfoVpcId=None, VipIsp=None, VpcId=None, ZoneId=None)
