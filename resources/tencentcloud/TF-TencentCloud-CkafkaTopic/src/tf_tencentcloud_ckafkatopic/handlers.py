...
TYPE_NAME = "TF::TencentCloud::CkafkaTopic"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_ckafka_topic/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'tencentcloud_ckafka_topic',
                'returnValues': ["CreateTime", "ForwardCosBucket", "ForwardInterval", "ForwardStatus", "Id", "MessageStorageLocation", "SegmentBytes"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_ckafka_topic/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_ckafka_topic/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CleanUpPolicy=None, CreateTime=None, EnableWhiteList=None, ForwardCosBucket=None, ForwardInterval=None, ForwardStatus=None, Id=None, InstanceId=None, IpWhiteList=None, MaxMessageBytes=None, MessageStorageLocation=None, Note=None, PartitionNum=None, ReplicaNum=None, Retention=None, Segment=None, SegmentBytes=None, SyncReplicaMinNum=None, TopicName=None, UncleanLeaderElectionEnable=None)
