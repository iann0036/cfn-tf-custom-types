...
TYPE_NAME = "TF::TencentCloud::KubernetesNodePool"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_kubernetes_node_pool/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'tencentcloud_kubernetes_node_pool',
                'returnValues': ["AutoScalingGroupId", "Id", "LaunchConfigId", "NodeCount", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_kubernetes_node_pool/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_kubernetes_node_pool/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoScalingGroupId=None, ClusterId=None, DeleteKeepInstance=None, DesiredCapacity=None, EnableAutoScale=None, Id=None, Labels=None, LaunchConfigId=None, MaxSize=None, MinSize=None, Name=None, NodeCount=None, NodeOs=None, NodeOsType=None, RetryPolicy=None, ScalingMode=None, Status=None, SubnetIds=None, Unschedulable=None, VpcId=None, AutoScalingConfig=None, NodeConfig=None, Taints=None)
