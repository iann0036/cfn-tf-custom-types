...
TYPE_NAME = "TF::TencentCloud::AsScalingConfig"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_as_scaling_config/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_as_scaling_config/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_as_scaling_config/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ConfigurationName=None, CreateTime=None, DiskTypePolicy=None, EnhancedMonitorService=None, EnhancedSecurityService=None, Id=None, ImageId=None, InstanceTags=None, InstanceTypes=None, InternetChargeType=None, InternetMaxBandwidthOut=None, KeepImageLogin=None, KeyIds=None, Password=None, ProjectId=None, PublicIpAssigned=None, SecurityGroupIds=None, Status=None, SystemDiskSize=None, SystemDiskType=None, UserData=None, DataDisk=None)
