...
TYPE_NAME = "TF::TencentCloud::VpnConnection"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_vpn_connection/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CreateTime", "EncryptProto", "Id", "IsCcnType", "NetStatus", "RouteType", "State", "VpnProto"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_vpn_connection/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_vpn_connection/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreateTime=None, CustomerGatewayId=None, EncryptProto=None, Id=None, IkeDhGroupName=None, IkeExchangeMode=None, IkeLocalAddress=None, IkeLocalFqdnName=None, IkeLocalIdentity=None, IkeProtoAuthenAlgorithm=None, IkeProtoEncryAlgorithm=None, IkeRemoteAddress=None, IkeRemoteFqdnName=None, IkeRemoteIdentity=None, IkeSaLifetimeSeconds=None, IkeVersion=None, IpsecEncryptAlgorithm=None, IpsecIntegrityAlgorithm=None, IpsecPfsDhGroup=None, IpsecSaLifetimeSeconds=None, IpsecSaLifetimeTraffic=None, IsCcnType=None, Name=None, NetStatus=None, PreShareKey=None, RouteType=None, State=None, Tags=None, VpcId=None, VpnGatewayId=None, VpnProto=None, SecurityGroupPolicy=None)
