...
TYPE_NAME = "TF::TencentCloud::DayuDdosPolicyCase"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_dayu_ddos_policy_case/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_dayu_ddos_policy_case/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_dayu_ddos_policy_case/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppProtocols=None, AppType=None, CreateTime=None, HasAbroad=None, HasInitiateTcp=None, HasInitiateUdp=None, HasVpn=None, Id=None, MaxTcpPackageLen=None, MaxUdpPackageLen=None, MinTcpPackageLen=None, MinUdpPackageLen=None, Name=None, PeerTcpPort=None, PeerUdpPort=None, PlatformTypes=None, ResourceType=None, SceneId=None, TcpEndPort=None, TcpFootprint=None, TcpStartPort=None, UdpEndPort=None, UdpFootprint=None, UdpStartPort=None, WebApiUrls=None)
