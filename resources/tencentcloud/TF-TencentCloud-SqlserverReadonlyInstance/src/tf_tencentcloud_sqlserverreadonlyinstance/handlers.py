...
TYPE_NAME = "TF::TencentCloud::SqlserverReadonlyInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_sqlserver_readonly_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'tencentcloud_sqlserver_readonly_instance',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_sqlserver_readonly_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_sqlserver_readonly_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZone=None, ChargeType=None, CreateTime=None, ForceUpgrade=None, Id=None, MasterInstanceId=None, Memory=None, Name=None, ReadonlyGroupId=None, ReadonlyGroupType=None, RoFlag=None, SecurityGroups=None, Status=None, Storage=None, SubnetId=None, Tags=None, Vip=None, VpcId=None, Vport=None)
