...
TYPE_NAME = "TF::TencentCloud::ScfFunction"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_scf_function/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/tencentcloud_scf_function/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/tencentcloud_scf_function/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClsLogsetId=None, ClsTopicId=None, CodeError=None, CodeResult=None, CodeSize=None, CosBucketName=None, CosBucketRegion=None, CosObjectName=None, Description=None, EipFixed=None, Eips=None, Environment=None, ErrNo=None, Handler=None, Host=None, Id=None, InstallDependency=None, L5Enable=None, MemSize=None, ModifyTime=None, Name=None, Namespace=None, Role=None, Runtime=None, Status=None, StatusDesc=None, SubnetId=None, Tags=None, Timeout=None, TriggerInfo=None, Vip=None, VpcId=None, ZipFile=None, Triggers=None)
