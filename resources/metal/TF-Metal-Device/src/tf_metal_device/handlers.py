...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/metal_device/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'equinix/metal',
                'providerTypeName': 'metal',
                'terraformTypeName': 'metal_device',
                'returnValues': ["AccessPrivateIpv4", "AccessPublicIpv4", "AccessPublicIpv6", "Created", "DeployedFacility", "DeployedHardwareReservationId", "Id", "Locked", "Network", "NetworkType", "Ports", "RootPassword", "SshKeyIds", "State", "Updated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/metal_device/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/metal_device/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessPrivateIpv4=None, AccessPublicIpv4=None, AccessPublicIpv6=None, AlwaysPxe=None, BillingCycle=None, Created=None, CustomData=None, DeployedFacility=None, DeployedHardwareReservationId=None, Description=None, Facilities=None, ForceDetachVolumes=None, HardwareReservationId=None, Hostname=None, Id=None, IpxeScriptUrl=None, Locked=None, Metro=None, Network=None, NetworkType=None, OperatingSystem=None, Plan=None, Ports=None, ProjectId=None, ProjectSshKeyIds=None, RootPassword=None, SshKeyIds=None, State=None, Storage=None, Tags=None, Updated=None, UserData=None, WaitForReservationDeprovision=None, IpAddress=None, Timeouts=None)
