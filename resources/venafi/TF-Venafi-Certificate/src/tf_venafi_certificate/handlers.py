...
TYPE_NAME = "TF::Venafi::Certificate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/venafi_certificate/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'Venafi/venafi',
                'providerTypeName': 'venafi',
                'terraformTypeName': 'venafi_certificate',
                'returnValues': ["Certificate", "Chain", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/venafi_certificate/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/venafi_certificate/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Algorithm=None, Certificate=None, CertificateDn=None, Chain=None, CommonName=None, CsrPem=None, CustomFields=None, EcdsaCurve=None, ExpirationWindow=None, Id=None, IssuerHint=None, KeyPassword=None, Pkcs12=None, PrivateKeyPem=None, RsaBits=None, SanDns=None, SanEmail=None, SanIp=None, ValidDays=None)
