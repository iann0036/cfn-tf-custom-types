...
def create_handler(
    ...
    try:
        ...
        tf['resource']['dnsimple_record'][request.logicalResourceIdentifier] = {}

        if "dnsimple" == "aws":
            creds = session.credentials
            tf["provider"]["dnsimple"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("dnsimple"))['SecretString']
            tf["provider"]["dnsimple"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['dnsimple_record'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
