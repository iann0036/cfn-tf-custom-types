...
def create_handler(
    ...
    try:
        ...
        tf['resource']['openstack_networking_qos_minimum_bandwidth_rule_v2'][request.logicalResourceIdentifier] = {}

        if "openstack" == "aws":
            creds = session.credentials
            tf["provider"]["openstack"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("openstack"))['SecretString']
            tf["provider"]["openstack"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['openstack_networking_qos_minimum_bandwidth_rule_v2'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
