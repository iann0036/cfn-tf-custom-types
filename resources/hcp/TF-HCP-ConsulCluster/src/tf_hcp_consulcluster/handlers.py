...
TYPE_NAME = "TF::HCP::ConsulCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcp_consul_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/hcp',
                'providerTypeName': 'hcp',
                'terraformTypeName': 'hcp_consul_cluster',
                'returnValues': ["CloudProvider", "ConsulAutomaticUpgrades", "ConsulCaFile", "ConsulConfigFile", "ConsulPrivateEndpointUrl", "ConsulPublicEndpointUrl", "ConsulRootTokenAccessorId", "ConsulRootTokenSecretId", "ConsulSnapshotInterval", "ConsulSnapshotRetention", "ConsulVersion", "Id", "OrganizationId", "ProjectId", "Region", "Scale", "SelfLink"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcp_consul_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/hcp_consul_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CloudProvider=None, ClusterId=None, ConnectEnabled=None, ConsulAutomaticUpgrades=None, ConsulCaFile=None, ConsulConfigFile=None, ConsulPrivateEndpointUrl=None, ConsulPublicEndpointUrl=None, ConsulRootTokenAccessorId=None, ConsulRootTokenSecretId=None, ConsulSnapshotInterval=None, ConsulSnapshotRetention=None, ConsulVersion=None, Datacenter=None, HvnId=None, Id=None, MinConsulVersion=None, OrganizationId=None, PrimaryLink=None, ProjectId=None, PublicEndpoint=None, Region=None, Scale=None, SelfLink=None, Size=None, Tier=None, Timeouts=None)
