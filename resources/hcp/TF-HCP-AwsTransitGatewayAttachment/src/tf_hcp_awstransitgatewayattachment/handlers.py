...
TYPE_NAME = "TF::HCP::AwsTransitGatewayAttachment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcp_aws_transit_gateway_attachment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'hcp_aws_transit_gateway_attachment',
                'returnValues': ["CreatedAt", "ExpiresAt", "Id", "OrganizationId", "ProjectId", "ProviderTransitGatewayAttachmentId", "SelfLink", "State"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcp_aws_transit_gateway_attachment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/hcp_aws_transit_gateway_attachment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreatedAt=None, ExpiresAt=None, HvnId=None, Id=None, OrganizationId=None, ProjectId=None, ProviderTransitGatewayAttachmentId=None, ResourceShareArn=None, SelfLink=None, State=None, TransitGatewayAttachmentId=None, TransitGatewayId=None, Timeouts=None)
