...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcp_vault_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'hcp_vault_cluster',
                'returnValues': ["CloudProvider", "CreatedAt", "Id", "Namespace", "OrganizationId", "ProjectId", "Region", "VaultPrivateEndpointUrl", "VaultPublicEndpointUrl", "VaultVersion"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcp_vault_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/hcp_vault_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CloudProvider=None, ClusterId=None, CreatedAt=None, HvnId=None, Id=None, MinVaultVersion=None, Namespace=None, OrganizationId=None, ProjectId=None, PublicEndpoint=None, Region=None, Tier=None, VaultPrivateEndpointUrl=None, VaultPublicEndpointUrl=None, VaultVersion=None, Timeouts=None)
