...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ecl_sss_user_v1/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ecl_sss_user_v1',
                'returnValues': ["BrandId", "ContractId", "ContractOwner", "ExternalReferenceId", "Id", "KeystoneEndpoint", "KeystoneName", "KeystonePassword", "LoginIntegration", "SssEndpoint", "StartTime", "UserId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ecl_sss_user_v1/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ecl_sss_user_v1/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BrandId=None, ContractId=None, ContractOwner=None, ExternalReferenceId=None, Id=None, KeystoneEndpoint=None, KeystoneName=None, KeystonePassword=None, LoginId=None, LoginIntegration=None, MailAddress=None, NotifyPassword=None, Password=None, SssEndpoint=None, StartTime=None, UserId=None)
