...
TYPE_NAME = "TF::ECL::NetworkLoadBalancerV2"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ecl_network_load_balancer_v2/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ecl_network_load_balancer_v2',
                'returnValues': ["AdminPassword", "AdminUsername", "Id", "Status", "UserPassword", "UserUsername"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ecl_network_load_balancer_v2/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ecl_network_load_balancer_v2/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminPassword=None, AdminUsername=None, AvailabilityZone=None, DefaultGateway=None, Description=None, Id=None, LoadBalancerPlanId=None, Name=None, Status=None, TenantId=None, UserPassword=None, UserUsername=None, Interfaces=None, SyslogServers=None, Timeouts=None)
