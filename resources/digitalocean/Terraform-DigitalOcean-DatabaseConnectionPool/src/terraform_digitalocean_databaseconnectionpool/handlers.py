...
def create_handler(
    ...
    try:
        ...
        tf['resource']['digitalocean_database_connection_pool'][request.logicalResourceIdentifier] = {}

        if "digitalocean" == "aws":
            creds = session.credentials
            tf["provider"]["digitalocean"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("digitalocean"))['SecretString']
            tf["provider"]["digitalocean"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['digitalocean_database_connection_pool'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
