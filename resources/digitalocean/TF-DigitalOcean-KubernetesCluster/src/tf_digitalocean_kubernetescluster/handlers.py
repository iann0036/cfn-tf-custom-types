...
TYPE_NAME = "TF::DigitalOcean::KubernetesCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/digitalocean_kubernetes_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ClusterSubnet", "CreatedAt", "Endpoint", "Id", "Ipv4Address", "KubeConfig", "ServiceSubnet", "Status", "UpdatedAt", "Urn"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/digitalocean_kubernetes_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/digitalocean_kubernetes_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoUpgrade=None, ClusterSubnet=None, CreatedAt=None, Endpoint=None, Id=None, Ipv4Address=None, KubeConfig=None, Name=None, Region=None, ServiceSubnet=None, Status=None, SurgeUpgrade=None, Tags=None, UpdatedAt=None, Urn=None, Version=None, VpcUuid=None, NodePool=None)
