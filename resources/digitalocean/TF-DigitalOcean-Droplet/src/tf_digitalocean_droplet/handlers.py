...
TYPE_NAME = "TF::DigitalOcean::Droplet"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/digitalocean_droplet/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'digitalocean/digitalocean',
                'providerTypeName': 'digitalocean',
                'terraformTypeName': 'digitalocean_droplet',
                'returnValues': ["CreatedAt", "Disk", "Id", "Ipv4Address", "Ipv4AddressPrivate", "Ipv6Address", "Locked", "Memory", "PriceHourly", "PriceMonthly", "Status", "Urn", "Vcpus"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/digitalocean_droplet/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/digitalocean_droplet/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Backups=None, CreatedAt=None, Disk=None, Id=None, Image=None, Ipv4Address=None, Ipv4AddressPrivate=None, Ipv6=None, Ipv6Address=None, Locked=None, Memory=None, Monitoring=None, Name=None, PriceHourly=None, PriceMonthly=None, PrivateNetworking=None, Region=None, ResizeDisk=None, Size=None, SshKeys=None, Status=None, Tags=None, Urn=None, UserData=None, Vcpus=None, VolumeIds=None, VpcUuid=None)
