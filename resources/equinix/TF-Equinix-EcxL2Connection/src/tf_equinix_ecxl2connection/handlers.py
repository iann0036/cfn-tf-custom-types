...
TYPE_NAME = "TF::Equinix::EcxL2Connection"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/equinix_ecx_l2_connection/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'equinix_ecx_l2_connection',
                'returnValues': ["Id", "ProviderStatus", "RedundancyType", "RedundantUuid", "Status", "Uuid"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/equinix_ecx_l2_connection/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/equinix_ecx_l2_connection/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthorizationKey=None, DeviceInterfaceId=None, DeviceUuid=None, Id=None, Name=None, NamedTag=None, Notifications=None, PortUuid=None, ProfileUuid=None, ProviderStatus=None, PurchaseOrderNumber=None, RedundancyType=None, RedundantUuid=None, SellerMetroCode=None, SellerRegion=None, Speed=None, SpeedUnit=None, Status=None, Uuid=None, VlanCtag=None, VlanStag=None, ZsidePortUuid=None, ZsideVlanCtag=None, ZsideVlanStag=None, AdditionalInfo=None, SecondaryConnection=None, Timeouts=None)
