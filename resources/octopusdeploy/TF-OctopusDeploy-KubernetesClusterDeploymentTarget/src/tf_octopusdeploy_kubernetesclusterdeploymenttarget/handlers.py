...
TYPE_NAME = "TF::OctopusDeploy::KubernetesClusterDeploymentTarget"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_kubernetes_cluster_deployment_target/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'octopusdeploy_kubernetes_cluster_deployment_target',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_kubernetes_cluster_deployment_target/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/octopusdeploy_kubernetes_cluster_deployment_target/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClusterCertificate=None, ClusterUrl=None, DefaultWorkerPoolId=None, Environments=None, HasLatestCalamari=None, HealthStatus=None, Id=None, IsDisabled=None, IsInProcess=None, MachinePolicyId=None, Name=None, Namespace=None, OperatingSystem=None, ProxyId=None, Roles=None, RunningInContainer=None, ShellName=None, ShellVersion=None, SkipTlsVerification=None, SpaceId=None, Status=None, StatusSummary=None, TenantTags=None, TenantedDeploymentParticipation=None, Tenants=None, Thumbprint=None, Uri=None, Authentication=None, AwsAccountAuthentication=None, AzureServicePrincipalAuthentication=None, CertificateAuthentication=None, Container=None, Endpoint=None)
