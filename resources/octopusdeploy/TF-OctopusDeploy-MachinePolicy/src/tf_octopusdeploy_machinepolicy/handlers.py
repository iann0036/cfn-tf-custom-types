...
TYPE_NAME = "TF::OctopusDeploy::MachinePolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_machine_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'octopusdeploy_machine_policy',
                'returnValues': ["Id", "IsDefault"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_machine_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/octopusdeploy_machine_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ConnectionConnectTimeout=None, ConnectionRetryCountLimit=None, ConnectionRetrySleepInterval=None, ConnectionRetryTimeLimit=None, Description=None, Id=None, IsDefault=None, Name=None, PollingRequestMaximumMessageProcessingTimeout=None, PollingRequestQueueTimeout=None, SpaceId=None, MachineCleanupPolicy=None, MachineConnectivityPolicy=None, MachineHealthCheckPolicy=None, MachineUpdatePolicy=None)
