...
TYPE_NAME = "TF::OctopusDeploy::Certificate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_certificate/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'octopusdeploy_certificate',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_certificate/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/octopusdeploy_certificate/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Archived=None, CertificateData=None, CertificateDataFormat=None, Environments=None, HasPrivateKey=None, Id=None, IsExpired=None, IssuerCommonName=None, IssuerDistinguishedName=None, IssuerOrganization=None, Name=None, NotAfter=None, NotBefore=None, Notes=None, Password=None, ReplacedBy=None, SelfSigned=None, SerialNumber=None, SignatureAlgorithmName=None, SubjectAlternativeNames=None, SubjectCommonName=None, SubjectDistinguishedName=None, SubjectOrganization=None, TenantTags=None, TenantedDeploymentParticipation=None, Tenants=None, Thumbprint=None, Version=None)
