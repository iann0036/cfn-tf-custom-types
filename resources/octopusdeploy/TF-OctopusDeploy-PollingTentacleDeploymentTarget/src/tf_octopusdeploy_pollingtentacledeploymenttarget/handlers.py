...
TYPE_NAME = "TF::OctopusDeploy::PollingTentacleDeploymentTarget"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_polling_tentacle_deployment_target/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'octopusdeploy_polling_tentacle_deployment_target',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_polling_tentacle_deployment_target/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/octopusdeploy_polling_tentacle_deployment_target/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CertificateSignatureAlgorithm=None, Environments=None, HasLatestCalamari=None, HealthStatus=None, Id=None, IsDisabled=None, IsInProcess=None, MachinePolicyId=None, Name=None, OperatingSystem=None, Roles=None, ShellName=None, ShellVersion=None, SpaceId=None, Status=None, StatusSummary=None, TenantTags=None, TenantedDeploymentParticipation=None, Tenants=None, TentacleUrl=None, Thumbprint=None, Uri=None, Endpoint=None, TentacleVersionDetails=None)
