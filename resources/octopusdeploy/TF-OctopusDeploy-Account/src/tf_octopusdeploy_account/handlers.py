...
TYPE_NAME = "TF::OctopusDeploy::Account"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_account/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'octopusdeploy_account',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/octopusdeploy_account/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/octopusdeploy_account/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessKey=None, AccountType=None, ActiveDirectoryEndpointBaseUri=None, ApplicationId=None, AuthenticationEndpoint=None, AzureEnvironment=None, CertificateData=None, CertificateThumbprint=None, ClientSecret=None, Description=None, Environments=None, Id=None, Name=None, Password=None, PrivateKeyFile=None, PrivateKeyPassphrase=None, ResourceManagerEndpoint=None, SecretKey=None, ServiceManagementEndpointBaseUri=None, ServiceManagementEndpointSuffix=None, SpaceId=None, SubscriptionId=None, TenantId=None, TenantTags=None, TenantedDeploymentParticipation=None, Tenants=None, Token=None, Username=None)
