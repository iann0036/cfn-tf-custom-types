...
TYPE_NAME = "TF::OpenNebula::VirtualNetwork"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/opennebula_virtual_network/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'OpenNebula/opennebula',
                'providerTypeName': 'opennebula',
                'terraformTypeName': 'opennebula_virtual_network',
                'returnValues': ["Gid", "Gname", "Id", "Uid", "Uname"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/opennebula_virtual_network/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/opennebula_virtual_network/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutomaticVlanId=None, Bridge=None, Clusters=None, Description=None, Dns=None, Gateway=None, Gid=None, Gname=None, Group=None, GuestMtu=None, HoldIps=None, HoldSize=None, Id=None, IpHold=None, Mtu=None, Name=None, NetworkMask=None, Permissions=None, PhysicalDevice=None, ReservationSize=None, ReservationVnet=None, SecurityGroups=None, Tags=None, Type=None, Uid=None, Uname=None, VlanId=None, Ar=None)
