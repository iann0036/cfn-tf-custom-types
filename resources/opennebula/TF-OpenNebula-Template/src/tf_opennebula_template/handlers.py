...
TYPE_NAME = "TF::OpenNebula::Template"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/opennebula_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'opennebula_template',
                'returnValues': ["Gid", "Gname", "Id", "RegTime", "Uid", "Uname"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/opennebula_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/opennebula_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Context=None, Cpu=None, Gid=None, Gname=None, Group=None, Id=None, Memory=None, Name=None, Permissions=None, RegTime=None, Tags=None, Template=None, Uid=None, Uname=None, Vcpu=None, Disk=None, Graphics=None, Nic=None, Os=None, Vmgroup=None)
