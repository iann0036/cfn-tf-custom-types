...
TYPE_NAME = "TF::OpenNebula::VirtualMachine"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/opennebula_virtual_machine/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'opennebula_virtual_machine',
                'returnValues': ["Gid", "Gname", "Id", "Instance", "Ip", "Lcmstate", "State", "Uid", "Uname"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/opennebula_virtual_machine/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/opennebula_virtual_machine/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Context=None, Cpu=None, Gid=None, Gname=None, Group=None, Id=None, Instance=None, Ip=None, Lcmstate=None, Memory=None, Name=None, Pending=None, Permissions=None, State=None, Tags=None, TemplateId=None, Timeout=None, Uid=None, Uname=None, Vcpu=None, Disk=None, Graphics=None, Nic=None, Os=None, Vmgroup=None)
