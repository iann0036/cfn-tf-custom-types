...
TYPE_NAME = "TF::CheckPoint::ManagementSimpleGateway"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/checkpoint_management_simple_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'CheckPointSW/checkpoint',
                'providerTypeName': 'checkpoint',
                'terraformTypeName': 'checkpoint_management_simple_gateway',
                'returnValues': ["DynamicIp", "Hardware", "Id", "SicName", "SicState"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/checkpoint_management_simple_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/checkpoint_management_simple_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AntiBot=None, AntiVirus=None, ApplicationControl=None, Color=None, Comments=None, ContentAwareness=None, DynamicIp=None, Firewall=None, FirewallSettings=None, Hardware=None, IcapServer=None, Id=None, IgnoreWarnings=None, Ips=None, Ipv4Address=None, Ipv6Address=None, LogsSettings=None, Name=None, OneTimePassword=None, OsName=None, SaveLogsLocally=None, SendAlertsToServer=None, SendLogsToBackupServer=None, SendLogsToServer=None, SicName=None, SicState=None, Tags=None, ThreatEmulation=None, ThreatExtraction=None, UrlFiltering=None, Version=None, Vpn=None, VpnSettings=None, Interfaces=None)
