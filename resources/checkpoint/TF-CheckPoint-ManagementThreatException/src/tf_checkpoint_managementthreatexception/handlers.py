...
TYPE_NAME = "TF::CheckPoint::ManagementThreatException"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/checkpoint_management_threat_exception/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'checkpoint_management_threat_exception',
                'returnValues': ["Id", "Owner"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/checkpoint_management_threat_exception/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/checkpoint_management_threat_exception/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, Comments=None, Destination=None, DestinationNegate=None, Enabled=None, ExceptionGroupName=None, ExceptionGroupUid=None, Id=None, IgnoreErrors=None, IgnoreWarnings=None, InstallOn=None, Layer=None, Name=None, Owner=None, Position=None, ProtectedScope=None, ProtectedScopeNegate=None, ProtectionOrSite=None, RuleName=None, RuleUid=None, Service=None, ServiceNegate=None, Source=None, SourceNegate=None, Track=None)
