...
TYPE_NAME = "TF::CheckPoint::ManagementSimpleCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/checkpoint_management_simple_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'checkpoint_management_simple_cluster',
                'returnValues': ["DynamicIp", "Id", "SicName", "SicState"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/checkpoint_management_simple_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/checkpoint_management_simple_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AntiBot=None, AntiVirus=None, ApplicationControl=None, ClusterMode=None, Color=None, Comments=None, ContentAwareness=None, DataAwareness=None, DynamicIp=None, Firewall=None, FirewallSettings=None, Hardware=None, Id=None, IgnoreErrors=None, IgnoreWarnings=None, Ips=None, Ipv4Address=None, Ipv6Address=None, Name=None, OsName=None, SaveLogsLocally=None, SendAlertsToServer=None, SendLogsToBackupServer=None, SendLogsToServer=None, SicName=None, SicState=None, Tags=None, ThreatEmulation=None, UrlFiltering=None, Version=None, Vpn=None, VpnSettings=None, Interfaces=None, Members=None)
