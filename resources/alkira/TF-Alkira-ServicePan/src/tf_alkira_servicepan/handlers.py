...
TYPE_NAME = "TF::Alkira::ServicePan"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/alkira_service_pan/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'alkiranet/alkira',
                'providerTypeName': 'alkira',
                'terraformTypeName': 'alkira_service_pan',
                'returnValues': ["Id", "ServiceId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/alkira_service_pan/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/alkira_service_pan/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BillingTags=None, CredentialId=None, Cxp=None, Group=None, Id=None, LicenseType=None, ManagementSegment=None, MaxInstanceCount=None, MinInstanceCount=None, Name=None, PanoramaDeviceGroup=None, PanoramaEnabled=None, PanoramaIpAddress=None, PanoramaTemplate=None, Segments=None, ServiceId=None, Size=None, Type=None, Version=None, Instance=None, ZonesToGroups=None)
