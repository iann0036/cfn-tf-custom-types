...
TYPE_NAME = "TF::Buildkite::Pipeline"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/buildkite_pipeline/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'buildkite/buildkite',
                'providerTypeName': 'buildkite',
                'terraformTypeName': 'buildkite_pipeline',
                'returnValues': ["Id", "Slug", "WebhookUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/buildkite_pipeline/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/buildkite_pipeline/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BranchConfiguration=None, CancelIntermediateBuilds=None, CancelIntermediateBuildsBranchFilter=None, DefaultBranch=None, Description=None, Id=None, Name=None, Repository=None, SkipIntermediateBuilds=None, SkipIntermediateBuildsBranchFilter=None, Slug=None, Steps=None, Team=None, WebhookUrl=None, ProviderSettings=None)
