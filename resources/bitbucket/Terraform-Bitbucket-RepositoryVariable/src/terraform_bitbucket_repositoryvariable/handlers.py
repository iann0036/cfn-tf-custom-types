...
def create_handler(
    ...
    try:
        ...
        tf['resource']['bitbucket_repository_variable'][request.logicalResourceIdentifier] = {}

        if "bitbucket" == "aws":
            creds = session.credentials
            tf["provider"]["bitbucket"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("bitbucket"))['SecretString']
            tf["provider"]["bitbucket"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['bitbucket_repository_variable'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
