...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'rancher/rancher2',
                'providerTypeName': 'rancher2',
                'terraformTypeName': 'rancher2_cluster',
                'returnValues': ["CaCert", "ClusterRegistrationToken", "DefaultProjectId", "EnableClusterIstio", "Id", "IstioEnabled", "KubeConfig", "SystemProjectId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/rancher2_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotations=None, CaCert=None, ClusterRegistrationToken=None, ClusterTemplateId=None, ClusterTemplateRevisionId=None, DefaultPodSecurityPolicyTemplateId=None, DefaultProjectId=None, Description=None, DesiredAgentImage=None, DesiredAuthImage=None, DockerRootDir=None, Driver=None, EnableClusterAlerting=None, EnableClusterIstio=None, EnableClusterMonitoring=None, EnableNetworkPolicy=None, Id=None, IstioEnabled=None, KubeConfig=None, Labels=None, Name=None, SystemProjectId=None, WindowsPreferedCluster=None, AgentEnvVars=None, AksConfig=None, ClusterAuthEndpoint=None, ClusterMonitoringInput=None, ClusterTemplateAnswers=None, ClusterTemplateQuestions=None, EksConfig=None, EksConfigV2=None, GkeConfig=None, GkeConfigV2=None, K3sConfig=None, OkeConfig=None, Rke2Config=None, RkeConfig=None, ScheduledClusterScan=None, Timeouts=None)
