...
TYPE_NAME = "TF::Rancher2::PodSecurityPolicyTemplate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_pod_security_policy_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'rancher2_pod_security_policy_template',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_pod_security_policy_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/rancher2_pod_security_policy_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowPrivilegeEscalation=None, AllowedCapabilities=None, AllowedProcMountTypes=None, AllowedUnsafeSysctls=None, Annotations=None, DefaultAddCapabilities=None, DefaultAllowPrivilegeEscalation=None, Description=None, ForbiddenSysctls=None, HostIpc=None, HostNetwork=None, HostPid=None, Id=None, Labels=None, Name=None, Privileged=None, ReadOnlyRootFilesystem=None, RequiredDropCapabilities=None, Volumes=None, AllowedCsiDriver=None, AllowedFlexVolume=None, AllowedHostPath=None, FsGroup=None, HostPort=None, RunAsGroup=None, RunAsUser=None, RuntimeClass=None, SeLinux=None, SupplementalGroup=None, Timeouts=None)
