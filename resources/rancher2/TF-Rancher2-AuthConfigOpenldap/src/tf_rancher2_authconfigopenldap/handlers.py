...
TYPE_NAME = "TF::Rancher2::AuthConfigOpenldap"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_auth_config_openldap/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'rancher2_auth_config_openldap',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_auth_config_openldap/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/rancher2_auth_config_openldap/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessMode=None, AllowedPrincipalIds=None, Annotations=None, Certificate=None, ConnectionTimeout=None, Enabled=None, GroupDnAttribute=None, GroupMemberMappingAttribute=None, GroupMemberUserAttribute=None, GroupNameAttribute=None, GroupObjectClass=None, GroupSearchAttribute=None, GroupSearchBase=None, Id=None, Labels=None, Name=None, NestedGroupMembershipEnabled=None, Port=None, Servers=None, ServiceAccountDistinguishedName=None, ServiceAccountPassword=None, TestPassword=None, TestUsername=None, Tls=None, Type=None, UserDisabledBitMask=None, UserEnabledAttribute=None, UserLoginAttribute=None, UserMemberAttribute=None, UserNameAttribute=None, UserObjectClass=None, UserSearchAttribute=None, UserSearchBase=None)
