...
TYPE_NAME = "TF::Rancher2::NodeTemplate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_node_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'rancher2_node_template',
                'returnValues': ["Driver", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/rancher2_node_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/rancher2_node_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotations=None, AuthCertificateAuthority=None, AuthKey=None, CloudCredentialId=None, Description=None, Driver=None, DriverId=None, EngineEnv=None, EngineInsecureRegistry=None, EngineInstallUrl=None, EngineLabel=None, EngineOpt=None, EngineRegistryMirror=None, EngineStorageDriver=None, Id=None, Labels=None, Name=None, UseInternalIpAddress=None, Amazonec2Config=None, AzureConfig=None, DigitaloceanConfig=None, HetznerConfig=None, LinodeConfig=None, NodeTaints=None, OpennebulaConfig=None, OpenstackConfig=None, Timeouts=None, VsphereConfig=None)
