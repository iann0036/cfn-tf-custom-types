...
def create_handler(
    ...
    try:
        ...
        tf['resource']['skytap_label_category'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("skytap"))['SecretString']
            tf["provider"]["skytap"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['skytap_label_category'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
