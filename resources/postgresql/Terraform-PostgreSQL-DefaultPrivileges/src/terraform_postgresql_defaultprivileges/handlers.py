...
def create_handler(
    ...
    try:
        ...
        tf['resource']['postgresql_default_privileges'][request.logicalResourceIdentifier] = {}

        if "postgresql" == "aws":
            creds = session.credentials
            tf["provider"]["postgresql"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("postgresql"))['SecretString']
            tf["provider"]["postgresql"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['postgresql_default_privileges'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
