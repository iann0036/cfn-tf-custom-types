...
TYPE_NAME = "TF::Artifactory::LocalRepository"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/artifactory_local_repository/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'artifactory_local_repository',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/artifactory_local_repository/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/artifactory_local_repository/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ArchiveBrowsingEnabled=None, BlackedOut=None, CalculateYumMetadata=None, ChecksumPolicyType=None, DebianTrivialLayout=None, Description=None, DockerApiVersion=None, EnableFileListsIndexing=None, ExcludesPattern=None, ForceNugetAuthentication=None, HandleReleases=None, HandleSnapshots=None, Id=None, IncludesPattern=None, Key=None, MaxUniqueSnapshots=None, MaxUniqueTags=None, Notes=None, PackageType=None, PropertySets=None, RepoLayoutRef=None, SnapshotVersionBehavior=None, SuppressPomConsistencyChecks=None, XrayIndex=None, YumRootDepth=None)
