...
TYPE_NAME = "TF::Artifactory::RemoteRepository"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/artifactory_remote_repository/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'jfrog/artifactory',
                'providerTypeName': 'artifactory',
                'terraformTypeName': 'artifactory_remote_repository',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/artifactory_remote_repository/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/artifactory_remote_repository/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowAnyHostAuth=None, BlackedOut=None, BlockMismatchingMimeTypes=None, BowerRegistryUrl=None, BypassHeadRequests=None, ClientTlsCertificate=None, Description=None, DownloadContextPath=None, EnableCookieManagement=None, EnableTokenAuthentication=None, ExcludesPattern=None, FeedContextPath=None, FetchJarsEagerly=None, FetchSourcesEagerly=None, ForceNugetAuthentication=None, HandleReleases=None, HandleSnapshots=None, HardFail=None, Id=None, IncludesPattern=None, Key=None, LocalAddress=None, MaxUniqueSnapshots=None, MissedCachePeriodSeconds=None, Notes=None, Offline=None, PackageType=None, Password=None, PropagateQueryParams=None, PropertySets=None, Proxy=None, PypiRegistryUrl=None, RemoteRepoChecksumPolicyType=None, RepoLayoutRef=None, RetrievalCachePeriodSeconds=None, ShareConfiguration=None, SocketTimeoutMillis=None, StoreArtifactsLocally=None, SuppressPomConsistencyChecks=None, SynchronizeProperties=None, UnusedArtifactsCleanupPeriodHours=None, Url=None, Username=None, V3FeedUrl=None, VcsGitDownloadUrl=None, VcsGitProvider=None, VcsType=None, XrayIndex=None, ContentSynchronisation=None, Nuget=None)
