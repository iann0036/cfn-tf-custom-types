...
TYPE_NAME = "TF::Artifactory::VirtualRepository"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/artifactory_virtual_repository/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'artifactory_virtual_repository',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/artifactory_virtual_repository/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/artifactory_virtual_repository/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ArtifactoryRequestsCanRetrieveRemoteArtifacts=None, DebianTrivialLayout=None, DefaultDeploymentRepo=None, Description=None, ExcludesPattern=None, ForceNugetAuthentication=None, Id=None, IncludesPattern=None, Key=None, KeyPair=None, Notes=None, PackageType=None, PomRepositoryReferencesCleanupPolicy=None, RepoLayoutRef=None, Repositories=None)
