...
TYPE_NAME = "TF::NSXT::PolicyLbVirtualServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_lb_virtual_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'nsxt_policy_lb_virtual_server',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_lb_virtual_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nsxt_policy_lb_virtual_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessLogEnabled=None, ApplicationProfilePath=None, DefaultPoolMemberPorts=None, Description=None, DisplayName=None, Enabled=None, Id=None, IpAddress=None, LogSignificantEventOnly=None, MaxConcurrentConnections=None, MaxNewConnectionRate=None, NsxId=None, Path=None, PersistenceProfilePath=None, PoolPath=None, Ports=None, Revision=None, ServicePath=None, SorryPoolPath=None, AccessListControl=None, ClientSsl=None, ServerSsl=None, Tag=None)
