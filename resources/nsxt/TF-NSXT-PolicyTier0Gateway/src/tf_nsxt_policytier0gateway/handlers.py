...
TYPE_NAME = "TF::NSXT::PolicyTier0Gateway"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_tier0_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'vmware/nsxt',
                'providerTypeName': 'nsxt',
                'terraformTypeName': 'nsxt_policy_tier0_gateway',
                'returnValues': ["Id", "Path", "RedistributionSet", "Revision"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_tier0_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nsxt_policy_tier0_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DefaultRuleLogging=None, Description=None, DhcpConfigPath=None, DisplayName=None, EdgeClusterPath=None, EnableFirewall=None, FailoverMode=None, ForceWhitelisting=None, HaMode=None, Id=None, InternalTransitSubnets=None, Ipv6DadProfilePath=None, Ipv6NdraProfilePath=None, NsxId=None, Path=None, RdAdminAddress=None, RedistributionSet=None, Revision=None, TransitSubnets=None, BgpConfig=None, IntersiteConfig=None, LocaleService=None, RedistributionConfig=None, Tag=None, VrfConfig=None)
