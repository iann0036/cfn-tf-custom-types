...
TYPE_NAME = "TF::NSXT::PolicyTier0GatewayInterface"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_tier0_gateway_interface/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'nsxt_policy_tier0_gateway_interface',
                'returnValues': ["Id", "IpAddresses", "LocaleServiceId", "Path", "Revision"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_tier0_gateway_interface/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nsxt_policy_tier0_gateway_interface/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessVlanId=None, Description=None, DisplayName=None, EdgeNodePath=None, EnablePim=None, GatewayPath=None, Id=None, IpAddresses=None, Ipv6NdraProfilePath=None, LocaleServiceId=None, Mtu=None, NsxId=None, Path=None, Revision=None, SegmentPath=None, SitePath=None, Subnets=None, Type=None, UrpfMode=None, Ospf=None, Tag=None)
