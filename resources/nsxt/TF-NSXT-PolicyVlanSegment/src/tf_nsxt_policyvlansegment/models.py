...
class ResourceModel(BaseModel):
    ...
    DiscoveryProfile: Optional[Sequence["_DiscoveryProfileDefinition"]]
    L2Extension: Optional[Sequence["_L2ExtensionDefinition"]]
    QosProfile: Optional[Sequence["_QosProfileDefinition"]]
    SecurityProfile: Optional[Sequence["_SecurityProfileDefinition"]]
    ...
    def _deserialize(
        ...
        return cls(
            ...
            DiscoveryProfile=deserialize_list(json_data.get("DiscoveryProfile"), DiscoveryProfileDefinition),
            L2Extension=deserialize_list(json_data.get("L2Extension"), L2ExtensionDefinition),
            QosProfile=deserialize_list(json_data.get("QosProfile"), QosProfileDefinition),
            SecurityProfile=deserialize_list(json_data.get("SecurityProfile"), SecurityProfileDefinition),
...
@dataclass
class DiscoveryProfileDefinition(BaseModel):
    BindingMapPath: Optional[str]
    IpDiscoveryProfilePath: Optional[str]
    MacDiscoveryProfilePath: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_DiscoveryProfileDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_DiscoveryProfileDefinition"]:
        if not json_data:
            return None
        return cls(
            BindingMapPath=json_data.get("BindingMapPath"),
            IpDiscoveryProfilePath=json_data.get("IpDiscoveryProfilePath"),
            MacDiscoveryProfilePath=json_data.get("MacDiscoveryProfilePath"),
        )


# work around possible type aliasing issues when variable has same name as a model
_DiscoveryProfileDefinition = DiscoveryProfileDefinition
...
@dataclass
class QosProfileDefinition(BaseModel):
    BindingMapPath: Optional[str]
    QosProfilePath: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_QosProfileDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_QosProfileDefinition"]:
        if not json_data:
            return None
        return cls(
            BindingMapPath=json_data.get("BindingMapPath"),
            QosProfilePath=json_data.get("QosProfilePath"),
        )


# work around possible type aliasing issues when variable has same name as a model
_QosProfileDefinition = QosProfileDefinition


@dataclass
class SecurityProfileDefinition(BaseModel):
    BindingMapPath: Optional[str]
    SecurityProfilePath: Optional[str]
    SpoofguardProfilePath: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_SecurityProfileDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_SecurityProfileDefinition"]:
        if not json_data:
            return None
        return cls(
            BindingMapPath=json_data.get("BindingMapPath"),
            SecurityProfilePath=json_data.get("SecurityProfilePath"),
            SpoofguardProfilePath=json_data.get("SpoofguardProfilePath"),
        )


# work around possible type aliasing issues when variable has same name as a model
_SecurityProfileDefinition = SecurityProfileDefinition
