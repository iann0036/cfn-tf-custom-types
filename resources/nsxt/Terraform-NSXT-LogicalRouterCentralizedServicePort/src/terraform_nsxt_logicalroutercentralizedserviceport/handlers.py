...
def create_handler(
    ...
    try:
        ...
        tf['resource']['nsxt_logical_router_centralized_service_port'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("nsxt"))['SecretString']
            tf["provider"]["nsxt"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['nsxt_logical_router_centralized_service_port'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
