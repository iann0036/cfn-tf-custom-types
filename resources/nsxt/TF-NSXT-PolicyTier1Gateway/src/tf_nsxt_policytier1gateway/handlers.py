...
TYPE_NAME = "TF::NSXT::PolicyTier1Gateway"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_tier1_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'nsxt_policy_tier1_gateway',
                'returnValues': ["Id", "Path", "Revision"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nsxt_policy_tier1_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nsxt_policy_tier1_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DefaultRuleLogging=None, Description=None, DhcpConfigPath=None, DisplayName=None, EdgeClusterPath=None, EgressQosProfilePath=None, EnableFirewall=None, EnableStandbyRelocation=None, FailoverMode=None, ForceWhitelisting=None, Id=None, IngressQosProfilePath=None, Ipv6DadProfilePath=None, Ipv6NdraProfilePath=None, NsxId=None, Path=None, PoolAllocation=None, Revision=None, RouteAdvertisementTypes=None, Tier0Path=None, IntersiteConfig=None, LocaleService=None, RouteAdvertisementRule=None, Tag=None)
