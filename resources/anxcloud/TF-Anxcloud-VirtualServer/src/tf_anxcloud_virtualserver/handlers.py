...
TYPE_NAME = "TF::Anxcloud::VirtualServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/anxcloud_virtual_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'anexia-it/anxcloud',
                'providerTypeName': 'anxcloud',
                'terraformTypeName': 'anxcloud_virtual_server',
                'returnValues': ["Id", "Info"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/anxcloud_virtual_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/anxcloud_virtual_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BootDelay=None, CpuPerformanceType=None, Cpus=None, CriticalOperationConfirmed=None, Dns=None, EnterBiosSetup=None, ForceRestartIfNeeded=None, Hostname=None, Id=None, Info=None, LocationId=None, Memory=None, Password=None, Script=None, Sockets=None, SshKey=None, Tags=None, TemplateId=None, TemplateType=None, Disk=None, Network=None, Timeouts=None)
