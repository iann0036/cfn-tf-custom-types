...
def create_handler(
    ...
    try:
        ...
        tf['resource']['kubernetes_horizontal_pod_autoscaler'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("kubernetes"))['SecretString']
            tf["provider"]["kubernetes"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['kubernetes_horizontal_pod_autoscaler'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
