...
def create_handler(
    ...
    try:
        ...
        tf['resource']['template_cloudinit_config'][request.logicalResourceIdentifier] = {}

        if "template" == "aws":
            creds = session.credentials
            tf["provider"]["template"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("template"))['SecretString']
            tf["provider"]["template"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['template_cloudinit_config'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
