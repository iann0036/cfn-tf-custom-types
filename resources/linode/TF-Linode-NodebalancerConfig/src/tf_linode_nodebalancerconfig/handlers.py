...
TYPE_NAME = "TF::Linode::NodebalancerConfig"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/linode_nodebalancer_config/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/linode_nodebalancer_config/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/linode_nodebalancer_config/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Algorithm=None, Check=None, CheckAttempts=None, CheckBody=None, CheckInterval=None, CheckPassive=None, CheckPath=None, CheckTimeout=None, CipherSuite=None, Id=None, NodeStatus=None, NodebalancerId=None, Port=None, Protocol=None, ProxyProtocol=None, SslCert=None, SslCommonname=None, SslFingerprint=None, SslKey=None, Stickiness=None)
