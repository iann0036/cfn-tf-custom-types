...
def create_handler(
    ...
    try:
        ...
        tf['resource']['linode_object_storage_bucket'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("linode"))['SecretString']
            tf["provider"]["linode"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['linode_object_storage_bucket'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
