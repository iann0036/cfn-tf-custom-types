...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/linode_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'linode/linode',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/linode_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/linode_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthorizedKeys=None, AuthorizedUsers=None, BackupId=None, Backups=None, BackupsEnabled=None, BootConfigLabel=None, Group=None, Id=None, Image=None, IpAddress=None, Ipv4=None, Ipv6=None, Label=None, PrivateIp=None, PrivateIpAddress=None, Region=None, RootPass=None, Specs=None, StackscriptData=None, StackscriptId=None, Status=None, SwapSize=None, Tags=None, Type=None, WatchdogEnabled=None, Alerts=None, Config=None, Disk=None, Interface=None, Timeouts=None)
