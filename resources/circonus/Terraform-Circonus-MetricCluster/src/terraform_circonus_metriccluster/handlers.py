...
def create_handler(
    ...
    try:
        ...
        tf['resource']['circonus_metric_cluster'][request.logicalResourceIdentifier] = {}

        if "circonus" == "aws":
            creds = session.credentials
            tf["provider"]["circonus"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("circonus"))['SecretString']
            tf["provider"]["circonus"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['circonus_metric_cluster'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
