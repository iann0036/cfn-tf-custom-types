...
TYPE_NAME = "TF::Circonus::ContactGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/circonus_contact_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/circonus_contact_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/circonus_contact_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AggregationWindow=None, AlwaysSendClear=None, GroupType=None, Id=None, LastModified=None, LastModifiedBy=None, LongMessage=None, LongSubject=None, LongSummary=None, Name=None, ShortMessage=None, ShortSummary=None, Tags=None, AlertOption=None, Email=None, Http=None, PagerDuty=None, Slack=None, Sms=None, Victorops=None, Xmpp=None)
