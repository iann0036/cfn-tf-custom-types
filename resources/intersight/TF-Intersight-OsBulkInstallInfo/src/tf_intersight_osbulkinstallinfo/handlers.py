...
TYPE_NAME = "TF::Intersight::OsBulkInstallInfo"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_os_bulk_install_info/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_os_bulk_install_info',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_os_bulk_install_info/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_os_bulk_install_info/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, ClassId=None, ConfigurationFile=None, CreateTime=None, DomainGroupMoid=None, FileContent=None, GlobalConfig=None, Id=None, IsFileContentSet=None, ModTime=None, Moid=None, Name=None, ObjectType=None, OperState=None, Organization=None, OsImage=None, Owners=None, Parent=None, PermissionResources=None, ScuImage=None, ServerConfigs=None, Servers=None, SharedScope=None, Tags=None, ValidationInfos=None, VersionContext=None)
