...
TYPE_NAME = "TF::Intersight::VirtualizationVirtualMachine"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_virtualization_virtual_machine/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_virtualization_virtual_machine',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_virtualization_virtual_machine/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_virtualization_virtual_machine/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, Action=None, ActionInfo=None, AdditionalProperties=None, AffinitySelectors=None, Ancestors=None, AntiAffinitySelectors=None, ClassId=None, CloudInitConfig=None, Cluster=None, ClusterEsxi=None, Cpu=None, CreateTime=None, Discovered=None, Disk=None, DomainGroupMoid=None, ForceDelete=None, GuestOs=None, Host=None, HostEsxi=None, HypervisorType=None, Id=None, Interfaces=None, Inventory=None, Labels=None, Memory=None, ModTime=None, Moid=None, Name=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, PowerState=None, ProvisionType=None, RegisteredDevice=None, SharedScope=None, Tags=None, VersionContext=None, VmConfig=None, WaitForCompletion=None, WorkflowInfo=None)
