...
TYPE_NAME = "TF::Intersight::SoftwarerepositoryCategorySupportConstraint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_softwarerepository_category_support_constraint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_softwarerepository_category_support_constraint',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_softwarerepository_category_support_constraint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_softwarerepository_category_support_constraint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, ClassId=None, ConstraintId=None, CreateTime=None, DomainGroupMoid=None, FilteredModels=None, Id=None, MdfId=None, MinVersion=None, ModTime=None, Moid=None, Name=None, ObjectType=None, Owners=None, Parent=None, ParseFromImageName=None, PermissionResources=None, SharedScope=None, SupportedModels=None, Tags=None, VersionContext=None)
