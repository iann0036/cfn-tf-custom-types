...
TYPE_NAME = "TF::Intersight::TechsupportmanagementCollectionControlPolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_techsupportmanagement_collection_control_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_techsupportmanagement_collection_control_policy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_techsupportmanagement_collection_control_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_techsupportmanagement_collection_control_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Account=None, AccountMoid=None, AdditionalProperties=None, Ancestors=None, ClassId=None, CreateTime=None, DeploymentType=None, DomainGroupMoid=None, Id=None, ModTime=None, Moid=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, SharedScope=None, Tags=None, TechSupportCollection=None, VersionContext=None)
