...
TYPE_NAME = "TF::Intersight::WorkflowWorkflowDefinition"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_workflow_workflow_definition/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_workflow_workflow_definition',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_workflow_workflow_definition/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_workflow_workflow_definition/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Catalog=None, ClassId=None, ClonedFrom=None, CreateTime=None, DefaultVersion=None, Description=None, DomainGroupMoid=None, Id=None, InputDefinition=None, InputParameterSet=None, Label=None, LicenseEntitlement=None, MaxTaskCount=None, MaxWorkerTaskCount=None, ModTime=None, Moid=None, Name=None, NrVersion=None, ObjectType=None, OutputDefinition=None, OutputParameters=None, Owners=None, Parent=None, PermissionResources=None, Properties=None, SharedScope=None, Tags=None, Tasks=None, UiInputFilters=None, UiRenderingData=None, ValidationInformation=None, VersionContext=None, WorkflowMetadata=None)
