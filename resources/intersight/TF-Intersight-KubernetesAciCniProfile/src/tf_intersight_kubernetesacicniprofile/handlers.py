...
TYPE_NAME = "TF::Intersight::KubernetesAciCniProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_kubernetes_aci_cni_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_kubernetes_aci_cni_profile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_kubernetes_aci_cni_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_kubernetes_aci_cni_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AaepName=None, AccountMoid=None, AdditionalProperties=None, Ancestors=None, ClassId=None, ClusterAciAllocations=None, ClusterProfiles=None, CreateTime=None, Description=None, DomainGroupMoid=None, ExtSvcDynSubnetStart=None, ExtSvcStaticSubnetStart=None, Id=None, InfraVlanId=None, L3OutNetworkName=None, L3OutPolicyName=None, L3OutTenant=None, ModTime=None, Moid=None, Name=None, NestedVmmDomain=None, NodeSvcSubnetStart=None, NodeVlanRangeEnd=None, NodeVlanRangeStart=None, NumberOfKubernetesClusters=None, ObjectType=None, OpflexMulticastAddressRange=None, Organization=None, Owners=None, Parent=None, PermissionResources=None, PodSubnetStart=None, RegisteredDevice=None, SharedScope=None, SrcTemplate=None, SvcSubnetStart=None, Tags=None, Type=None, VersionContext=None, Vrf=None)
