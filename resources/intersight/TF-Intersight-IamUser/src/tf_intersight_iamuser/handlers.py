...
TYPE_NAME = "TF::Intersight::IamUser"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_iam_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_iam_user',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_iam_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_iam_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, ApiKeys=None, AppRegistrations=None, ClassId=None, ClientIpAddress=None, CreateTime=None, DomainGroupMoid=None, Email=None, FirstName=None, Id=None, Idp=None, Idpreference=None, LastLoginTime=None, LastName=None, LastRoleModifiedTime=None, LocalUserPassword=None, ModTime=None, Moid=None, Name=None, OauthTokens=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, Permissions=None, Sessions=None, SharedScope=None, Tags=None, UserIdOrEmail=None, UserType=None, UserUniqueIdentifier=None, VersionContext=None)
