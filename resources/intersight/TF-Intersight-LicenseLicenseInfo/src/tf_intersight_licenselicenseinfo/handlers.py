...
TYPE_NAME = "TF::Intersight::LicenseLicenseInfo"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_license_license_info/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_license_license_info',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_license_license_info/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_license_license_info/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountLicenseData=None, AccountMoid=None, ActiveAdmin=None, AdditionalProperties=None, Ancestors=None, ClassId=None, CreateTime=None, DaysLeft=None, DomainGroupMoid=None, EndTime=None, EnforceMode=None, ErrorDesc=None, EvaluationPeriod=None, ExtraEvaluation=None, Id=None, LicenseCount=None, LicenseState=None, LicenseType=None, ModTime=None, Moid=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, SharedScope=None, StartTime=None, Tags=None, TrialAdmin=None, VersionContext=None)
