...
TYPE_NAME = "TF::Intersight::SoftwarerepositoryOperatingSystemFile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_softwarerepository_operating_system_file/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_softwarerepository_operating_system_file',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_softwarerepository_operating_system_file/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_softwarerepository_operating_system_file/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Catalog=None, ClassId=None, CreateTime=None, Description=None, DomainGroupMoid=None, DownloadCount=None, Id=None, ImportAction=None, ImportState=None, ImportedTime=None, LastAccessTime=None, Md5eTag=None, Md5sum=None, ModTime=None, Moid=None, Name=None, NrSource=None, NrVersion=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, ReleaseDate=None, Sha512sum=None, SharedScope=None, Size=None, SoftwareAdvisoryUrl=None, Tags=None, Vendor=None, VersionContext=None)
