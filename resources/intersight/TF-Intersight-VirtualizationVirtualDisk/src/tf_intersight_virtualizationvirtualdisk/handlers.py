...
TYPE_NAME = "TF::Intersight::VirtualizationVirtualDisk"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_virtualization_virtual_disk/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_virtualization_virtual_disk',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_virtualization_virtual_disk/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_virtualization_virtual_disk/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Capacity=None, ClassId=None, Cluster=None, CreateTime=None, Discovered=None, DomainGroupMoid=None, Id=None, Inventory=None, ModTime=None, Mode=None, Moid=None, Name=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, RegisteredDevice=None, SharedScope=None, SourceCerts=None, SourceDiskToClone=None, SourceFilePath=None, Tags=None, VersionContext=None, WaitForCompletion=None, WorkflowInfo=None)
