...
TYPE_NAME = "TF::Intersight::SoftwarerepositoryCategoryMapper"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_softwarerepository_category_mapper/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_softwarerepository_category_mapper',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_softwarerepository_category_mapper/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_softwarerepository_category_mapper/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Category=None, ClassId=None, CreateTime=None, DomainGroupMoid=None, FileType=None, Id=None, ImageType=None, MdfId=None, ModTime=None, Moid=None, Name=None, NrSource=None, NrVersion=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, RegexPattern=None, SharedScope=None, SupportedModels=None, SwId=None, TagTypes=None, Tags=None, VersionContext=None)
