...
TYPE_NAME = "TF::Intersight::CapabilityEquipmentSlotArray"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_capability_equipment_slot_array/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_capability_equipment_slot_array',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_capability_equipment_slot_array/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_capability_equipment_slot_array/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, ClassId=None, CreateTime=None, DomainGroupMoid=None, FirstIndex=None, Height=None, HorizontalStartOffset=None, Id=None, InlineGroupSeparation=None, InlineGroupSize=None, InlineOffset=None, Location=None, ModTime=None, Moid=None, Name=None, NumberOfSlots=None, ObjectType=None, Orientation=None, Owners=None, Parent=None, PermissionResources=None, Pid=None, Selector=None, SharedScope=None, Sku=None, SlotsPerLine=None, Tags=None, TransverseGroupSeparation=None, TransverseGroupSize=None, TransverseOffset=None, VersionContext=None, VerticalStartOffset=None, Vid=None, Width=None)
