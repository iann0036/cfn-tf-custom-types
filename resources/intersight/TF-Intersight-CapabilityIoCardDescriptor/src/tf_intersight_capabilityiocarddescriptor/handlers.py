...
TYPE_NAME = "TF::Intersight::CapabilityIoCardDescriptor"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_capability_io_card_descriptor/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_capability_io_card_descriptor',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_capability_io_card_descriptor/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_capability_io_card_descriptor/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Capabilities=None, ClassId=None, CreateTime=None, Description=None, DomainGroupMoid=None, Id=None, ModTime=None, Model=None, Moid=None, NrVersion=None, NumHifPorts=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, Revision=None, SharedScope=None, Tags=None, UifConnectivity=None, Vendor=None, VersionContext=None)
