...
TYPE_NAME = "TF::Intersight::CapabilitySwitchCapability"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_capability_switch_capability/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_capability_switch_capability',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_capability_switch_capability/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_capability_switch_capability/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, ClassId=None, CreateTime=None, DefaultFcoeVlan=None, DomainGroupMoid=None, DynamicVifsSupported=None, FanModulesSupported=None, FcEndHostModeReservedVsans=None, FcUplinkPortsAutoNegotiationSupported=None, Id=None, LocatorBeaconSupported=None, MaxPorts=None, MaxSlots=None, ModTime=None, Moid=None, Name=None, NetworkLimits=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, Pid=None, PortsSupporting100gSpeed=None, PortsSupporting10gSpeed=None, PortsSupporting1gSpeed=None, PortsSupporting25gSpeed=None, PortsSupporting40gSpeed=None, PortsSupportingBreakout=None, PortsSupportingFcoe=None, PortsSupportingServerRole=None, ReservedVsans=None, SerenoNetflowSupported=None, SharedScope=None, Sku=None, StorageLimits=None, SwitchingModeCapabilities=None, SystemLimits=None, Tags=None, UnifiedPorts=None, UnifiedRule=None, VersionContext=None, Vid=None)
