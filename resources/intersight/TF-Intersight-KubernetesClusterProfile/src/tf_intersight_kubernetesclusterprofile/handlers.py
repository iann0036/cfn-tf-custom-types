...
TYPE_NAME = "TF::Intersight::KubernetesClusterProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_kubernetes_cluster_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_kubernetes_cluster_profile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_kubernetes_cluster_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_kubernetes_cluster_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AciCniProfile=None, Action=None, ActionInfo=None, AdditionalProperties=None, Ancestors=None, AssociatedCluster=None, CertConfig=None, ClassId=None, ClusterIpPools=None, ConfigContext=None, ContainerRuntimeConfig=None, CreateTime=None, Description=None, DomainGroupMoid=None, EssentialAddons=None, Id=None, KubeConfig=None, LoadbalancerIpLeases=None, ManagedMode=None, ManagementConfig=None, MasterVipLease=None, ModTime=None, Moid=None, Name=None, NetConfig=None, NodeGroups=None, ObjectType=None, Organization=None, Owners=None, Parent=None, PermissionResources=None, PolicyBucket=None, SharedScope=None, SrcTemplate=None, Status=None, SysConfig=None, Tags=None, TrustedRegistries=None, Type=None, VersionContext=None, WaitForCompletion=None, WorkflowInfo=None)
