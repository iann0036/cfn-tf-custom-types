...
TYPE_NAME = "TF::Intersight::HyperflexHealthCheckDefinition"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_hyperflex_health_check_definition/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_hyperflex_health_check_definition',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_hyperflex_health_check_definition/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_hyperflex_health_check_definition/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Category=None, ClassId=None, CommonCauses=None, Configuration=None, CreateTime=None, DefaultHealthCheckScriptInfo=None, Description=None, DomainGroupMoid=None, HealthCheckScriptInfos=None, HealthImpact=None, Id=None, InternalName=None, MinimumHyperFlexVersion=None, ModTime=None, Moid=None, Name=None, ObjectType=None, Owners=None, Parent=None, PermissionResources=None, Reference=None, Resolution=None, ScriptExecutionMode=None, ScriptExecutionOnComputeNodes=None, SharedScope=None, Tags=None, TargetExecutionType=None, Timeout=None, UnsupportedHyperFlexVersions=None, VersionContext=None)
