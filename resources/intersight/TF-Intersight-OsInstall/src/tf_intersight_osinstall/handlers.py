...
TYPE_NAME = "TF::Intersight::OsInstall"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_os_install/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_os_install',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_os_install/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_os_install/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalParameters=None, AdditionalProperties=None, Ancestors=None, Answers=None, ClassId=None, ConfigurationFile=None, CreateTime=None, Description=None, DomainGroupMoid=None, ErrorMsg=None, Id=None, Image=None, InstallMethod=None, InstallTarget=None, ModTime=None, Moid=None, Name=None, ObjectType=None, OperState=None, OperatingSystemParameters=None, Organization=None, OsduImage=None, Owners=None, Parent=None, PermissionResources=None, Server=None, SharedScope=None, Tags=None, VersionContext=None, WaitForCompletion=None, WorkflowInfo=None)
