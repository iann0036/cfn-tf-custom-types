...
TYPE_NAME = "TF::Intersight::VnicEthIf"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_vnic_eth_if/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'intersight_vnic_eth_if',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/intersight_vnic_eth_if/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/intersight_vnic_eth_if/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountMoid=None, AdditionalProperties=None, Ancestors=None, Cdn=None, ClassId=None, CreateTime=None, DomainGroupMoid=None, EthAdapterPolicy=None, EthNetworkPolicy=None, EthQosPolicy=None, FabricEthNetworkControlPolicy=None, FabricEthNetworkGroupPolicy=None, FailoverEnabled=None, Id=None, IpLease=None, IscsiBootPolicy=None, IscsiIpV4AddressAllocationType=None, IscsiIpV4Config=None, IscsiIpv4Address=None, LanConnectivityPolicy=None, LcpVnic=None, MacAddress=None, MacAddressType=None, MacLease=None, MacPool=None, ModTime=None, Moid=None, Name=None, ObjectType=None, Order=None, Owners=None, Parent=None, PermissionResources=None, Placement=None, Profile=None, SharedScope=None, SpVnics=None, StandbyVifId=None, StaticMacAddress=None, Tags=None, UsnicSettings=None, VersionContext=None, VifId=None, VmqSettings=None)
