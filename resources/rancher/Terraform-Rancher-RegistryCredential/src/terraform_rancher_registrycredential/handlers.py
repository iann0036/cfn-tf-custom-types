...
def create_handler(
    ...
    try:
        ...
        tf['resource']['rancher_registry_credential'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("rancher"))['SecretString']
            tf["provider"]["rancher"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['rancher_registry_credential'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
