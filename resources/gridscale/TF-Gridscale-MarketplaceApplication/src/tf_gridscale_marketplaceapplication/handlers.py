...
TYPE_NAME = "TF::Gridscale::MarketplaceApplication"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_marketplace_application/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'gridscale_marketplace_application',
                'returnValues': ["ChangeTime", "CreateTime", "Id", "IsApplicationOwner", "IsPublishGlobal", "IsPublishGlobalRequested", "IsPublishRequested", "IsPublished", "PublishGlobalRequestedDate", "PublishRequestedDate", "PublishedDate", "PublishedGlobalDate", "Status", "Type", "UniqueHash"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_marketplace_application/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gridscale_marketplace_application/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Category=None, ChangeTime=None, CreateTime=None, Id=None, IsApplicationOwner=None, IsPublishGlobal=None, IsPublishGlobalRequested=None, IsPublishRequested=None, IsPublished=None, MetaAdvices=None, MetaAuthor=None, MetaComponents=None, MetaFeatures=None, MetaHints=None, MetaIcon=None, MetaLicense=None, MetaOs=None, MetaOverview=None, MetaTermsOfUse=None, Name=None, ObjectStoragePath=None, Publish=None, PublishGlobalRequestedDate=None, PublishRequestedDate=None, PublishedDate=None, PublishedGlobalDate=None, SetupCores=None, SetupMemory=None, SetupStorageCapacity=None, Status=None, Type=None, UniqueHash=None, Timeouts=None)
