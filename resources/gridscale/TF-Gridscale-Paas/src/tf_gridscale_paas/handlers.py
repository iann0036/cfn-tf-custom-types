...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_paas/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'gridscale_paas',
                'returnValues': ["ChangeTime", "CreateTime", "CurrentPrice", "Id", "Kubeconfig", "ListenPort", "NetworkUuid", "Password", "ServiceTemplateUuidComputed", "Status", "UsageInMinute", "Username"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_paas/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gridscale_paas/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ChangeTime=None, CreateTime=None, CurrentPrice=None, Id=None, Kubeconfig=None, Labels=None, ListenPort=None, Name=None, NetworkUuid=None, Password=None, SecurityZoneUuid=None, ServiceTemplateUuid=None, ServiceTemplateUuidComputed=None, Status=None, UsageInMinute=None, Username=None, Parameter=None, ResourceLimit=None, Timeouts=None)
