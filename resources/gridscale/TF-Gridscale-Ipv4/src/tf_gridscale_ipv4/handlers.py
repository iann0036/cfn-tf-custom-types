...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_ipv4/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'gridscale_ipv4',
                'returnValues': ["ChangeTime", "CreateTime", "CurrentPrice", "DeleteBlock", "Id", "Ip", "LocationCountry", "LocationIata", "LocationName", "LocationUuid", "Prefix", "Status", "UsageInMinutes"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_ipv4/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gridscale_ipv4/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ChangeTime=None, CreateTime=None, CurrentPrice=None, DeleteBlock=None, Failover=None, Id=None, Ip=None, Labels=None, LocationCountry=None, LocationIata=None, LocationName=None, LocationUuid=None, Name=None, Prefix=None, ReverseDns=None, Status=None, UsageInMinutes=None, Timeouts=None)
