...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'gridscale_server',
                'returnValues': ["ChangeTime", "ConsoleToken", "CreateTime", "CurrentPrice", "Id", "Legacy", "LocationUuid", "Status", "UsageInMinutesCores", "UsageInMinutesMemory"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gridscale_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRecovery=None, AvailabilityZone=None, ChangeTime=None, ConsoleToken=None, Cores=None, CreateTime=None, CurrentPrice=None, HardwareProfile=None, Id=None, Ipv4=None, Ipv6=None, Isoimage=None, Labels=None, Legacy=None, LocationUuid=None, Memory=None, Name=None, Power=None, Status=None, UsageInMinutesCores=None, UsageInMinutesMemory=None, Network=None, Storage=None, Timeouts=None)
