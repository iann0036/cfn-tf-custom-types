...
class ResourceModel(BaseModel):
    ...
    S3Backup: Optional[Sequence["_S3BackupDefinition"]]
    ...
    def _deserialize(
        ...
        return cls(
            ...
            S3Backup=deserialize_list(json_data.get("S3Backup"), S3BackupDefinition),
...
@dataclass
class S3BackupDefinition(BaseModel):
    BackupAccessKey: Optional[str]
    BackupBucket: Optional[str]
    BackupSecretKey: Optional[str]
    BackupServerUrl: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_S3BackupDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_S3BackupDefinition"]:
        if not json_data:
            return None
        return cls(
            BackupAccessKey=json_data.get("BackupAccessKey"),
            BackupBucket=json_data.get("BackupBucket"),
            BackupSecretKey=json_data.get("BackupSecretKey"),
            BackupServerUrl=json_data.get("BackupServerUrl"),
        )


# work around possible type aliasing issues when variable has same name as a model
_S3BackupDefinition = S3BackupDefinition
