...
TYPE_NAME = "TF::Gridscale::Firewall"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_firewall/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'gridscale_firewall',
                'returnValues': ["ChangeTime", "CreateTime", "Description", "Id", "LocationName", "Network", "Private", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gridscale_firewall/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gridscale_firewall/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ChangeTime=None, CreateTime=None, Description=None, Id=None, Labels=None, LocationName=None, Name=None, Network=None, Private=None, Status=None, RulesV4In=None, RulesV4Out=None, RulesV6In=None, RulesV6Out=None, Timeouts=None)
