...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_vrf/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_vrf/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_vrf/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, BdEnforcedEnable=None, Description=None, Id=None, IpDataPlaneLearning=None, KnwMcastAct=None, Name=None, NameAlias=None, PcEnfDir=None, PcEnfPref=None, RelationFvRsBgpCtxPol=None, RelationFvRsCtxMcastTo=None, RelationFvRsCtxMonPol=None, RelationFvRsCtxToEpRet=None, RelationFvRsCtxToExtRouteTagPol=None, RelationFvRsOspfCtxPol=None, RelationFvRsVrfValidationPol=None, TenantDn=None, RelationFvRsCtxToBgpCtxAfPol=None, RelationFvRsCtxToEigrpCtxAfPol=None, RelationFvRsCtxToOspfCtxPol=None)
