...
TYPE_NAME = "TF::ACI::CloudExternalEpg"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_cloud_external_epg/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_cloud_external_epg',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_cloud_external_epg/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_cloud_external_epg/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, CloudApplicationcontainerDn=None, Description=None, ExceptionTag=None, FloodOnEncap=None, Id=None, MatchT=None, Name=None, NameAlias=None, PrefGrMemb=None, Prio=None, RelationCloudRsCloudEpgCtx=None, RelationFvRsCons=None, RelationFvRsConsIf=None, RelationFvRsCustQosPol=None, RelationFvRsIntraEpg=None, RelationFvRsProtBy=None, RelationFvRsProv=None, RelationFvRsSecInherited=None, RouteReachability=None)
