...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_epg_to_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_epg_to_domain',
                'returnValues': ["Id", "VmmId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_epg_to_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_epg_to_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowMicroSeg=None, Annotation=None, ApplicationEpgDn=None, BindingType=None, Delimiter=None, Description=None, Encap=None, EncapMode=None, EpgCos=None, EpgCosPref=None, Id=None, InstrImedcy=None, LagPolicyName=None, NetflowDir=None, NetflowPref=None, NumPorts=None, PortAllocation=None, PrimaryEncap=None, PrimaryEncapInner=None, ResImedcy=None, SecondaryEncapInner=None, SwitchingMode=None, Tdn=None, VmmAllowPromiscuous=None, VmmForgedTransmits=None, VmmId=None, VmmMacChanges=None)
