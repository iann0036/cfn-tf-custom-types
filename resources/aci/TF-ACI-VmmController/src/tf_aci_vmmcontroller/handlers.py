...
TYPE_NAME = "TF::ACI::VmmController"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_vmm_controller/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_vmm_controller',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_vmm_controller/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_vmm_controller/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, Description=None, DvsVersion=None, HostOrIp=None, Id=None, InventoryTrigSt=None, Mode=None, MsftConfigErrMsg=None, MsftConfigIssues=None, N1kvStatsMode=None, Name=None, NameAlias=None, Port=None, RelationVmmRsAcc=None, RelationVmmRsCtrlrPMonPol=None, RelationVmmRsMcastAddrNs=None, RelationVmmRsMgmtEPg=None, RelationVmmRsToExtDevMgr=None, RelationVmmRsVxlanNs=None, RelationVmmRsVxlanNsDef=None, RootContName=None, Scope=None, SeqNum=None, StatsMode=None, VmmDomainDn=None, VxlanDeplPref=None, RelationVmmRsVmmCtrlrP=None)
