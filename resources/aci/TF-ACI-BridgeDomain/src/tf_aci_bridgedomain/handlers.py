...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_bridge_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_bridge_domain',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_bridge_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_bridge_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, ArpFlood=None, BridgeDomainType=None, Description=None, EpClear=None, EpMoveDetectMode=None, HostBasedRouting=None, Id=None, IntersiteBumTrafficAllow=None, IntersiteL2Stretch=None, IpLearning=None, Ipv6McastAllow=None, LimitIpLearnToSubnets=None, LlAddr=None, Mac=None, McastAllow=None, MultiDstPktAct=None, Name=None, NameAlias=None, OptimizeWanBandwidth=None, RelationFvRsAbdPolMonPol=None, RelationFvRsBdFloodTo=None, RelationFvRsBdToEpRet=None, RelationFvRsBdToFhs=None, RelationFvRsBdToNdP=None, RelationFvRsBdToOut=None, RelationFvRsBdToProfile=None, RelationFvRsBdToRelayP=None, RelationFvRsCtx=None, RelationFvRsIgmpsn=None, RelationFvRsMldsn=None, TenantDn=None, UnicastRoute=None, UnkMacUcastAct=None, UnkMcastAct=None, V6unkMcastAct=None, Vmac=None, RelationFvRsBdToNetflowMonitorPol=None)
