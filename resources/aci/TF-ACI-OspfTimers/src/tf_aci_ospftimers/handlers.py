...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_ospf_timers/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_ospf_timers',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_ospf_timers/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_ospf_timers/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, BwRef=None, Ctrl=None, Description=None, Dist=None, GrCtrl=None, Id=None, LsaArrivalIntvl=None, LsaGpPacingIntvl=None, LsaHoldIntvl=None, LsaMaxIntvl=None, LsaStartIntvl=None, MaxEcmp=None, MaxLsaAction=None, MaxLsaNum=None, MaxLsaResetIntvl=None, MaxLsaSleepCnt=None, MaxLsaSleepIntvl=None, MaxLsaThresh=None, Name=None, NameAlias=None, SpfHoldIntvl=None, SpfInitIntvl=None, SpfMaxIntvl=None, TenantDn=None)
