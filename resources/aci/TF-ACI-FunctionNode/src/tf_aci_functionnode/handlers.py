...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_function_node/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_function_node',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_function_node/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_function_node/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, ConnConsumerDn=None, ConnProviderDn=None, Description=None, FuncTemplateType=None, FuncType=None, Id=None, IsCopy=None, L4L7ServiceGraphTemplateDn=None, Managed=None, Name=None, NameAlias=None, RelationVnsRsDefaultScopeToTerm=None, RelationVnsRsNodeToAbsFuncProf=None, RelationVnsRsNodeToCloudLDev=None, RelationVnsRsNodeToLDev=None, RelationVnsRsNodeToMFunc=None, RoutingMode=None, SequenceNumber=None, ShareEncap=None)
