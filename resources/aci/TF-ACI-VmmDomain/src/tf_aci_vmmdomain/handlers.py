...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_vmm_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aci_vmm_domain',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_vmm_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_vmm_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessMode=None, Annotation=None, ArpLearning=None, AveTimeOut=None, ConfigInfraPg=None, CtrlKnob=None, Delimiter=None, Description=None, EnableAve=None, EnableTag=None, EncapMode=None, EnfPref=None, EpInventoryType=None, EpRetTime=None, HvAvailMonitor=None, Id=None, McastAddr=None, Mode=None, Name=None, NameAlias=None, PrefEncapMode=None, ProviderProfileDn=None, RelationInfraRsDomVxlanNsDef=None, RelationInfraRsVipAddrNs=None, RelationInfraRsVlanNs=None, RelationInfraRsVlanNsDef=None, RelationVmmRsDefaultCdpIfPol=None, RelationVmmRsDefaultFwPol=None, RelationVmmRsDefaultL2InstPol=None, RelationVmmRsDefaultLacpLagPol=None, RelationVmmRsDefaultLldpIfPol=None, RelationVmmRsDefaultStpIfPol=None, RelationVmmRsDomMcastAddrNs=None, RelationVmmRsPrefEnhancedLagPol=None)
