...
TYPE_NAME = "TF::ACI::LeafAccessPortPolicyGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_leaf_access_port_policy_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'CiscoDevNet/aci',
                'providerTypeName': 'aci',
                'terraformTypeName': 'aci_leaf_access_port_policy_group',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aci_leaf_access_port_policy_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aci_leaf_access_port_policy_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Annotation=None, Description=None, Id=None, Name=None, NameAlias=None, RelationInfraRsAttEntP=None, RelationInfraRsCdpIfPol=None, RelationInfraRsCoppIfPol=None, RelationInfraRsDwdmIfPol=None, RelationInfraRsFcIfPol=None, RelationInfraRsHIfPol=None, RelationInfraRsL2IfPol=None, RelationInfraRsL2InstPol=None, RelationInfraRsL2PortAuthPol=None, RelationInfraRsL2PortSecurityPol=None, RelationInfraRsLldpIfPol=None, RelationInfraRsMacsecIfPol=None, RelationInfraRsMcpIfPol=None, RelationInfraRsMonIfInfraPol=None, RelationInfraRsPoeIfPol=None, RelationInfraRsQosDppIfPol=None, RelationInfraRsQosEgressDppIfPol=None, RelationInfraRsQosIngressDppIfPol=None, RelationInfraRsQosPfcIfPol=None, RelationInfraRsQosSdIfPol=None, RelationInfraRsSpanVDestGrp=None, RelationInfraRsSpanVSrcGrp=None, RelationInfraRsStormctrlIfPol=None, RelationInfraRsStpIfPol=None, RelationInfraRsNetflowMonitorPol=None)
