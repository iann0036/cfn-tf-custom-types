...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/time_offset/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/time',
                'providerTypeName': 'time',
                'terraformTypeName': 'time_offset',
                'returnValues': ["Day", "Hour", "Id", "Minute", "Month", "Rfc3339", "Second", "Unix", "Year"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/time_offset/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/time_offset/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BaseRfc3339=None, Day=None, Hour=None, Id=None, Minute=None, Month=None, OffsetDays=None, OffsetHours=None, OffsetMinutes=None, OffsetMonths=None, OffsetSeconds=None, OffsetYears=None, Rfc3339=None, Second=None, Triggers=None, Unix=None, Year=None)
