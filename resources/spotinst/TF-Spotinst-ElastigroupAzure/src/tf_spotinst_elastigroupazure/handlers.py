...
TYPE_NAME = "TF::Spotinst::ElastigroupAzure"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_elastigroup_azure/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_elastigroup_azure/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/spotinst_elastigroup_azure/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CustomData=None, DesiredCapacity=None, Id=None, LowPrioritySizes=None, MaxSize=None, MinSize=None, Name=None, OdSizes=None, Product=None, Region=None, ResourceGroupName=None, ShutdownScript=None, UserData=None, HealthCheck=None, Image=None, IntegrationKubernetes=None, IntegrationMultaiRuntime=None, LoadBalancers=None, Login=None, ManagedServiceIdentities=None, Network=None, ScalingDownPolicy=None, ScalingUpPolicy=None, ScheduledTask=None, Strategy=None, UpdatePolicy=None)
