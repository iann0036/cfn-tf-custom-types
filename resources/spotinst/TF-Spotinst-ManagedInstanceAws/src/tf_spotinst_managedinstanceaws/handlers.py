...
TYPE_NAME = "TF::Spotinst::ManagedInstanceAws"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_managed_instance_aws/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_managed_instance_aws/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/spotinst_managed_instance_aws/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoHealing=None, BlockDevicesMode=None, CpuCredits=None, Description=None, DrainingTimeout=None, EbsOptimized=None, ElasticIp=None, EnableMonitoring=None, FallBackToOd=None, GracePeriod=None, HealthCheckType=None, IamInstanceProfile=None, Id=None, ImageId=None, InstanceTypes=None, KeyPair=None, LifeCycle=None, MinimumInstanceLifetime=None, Name=None, OptimizationWindows=None, Orientation=None, PersistBlockDevices=None, PersistPrivateIp=None, PersistRootDevice=None, PlacementTenancy=None, PreferredType=None, PrivateIp=None, Product=None, Region=None, SecurityGroupIds=None, ShutdownScript=None, SubnetIds=None, UnhealthyDuration=None, UserData=None, UtilizeReservedInstances=None, VpcId=None, BlockDeviceMappings=None, IntegrationRoute53=None, LoadBalancers=None, ManagedInstanceAction=None, NetworkInterface=None, RevertToSpot=None, ScheduledTask=None, Tags=None)
