...
TYPE_NAME = "TF::Spotinst::ElastigroupGcp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_elastigroup_gcp/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_elastigroup_gcp/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/spotinst_elastigroup_gcp/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoHealing=None, AvailabilityZones=None, Description=None, DesiredCapacity=None, DrainingTimeout=None, FallbackToOndemand=None, HealthCheckGracePeriod=None, HealthCheckType=None, Id=None, InstanceTypesOndemand=None, InstanceTypesPreemptible=None, IpForwarding=None, MaxSize=None, MinSize=None, Name=None, OndemandCount=None, PreemptiblePercentage=None, ServiceAccount=None, ShutdownScript=None, StartupScript=None, Tags=None, UnhealthyDuration=None, BackendServices=None, Disk=None, Gpu=None, InstanceTypesCustom=None, IntegrationDockerSwarm=None, IntegrationGke=None, Labels=None, Metadata=None, NetworkInterface=None, ScalingDownPolicy=None, ScalingUpPolicy=None, ScheduledTask=None, Subnets=None)
