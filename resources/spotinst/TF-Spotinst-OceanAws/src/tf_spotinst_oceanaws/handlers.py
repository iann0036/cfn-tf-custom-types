...
TYPE_NAME = "TF::Spotinst::OceanAws"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_ocean_aws/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_ocean_aws/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/spotinst_ocean_aws/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AssociatePublicIpAddress=None, Blacklist=None, ControllerId=None, DesiredCapacity=None, DrainingTimeout=None, EbsOptimized=None, FallbackToOndemand=None, GracePeriod=None, IamInstanceProfile=None, Id=None, ImageId=None, KeyName=None, MaxSize=None, MinSize=None, Monitoring=None, Name=None, Region=None, RootVolumeSize=None, SecurityGroups=None, SpotPercentage=None, SubnetIds=None, UseAsTemplateOnly=None, UserData=None, UtilizeCommitments=None, UtilizeReservedInstances=None, Whitelist=None, Autoscaler=None, LoadBalancers=None, ScheduledTask=None, Tags=None, UpdatePolicy=None)
