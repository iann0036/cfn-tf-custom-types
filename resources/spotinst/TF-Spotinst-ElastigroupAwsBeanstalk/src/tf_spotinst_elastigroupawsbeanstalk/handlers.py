...
TYPE_NAME = "TF::Spotinst::ElastigroupAwsBeanstalk"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_elastigroup_aws_beanstalk/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/spotinst_elastigroup_aws_beanstalk/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/spotinst_elastigroup_aws_beanstalk/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BeanstalkEnvironmentId=None, BeanstalkEnvironmentName=None, DesiredCapacity=None, Id=None, InstanceTypesSpot=None, Maintenance=None, MaxSize=None, MinSize=None, Name=None, Product=None, Region=None, DeploymentPreferences=None, ManagedActions=None, ScheduledTask=None)
