...
def create_handler(
    ...
    try:
        ...
        tf['resource']['ucloud_vpc_peering_connection'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("ucloud"))['SecretString']
            tf["provider"]["ucloud"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['ucloud_vpc_peering_connection'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
