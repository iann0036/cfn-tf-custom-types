...
def create_handler(
    ...
    try:
        ...
        tf['resource']['logicmonitor_collector_group'][request.logicalResourceIdentifier] = {}

        if "logicmonitor" == "aws":
            creds = session.credentials
            tf["provider"]["logicmonitor"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("logicmonitor"))['SecretString']
            tf["provider"]["logicmonitor"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['logicmonitor_collector_group'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
