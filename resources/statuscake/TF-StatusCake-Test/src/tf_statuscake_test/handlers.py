...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/statuscake_test/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'StatusCakeDev/statuscake',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/statuscake_test/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/statuscake_test/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BasicPass=None, BasicUser=None, Branding=None, CheckRate=None, Confirmations=None, ContactGroup=None, ContactId=None, CustomHeader=None, DoNotFind=None, EnableSslAlert=None, FinalEndpoint=None, FindString=None, FollowRedirect=None, Id=None, LogoImage=None, NodeLocations=None, Paused=None, PingUrl=None, Port=None, PostRaw=None, Public=None, RealBrowser=None, Status=None, StatusCodes=None, TestId=None, TestTags=None, TestType=None, Timeout=None, TriggerRate=None, Uptime=None, UseJar=None, UserAgent=None, Virus=None, WebsiteHost=None, WebsiteName=None, WebsiteUrl=None)
