...
def create_handler(
    ...
    try:
        ...
        tf['resource']['statuscake_test'][request.logicalResourceIdentifier] = {}

        if "statuscake" == "aws":
            creds = session.credentials
            tf["provider"]["statuscake"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("statuscake"))['SecretString']
            tf["provider"]["statuscake"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['statuscake_test'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
