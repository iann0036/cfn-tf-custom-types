...
TYPE_NAME = "TF::Brightbox::OrbitContainer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/brightbox_orbit_container/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/brightbox_orbit_container/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/brightbox_orbit_container/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BytesUsed=None, ContainerRead=None, ContainerSyncKey=None, ContainerSyncTo=None, ContainerWrite=None, CreatedAt=None, HistoryLocation=None, Id=None, Metadata=None, Name=None, ObjectCount=None, StoragePolicy=None, VersionsLocation=None, Timeouts=None)
