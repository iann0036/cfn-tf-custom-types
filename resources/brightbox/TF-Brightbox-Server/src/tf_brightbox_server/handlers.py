...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/brightbox_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'brightbox/brightbox',
                'providerTypeName': 'brightbox',
                'terraformTypeName': 'brightbox_server',
                'returnValues': ["Fqdn", "Hostname", "Id", "Interface", "Ipv4Address", "Ipv4AddressPrivate", "Ipv6Address", "Ipv6Hostname", "PublicHostname", "Status", "Username"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/brightbox_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/brightbox_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DiskEncrypted=None, Fqdn=None, Hostname=None, Id=None, Image=None, Interface=None, Ipv4Address=None, Ipv4AddressPrivate=None, Ipv6Address=None, Ipv6Hostname=None, Locked=None, Name=None, PublicHostname=None, ServerGroups=None, Status=None, Type=None, UserData=None, UserDataBase64=None, Username=None, Zone=None, Timeouts=None)
