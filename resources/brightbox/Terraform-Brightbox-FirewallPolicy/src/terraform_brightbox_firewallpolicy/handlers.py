...
def create_handler(
    ...
    try:
        ...
        tf['resource']['brightbox_firewall_policy'][request.logicalResourceIdentifier] = {}

        if "brightbox" == "aws":
            creds = session.credentials
            tf["provider"]["brightbox"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("brightbox"))['SecretString']
            tf["provider"]["brightbox"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['brightbox_firewall_policy'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
