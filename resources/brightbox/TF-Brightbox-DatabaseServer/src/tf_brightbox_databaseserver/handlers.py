...
TYPE_NAME = "TF::Brightbox::DatabaseServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/brightbox_database_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AdminPassword", "AdminUsername", "Id", "SnapshotsScheduleNextAt", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/brightbox_database_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/brightbox_database_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminPassword=None, AdminUsername=None, AllowAccess=None, DatabaseEngine=None, DatabaseType=None, DatabaseVersion=None, Description=None, Id=None, Locked=None, MaintenanceHour=None, MaintenanceWeekday=None, Name=None, Snapshot=None, SnapshotsSchedule=None, SnapshotsScheduleNextAt=None, Status=None, Zone=None, Timeouts=None)
