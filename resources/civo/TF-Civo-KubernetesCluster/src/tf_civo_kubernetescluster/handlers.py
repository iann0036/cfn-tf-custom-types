...
TYPE_NAME = "TF::Civo::KubernetesCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/civo_kubernetes_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'civo/civo',
                'providerTypeName': 'civo',
                'terraformTypeName': 'civo_kubernetes_cluster',
                'returnValues': ["ApiEndpoint", "BuiltAt", "CreatedAt", "DnsEntry", "Id", "InstalledApplications", "Instances", "Kubeconfig", "MasterIp", "Pools", "Ready", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/civo_kubernetes_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/civo_kubernetes_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiEndpoint=None, Applications=None, BuiltAt=None, CreatedAt=None, DnsEntry=None, Id=None, InstalledApplications=None, Instances=None, Kubeconfig=None, KubernetesVersion=None, MasterIp=None, Name=None, NetworkId=None, NumTargetNodes=None, Pools=None, Ready=None, Region=None, Status=None, Tags=None, TargetNodesSize=None)
