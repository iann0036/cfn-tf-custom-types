...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/civo_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'civo_instance',
                'returnValues': ["CpuCores", "CreatedAt", "DiskGb", "Id", "InitialPassword", "PrivateIp", "PseudoIp", "PublicIp", "RamMb", "SourceId", "SourceType", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/civo_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/civo_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CpuCores=None, CreatedAt=None, DiskGb=None, FirewallId=None, Hostname=None, Id=None, InitialPassword=None, InitialUser=None, NetworkId=None, Notes=None, PrivateIp=None, PseudoIp=None, PublicIp=None, PublicIpRequired=None, RamMb=None, Region=None, ReverseDns=None, Script=None, Size=None, SourceId=None, SourceType=None, SshkeyId=None, Status=None, Tags=None, Template=None)
