...
TYPE_NAME = "TF::OPC::ComputeInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/opc_compute_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/opc',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/opc_compute_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/opc_compute_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Attributes=None, AvailabilityDomain=None, BootOrder=None, DesiredState=None, Domain=None, Entry=None, Fingerprint=None, Fqdn=None, Hostname=None, Id=None, ImageFormat=None, ImageList=None, InstanceAttributes=None, IpAddress=None, Label=None, Name=None, PlacementRequirements=None, Platform=None, Priority=None, QuotaReservation=None, Relationships=None, Resolvers=None, ReverseDns=None, Shape=None, Site=None, SshKeys=None, StartTime=None, State=None, Tags=None, Vcable=None, Virtio=None, VncAddress=None, NetworkingInfo=None, Storage=None, Timeouts=None)
