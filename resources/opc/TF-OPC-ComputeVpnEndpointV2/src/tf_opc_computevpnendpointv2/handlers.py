...
TYPE_NAME = "TF::OPC::ComputeVpnEndpointV2"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/opc_compute_vpn_endpoint_v2/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/opc_compute_vpn_endpoint_v2/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/opc_compute_vpn_endpoint_v2/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CustomerVpnGateway=None, Description=None, Enabled=None, Id=None, IkeIdentifier=None, IpNetwork=None, LocalGatewayIpAddress=None, LocalGatewayPrivateIpAddress=None, Name=None, PreSharedKey=None, ReachableRoutes=None, RequirePerfectForwardSecrecy=None, Tags=None, TunnelStatus=None, Uri=None, VnicSets=None, PhaseOneSettings=None, PhaseTwoSettings=None, Timeouts=None)
