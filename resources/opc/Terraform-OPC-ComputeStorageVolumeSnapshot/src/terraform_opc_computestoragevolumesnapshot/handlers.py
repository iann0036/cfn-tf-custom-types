...
def create_handler(
    ...
    try:
        ...
        tf['resource']['opc_compute_storage_volume_snapshot'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("opc"))['SecretString']
            tf["provider"]["opc"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['opc_compute_storage_volume_snapshot'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
