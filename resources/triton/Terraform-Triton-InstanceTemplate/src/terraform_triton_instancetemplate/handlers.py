...
def create_handler(
    ...
    try:
        ...
        tf['resource']['triton_instance_template'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("triton"))['SecretString']
            tf["provider"]["triton"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['triton_instance_template'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
