...
TYPE_NAME = "TF::OCI::DataintegrationWorkspace"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_dataintegration_workspace/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_dataintegration_workspace',
                'returnValues': ["Id", "State", "StateMessage", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_dataintegration_workspace/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_dataintegration_workspace/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, Description=None, DisplayName=None, DnsServerIp=None, DnsServerZone=None, FreeformTags=None, Id=None, IsForceOperation=None, IsPrivateNetworkEnabled=None, QuiesceTimeout=None, State=None, StateMessage=None, SubnetId=None, TimeCreated=None, TimeUpdated=None, VcnId=None, Timeouts=None)
