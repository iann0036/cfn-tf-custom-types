...
TYPE_NAME = "TF::OCI::CoreDrgAttachmentManagement"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_drg_attachment_management/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_core_drg_attachment_management',
                'returnValues': ["Id", "IsCrossTenancy", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_drg_attachment_management/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_drg_attachment_management/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AttachmentType=None, CompartmentId=None, DefinedTags=None, DisplayName=None, DrgId=None, DrgRouteTableId=None, ExportDrgRouteDistributionId=None, FreeformTags=None, Id=None, IsCrossTenancy=None, NetworkId=None, RemoveExportDrgRouteDistributionTrigger=None, RouteTableId=None, State=None, TimeCreated=None, VcnId=None, NetworkDetails=None, Timeouts=None)
