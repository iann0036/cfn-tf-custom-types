...
TYPE_NAME = "TF::OCI::DatabaseDatabaseUpgrade"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_database_upgrade/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_database_upgrade',
                'returnValues': ["CharacterSet", "CompartmentId", "ConnectionStrings", "DatabaseSoftwareImageId", "DbBackupConfig", "DbHomeId", "DbName", "DbSystemId", "DbUniqueName", "DbWorkload", "DefinedTags", "FreeformTags", "Id", "LastBackupTimestamp", "LifecycleDetails", "NcharacterSet", "PdbName", "SourceDatabasePointInTimeRecoveryTimestamp", "State", "TimeCreated", "VmClusterId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_database_upgrade/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_database_upgrade/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, CharacterSet=None, CompartmentId=None, ConnectionStrings=None, DatabaseId=None, DatabaseSoftwareImageId=None, DbBackupConfig=None, DbHomeId=None, DbName=None, DbSystemId=None, DbUniqueName=None, DbWorkload=None, DefinedTags=None, FreeformTags=None, Id=None, LastBackupTimestamp=None, LifecycleDetails=None, NcharacterSet=None, PdbName=None, SourceDatabasePointInTimeRecoveryTimestamp=None, State=None, TimeCreated=None, VmClusterId=None, DatabaseUpgradeSourceDetails=None, Timeouts=None)
