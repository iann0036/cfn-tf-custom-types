...
TYPE_NAME = "TF::OCI::GoldenGateDatabaseRegistration"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_golden_gate_database_registration/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_golden_gate_database_registration',
                'returnValues': ["Id", "LifecycleDetails", "RcePrivateIp", "SecretId", "State", "SystemTags", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_golden_gate_database_registration/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_golden_gate_database_registration/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AliasName=None, CompartmentId=None, ConnectionString=None, DatabaseId=None, DefinedTags=None, Description=None, DisplayName=None, Fqdn=None, FreeformTags=None, Id=None, IpAddress=None, KeyId=None, LifecycleDetails=None, Password=None, RcePrivateIp=None, SecretCompartmentId=None, SecretId=None, State=None, SubnetId=None, SystemTags=None, TimeCreated=None, TimeUpdated=None, Username=None, VaultId=None, Wallet=None, Timeouts=None)
