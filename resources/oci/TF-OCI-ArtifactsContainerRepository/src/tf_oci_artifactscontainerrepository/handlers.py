...
TYPE_NAME = "TF::OCI::ArtifactsContainerRepository"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_artifacts_container_repository/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_artifacts_container_repository',
                'returnValues': ["BillableSizeInGbs", "CreatedBy", "Id", "ImageCount", "LayerCount", "LayersSizeInBytes", "State", "TimeCreated", "TimeLastPushed"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_artifacts_container_repository/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_artifacts_container_repository/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BillableSizeInGbs=None, CompartmentId=None, CreatedBy=None, DisplayName=None, Id=None, ImageCount=None, IsImmutable=None, IsPublic=None, LayerCount=None, LayersSizeInBytes=None, State=None, TimeCreated=None, TimeLastPushed=None, Readme=None, Timeouts=None)
