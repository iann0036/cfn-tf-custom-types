...
TYPE_NAME = "TF::OCI::AnalyticsAnalyticsInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_analytics_analytics_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Id", "PrivateAccessChannels", "ServiceUrl", "TimeCreated", "TimeUpdated", "VanityUrlDetails"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_analytics_analytics_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_analytics_analytics_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, Description=None, EmailNotification=None, FeatureSet=None, FreeformTags=None, Id=None, IdcsAccessToken=None, LicenseType=None, Name=None, PrivateAccessChannels=None, ServiceUrl=None, State=None, TimeCreated=None, TimeUpdated=None, VanityUrlDetails=None, Capacity=None, NetworkEndpointDetails=None, Timeouts=None)
