# DO NOT modify this file by hand, changes will be overwritten
import sys
from dataclasses import dataclass
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

from cloudformation_cli_python_lib.interface import (
    BaseModel,
    BaseResourceHandlerRequest,
)
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class ResourceHandlerRequest(BaseResourceHandlerRequest):
    # pylint: disable=invalid-name
    desiredResourceState: Optional["ResourceModel"]
    previousResourceState: Optional["ResourceModel"]


@dataclass
class ResourceModel(BaseModel):
    tfcfnid: Optional[str]
    CompartmentId: Optional[str]
    DefinedTags: Optional[Sequence["_DefinedTagsDefinition"]]
    Description: Optional[str]
    EmailNotification: Optional[str]
    FeatureSet: Optional[str]
    FreeformTags: Optional[Sequence["_FreeformTagsDefinition"]]
    Id: Optional[str]
    IdcsAccessToken: Optional[str]
    LicenseType: Optional[str]
    Name: Optional[str]
    PrivateAccessChannels: Optional[Sequence["_PrivateAccessChannelsDefinition"]]
    ServiceUrl: Optional[str]
    State: Optional[str]
    TimeCreated: Optional[str]
    TimeUpdated: Optional[str]
    VanityUrlDetails: Optional[Sequence["_VanityUrlDetailsDefinition"]]
    Capacity: Optional[Sequence["_CapacityDefinition"]]
    NetworkEndpointDetails: Optional[Sequence["_NetworkEndpointDetailsDefinition"]]
    Timeouts: Optional["_TimeoutsDefinition"]

    @classmethod
    def _deserialize(
        cls: Type["_ResourceModel"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_ResourceModel"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            tfcfnid=json_data.get("tfcfnid"),
            CompartmentId=json_data.get("CompartmentId"),
            DefinedTags=deserialize_list(json_data.get("DefinedTags"), DefinedTagsDefinition),
            Description=json_data.get("Description"),
            EmailNotification=json_data.get("EmailNotification"),
            FeatureSet=json_data.get("FeatureSet"),
            FreeformTags=deserialize_list(json_data.get("FreeformTags"), FreeformTagsDefinition),
            Id=json_data.get("Id"),
            IdcsAccessToken=json_data.get("IdcsAccessToken"),
            LicenseType=json_data.get("LicenseType"),
            Name=json_data.get("Name"),
            PrivateAccessChannels=deserialize_list(json_data.get("PrivateAccessChannels"), PrivateAccessChannelsDefinition),
            ServiceUrl=json_data.get("ServiceUrl"),
            State=json_data.get("State"),
            TimeCreated=json_data.get("TimeCreated"),
            TimeUpdated=json_data.get("TimeUpdated"),
            VanityUrlDetails=deserialize_list(json_data.get("VanityUrlDetails"), VanityUrlDetailsDefinition),
            Capacity=deserialize_list(json_data.get("Capacity"), CapacityDefinition),
            NetworkEndpointDetails=deserialize_list(json_data.get("NetworkEndpointDetails"), NetworkEndpointDetailsDefinition),
            Timeouts=TimeoutsDefinition._deserialize(json_data.get("Timeouts")),
        )


# work around possible type aliasing issues when variable has same name as a model
_ResourceModel = ResourceModel


@dataclass
class DefinedTagsDefinition(BaseModel):
    MapKey: Optional[str]
    MapValue: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_DefinedTagsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_DefinedTagsDefinition"]:
        if not json_data:
            return None
        return cls(
            MapKey=json_data.get("MapKey"),
            MapValue=json_data.get("MapValue"),
        )


# work around possible type aliasing issues when variable has same name as a model
_DefinedTagsDefinition = DefinedTagsDefinition


@dataclass
class FreeformTagsDefinition(BaseModel):
    MapKey: Optional[str]
    MapValue: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_FreeformTagsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_FreeformTagsDefinition"]:
        if not json_data:
            return None
        return cls(
            MapKey=json_data.get("MapKey"),
            MapValue=json_data.get("MapValue"),
        )


# work around possible type aliasing issues when variable has same name as a model
_FreeformTagsDefinition = FreeformTagsDefinition


@dataclass
class PrivateAccessChannelsDefinition(BaseModel):
    MapKey: Optional[str]
    MapValue: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_PrivateAccessChannelsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_PrivateAccessChannelsDefinition"]:
        if not json_data:
            return None
        return cls(
            MapKey=json_data.get("MapKey"),
            MapValue=json_data.get("MapValue"),
        )


# work around possible type aliasing issues when variable has same name as a model
_PrivateAccessChannelsDefinition = PrivateAccessChannelsDefinition


@dataclass
class VanityUrlDetailsDefinition(BaseModel):
    MapKey: Optional[str]
    MapValue: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_VanityUrlDetailsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_VanityUrlDetailsDefinition"]:
        if not json_data:
            return None
        return cls(
            MapKey=json_data.get("MapKey"),
            MapValue=json_data.get("MapValue"),
        )


# work around possible type aliasing issues when variable has same name as a model
_VanityUrlDetailsDefinition = VanityUrlDetailsDefinition


@dataclass
class CapacityDefinition(BaseModel):
    CapacityType: Optional[str]
    CapacityValue: Optional[float]

    @classmethod
    def _deserialize(
        cls: Type["_CapacityDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_CapacityDefinition"]:
        if not json_data:
            return None
        return cls(
            CapacityType=json_data.get("CapacityType"),
            CapacityValue=json_data.get("CapacityValue"),
        )


# work around possible type aliasing issues when variable has same name as a model
_CapacityDefinition = CapacityDefinition


@dataclass
class NetworkEndpointDetailsDefinition(BaseModel):
    NetworkEndpointType: Optional[str]
    SubnetId: Optional[str]
    VcnId: Optional[str]
    WhitelistedIps: Optional[Sequence[str]]
    WhitelistedVcns: Optional[Sequence["_WhitelistedVcnsDefinition"]]

    @classmethod
    def _deserialize(
        cls: Type["_NetworkEndpointDetailsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_NetworkEndpointDetailsDefinition"]:
        if not json_data:
            return None
        return cls(
            NetworkEndpointType=json_data.get("NetworkEndpointType"),
            SubnetId=json_data.get("SubnetId"),
            VcnId=json_data.get("VcnId"),
            WhitelistedIps=json_data.get("WhitelistedIps"),
            WhitelistedVcns=deserialize_list(json_data.get("WhitelistedVcns"), WhitelistedVcnsDefinition),
        )


# work around possible type aliasing issues when variable has same name as a model
_NetworkEndpointDetailsDefinition = NetworkEndpointDetailsDefinition


@dataclass
class WhitelistedVcnsDefinition(BaseModel):
    Id: Optional[str]
    WhitelistedIps: Optional[Sequence[str]]

    @classmethod
    def _deserialize(
        cls: Type["_WhitelistedVcnsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_WhitelistedVcnsDefinition"]:
        if not json_data:
            return None
        return cls(
            Id=json_data.get("Id"),
            WhitelistedIps=json_data.get("WhitelistedIps"),
        )


# work around possible type aliasing issues when variable has same name as a model
_WhitelistedVcnsDefinition = WhitelistedVcnsDefinition


@dataclass
class TimeoutsDefinition(BaseModel):
    Create: Optional[str]
    Delete: Optional[str]
    Update: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_TimeoutsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_TimeoutsDefinition"]:
        if not json_data:
            return None
        return cls(
            Create=json_data.get("Create"),
            Delete=json_data.get("Delete"),
            Update=json_data.get("Update"),
        )


# work around possible type aliasing issues when variable has same name as a model
_TimeoutsDefinition = TimeoutsDefinition


