...
TYPE_NAME = "TF::OCI::ObjectstorageReplicationPolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_objectstorage_replication_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_objectstorage_replication_policy',
                'returnValues': ["Id", "Status", "StatusMessage", "TimeCreated", "TimeLastSync"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_objectstorage_replication_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_objectstorage_replication_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Bucket=None, DeleteObjectInDestinationBucket=None, DestinationBucketName=None, DestinationRegionName=None, Id=None, Name=None, Namespace=None, Status=None, StatusMessage=None, TimeCreated=None, TimeLastSync=None, Timeouts=None)
