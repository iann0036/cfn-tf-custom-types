...
TYPE_NAME = "TF::OCI::DatabaseAutonomousContainerDatabase"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_autonomous_container_database/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AvailabilityDomain", "DbVersion", "Id", "InfrastructureType", "KeyStoreWalletName", "LastMaintenanceRunId", "LifecycleDetails", "MaintenanceWindow", "NextMaintenanceRunId", "PatchId", "Role", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_autonomous_container_database/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_autonomous_container_database/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutonomousExadataInfrastructureId=None, AutonomousVmClusterId=None, AvailabilityDomain=None, CompartmentId=None, DbUniqueName=None, DbVersion=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, InfrastructureType=None, KeyStoreId=None, KeyStoreWalletName=None, KmsKeyId=None, LastMaintenanceRunId=None, LifecycleDetails=None, MaintenanceWindow=None, NextMaintenanceRunId=None, PatchId=None, PatchModel=None, PeerAutonomousContainerDatabaseCompartmentId=None, PeerAutonomousContainerDatabaseDisplayName=None, PeerAutonomousExadataInfrastructureId=None, PeerAutonomousVmClusterId=None, PeerDbUniqueName=None, ProtectionMode=None, Role=None, RotateKeyTrigger=None, ServiceLevelAgreementType=None, StandbyMaintenanceBufferInDays=None, State=None, TimeCreated=None, VaultId=None, BackupConfig=None, MaintenanceWindowDetails=None, PeerAutonomousContainerDatabaseBackupConfig=None, Timeouts=None)
