...
TYPE_NAME = "TF::OCI::CoreInstancePoolInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_instance_pool_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_core_instance_pool_instance',
                'returnValues': ["AvailabilityDomain", "CompartmentId", "DisplayName", "FaultDomain", "Id", "InstanceConfigurationId", "LoadBalancerBackends", "Region", "Shape", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_instance_pool_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_instance_pool_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoTerminateInstanceOnDelete=None, AvailabilityDomain=None, CompartmentId=None, DecrementSizeOnDelete=None, DisplayName=None, FaultDomain=None, Id=None, InstanceConfigurationId=None, InstanceId=None, InstancePoolId=None, LoadBalancerBackends=None, Region=None, Shape=None, State=None, TimeCreated=None, Timeouts=None)
