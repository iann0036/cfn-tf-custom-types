...
TYPE_NAME = "TF::OCI::DatabaseExternalDatabaseConnector"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_external_database_connector/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_external_database_connector',
                'returnValues': ["CompartmentId", "ConnectionStatus", "Id", "LifecycleDetails", "State", "TimeConnectionStatusLastUpdated", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_external_database_connector/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_external_database_connector/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, ConnectionStatus=None, ConnectorAgentId=None, ConnectorType=None, DefinedTags=None, DisplayName=None, ExternalDatabaseId=None, FreeformTags=None, Id=None, LifecycleDetails=None, State=None, TimeConnectionStatusLastUpdated=None, TimeCreated=None, ConnectionCredentials=None, ConnectionString=None, Timeouts=None)
