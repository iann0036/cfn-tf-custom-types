...
TYPE_NAME = "TF::OCI::DatabaseExadataInfrastructureStorage"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_exadata_infrastructure_storage/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_exadata_infrastructure_storage',
                'returnValues': ["ActivatedStorageCount", "AdditionalStorageCount", "CpusEnabled", "CsiNumber", "DataStorageSizeInTbs", "DbNodeStorageSizeInGbs", "Id", "LifecycleDetails", "MaintenanceSloStatus", "MaxCpuCount", "MaxDataStorageInTbs", "MaxDbNodeStorageInGbs", "MaxMemoryInGbs", "MemorySizeInGbs", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_exadata_infrastructure_storage/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_exadata_infrastructure_storage/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActivatedStorageCount=None, ActivationFile=None, AdditionalStorageCount=None, AdminNetworkCidr=None, CloudControlPlaneServer1=None, CloudControlPlaneServer2=None, CompartmentId=None, ComputeCount=None, CorporateProxy=None, CpusEnabled=None, CsiNumber=None, DataStorageSizeInTbs=None, DbNodeStorageSizeInGbs=None, DefinedTags=None, DisplayName=None, DnsServer=None, ExadataInfrastructureId=None, FreeformTags=None, Gateway=None, Id=None, InfiniBandNetworkCidr=None, LifecycleDetails=None, MaintenanceSloStatus=None, MaxCpuCount=None, MaxDataStorageInTbs=None, MaxDbNodeStorageInGbs=None, MaxMemoryInGbs=None, MemorySizeInGbs=None, Netmask=None, NtpServer=None, Shape=None, State=None, StorageCount=None, TimeCreated=None, TimeZone=None, Contacts=None, MaintenanceWindow=None, Timeouts=None)
