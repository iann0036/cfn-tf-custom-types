...
TYPE_NAME = "TF::OCI::GoldenGateDeploymentBackup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_golden_gate_deployment_backup/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_golden_gate_deployment_backup',
                'returnValues': ["BackupType", "Id", "IsAutomatic", "LifecycleDetails", "OggVersion", "State", "SystemTags", "TimeCreated", "TimeOfBackup", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_golden_gate_deployment_backup/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_golden_gate_deployment_backup/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BackupType=None, Bucket=None, CompartmentId=None, DefinedTags=None, DeploymentId=None, DisplayName=None, FreeformTags=None, Id=None, IsAutomatic=None, LifecycleDetails=None, Namespace=None, Object=None, OggVersion=None, State=None, SystemTags=None, TimeCreated=None, TimeOfBackup=None, TimeUpdated=None, Timeouts=None)
