...
TYPE_NAME = "TF::OCI::BdsBdsInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_bds_bds_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_bds_bds_instance',
                'returnValues': ["ClusterDetails", "CreatedBy", "Id", "Nodes", "NumberOfNodes", "State", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_bds_bds_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_bds_bds_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClusterAdminPassword=None, ClusterDetails=None, ClusterPublicKey=None, ClusterVersion=None, CompartmentId=None, CreatedBy=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, IsCloudSqlConfigured=None, IsHighAvailability=None, IsSecure=None, Nodes=None, NumberOfNodes=None, State=None, TimeCreated=None, TimeUpdated=None, CloudSqlDetails=None, MasterNode=None, NetworkConfig=None, Timeouts=None, UtilNode=None, WorkerNode=None)
