...
TYPE_NAME = "TF::OCI::CoreVolumeBackup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_volume_backup/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_volume_backup/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_volume_backup/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, DisplayName=None, ExpirationTime=None, FreeformTags=None, Id=None, KmsKeyId=None, SizeInGbs=None, SizeInMbs=None, SourceType=None, SourceVolumeBackupId=None, State=None, SystemTags=None, TimeCreated=None, TimeRequestReceived=None, Type=None, UniqueSizeInGbs=None, UniqueSizeInMbs=None, VolumeId=None, SourceDetails=None, Timeouts=None)
