...
TYPE_NAME = "TF::OCI::DatabaseMaintenanceRun"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_maintenance_run/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CompartmentId", "Description", "DisplayName", "Id", "LifecycleDetails", "MaintenanceSubtype", "MaintenanceType", "PatchFailureCount", "PeerMaintenanceRunId", "State", "TargetResourceId", "TargetResourceType", "TimeEnded", "TimeStarted"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_maintenance_run/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_maintenance_run/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, Description=None, DisplayName=None, Id=None, IsEnabled=None, IsPatchNowEnabled=None, LifecycleDetails=None, MaintenanceRunId=None, MaintenanceSubtype=None, MaintenanceType=None, PatchFailureCount=None, PatchId=None, PatchingMode=None, PeerMaintenanceRunId=None, State=None, TargetResourceId=None, TargetResourceType=None, TimeEnded=None, TimeScheduled=None, TimeStarted=None, Timeouts=None)
