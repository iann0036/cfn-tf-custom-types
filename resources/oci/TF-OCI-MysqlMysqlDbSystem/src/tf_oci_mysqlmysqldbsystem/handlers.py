...
TYPE_NAME = "TF::OCI::MysqlMysqlDbSystem"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_mysql_mysql_db_system/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_mysql_mysql_db_system',
                'returnValues': ["AnalyticsCluster", "Channels", "CurrentPlacement", "Endpoints", "HeatWaveCluster", "Id", "IsAnalyticsClusterAttached", "IsHeatWaveClusterAttached", "LifecycleDetails", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_mysql_mysql_db_system/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_mysql_mysql_db_system/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminPassword=None, AdminUsername=None, AnalyticsCluster=None, AvailabilityDomain=None, Channels=None, CompartmentId=None, ConfigurationId=None, CurrentPlacement=None, DataStorageSizeInGb=None, DefinedTags=None, Description=None, DisplayName=None, Endpoints=None, FaultDomain=None, FreeformTags=None, HeatWaveCluster=None, HostnameLabel=None, Id=None, IpAddress=None, IsAnalyticsClusterAttached=None, IsHeatWaveClusterAttached=None, IsHighlyAvailable=None, LifecycleDetails=None, MysqlVersion=None, Port=None, PortX=None, ShapeName=None, ShutdownType=None, State=None, SubnetId=None, TimeCreated=None, TimeUpdated=None, BackupPolicy=None, Maintenance=None, Source=None, Timeouts=None)
