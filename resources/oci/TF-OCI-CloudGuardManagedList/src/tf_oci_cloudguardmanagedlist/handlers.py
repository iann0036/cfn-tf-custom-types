...
TYPE_NAME = "TF::OCI::CloudGuardManagedList"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_cloud_guard_managed_list/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_cloud_guard_managed_list',
                'returnValues': ["FeedProvider", "Id", "IsEditable", "LifecyleDetails", "State", "SystemTags", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_cloud_guard_managed_list/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_cloud_guard_managed_list/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, Description=None, DisplayName=None, FeedProvider=None, FreeformTags=None, Id=None, IsEditable=None, LifecyleDetails=None, ListItems=None, ListType=None, SourceManagedListId=None, State=None, SystemTags=None, TimeCreated=None, TimeUpdated=None, Timeouts=None)
