...
TYPE_NAME = "TF::OCI::DatabaseAutonomousDatabaseBackup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_autonomous_database_backup/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CompartmentId", "DatabaseSizeInTbs", "Id", "IsAutomatic", "IsRestorable", "KeyStoreId", "KeyStoreWalletName", "KmsKeyId", "LifecycleDetails", "State", "TimeEnded", "TimeStarted", "Type", "VaultId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_autonomous_database_backup/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_autonomous_database_backup/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutonomousDatabaseId=None, CompartmentId=None, DatabaseSizeInTbs=None, DisplayName=None, Id=None, IsAutomatic=None, IsRestorable=None, KeyStoreId=None, KeyStoreWalletName=None, KmsKeyId=None, LifecycleDetails=None, State=None, TimeEnded=None, TimeStarted=None, Type=None, VaultId=None, Timeouts=None)
