...
TYPE_NAME = "TF::OCI::ObjectstorageBucket"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_objectstorage_bucket/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ApproximateCount", "ApproximateSize", "BucketId", "CreatedBy", "Etag", "Id", "IsReadOnly", "ObjectLifecyclePolicyEtag", "ReplicationEnabled", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_objectstorage_bucket/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_objectstorage_bucket/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessType=None, ApproximateCount=None, ApproximateSize=None, AutoTiering=None, BucketId=None, CompartmentId=None, CreatedBy=None, DefinedTags=None, Etag=None, FreeformTags=None, Id=None, IsReadOnly=None, KmsKeyId=None, Metadata=None, Name=None, Namespace=None, ObjectEventsEnabled=None, ObjectLifecyclePolicyEtag=None, ReplicationEnabled=None, StorageTier=None, TimeCreated=None, Versioning=None, RetentionRules=None, Timeouts=None)
