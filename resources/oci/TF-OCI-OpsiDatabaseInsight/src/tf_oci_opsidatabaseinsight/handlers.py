...
TYPE_NAME = "TF::OCI::OpsiDatabaseInsight"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_opsi_database_insight/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_opsi_database_insight',
                'returnValues': ["DatabaseDisplayName", "DatabaseId", "DatabaseName", "DatabaseType", "DatabaseVersion", "EnterpriseManagerEntityDisplayName", "EnterpriseManagerEntityName", "EnterpriseManagerEntityType", "Id", "LifecycleDetails", "ProcessorCount", "State", "SystemTags", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_opsi_database_insight/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_opsi_database_insight/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DatabaseDisplayName=None, DatabaseId=None, DatabaseName=None, DatabaseType=None, DatabaseVersion=None, DefinedTags=None, EnterpriseManagerBridgeId=None, EnterpriseManagerEntityDisplayName=None, EnterpriseManagerEntityIdentifier=None, EnterpriseManagerEntityName=None, EnterpriseManagerEntityType=None, EnterpriseManagerIdentifier=None, EntitySource=None, FreeformTags=None, Id=None, LifecycleDetails=None, ProcessorCount=None, State=None, Status=None, SystemTags=None, TimeCreated=None, TimeUpdated=None, Timeouts=None)
