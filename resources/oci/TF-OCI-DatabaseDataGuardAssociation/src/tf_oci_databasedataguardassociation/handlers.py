...
TYPE_NAME = "TF::OCI::DatabaseDataGuardAssociation"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_data_guard_association/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ApplyLag", "ApplyRate", "Id", "LifecycleDetails", "PeerDataGuardAssociationId", "PeerDatabaseId", "PeerRole", "Role", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_data_guard_association/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_data_guard_association/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplyLag=None, ApplyRate=None, AvailabilityDomain=None, BackupNetworkNsgIds=None, CreateAsync=None, CreationType=None, DatabaseAdminPassword=None, DatabaseId=None, DatabaseSoftwareImageId=None, DeleteStandbyDbHomeOnDelete=None, DisplayName=None, Hostname=None, Id=None, LifecycleDetails=None, NsgIds=None, PeerDataGuardAssociationId=None, PeerDatabaseId=None, PeerDbHomeId=None, PeerDbSystemId=None, PeerRole=None, PeerVmClusterId=None, ProtectionMode=None, Role=None, Shape=None, State=None, SubnetId=None, TimeCreated=None, TransportType=None, Timeouts=None)
