...
TYPE_NAME = "TF::OCI::DatabaseCloudVmCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_cloud_vm_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_cloud_vm_cluster',
                'returnValues': ["AvailabilityDomain", "DiskRedundancy", "Id", "IormConfigCache", "LastUpdateHistoryEntryId", "LifecycleDetails", "ListenerPort", "NodeCount", "ScanDnsName", "ScanDnsRecordId", "ScanIpIds", "Shape", "State", "StorageSizeInGbs", "SystemVersion", "TimeCreated", "VipIds", "ZoneId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_cloud_vm_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_cloud_vm_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, BackupNetworkNsgIds=None, BackupSubnetId=None, CloudExadataInfrastructureId=None, ClusterName=None, CompartmentId=None, CpuCoreCount=None, CreateAsync=None, DataStoragePercentage=None, DefinedTags=None, DiskRedundancy=None, DisplayName=None, Domain=None, FreeformTags=None, GiVersion=None, Hostname=None, Id=None, IormConfigCache=None, IsLocalBackupEnabled=None, IsSparseDiskgroupEnabled=None, LastUpdateHistoryEntryId=None, LicenseModel=None, LifecycleDetails=None, ListenerPort=None, NodeCount=None, NsgIds=None, ScanDnsName=None, ScanDnsRecordId=None, ScanIpIds=None, Shape=None, SshPublicKeys=None, State=None, StorageSizeInGbs=None, SubnetId=None, SystemVersion=None, TimeCreated=None, TimeZone=None, VipIds=None, ZoneId=None, Timeouts=None)
