...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_kms_vault/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CryptoEndpoint", "Id", "IsPrimary", "ManagementEndpoint", "ReplicaDetails", "RestoredFromVaultId", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_kms_vault/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_kms_vault/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, CryptoEndpoint=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, IsPrimary=None, ManagementEndpoint=None, ReplicaDetails=None, RestoreTrigger=None, RestoredFromVaultId=None, State=None, TimeCreated=None, TimeOfDeletion=None, VaultType=None, RestoreFromFile=None, RestoreFromObjectStore=None, Timeouts=None)
