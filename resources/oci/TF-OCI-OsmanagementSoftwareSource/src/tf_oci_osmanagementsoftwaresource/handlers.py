...
TYPE_NAME = "TF::OCI::OsmanagementSoftwareSource"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_osmanagement_software_source/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_osmanagement_software_source/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_osmanagement_software_source/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ArchType=None, AssociatedManagedInstances=None, ChecksumType=None, CompartmentId=None, DefinedTags=None, Description=None, DisplayName=None, FreeformTags=None, GpgKeyFingerprint=None, GpgKeyId=None, GpgKeyUrl=None, Id=None, MaintainerEmail=None, MaintainerName=None, MaintainerPhone=None, Packages=None, ParentId=None, ParentName=None, RepoType=None, State=None, Status=None, Url=None, Timeouts=None)
