...
TYPE_NAME = "TF::OCI::DataflowPrivateEndpoint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_dataflow_private_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_dataflow_private_endpoint',
                'returnValues': ["Id", "LifecycleDetails", "OwnerPrincipalId", "OwnerUserName", "State", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_dataflow_private_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_dataflow_private_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, Description=None, DisplayName=None, DnsZones=None, FreeformTags=None, Id=None, LifecycleDetails=None, MaxHostCount=None, NsgIds=None, OwnerPrincipalId=None, OwnerUserName=None, State=None, SubnetId=None, TimeCreated=None, TimeUpdated=None, Timeouts=None)
