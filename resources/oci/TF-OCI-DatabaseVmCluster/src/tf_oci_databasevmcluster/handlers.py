...
TYPE_NAME = "TF::OCI::DatabaseVmCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_vm_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CpusEnabled", "Id", "LastPatchHistoryEntryId", "LifecycleDetails", "Shape", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_vm_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_vm_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, CpuCoreCount=None, CpusEnabled=None, DataStorageSizeInTbs=None, DbNodeStorageSizeInGbs=None, DefinedTags=None, DisplayName=None, ExadataInfrastructureId=None, FreeformTags=None, GiVersion=None, Id=None, IsLocalBackupEnabled=None, IsSparseDiskgroupEnabled=None, LastPatchHistoryEntryId=None, LicenseModel=None, LifecycleDetails=None, MemorySizeInGbs=None, Shape=None, SshPublicKeys=None, State=None, TimeCreated=None, TimeZone=None, VmClusterNetworkId=None, Timeouts=None)
