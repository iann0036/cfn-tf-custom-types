...
TYPE_NAME = "TF::OCI::LogAnalyticsLogAnalyticsEntity"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_log_analytics_log_analytics_entity/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_log_analytics_log_analytics_entity',
                'returnValues': ["AreLogsCollected", "EntityTypeInternalName", "Id", "LifecycleDetails", "ManagementAgentCompartmentId", "ManagementAgentDisplayName", "State", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_log_analytics_log_analytics_entity/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_log_analytics_log_analytics_entity/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AreLogsCollected=None, CloudResourceId=None, CompartmentId=None, DefinedTags=None, EntityTypeInternalName=None, EntityTypeName=None, FreeformTags=None, Hostname=None, Id=None, LifecycleDetails=None, ManagementAgentCompartmentId=None, ManagementAgentDisplayName=None, ManagementAgentId=None, Name=None, Namespace=None, Properties=None, SourceId=None, State=None, TimeCreated=None, TimeUpdated=None, TimezoneRegion=None, Timeouts=None)
