...
TYPE_NAME = "TF::OCI::KmsKeyVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_kms_key_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CompartmentId", "Id", "IsPrimary", "KeyVersionId", "PublicKey", "ReplicaDetails", "RestoredFromKeyId", "RestoredFromKeyVersionId", "State", "TimeCreated", "VaultId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_kms_key_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_kms_key_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, Id=None, IsPrimary=None, KeyId=None, KeyVersionId=None, ManagementEndpoint=None, PublicKey=None, ReplicaDetails=None, RestoredFromKeyId=None, RestoredFromKeyVersionId=None, State=None, TimeCreated=None, TimeOfDeletion=None, VaultId=None, Timeouts=None)
