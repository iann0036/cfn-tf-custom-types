...
TYPE_NAME = "TF::OCI::DatabaseDatabaseSoftwareImage"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_database_software_image/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_database_software_image',
                'returnValues': ["DatabaseSoftwareImageIncludedPatches", "Id", "IncludedPatchesSummary", "IsUpgradeSupported", "LifecycleDetails", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_database_software_image/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_database_software_image/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DatabaseSoftwareImageIncludedPatches=None, DatabaseSoftwareImageOneOffPatches=None, DatabaseVersion=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, ImageShapeFamily=None, ImageType=None, IncludedPatchesSummary=None, IsUpgradeSupported=None, LifecycleDetails=None, LsInventory=None, PatchSet=None, SourceDbHomeId=None, State=None, TimeCreated=None, Timeouts=None)
