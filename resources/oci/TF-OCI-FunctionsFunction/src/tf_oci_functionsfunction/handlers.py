...
TYPE_NAME = "TF::OCI::FunctionsFunction"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_functions_function/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_functions_function/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_functions_function/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplicationId=None, CompartmentId=None, Config=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, Image=None, ImageDigest=None, InvokeEndpoint=None, MemoryInMbs=None, State=None, TimeCreated=None, TimeUpdated=None, TimeoutInSeconds=None, Timeouts=None, TraceConfig=None)
