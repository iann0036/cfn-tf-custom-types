...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_volume/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AutoTunedVpusPerGb", "Id", "IsHydrated", "State", "SystemTags", "TimeCreated", "VolumeGroupId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_volume/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_volume/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoTunedVpusPerGb=None, AvailabilityDomain=None, BackupPolicyId=None, BlockVolumeReplicasDeletion=None, CompartmentId=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, IsAutoTuneEnabled=None, IsHydrated=None, KmsKeyId=None, SizeInGbs=None, SizeInMbs=None, State=None, SystemTags=None, TimeCreated=None, VolumeBackupId=None, VolumeGroupId=None, VpusPerGb=None, BlockVolumeReplicas=None, SourceDetails=None, Timeouts=None)
