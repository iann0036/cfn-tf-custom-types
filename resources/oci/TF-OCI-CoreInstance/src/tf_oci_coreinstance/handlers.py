...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, BootVolumeId=None, CapacityReservationId=None, CompartmentId=None, DedicatedVmHostId=None, DefinedTags=None, DisplayName=None, ExtendedMetadata=None, FaultDomain=None, FreeformTags=None, HostnameLabel=None, Id=None, Image=None, IpxeScript=None, IsPvEncryptionInTransitEnabled=None, LaunchMode=None, Metadata=None, PreserveBootVolume=None, PrivateIp=None, PublicIp=None, Region=None, Shape=None, State=None, SubnetId=None, SystemTags=None, TimeCreated=None, TimeMaintenanceRebootDue=None, AgentConfig=None, AvailabilityConfig=None, CreateVnicDetails=None, InstanceOptions=None, LaunchOptions=None, PlatformConfig=None, PreemptibleInstanceConfig=None, ShapeConfig=None, SourceDetails=None, Timeouts=None)
