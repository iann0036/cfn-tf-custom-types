...
TYPE_NAME = "TF::OCI::OptimizerResourceAction"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_optimizer_resource_action/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_optimizer_resource_action',
                'returnValues': ["Action", "CategoryId", "CompartmentId", "CompartmentName", "EstimatedCostSaving", "ExtendedMetadata", "Id", "Metadata", "Name", "RecommendationId", "ResourceId", "ResourceType", "State", "TimeCreated", "TimeStatusBegin", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_optimizer_resource_action/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_optimizer_resource_action/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, CategoryId=None, CompartmentId=None, CompartmentName=None, EstimatedCostSaving=None, ExtendedMetadata=None, Id=None, Metadata=None, Name=None, RecommendationId=None, ResourceActionId=None, ResourceId=None, ResourceType=None, State=None, Status=None, TimeCreated=None, TimeStatusBegin=None, TimeStatusEnd=None, TimeUpdated=None, Timeouts=None)
