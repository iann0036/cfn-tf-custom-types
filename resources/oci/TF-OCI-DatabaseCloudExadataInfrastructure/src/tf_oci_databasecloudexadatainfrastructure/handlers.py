...
TYPE_NAME = "TF::OCI::DatabaseCloudExadataInfrastructure"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_cloud_exadata_infrastructure/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_cloud_exadata_infrastructure',
                'returnValues': ["AvailableStorageSizeInGbs", "Id", "LastMaintenanceRunId", "LifecycleDetails", "NextMaintenanceRunId", "State", "TimeCreated", "TotalStorageSizeInGbs"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_cloud_exadata_infrastructure/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_cloud_exadata_infrastructure/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, AvailableStorageSizeInGbs=None, CompartmentId=None, ComputeCount=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, LastMaintenanceRunId=None, LifecycleDetails=None, NextMaintenanceRunId=None, Shape=None, State=None, StorageCount=None, TimeCreated=None, TotalStorageSizeInGbs=None, MaintenanceWindow=None, Timeouts=None)
