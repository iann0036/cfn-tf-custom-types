...
TYPE_NAME = "TF::OCI::ManagementAgentManagementAgent"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_management_agent_management_agent/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_management_agent_management_agent',
                'returnValues': ["AvailabilityStatus", "CompartmentId", "Host", "Id", "InstallKeyId", "InstallPath", "LifecycleDetails", "PlatformName", "PlatformType", "PlatformVersion", "PluginList", "State", "TimeCreated", "TimeLastHeartbeat", "TimeUpdated", "Version"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_management_agent_management_agent/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_management_agent_management_agent/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityStatus=None, CompartmentId=None, DefinedTags=None, DeployPluginsId=None, DisplayName=None, FreeformTags=None, Host=None, Id=None, InstallKeyId=None, InstallPath=None, IsAgentAutoUpgradable=None, LifecycleDetails=None, ManagedAgentId=None, PlatformName=None, PlatformType=None, PlatformVersion=None, PluginList=None, State=None, TimeCreated=None, TimeLastHeartbeat=None, TimeUpdated=None, Version=None, Timeouts=None)
