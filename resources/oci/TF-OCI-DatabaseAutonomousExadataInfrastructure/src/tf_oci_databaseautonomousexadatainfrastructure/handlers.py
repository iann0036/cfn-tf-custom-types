...
TYPE_NAME = "TF::OCI::DatabaseAutonomousExadataInfrastructure"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_autonomous_exadata_infrastructure/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Hostname", "Id", "LastMaintenanceRunId", "LifecycleDetails", "MaintenanceWindow", "NextMaintenanceRunId", "ScanDnsName", "State", "TimeCreated", "ZoneId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_autonomous_exadata_infrastructure/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_autonomous_exadata_infrastructure/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, CompartmentId=None, CreateAsync=None, DefinedTags=None, DisplayName=None, Domain=None, FreeformTags=None, Hostname=None, Id=None, LastMaintenanceRunId=None, LicenseModel=None, LifecycleDetails=None, MaintenanceWindow=None, NextMaintenanceRunId=None, NsgIds=None, ScanDnsName=None, Shape=None, State=None, SubnetId=None, TimeCreated=None, ZoneId=None, MaintenanceWindowDetails=None, Timeouts=None)
