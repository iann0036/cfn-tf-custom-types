...
TYPE_NAME = "TF::OCI::CoreVirtualNetwork"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_virtual_network/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["DefaultDhcpOptionsId", "DefaultRouteTableId", "DefaultSecurityListId", "Id", "Ipv6cidrBlocks", "State", "TimeCreated", "VcnDomainName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_virtual_network/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_virtual_network/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CidrBlock=None, CidrBlocks=None, CompartmentId=None, DefaultDhcpOptionsId=None, DefaultRouteTableId=None, DefaultSecurityListId=None, DefinedTags=None, DisplayName=None, DnsLabel=None, FreeformTags=None, Id=None, Ipv6cidrBlocks=None, IsIpv6enabled=None, State=None, TimeCreated=None, VcnDomainName=None, Timeouts=None)
