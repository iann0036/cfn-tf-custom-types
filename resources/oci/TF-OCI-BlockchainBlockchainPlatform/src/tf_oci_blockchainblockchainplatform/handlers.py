...
TYPE_NAME = "TF::OCI::BlockchainBlockchainPlatform"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_blockchain_blockchain_platform/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_blockchain_blockchain_platform',
                'returnValues': ["ComponentDetails", "HostOcpuUtilizationInfo", "Id", "IsMultiAd", "LifecycleDetails", "PlatformShapeType", "ServiceEndpoint", "ServiceVersion", "State", "StorageUsedInTbs", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_blockchain_blockchain_platform/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_blockchain_blockchain_platform/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CaCertArchiveText=None, CompartmentId=None, ComponentDetails=None, ComputeShape=None, DefinedTags=None, Description=None, DisplayName=None, FederatedUserId=None, FreeformTags=None, HostOcpuUtilizationInfo=None, Id=None, IdcsAccessToken=None, IsByol=None, IsMultiAd=None, LifecycleDetails=None, LoadBalancerShape=None, PlatformRole=None, PlatformShapeType=None, ServiceEndpoint=None, ServiceVersion=None, State=None, StorageSizeInTbs=None, StorageUsedInTbs=None, TimeCreated=None, TimeUpdated=None, TotalOcpuCapacity=None, Replicas=None, Timeouts=None)
