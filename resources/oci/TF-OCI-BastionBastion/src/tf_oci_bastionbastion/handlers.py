...
TYPE_NAME = "TF::OCI::BastionBastion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_bastion_bastion/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_bastion_bastion',
                'returnValues': ["Id", "LifecycleDetails", "MaxSessionsAllowed", "PrivateEndpointIpAddress", "State", "SystemTags", "TargetVcnId", "TimeCreated", "TimeUpdated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_bastion_bastion/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_bastion_bastion/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BastionType=None, ClientCidrBlockAllowList=None, CompartmentId=None, DefinedTags=None, FreeformTags=None, Id=None, LifecycleDetails=None, MaxSessionTtlInSeconds=None, MaxSessionsAllowed=None, Name=None, PhoneBookEntry=None, PrivateEndpointIpAddress=None, State=None, StaticJumpHostIpAddresses=None, SystemTags=None, TargetSubnetId=None, TargetVcnId=None, TimeCreated=None, TimeUpdated=None, Timeouts=None)
