...
TYPE_NAME = "TF::OCI::DatabaseBackup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_backup/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AvailabilityDomain", "CompartmentId", "DatabaseEdition", "DatabaseSizeInGbs", "Id", "KmsKeyId", "LifecycleDetails", "Shape", "State", "TimeEnded", "TimeStarted", "Type", "Version"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_backup/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_backup/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, CompartmentId=None, DatabaseEdition=None, DatabaseId=None, DatabaseSizeInGbs=None, DisplayName=None, Id=None, KmsKeyId=None, LifecycleDetails=None, Shape=None, State=None, TimeEnded=None, TimeStarted=None, Type=None, Version=None, Timeouts=None)
