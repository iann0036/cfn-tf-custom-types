...
TYPE_NAME = "TF::OCI::CoreVolumeAttachment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_volume_attachment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AvailabilityDomain", "ChapSecret", "ChapUsername", "Id", "Ipv4", "Iqn", "IsMultipath", "IscsiLoginState", "MultipathDevices", "Port", "State", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_volume_attachment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_volume_attachment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AttachmentType=None, AvailabilityDomain=None, ChapSecret=None, ChapUsername=None, CompartmentId=None, Device=None, DisplayName=None, EncryptionInTransitType=None, Id=None, InstanceId=None, Ipv4=None, Iqn=None, IsMultipath=None, IsPvEncryptionInTransitEnabled=None, IsReadOnly=None, IsShareable=None, IscsiLoginState=None, MultipathDevices=None, Port=None, State=None, TimeCreated=None, UseChap=None, VolumeId=None, Timeouts=None)
