...
TYPE_NAME = "TF::OCI::HealthChecksPingMonitor"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_health_checks_ping_monitor/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_health_checks_ping_monitor/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_health_checks_ping_monitor/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, DisplayName=None, FreeformTags=None, HomeRegion=None, Id=None, IntervalInSeconds=None, IsEnabled=None, Port=None, Protocol=None, ResultsUrl=None, Targets=None, TimeCreated=None, TimeoutInSeconds=None, VantagePointNames=None, Timeouts=None)
