...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_subnet/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Id", "Ipv6virtualRouterIp", "State", "SubnetDomainName", "TimeCreated", "VirtualRouterIp", "VirtualRouterMac"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_core_subnet/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_core_subnet/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, CidrBlock=None, CompartmentId=None, DefinedTags=None, DhcpOptionsId=None, DisplayName=None, DnsLabel=None, FreeformTags=None, Id=None, Ipv6cidrBlock=None, Ipv6virtualRouterIp=None, ProhibitInternetIngress=None, ProhibitPublicIpOnVnic=None, RouteTableId=None, SecurityListIds=None, State=None, SubnetDomainName=None, TimeCreated=None, VcnId=None, VirtualRouterIp=None, VirtualRouterMac=None, Timeouts=None)
