...
TYPE_NAME = "TF::OCI::DataflowApplication"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_dataflow_application/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_dataflow_application/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_dataflow_application/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ArchiveUri=None, Arguments=None, ClassName=None, CompartmentId=None, Configuration=None, DefinedTags=None, Description=None, DisplayName=None, DriverShape=None, Execute=None, ExecutorShape=None, FileUri=None, FreeformTags=None, Id=None, Language=None, LogsBucketUri=None, NumExecutors=None, OwnerPrincipalId=None, OwnerUserName=None, PrivateEndpointId=None, SparkVersion=None, State=None, TimeCreated=None, TimeUpdated=None, WarehouseBucketUri=None, Parameters=None, Timeouts=None)
