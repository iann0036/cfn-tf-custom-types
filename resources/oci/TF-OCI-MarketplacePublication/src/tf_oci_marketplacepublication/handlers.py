...
TYPE_NAME = "TF::OCI::MarketplacePublication"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_marketplace_publication/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_marketplace_publication',
                'returnValues': ["Icon", "Id", "PackageType", "State", "SupportedOperatingSystems", "TimeCreated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_marketplace_publication/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_marketplace_publication/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompartmentId=None, DefinedTags=None, FreeformTags=None, Icon=None, Id=None, IsAgreementAcknowledged=None, ListingType=None, LongDescription=None, Name=None, PackageType=None, ShortDescription=None, State=None, SupportedOperatingSystems=None, TimeCreated=None, PackageDetails=None, SupportContacts=None, Timeouts=None)
