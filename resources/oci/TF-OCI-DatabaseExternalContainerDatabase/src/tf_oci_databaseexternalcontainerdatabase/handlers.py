...
TYPE_NAME = "TF::OCI::DatabaseExternalContainerDatabase"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_external_container_database/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'oci_database_external_container_database',
                'returnValues': ["CharacterSet", "DatabaseConfiguration", "DatabaseEdition", "DatabaseManagementConfig", "DatabaseVersion", "DbId", "DbPacks", "DbUniqueName", "Id", "LifecycleDetails", "NcharacterSet", "State", "TimeCreated", "TimeZone"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oci_database_external_container_database/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oci_database_external_container_database/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CharacterSet=None, CompartmentId=None, DatabaseConfiguration=None, DatabaseEdition=None, DatabaseManagementConfig=None, DatabaseVersion=None, DbId=None, DbPacks=None, DbUniqueName=None, DefinedTags=None, DisplayName=None, FreeformTags=None, Id=None, LifecycleDetails=None, NcharacterSet=None, State=None, TimeCreated=None, TimeZone=None, Timeouts=None)
