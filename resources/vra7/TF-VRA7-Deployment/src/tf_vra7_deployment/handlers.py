...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra7_deployment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'vmware/vra7',
                'providerTypeName': 'vra7',
                'terraformTypeName': 'vra7_deployment',
                'returnValues': ["CreatedDate", "DeploymentId", "Id", "Name", "Owners", "RequestStatus"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra7_deployment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra7_deployment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BusinessgroupId=None, BusinessgroupName=None, CatalogItemId=None, CatalogItemName=None, CreatedDate=None, DeploymentConfiguration=None, DeploymentDestroy=None, DeploymentId=None, Description=None, ExpiryDate=None, Id=None, LeaseDays=None, Name=None, Owners=None, Reasons=None, RequestStatus=None, WaitTimeout=None, ResourceConfiguration=None)
