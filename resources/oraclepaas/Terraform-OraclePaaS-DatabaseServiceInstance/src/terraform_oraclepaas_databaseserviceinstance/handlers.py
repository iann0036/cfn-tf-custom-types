...
def create_handler(
    ...
    try:
        ...
        tf['resource']['oraclepaas_database_service_instance'][request.logicalResourceIdentifier] = {}

        if "oraclepaas" == "aws":
            creds = session.credentials
            tf["provider"]["oraclepaas"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("oraclepaas"))['SecretString']
            tf["provider"]["oraclepaas"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['oraclepaas_database_service_instance'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
