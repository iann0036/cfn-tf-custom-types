...
TYPE_NAME = "TF::OraclePaaS::ApplicationContainer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oraclepaas_application_container/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oraclepaas_application_container/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oraclepaas_application_container/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppUrl=None, ArchiveUrl=None, AuthType=None, AvailabilityDomain=None, DeploymentFile=None, GitPassword=None, GitRepository=None, GitUsername=None, Id=None, LoadBalancerSubnets=None, ManifestFile=None, Name=None, Notes=None, NotificationEmail=None, Region=None, Runtime=None, SubscriptionType=None, Tags=None, WebUrl=None, Deployment=None, Manifest=None, Timeouts=None)
