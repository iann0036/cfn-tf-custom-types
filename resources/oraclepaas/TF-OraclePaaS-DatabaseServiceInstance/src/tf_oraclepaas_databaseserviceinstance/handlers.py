...
TYPE_NAME = "TF::OraclePaaS::DatabaseServiceInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oraclepaas_database_service_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/oraclepaas',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oraclepaas_database_service_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oraclepaas_database_service_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, BringYourOwnLicense=None, CloudStorageContainer=None, ComputeSiteName=None, DbaasMonitorUrl=None, Description=None, DesiredState=None, Edition=None, EmUrl=None, GlassfishUrl=None, HighPerformanceStorage=None, Id=None, IdentityDomain=None, IpNetwork=None, IpReservations=None, Level=None, Name=None, NotificationEmail=None, Region=None, Shape=None, SshPublicKey=None, Status=None, Subnet=None, SubscriptionType=None, Uri=None, Version=None, Backups=None, DatabaseConfiguration=None, DefaultAccessRules=None, HybridDisasterRecovery=None, InstantiateFromBackup=None, Standby=None, Timeouts=None)
