...
TYPE_NAME = "TF::OraclePaaS::JavaServiceInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/oraclepaas_java_service_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/oraclepaas_java_service_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/oraclepaas_java_service_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AssignPublicIp=None, AvailabilityDomain=None, BackupDestination=None, BringYourOwnLicense=None, Description=None, DesiredState=None, Edition=None, EnableAdminConsole=None, ForceDelete=None, Id=None, IpNetwork=None, Level=None, MeteringFrequency=None, Name=None, NotificationEmail=None, Region=None, ServiceVersion=None, SnapshotName=None, SourceServiceName=None, SshPublicKey=None, Status=None, Subnet=None, UseIdentityService=None, Backups=None, LoadBalancer=None, OracleTrafficDirector=None, Timeouts=None, WeblogicServer=None)
