...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/splunk_indexes/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'splunk_indexes',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/splunk_indexes/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/splunk_indexes/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BlockSignSize=None, BucketRebuildMemoryHint=None, ColdPath=None, ColdToFrozenDir=None, ColdToFrozenScript=None, CompressRawdata=None, Datatype=None, EnableOnlineBucketRepair=None, FrozenTimePeriodInSecs=None, HomePath=None, Id=None, MaxBloomBackfillBucketAge=None, MaxConcurrentOptimizes=None, MaxDataSize=None, MaxHotBuckets=None, MaxHotIdleSecs=None, MaxHotSpanSecs=None, MaxMemMb=None, MaxMetaEntries=None, MaxTimeUnreplicatedNoAcks=None, MaxTimeUnreplicatedWithAcks=None, MaxTotalDataSizeMb=None, MaxWarmDbCount=None, MinRawFileSyncSecs=None, MinStreamGroupQueueSize=None, Name=None, PartialServiceMetaPeriod=None, ProcessTrackerServiceInterval=None, QuarantineFutureSecs=None, QuarantinePastSecs=None, RawChunkSizeBytes=None, RepFactor=None, RotatePeriodInSecs=None, ServiceMetaPeriod=None, SyncMeta=None, ThawedPath=None, ThrottleCheckPeriod=None, TstatsHomePath=None, WarmToColdScript=None, Acl=None)
