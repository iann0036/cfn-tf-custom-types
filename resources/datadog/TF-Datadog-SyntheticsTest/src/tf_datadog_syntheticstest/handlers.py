...
TYPE_NAME = "TF::Datadog::SyntheticsTest"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/datadog_synthetics_test/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/datadog_synthetics_test/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/datadog_synthetics_test/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DeviceIds=None, Id=None, Locations=None, Message=None, MonitorId=None, Name=None, RequestHeaders=None, RequestQuery=None, SetCookie=None, Status=None, Subtype=None, Tags=None, Type=None, ApiStep=None, Assertion=None, BrowserStep=None, BrowserVariable=None, ConfigVariable=None, OptionsList=None, RequestBasicauth=None, RequestClientCertificate=None, RequestDefinition=None)
