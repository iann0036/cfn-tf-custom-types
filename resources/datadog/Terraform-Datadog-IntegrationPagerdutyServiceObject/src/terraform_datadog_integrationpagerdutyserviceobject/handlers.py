...
def create_handler(
    ...
    try:
        ...
        tf['resource']['datadog_integration_pagerduty_service_object'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("datadog"))['SecretString']
            tf["provider"]["datadog"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['datadog_integration_pagerduty_service_object'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
