...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/datadog_monitor/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'DataDog/datadog',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/datadog_monitor/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/datadog_monitor/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EnableLogsSample=None, EscalationMessage=None, EvaluationDelay=None, ForceDelete=None, GroupbySimpleMonitor=None, Id=None, IncludeTags=None, Locked=None, Message=None, Name=None, NewHostDelay=None, NoDataTimeframe=None, NotifyAudit=None, NotifyNoData=None, Priority=None, Query=None, RenotifyInterval=None, RequireFullWindow=None, RestrictedRoles=None, Tags=None, TimeoutH=None, Type=None, Validate=None, MonitorThresholdWindows=None, MonitorThresholds=None)
