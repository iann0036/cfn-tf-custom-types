...
def create_handler(
    ...
    try:
        ...
        tf['resource']['packet_port_vlan_attachment'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("packet"))['SecretString']
            tf["provider"]["packet"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['packet_port_vlan_attachment'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
