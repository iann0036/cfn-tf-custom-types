...
def create_handler(
    ...
    try:
        ...
        tf['resource']['consul_acl_token_policy_attachment'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("consul"))['SecretString']
            tf["provider"]["consul"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['consul_acl_token_policy_attachment'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
