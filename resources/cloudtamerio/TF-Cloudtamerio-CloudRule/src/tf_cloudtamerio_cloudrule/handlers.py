...
TYPE_NAME = "TF::Cloudtamerio::CloudRule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudtamerio_cloud_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'cloudtamerio_cloud_rule',
                'returnValues': ["BuiltIn", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudtamerio_cloud_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudtamerio_cloud_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BuiltIn=None, Description=None, Id=None, LastUpdated=None, Name=None, PostWebhookId=None, PreWebhookId=None, AwsCloudformationTemplates=None, AwsIamPolicies=None, AzureArmTemplateDefinitions=None, AzurePolicyDefinitions=None, AzureRoleDefinitions=None, ComplianceStandards=None, InternalAwsAmis=None, InternalAwsServiceCatalogPortfolios=None, Ous=None, OwnerUserGroups=None, OwnerUsers=None, Projects=None, ServiceControlPolicies=None)
