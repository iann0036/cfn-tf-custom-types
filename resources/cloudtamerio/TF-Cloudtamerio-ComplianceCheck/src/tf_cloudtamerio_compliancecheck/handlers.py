...
TYPE_NAME = "TF::Cloudtamerio::ComplianceCheck"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudtamerio_compliance_check/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'cloudtamer-io/cloudtamerio',
                'providerTypeName': 'cloudtamerio',
                'terraformTypeName': 'cloudtamerio_compliance_check',
                'returnValues': ["CreatedAt", "CtManaged", "Id", "LastScanId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudtamerio_compliance_check/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudtamerio_compliance_check/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AzurePolicyId=None, Body=None, CloudProviderId=None, ComplianceCheckTypeId=None, CreatedAt=None, CreatedByUserId=None, CtManaged=None, Description=None, FrequencyMinutes=None, FrequencyTypeId=None, Id=None, IsAllRegions=None, IsAutoArchived=None, LastScanId=None, LastUpdated=None, Name=None, Regions=None, SeverityTypeId=None, OwnerUserGroups=None, OwnerUsers=None)
