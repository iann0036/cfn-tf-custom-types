...
def create_handler(
    ...
    try:
        ...
        tf['resource']['mailgun_domain'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("mailgun"))['SecretString']
            tf["provider"]["mailgun"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['mailgun_domain'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
