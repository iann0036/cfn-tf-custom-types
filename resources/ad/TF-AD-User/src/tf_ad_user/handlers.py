...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ad_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/ad',
                'providerTypeName': 'ad',
                'terraformTypeName': 'ad_user',
                'returnValues': ["Id", "Sid"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ad_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ad_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CannotChangePassword=None, City=None, Company=None, Container=None, Country=None, CustomAttributes=None, Department=None, Description=None, DisplayName=None, Division=None, EmailAddress=None, EmployeeId=None, EmployeeNumber=None, Enabled=None, Fax=None, GivenName=None, HomeDirectory=None, HomeDrive=None, HomePage=None, HomePhone=None, Id=None, InitialPassword=None, Initials=None, MobilePhone=None, Office=None, OfficePhone=None, Organization=None, OtherName=None, PasswordNeverExpires=None, PoBox=None, PostalCode=None, PrincipalName=None, SamAccountName=None, Sid=None, SmartCardLogonRequired=None, State=None, StreetAddress=None, Surname=None, Title=None, TrustedForDelegation=None)
