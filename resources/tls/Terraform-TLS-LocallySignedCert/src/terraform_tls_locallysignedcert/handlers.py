...
def create_handler(
    ...
    try:
        ...
        tf['resource']['tls_locally_signed_cert'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("tls"))['SecretString']
            tf["provider"]["tls"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['tls_locally_signed_cert'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
