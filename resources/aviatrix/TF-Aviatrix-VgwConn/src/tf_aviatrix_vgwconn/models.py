# DO NOT modify this file by hand, changes will be overwritten
import sys
from dataclasses import dataclass
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

from cloudformation_cli_python_lib.interface import (
    BaseModel,
    BaseResourceHandlerRequest,
)
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class ResourceHandlerRequest(BaseResourceHandlerRequest):
    # pylint: disable=invalid-name
    desiredResourceState: Optional["ResourceModel"]
    previousResourceState: Optional["ResourceModel"]


@dataclass
class ResourceModel(BaseModel):
    tfcfnid: Optional[str]
    BgpLocalAsNum: Optional[str]
    BgpVgwAccount: Optional[str]
    BgpVgwId: Optional[str]
    BgpVgwRegion: Optional[str]
    ConnName: Optional[str]
    EnableEventTriggeredHa: Optional[bool]
    EnableLearnedCidrsApproval: Optional[bool]
    GwName: Optional[str]
    Id: Optional[str]
    ManualBgpAdvertisedCidrs: Optional[Sequence[str]]
    VpcId: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_ResourceModel"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_ResourceModel"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            tfcfnid=json_data.get("tfcfnid"),
            BgpLocalAsNum=json_data.get("BgpLocalAsNum"),
            BgpVgwAccount=json_data.get("BgpVgwAccount"),
            BgpVgwId=json_data.get("BgpVgwId"),
            BgpVgwRegion=json_data.get("BgpVgwRegion"),
            ConnName=json_data.get("ConnName"),
            EnableEventTriggeredHa=json_data.get("EnableEventTriggeredHa"),
            EnableLearnedCidrsApproval=json_data.get("EnableLearnedCidrsApproval"),
            GwName=json_data.get("GwName"),
            Id=json_data.get("Id"),
            ManualBgpAdvertisedCidrs=json_data.get("ManualBgpAdvertisedCidrs"),
            VpcId=json_data.get("VpcId"),
        )


# work around possible type aliasing issues when variable has same name as a model
_ResourceModel = ResourceModel


