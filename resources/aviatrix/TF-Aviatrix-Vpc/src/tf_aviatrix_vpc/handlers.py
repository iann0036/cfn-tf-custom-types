...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_vpc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_vpc',
                'returnValues': ["AzureVnetResourceId", "Id", "PrivateSubnets", "PublicSubnets", "RouteTables", "VpcId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_vpc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_vpc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountName=None, AviatrixFirenetVpc=None, AviatrixTransitVpc=None, AzureVnetResourceId=None, Cidr=None, CloudType=None, EnableNativeGwlb=None, EnablePrivateOobSubnet=None, Id=None, Name=None, NumOfSubnetPairs=None, PrivateSubnets=None, PublicSubnets=None, Region=None, ResourceGroup=None, RouteTables=None, SubnetSize=None, VpcId=None, Subnets=None)
