...
TYPE_NAME = "TF::Aviatrix::DeviceTransitGatewayAttachment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_device_transit_gateway_attachment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_device_transit_gateway_attachment',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_device_transit_gateway_attachment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_device_transit_gateway_attachment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ConnectionName=None, DeviceBgpAsn=None, DeviceName=None, EnableEventTriggeredHa=None, EnableGlobalAccelerator=None, EnableLearnedCidrsApproval=None, Id=None, LocalTunnelIp=None, ManualBgpAdvertisedCidrs=None, Phase1Authentication=None, Phase1DhGroups=None, Phase1Encryption=None, Phase2Authentication=None, Phase2DhGroups=None, Phase2Encryption=None, PreSharedKey=None, RemoteTunnelIp=None, TransitGatewayBgpAsn=None, TransitGatewayName=None, VpcId=None)
