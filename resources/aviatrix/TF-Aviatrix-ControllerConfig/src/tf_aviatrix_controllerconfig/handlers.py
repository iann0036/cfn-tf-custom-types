...
TYPE_NAME = "TF::Aviatrix::ControllerConfig"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_controller_config/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_controller_config',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_controller_config/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_controller_config/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AwsGuardDutyScanningInterval=None, BackupAccountName=None, BackupBucketName=None, BackupCloudType=None, BackupConfiguration=None, BackupContainerName=None, BackupRegion=None, BackupStorageName=None, CaCertificateFilePath=None, EnableVpcDnsServer=None, FqdnExceptionRule=None, HttpAccess=None, Id=None, MultipleBackups=None, SecurityGroupManagement=None, ServerPrivateKeyFilePath=None, ServerPublicCertificateFilePath=None, SgManagementAccountName=None, TargetVersion=None, Version=None)
