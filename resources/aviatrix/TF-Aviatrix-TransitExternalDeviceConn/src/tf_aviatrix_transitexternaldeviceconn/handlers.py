...
TYPE_NAME = "TF::Aviatrix::TransitExternalDeviceConn"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_transit_external_device_conn/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_transit_external_device_conn',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_transit_external_device_conn/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_transit_external_device_conn/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApprovedCidrs=None, BackupBgpRemoteAsNum=None, BackupDirectConnect=None, BackupLocalLanIp=None, BackupLocalTunnelCidr=None, BackupPreSharedKey=None, BackupRemoteGatewayIp=None, BackupRemoteLanIp=None, BackupRemoteTunnelCidr=None, BgpLocalAsNum=None, BgpRemoteAsNum=None, ConnectionName=None, ConnectionType=None, CustomAlgorithms=None, DirectConnect=None, EnableEdgeSegmentation=None, EnableEventTriggeredHa=None, EnableIkev2=None, EnableLearnedCidrsApproval=None, GwName=None, HaEnabled=None, Id=None, LocalLanIp=None, LocalTunnelCidr=None, ManualBgpAdvertisedCidrs=None, Phase1RemoteIdentifier=None, Phase1Authentication=None, Phase1DhGroups=None, Phase1Encryption=None, Phase2Authentication=None, Phase2DhGroups=None, Phase2Encryption=None, PreSharedKey=None, RemoteGatewayIp=None, RemoteLanIp=None, RemoteSubnet=None, RemoteTunnelCidr=None, RemoteVpcName=None, SwitchToHaStandbyGateway=None, TunnelProtocol=None, VpcId=None)
