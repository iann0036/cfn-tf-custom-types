...
TYPE_NAME = "TF::Aviatrix::Site2cloud"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_site2cloud/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_site2cloud',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_site2cloud/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_site2cloud/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BackupGatewayName=None, BackupLocalTunnelIp=None, BackupPreSharedKey=None, BackupRemoteGatewayIp=None, BackupRemoteGatewayLatitude=None, BackupRemoteGatewayLongitude=None, BackupRemoteTunnelIp=None, ConnectionName=None, ConnectionType=None, CustomAlgorithms=None, CustomMapped=None, EnableActiveActive=None, EnableDeadPeerDetection=None, EnableEventTriggeredHa=None, EnableIkev2=None, EnableSingleIpHa=None, ForwardTrafficToTransit=None, HaEnabled=None, Id=None, LocalDestinationRealCidrs=None, LocalDestinationVirtualCidrs=None, LocalSourceRealCidrs=None, LocalSourceVirtualCidrs=None, LocalSubnetCidr=None, LocalSubnetVirtual=None, LocalTunnelIp=None, Phase1RemoteIdentifier=None, Phase1Authentication=None, Phase1DhGroups=None, Phase1Encryption=None, Phase2Authentication=None, Phase2DhGroups=None, Phase2Encryption=None, PreSharedKey=None, PrimaryCloudGatewayName=None, PrivateRouteEncryption=None, RemoteDestinationRealCidrs=None, RemoteDestinationVirtualCidrs=None, RemoteGatewayIp=None, RemoteGatewayLatitude=None, RemoteGatewayLongitude=None, RemoteGatewayType=None, RemoteSourceRealCidrs=None, RemoteSourceVirtualCidrs=None, RemoteSubnetCidr=None, RemoteSubnetVirtual=None, RemoteTunnelIp=None, RouteTableList=None, SslServerPool=None, TunnelType=None, VpcId=None)
