...
TYPE_NAME = "TF::Aviatrix::FirewallInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_firewall_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_firewall_instance',
                'returnValues': ["CloudType", "EgressInterface", "GcpVpcId", "Id", "InstanceId", "LanInterface", "ManagementInterface", "PublicIp"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_firewall_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_firewall_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityDomain=None, BootstrapBucketName=None, BootstrapStorageName=None, CloudType=None, ContainerFolder=None, EgressInterface=None, EgressSubnet=None, EgressVpcId=None, FaultDomain=None, FileShareFolder=None, FirenetGwName=None, FirewallImage=None, FirewallImageId=None, FirewallImageVersion=None, FirewallName=None, FirewallSize=None, GcpVpcId=None, IamRole=None, Id=None, InstanceId=None, KeyName=None, LanInterface=None, ManagementInterface=None, ManagementSubnet=None, ManagementVpcId=None, Password=None, PublicIp=None, SasUrlConfig=None, SasUrlLicense=None, ShareDirectory=None, SicKey=None, SshPublicKey=None, StorageAccessKey=None, Tags=None, UserData=None, Username=None, VpcId=None, Zone=None)
