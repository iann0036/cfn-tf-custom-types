...
TYPE_NAME = "TF::Aviatrix::TransitGatewayPeering"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_transit_gateway_peering/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aviatrix_transit_gateway_peering',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aviatrix_transit_gateway_peering/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aviatrix_transit_gateway_peering/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EnableInsaneModeEncryptionOverInternet=None, EnablePeeringOverPrivateNetwork=None, EnableSingleTunnelMode=None, Gateway1ExcludedNetworkCidrs=None, Gateway1ExcludedTgwConnections=None, Gateway2ExcludedNetworkCidrs=None, Gateway2ExcludedTgwConnections=None, Id=None, PrependAsPath1=None, PrependAsPath2=None, TransitGatewayName1=None, TransitGatewayName2=None, TunnelCount=None)
