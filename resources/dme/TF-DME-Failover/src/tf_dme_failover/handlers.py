...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/dme_failover/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'DNSMadeEasy/dme',
                'providerTypeName': 'dme',
                'terraformTypeName': 'dme_failover',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/dme_failover/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/dme_failover/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoFailover=None, ContactList=None, DnsFqdn=None, DnsTimeout=None, Failover=None, HttpFile=None, HttpFqdn=None, HttpQueryString=None, Id=None, Ip1=None, Ip2=None, Ip3=None, Ip4=None, Ip5=None, MaxEmails=None, Monitor=None, Port=None, ProtocolId=None, RecordId=None, SendString=None, Sensitivity=None, SystemDescription=None, Timeout=None)
