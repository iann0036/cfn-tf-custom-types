...
def create_handler(
    ...
    try:
        ...
        tf['resource']['acme_registration'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("acme"))['SecretString']
            tf["provider"]["acme"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['acme_registration'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
