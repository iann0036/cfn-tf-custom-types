...
TYPE_NAME = "TF::AzureStack::NetworkInterface"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_network_interface/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_network_interface/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurestack_network_interface/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppliedDnsServers=None, DnsServers=None, EnableIpForwarding=None, Id=None, InternalDnsNameLabel=None, InternalFqdn=None, Location=None, MacAddress=None, Name=None, NetworkSecurityGroupId=None, PrivateIpAddress=None, PrivateIpAddresses=None, ResourceGroupName=None, Tags=None, VirtualMachineId=None, IpConfiguration=None)
