...
TYPE_NAME = "TF::AzureStack::VirtualNetworkGatewayConnection"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_virtual_network_gateway_connection/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_virtual_network_gateway_connection/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurestack_virtual_network_gateway_connection/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthorizationKey=None, EnableBgp=None, ExpressRouteCircuitId=None, Id=None, LocalNetworkGatewayId=None, Location=None, Name=None, PeerVirtualNetworkGatewayId=None, ResourceGroupName=None, RoutingWeight=None, SharedKey=None, Tags=None, Type=None, VirtualNetworkGatewayId=None)
