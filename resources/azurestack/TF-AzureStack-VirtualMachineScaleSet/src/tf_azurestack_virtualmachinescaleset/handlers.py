...
TYPE_NAME = "TF::AzureStack::VirtualMachineScaleSet"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_virtual_machine_scale_set/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_virtual_machine_scale_set/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurestack_virtual_machine_scale_set/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Id=None, LicenseType=None, Location=None, Name=None, Overprovision=None, Priority=None, ResourceGroupName=None, SinglePlacementGroup=None, Tags=None, UpgradePolicyMode=None, Extension=None, Identity=None, NetworkProfile=None, OsProfile=None, OsProfileLinuxConfig=None, OsProfileSecrets=None, OsProfileWindowsConfig=None, Plan=None, Sku=None, StorageProfileDataDisk=None, StorageProfileImageReference=None, StorageProfileOsDisk=None)
