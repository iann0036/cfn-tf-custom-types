...
TYPE_NAME = "TF::AzureStack::StorageAccount"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_storage_account/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/azurestack',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurestack_storage_account/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurestack_storage_account/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountEncryptionSource=None, AccountKind=None, AccountReplicationType=None, AccountTier=None, AccountType=None, EnableBlobEncryption=None, Id=None, Location=None, Name=None, PrimaryAccessKey=None, PrimaryBlobConnectionString=None, PrimaryBlobEndpoint=None, PrimaryConnectionString=None, PrimaryFileEndpoint=None, PrimaryLocation=None, PrimaryQueueEndpoint=None, PrimaryTableEndpoint=None, ResourceGroupName=None, SecondaryAccessKey=None, SecondaryBlobConnectionString=None, SecondaryBlobEndpoint=None, SecondaryConnectionString=None, SecondaryLocation=None, SecondaryQueueEndpoint=None, SecondaryTableEndpoint=None, Tags=None, CustomDomain=None)
