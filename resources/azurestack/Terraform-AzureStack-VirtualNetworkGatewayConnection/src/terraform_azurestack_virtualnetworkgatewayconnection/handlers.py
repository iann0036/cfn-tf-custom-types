...
def create_handler(
    ...
    try:
        ...
        tf['resource']['azurestack_virtual_network_gateway_connection'][request.logicalResourceIdentifier] = {}

        if "azurestack" == "aws":
            creds = session.credentials
            tf["provider"]["azurestack"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("azurestack"))['SecretString']
            tf["provider"]["azurestack"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['azurestack_virtual_network_gateway_connection'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
