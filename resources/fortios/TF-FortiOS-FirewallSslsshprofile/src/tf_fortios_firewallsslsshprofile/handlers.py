...
TYPE_NAME = "TF::FortiOS::FirewallSslsshprofile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_sslsshprofile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_firewall_sslsshprofile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_sslsshprofile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_firewall_sslsshprofile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Allowlist=None, BlockBlacklistedCertificates=None, BlockBlocklistedCertificates=None, Caname=None, Comment=None, DynamicSortSubtable=None, Id=None, MapiOverHttps=None, Name=None, RpcOverHttps=None, ServerCert=None, ServerCertMode=None, SslAnomaliesLog=None, SslExemptionsLog=None, SslNegotiationLog=None, SupportedAlpn=None, UntrustedCaname=None, UseSslServer=None, Vdomparam=None, Whitelist=None, Ftps=None, Https=None, Imaps=None, Pop3s=None, Smtps=None, Ssh=None, Ssl=None, SslExempt=None, SslServer=None)
