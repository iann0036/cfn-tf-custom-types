...
TYPE_NAME = "TF::FortiOS::SystemWccp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_wccp/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_wccp',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_wccp/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_wccp/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AssignmentBucketFormat=None, AssignmentDstaddrMask=None, AssignmentMethod=None, AssignmentSrcaddrMask=None, AssignmentWeight=None, Authentication=None, CacheEngineMethod=None, CacheId=None, ForwardMethod=None, GroupAddress=None, Id=None, Password=None, Ports=None, PortsDefined=None, PrimaryHash=None, Priority=None, Protocol=None, ReturnMethod=None, RouterId=None, RouterList=None, ServerList=None, ServerType=None, ServiceId=None, ServiceType=None, Vdomparam=None)
