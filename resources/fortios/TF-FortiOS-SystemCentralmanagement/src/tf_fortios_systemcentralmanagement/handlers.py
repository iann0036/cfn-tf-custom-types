...
TYPE_NAME = "TF::FortiOS::SystemCentralmanagement"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_centralmanagement/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_centralmanagement',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_centralmanagement/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_centralmanagement/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowMonitor=None, AllowPushConfiguration=None, AllowPushFirmware=None, AllowRemoteFirmwareUpgrade=None, CaCert=None, DynamicSortSubtable=None, EncAlgorithm=None, Fmg=None, FmgSourceIp=None, FmgSourceIp6=None, FmgUpdatePort=None, Id=None, IncludeDefaultServers=None, Interface=None, InterfaceSelectMethod=None, LocalCert=None, Mode=None, ScheduleConfigRestore=None, ScheduleScriptRestore=None, SerialNumber=None, Type=None, Vdom=None, Vdomparam=None, ServerList=None)
