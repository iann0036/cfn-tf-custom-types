...
TYPE_NAME = "TF::FortiOS::UserNacpolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_nacpolicy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_user_nacpolicy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_nacpolicy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_user_nacpolicy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Category=None, Description=None, DynamicSortSubtable=None, EmsTag=None, Family=None, Host=None, HwVendor=None, HwVersion=None, Id=None, Mac=None, Name=None, Os=None, Src=None, Status=None, SwVersion=None, SwitchAutoAuth=None, SwitchFortilink=None, SwitchMacPolicy=None, SwitchPortPolicy=None, Type=None, User=None, UserGroup=None, Vdomparam=None, SwitchScope=None)
