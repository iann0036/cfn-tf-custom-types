...
TYPE_NAME = "TF::FortiOS::LogmemoryFilter"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_logmemory_filter/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_logmemory_filter',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_logmemory_filter/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_logmemory_filter/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Admin=None, Anomaly=None, Auth=None, CpuMemoryUsage=None, Dhcp=None, Dns=None, DynamicSortSubtable=None, Event=None, Filter=None, FilterType=None, ForwardTraffic=None, Gtp=None, Ha=None, Id=None, Ipsec=None, LdbMonitor=None, LocalTraffic=None, MulticastTraffic=None, NetscanDiscovery=None, NetscanVulnerability=None, Pattern=None, Ppp=None, Radius=None, Severity=None, SnifferTraffic=None, Ssh=None, SslvpnLogAdm=None, SslvpnLogAuth=None, SslvpnLogSession=None, System=None, Vdomparam=None, VipSsl=None, Voip=None, WanOpt=None, WirelessActivity=None, FreeStyle=None)
