...
TYPE_NAME = "TF::FortiOS::FirewallAddress"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_address/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_firewall_address',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_address/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_firewall_address/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowRouting=None, AssociatedInterface=None, CacheTtl=None, ClearpassSpt=None, Color=None, Comment=None, Country=None, DynamicSortSubtable=None, EndIp=None, EndMac=None, EpgName=None, Filter=None, Fqdn=None, Id=None, Interface=None, Name=None, NodeIpOnly=None, ObjId=None, ObjTag=None, ObjType=None, Organization=None, PolicyGroup=None, Sdn=None, SdnAddrType=None, SdnTag=None, StartIp=None, StartMac=None, SubType=None, Subnet=None, SubnetName=None, Tenant=None, Type=None, Uuid=None, Vdomparam=None, Visibility=None, Wildcard=None, WildcardFqdn=None, FssoGroup=None, List=None, Tagging=None)
