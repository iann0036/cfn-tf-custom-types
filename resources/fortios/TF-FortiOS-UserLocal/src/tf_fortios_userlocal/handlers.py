...
TYPE_NAME = "TF::FortiOS::UserLocal"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_local/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_user_local',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_local/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_user_local/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthConcurrentOverride=None, AuthConcurrentValue=None, Authtimeout=None, EmailTo=None, Fortitoken=None, Fosid=None, Id=None, LdapServer=None, Name=None, Passwd=None, PasswdPolicy=None, PasswdTime=None, PpkIdentity=None, PpkSecret=None, RadiusServer=None, SmsCustomServer=None, SmsPhone=None, SmsServer=None, Status=None, TacacsServer=None, TwoFactor=None, TwoFactorAuthentication=None, TwoFactorNotification=None, Type=None, UsernameCaseInsensitivity=None, UsernameCaseSensitivity=None, Vdomparam=None, Workstation=None)
