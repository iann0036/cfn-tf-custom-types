...
TYPE_NAME = "TF::FortiOS::VpncertificateLocal"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_vpncertificate_local/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_vpncertificate_local',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_vpncertificate_local/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_vpncertificate_local/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoRegenerateDays=None, AutoRegenerateDaysWarning=None, CaIdentifier=None, Certificate=None, CmpPath=None, CmpRegenerationMethod=None, CmpServer=None, CmpServerCert=None, Comments=None, Csr=None, EnrollProtocol=None, Id=None, IkeLocalid=None, IkeLocalidType=None, LastUpdated=None, Name=None, NameEncoding=None, Password=None, PrivateKey=None, Range=None, ScepPassword=None, ScepUrl=None, Source=None, SourceIp=None, State=None, Vdomparam=None)
