...
TYPE_NAME = "TF::FortiOS::Wirelesscontrollerhotspot20Hsprofile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_wirelesscontrollerhotspot20_hsprofile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_wirelesscontrollerhotspot20_hsprofile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_wirelesscontrollerhotspot20_hsprofile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_wirelesscontrollerhotspot20_hsprofile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessNetworkAsra=None, AccessNetworkEsr=None, AccessNetworkInternet=None, AccessNetworkType=None, AccessNetworkUesa=None, AnqpDomainId=None, BssTransition=None, ConnCap=None, DeauthRequestTimeout=None, Dgaf=None, DomainName=None, DynamicSortSubtable=None, GasComebackDelay=None, GasFragmentationLimit=None, Hessid=None, Id=None, IpAddrType=None, L2tif=None, N3gppPlmn=None, NaiRealm=None, Name=None, NetworkAuth=None, OperFriendlyName=None, OsuSsid=None, PameBi=None, ProxyArp=None, QosMap=None, RoamingConsortium=None, Vdomparam=None, VenueGroup=None, VenueName=None, VenueType=None, WanMetrics=None, WnmSleepMode=None, OsuProvider=None)
