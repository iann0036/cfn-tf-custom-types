...
TYPE_NAME = "TF::FortiOS::RouterPolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_router_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_router_policy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_router_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_router_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, Comments=None, DstNegate=None, DynamicSortSubtable=None, EndPort=None, EndSourcePort=None, Gateway=None, Id=None, InputDeviceNegate=None, OutputDevice=None, Protocol=None, SeqNum=None, SrcNegate=None, StartPort=None, StartSourcePort=None, Status=None, Tos=None, TosMask=None, Vdomparam=None, Dst=None, Dstaddr=None, InputDevice=None, InternetServiceCustom=None, InternetServiceId=None, Src=None, Srcaddr=None)
