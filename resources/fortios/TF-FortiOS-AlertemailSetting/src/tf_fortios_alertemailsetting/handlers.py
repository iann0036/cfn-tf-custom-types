...
TYPE_NAME = "TF::FortiOS::AlertemailSetting"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_alertemail_setting/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_alertemail_setting',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_alertemail_setting/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_alertemail_setting/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminLoginLogs=None, AlertInterval=None, AmcInterfaceBypassMode=None, AntivirusLogs=None, ConfigurationChangesLogs=None, CriticalInterval=None, DebugInterval=None, EmailInterval=None, EmergencyInterval=None, ErrorInterval=None, FdsLicenseExpiringDays=None, FdsLicenseExpiringWarning=None, FdsUpdateLogs=None, FilterMode=None, FipsCcErrors=None, FirewallAuthenticationFailureLogs=None, FortiguardLogQuotaWarning=None, FssoDisconnectLogs=None, HaLogs=None, Id=None, InformationInterval=None, IpsLogs=None, IpsecErrorsLogs=None, LocalDiskUsage=None, LogDiskUsageWarning=None, Mailto1=None, Mailto2=None, Mailto3=None, NotificationInterval=None, PppErrorsLogs=None, Severity=None, SshLogs=None, SslvpnAuthenticationErrorsLogs=None, Username=None, Vdomparam=None, ViolationTrafficLogs=None, WarningInterval=None, WebfilterLogs=None)
