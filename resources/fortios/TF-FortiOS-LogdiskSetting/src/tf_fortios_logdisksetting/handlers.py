...
TYPE_NAME = "TF::FortiOS::LogdiskSetting"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_logdisk_setting/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_logdisk_setting',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_logdisk_setting/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_logdisk_setting/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Diskfull=None, DlpArchiveQuota=None, FullFinalWarningThreshold=None, FullFirstWarningThreshold=None, FullSecondWarningThreshold=None, Id=None, Interface=None, InterfaceSelectMethod=None, IpsArchive=None, LogQuota=None, MaxLogFileSize=None, MaxPolicyPacketCaptureSize=None, MaximumLogAge=None, ReportQuota=None, RollDay=None, RollSchedule=None, RollTime=None, SourceIp=None, Status=None, Upload=None, UploadDeleteFiles=None, UploadDestination=None, UploadSslConn=None, Uploaddir=None, Uploadip=None, Uploadpass=None, Uploadport=None, Uploadsched=None, Uploadtime=None, Uploadtype=None, Uploaduser=None, Vdomparam=None)
