...
TYPE_NAME = "TF::FortiOS::IcapProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_icap_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_icap_profile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_icap_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_icap_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DynamicSortSubtable=None, Id=None, Methods=None, Name=None, Preview=None, PreviewDataLength=None, ReplacemsgGroup=None, Request=None, RequestFailure=None, RequestPath=None, RequestServer=None, RespmodDefaultAction=None, Response=None, ResponseFailure=None, ResponsePath=None, ResponseReqHdr=None, ResponseServer=None, StreamingContentBypass=None, Vdomparam=None, IcapHeaders=None, RespmodForwardRules=None)
