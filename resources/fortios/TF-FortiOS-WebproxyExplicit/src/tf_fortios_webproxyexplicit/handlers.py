...
TYPE_NAME = "TF::FortiOS::WebproxyExplicit"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_webproxy_explicit/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_webproxy_explicit',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_webproxy_explicit/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_webproxy_explicit/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DynamicSortSubtable=None, FtpIncomingPort=None, FtpOverHttp=None, HttpIncomingPort=None, HttpsIncomingPort=None, HttpsReplacementMessage=None, Id=None, IncomingIp=None, IncomingIp6=None, Ipv6Status=None, MessageUponServerError=None, OutgoingIp=None, OutgoingIp6=None, PacFileData=None, PacFileName=None, PacFileServerPort=None, PacFileServerStatus=None, PacFileUrl=None, PrefDnsResult=None, Realm=None, SecDefaultAction=None, Socks=None, SocksIncomingPort=None, SslAlgorithm=None, Status=None, StrictGuest=None, TraceAuthNoRsp=None, UnknownHttpVersion=None, Vdomparam=None, PacPolicy=None)
