...
TYPE_NAME = "TF::FortiOS::RouterOspf"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_router_ospf/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_router_ospf',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_router_ospf/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_router_ospf/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AbrType=None, AutoCostRefBandwidth=None, Bfd=None, DatabaseOverflow=None, DatabaseOverflowMaxLsas=None, DatabaseOverflowTimeToRecover=None, DefaultInformationMetric=None, DefaultInformationMetricType=None, DefaultInformationOriginate=None, DefaultInformationRouteMap=None, DefaultMetric=None, Distance=None, DistanceExternal=None, DistanceInterArea=None, DistanceIntraArea=None, DistributeListIn=None, DistributeRouteMapIn=None, DynamicSortSubtable=None, Id=None, LogNeighbourChanges=None, RestartMode=None, RestartPeriod=None, Rfc1583Compatible=None, RouterId=None, SpfTimers=None, Vdomparam=None, Area=None, DistributeList=None, Neighbor=None, Network=None, OspfInterface=None, PassiveInterface=None, Redistribute=None, SummaryAddress=None)
