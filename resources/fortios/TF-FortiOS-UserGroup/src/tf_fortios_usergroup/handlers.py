...
TYPE_NAME = "TF::FortiOS::UserGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_user_group',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_user_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthConcurrentOverride=None, AuthConcurrentValue=None, Authtimeout=None, Company=None, DynamicSortSubtable=None, Email=None, Expire=None, ExpireType=None, Fosid=None, GroupType=None, HttpDigestRealm=None, Id=None, MaxAccounts=None, MobilePhone=None, MultipleGuestAdd=None, Name=None, Password=None, SmsCustomServer=None, SmsServer=None, Sponsor=None, SsoAttributeValue=None, UserId=None, UserName=None, Vdomparam=None, Guest=None, Match=None, Member=None)
