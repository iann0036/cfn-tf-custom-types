...
TYPE_NAME = "TF::FortiOS::FirewallInterfacepolicy6"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_interfacepolicy6/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_firewall_interfacepolicy6',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_interfacepolicy6/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_firewall_interfacepolicy6/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddressType=None, ApplicationList=None, ApplicationListStatus=None, AvProfile=None, AvProfileStatus=None, Comments=None, DlpSensor=None, DlpSensorStatus=None, Dsri=None, DynamicSortSubtable=None, EmailfilterProfile=None, EmailfilterProfileStatus=None, Id=None, Interface=None, IpsSensor=None, IpsSensorStatus=None, Label=None, Logtraffic=None, Policyid=None, ScanBotnetConnections=None, SpamfilterProfile=None, SpamfilterProfileStatus=None, Status=None, Vdomparam=None, WebfilterProfile=None, WebfilterProfileStatus=None, Dstaddr6=None, Service6=None, Srcaddr6=None)
