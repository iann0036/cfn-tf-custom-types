...
TYPE_NAME = "TF::FortiOS::SystemResourcelimits"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_resourcelimits/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_resourcelimits',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_resourcelimits/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_resourcelimits/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CustomService=None, DialupTunnel=None, FirewallAddress=None, FirewallAddrgrp=None, FirewallPolicy=None, Id=None, IpsecPhase1=None, IpsecPhase1Interface=None, IpsecPhase2=None, IpsecPhase2Interface=None, LogDiskQuota=None, OnetimeSchedule=None, Proxy=None, RecurringSchedule=None, ServiceGroup=None, Session=None, Sslvpn=None, User=None, UserGroup=None, Vdomparam=None)
