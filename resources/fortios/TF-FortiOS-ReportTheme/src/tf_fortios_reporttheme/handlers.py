...
TYPE_NAME = "TF::FortiOS::ReportTheme"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_report_theme/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_report_theme',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_report_theme/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_report_theme/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BulletListStyle=None, ColumnCount=None, DefaultHtmlStyle=None, DefaultPdfStyle=None, GraphChartStyle=None, Heading1Style=None, Heading2Style=None, Heading3Style=None, Heading4Style=None, HlineStyle=None, Id=None, ImageStyle=None, Name=None, NormalTextStyle=None, NumberedListStyle=None, PageFooterStyle=None, PageHeaderStyle=None, PageOrient=None, PageStyle=None, ReportSubtitleStyle=None, ReportTitleStyle=None, TableChartCaptionStyle=None, TableChartEvenRowStyle=None, TableChartHeadStyle=None, TableChartOddRowStyle=None, TableChartStyle=None, TocHeading1Style=None, TocHeading2Style=None, TocHeading3Style=None, TocHeading4Style=None, TocTitleStyle=None, Vdomparam=None)
