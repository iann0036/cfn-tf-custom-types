...
TYPE_NAME = "TF::FortiOS::SpamfilterProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_spamfilter_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_spamfilter_profile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_spamfilter_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_spamfilter_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Comment=None, External=None, FlowBased=None, Id=None, Name=None, Options=None, ReplacemsgGroup=None, SpamBwlTable=None, SpamBwordTable=None, SpamBwordThreshold=None, SpamFiltering=None, SpamIptrustTable=None, SpamLog=None, SpamLogFortiguardResponse=None, SpamMheaderTable=None, SpamRblTable=None, Vdomparam=None, Gmail=None, Imap=None, Mapi=None, MsnHotmail=None, Pop3=None, Smtp=None, YahooMail=None)
