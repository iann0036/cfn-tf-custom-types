...
TYPE_NAME = "TF::FortiOS::RouterospfOspfinterface"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_routerospf_ospfinterface/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_routerospf_ospfinterface',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_routerospf_ospfinterface/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_routerospf_ospfinterface/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Authentication=None, AuthenticationKey=None, Bfd=None, Cost=None, DatabaseFilterOut=None, DeadInterval=None, DynamicSortSubtable=None, HelloInterval=None, HelloMultiplier=None, Id=None, Interface=None, Ip=None, Md5Key=None, Md5Keychain=None, Mtu=None, MtuIgnore=None, Name=None, NetworkType=None, PrefixLength=None, Priority=None, ResyncTimeout=None, RetransmitInterval=None, Status=None, TransmitDelay=None, Vdomparam=None, Md5Keys=None)
