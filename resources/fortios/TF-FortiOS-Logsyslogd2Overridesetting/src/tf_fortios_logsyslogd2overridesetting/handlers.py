...
TYPE_NAME = "TF::FortiOS::Logsyslogd2Overridesetting"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_logsyslogd2_overridesetting/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_logsyslogd2_overridesetting',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_logsyslogd2_overridesetting/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_logsyslogd2_overridesetting/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Certificate=None, DynamicSortSubtable=None, EncAlgorithm=None, Facility=None, Format=None, Id=None, Interface=None, InterfaceSelectMethod=None, MaxLogRate=None, Mode=None, Override=None, Port=None, Priority=None, Server=None, SourceIp=None, SslMinProtoVersion=None, Status=None, SyslogType=None, Vdomparam=None, CustomFieldName=None)
