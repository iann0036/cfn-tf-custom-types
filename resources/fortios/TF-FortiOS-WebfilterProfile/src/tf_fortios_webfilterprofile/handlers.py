...
TYPE_NAME = "TF::FortiOS::WebfilterProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_webfilter_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_webfilter_profile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_webfilter_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_webfilter_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Comment=None, DynamicSortSubtable=None, ExtendedLog=None, FeatureSet=None, HttpsReplacemsg=None, Id=None, InspectionMode=None, LogAllUrl=None, Name=None, Options=None, OvrdPerm=None, PostAction=None, ReplacemsgGroup=None, Vdomparam=None, WebAntiphishingLog=None, WebContentLog=None, WebExtendedAllActionLog=None, WebFilterActivexLog=None, WebFilterAppletLog=None, WebFilterCommandBlockLog=None, WebFilterCookieLog=None, WebFilterCookieRemovalLog=None, WebFilterJsLog=None, WebFilterJscriptLog=None, WebFilterRefererLog=None, WebFilterUnknownLog=None, WebFilterVbsLog=None, WebFtgdErrLog=None, WebFtgdQuotaUsage=None, WebInvalidDomainLog=None, WebUrlLog=None, Wisp=None, WispAlgorithm=None, YoutubeChannelStatus=None, Antiphish=None, FileFilter=None, FtgdWf=None, Override=None, Web=None, WispServers=None, YoutubeChannelFilter=None)
