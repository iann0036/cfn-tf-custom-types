...
TYPE_NAME = "TF::FortiOS::AntivirusProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_antivirus_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_antivirus_profile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_antivirus_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_antivirus_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AnalyticsAcceptFiletype=None, AnalyticsBlFiletype=None, AnalyticsDb=None, AnalyticsIgnoreFiletype=None, AnalyticsMaxUpload=None, AnalyticsWlFiletype=None, AvBlockLog=None, AvVirusLog=None, Comment=None, ExtendedLog=None, FeatureSet=None, FtgdAnalytics=None, Id=None, InspectionMode=None, MobileMalwareDb=None, Name=None, ReplacemsgGroup=None, ScanMode=None, Vdomparam=None, Cifs=None, ContentDisarm=None, Ftp=None, Http=None, Imap=None, Mapi=None, NacQuar=None, Nntp=None, OutbreakPrevention=None, Pop3=None, Smb=None, Smtp=None, Ssh=None)
