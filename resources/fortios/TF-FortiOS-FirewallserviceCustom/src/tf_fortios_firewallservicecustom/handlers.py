...
TYPE_NAME = "TF::FortiOS::FirewallserviceCustom"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewallservice_custom/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_firewallservice_custom',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewallservice_custom/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_firewallservice_custom/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppServiceType=None, Category=None, CheckResetRange=None, Color=None, Comment=None, DynamicSortSubtable=None, Fqdn=None, Helper=None, Icmpcode=None, Icmptype=None, Id=None, Iprange=None, Name=None, Protocol=None, ProtocolNumber=None, Proxy=None, SctpPortrange=None, SessionTtl=None, TcpHalfcloseTimer=None, TcpHalfopenTimer=None, TcpPortrange=None, TcpTimewaitTimer=None, UdpIdleTimer=None, UdpPortrange=None, Vdomparam=None, Visibility=None, AppCategory=None, Application=None)
