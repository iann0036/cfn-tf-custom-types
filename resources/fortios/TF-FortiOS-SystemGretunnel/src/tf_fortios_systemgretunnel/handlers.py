...
TYPE_NAME = "TF::FortiOS::SystemGretunnel"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_gretunnel/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_gretunnel',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_gretunnel/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_gretunnel/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ChecksumReception=None, ChecksumTransmission=None, Diffservcode=None, DscpCopying=None, Id=None, Interface=None, IpVersion=None, KeepaliveFailtimes=None, KeepaliveInterval=None, KeyInbound=None, KeyOutbound=None, LocalGw=None, LocalGw6=None, Name=None, RemoteGw=None, RemoteGw6=None, SequenceNumberReception=None, SequenceNumberTransmission=None, Vdomparam=None)
