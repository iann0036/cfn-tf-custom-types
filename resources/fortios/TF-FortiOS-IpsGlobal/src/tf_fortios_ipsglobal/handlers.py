...
TYPE_NAME = "TF::FortiOS::IpsGlobal"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_ips_global/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_ips_global',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_ips_global/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_ips_global/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AnomalyMode=None, Database=None, DeepAppInspDbLimit=None, DeepAppInspTimeout=None, EngineCount=None, ExcludeSignatures=None, FailOpen=None, Id=None, IntelligentMode=None, NgfwMaxScanRange=None, PacketLogQueueDepth=None, SessionLimitMode=None, SkypeClientPublicIpaddr=None, SocketSize=None, SyncSessionTtl=None, TrafficSubmit=None, Vdomparam=None, TlsActiveProbe=None)
