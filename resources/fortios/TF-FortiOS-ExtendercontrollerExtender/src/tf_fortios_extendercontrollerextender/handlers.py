...
TYPE_NAME = "TF::FortiOS::ExtendercontrollerExtender"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_extendercontroller_extender/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_extendercontroller_extender',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_extendercontroller_extender/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_extendercontroller_extender/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AaaSharedSecret=None, AccessPointName=None, Admin=None, AtDialScript=None, BillingStartDay=None, CdmaAaaSpi=None, CdmaHaSpi=None, CdmaNai=None, ConnStatus=None, Description=None, DialMode=None, DialStatus=None, ExtName=None, Fosid=None, HaSharedSecret=None, Id=None, Ifname=None, InitiatedUpdate=None, Mode=None, ModemPasswd=None, ModemType=None, MultiMode=None, PppAuthProtocol=None, PppEchoRequest=None, PppPassword=None, PppUsername=None, PrimaryHa=None, QuotaLimitMb=None, Redial=None, RedundantIntf=None, Roaming=None, Role=None, SecondaryHa=None, SimPin=None, Vdom=None, Vdomparam=None, WimaxAuthProtocol=None, WimaxCarrier=None, WimaxRealm=None)
