...
TYPE_NAME = "TF::FortiOS::FirewallSniffer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_sniffer/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_firewall_sniffer',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_firewall_sniffer/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_firewall_sniffer/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplicationList=None, ApplicationListStatus=None, AvProfile=None, AvProfileStatus=None, DlpSensor=None, DlpSensorStatus=None, Dsri=None, DynamicSortSubtable=None, EmailfilterProfile=None, EmailfilterProfileStatus=None, Fosid=None, Host=None, Id=None, Interface=None, IpThreatfeedStatus=None, IpsDosStatus=None, IpsSensor=None, IpsSensorStatus=None, Ipv6=None, Logtraffic=None, MaxPacketCount=None, NonIp=None, Port=None, Protocol=None, ScanBotnetConnections=None, SpamfilterProfile=None, SpamfilterProfileStatus=None, Status=None, Vdomparam=None, Vlan=None, WebfilterProfile=None, WebfilterProfileStatus=None, Anomaly=None, IpThreatfeed=None)
