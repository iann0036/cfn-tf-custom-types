...
TYPE_NAME = "TF::FortiOS::UserRadius"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_radius/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_user_radius',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_user_radius/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_user_radius/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcctAllServers=None, AcctInterimInterval=None, AllUsergroup=None, AuthType=None, DynamicSortSubtable=None, GroupOverrideAttrType=None, H3cCompatibility=None, Id=None, Interface=None, InterfaceSelectMethod=None, Name=None, NasIp=None, PasswordEncoding=None, PasswordRenewal=None, RadiusCoa=None, RadiusPort=None, Rsso=None, RssoContextTimeout=None, RssoEndpointAttribute=None, RssoEndpointBlockAttribute=None, RssoEpOneIpOnly=None, RssoFlushIpSession=None, RssoLogFlags=None, RssoLogPeriod=None, RssoRadiusResponse=None, RssoRadiusServerPort=None, RssoSecret=None, RssoValidateRequestSecret=None, SecondarySecret=None, SecondaryServer=None, Secret=None, Server=None, SourceIp=None, SsoAttribute=None, SsoAttributeKey=None, SsoAttributeValueOverride=None, SwitchControllerAcctFastFramedipDetect=None, SwitchControllerServiceType=None, TertiarySecret=None, TertiaryServer=None, Timeout=None, UseManagementVdom=None, UsernameCaseSensitive=None, Vdomparam=None, AccountingServer=None, Class=None)
