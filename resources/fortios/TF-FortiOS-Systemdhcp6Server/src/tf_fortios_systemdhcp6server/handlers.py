...
TYPE_NAME = "TF::FortiOS::Systemdhcp6Server"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_systemdhcp6_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_systemdhcp6_server',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_systemdhcp6_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_systemdhcp6_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DnsSearchList=None, DnsServer1=None, DnsServer2=None, DnsServer3=None, DnsServer4=None, DnsService=None, Domain=None, DynamicSortSubtable=None, Fosid=None, Id=None, Interface=None, IpMode=None, LeaseTime=None, Option1=None, Option2=None, Option3=None, PrefixMode=None, RapidCommit=None, Status=None, Subnet=None, UpstreamInterface=None, Vdomparam=None, IpRange=None, PrefixRange=None)
