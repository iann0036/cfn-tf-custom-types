...
TYPE_NAME = "TF::FortiOS::Routerospf6Ospf6interface"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_routerospf6_ospf6interface/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_routerospf6_ospf6interface',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_routerospf6_ospf6interface/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_routerospf6_ospf6interface/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AreaId=None, Authentication=None, Bfd=None, Cost=None, DeadInterval=None, DynamicSortSubtable=None, HelloInterval=None, Id=None, Interface=None, IpsecAuthAlg=None, IpsecEncAlg=None, KeyRolloverInterval=None, Mtu=None, MtuIgnore=None, Name=None, NetworkType=None, Priority=None, RetransmitInterval=None, Status=None, TransmitDelay=None, Vdomparam=None, IpsecKeys=None, Neighbor=None)
