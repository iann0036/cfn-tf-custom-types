...
TYPE_NAME = "TF::FortiOS::VpncertificateSetting"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_vpncertificate_setting/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_vpncertificate_setting',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_vpncertificate_setting/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_vpncertificate_setting/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CertnameDsa1024=None, CertnameDsa2048=None, CertnameEcdsa256=None, CertnameEcdsa384=None, CertnameEcdsa521=None, CertnameEd25519=None, CertnameEd448=None, CertnameRsa1024=None, CertnameRsa2048=None, CertnameRsa4096=None, CheckCaCert=None, CheckCaChain=None, CmpKeyUsageChecking=None, CmpSaveExtraCerts=None, CnMatch=None, Id=None, Interface=None, InterfaceSelectMethod=None, OcspDefaultServer=None, OcspOption=None, OcspStatus=None, SslMinProtoVersion=None, SslOcspSourceIp=None, StrictCrlCheck=None, StrictOcspCheck=None, SubjectMatch=None, Vdomparam=None)
