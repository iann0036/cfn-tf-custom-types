...
TYPE_NAME = "TF::FortiOS::SystemAccprofile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_accprofile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_accprofile',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_accprofile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_accprofile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Admintimeout=None, AdmintimeoutOverride=None, Authgrp=None, Comments=None, Ftviewgrp=None, Fwgrp=None, Id=None, Loggrp=None, Name=None, Netgrp=None, Scope=None, Secfabgrp=None, Sysgrp=None, SystemDiagnostics=None, Utmgrp=None, Vdomparam=None, Vpngrp=None, Wanoptgrp=None, Wifi=None, FwgrpPermission=None, LoggrpPermission=None, NetgrpPermission=None, SysgrpPermission=None, UtmgrpPermission=None)
