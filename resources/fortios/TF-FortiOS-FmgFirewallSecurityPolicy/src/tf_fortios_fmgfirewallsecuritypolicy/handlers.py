...
TYPE_NAME = "TF::FortiOS::FmgFirewallSecurityPolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_fmg_firewall_security_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_fmg_firewall_security_policy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_fmg_firewall_security_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_fmg_firewall_security_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, Adom=None, ApplicationList=None, AvProfile=None, CapturePacket=None, Comments=None, DnsfilterProfile=None, Dstaddr=None, Dstintf=None, Fixedport=None, Fsso=None, Groups=None, Id=None, Inbound=None, InternetService=None, InternetServiceId=None, InternetServiceName=None, InternetServiceSrc=None, InternetServiceSrcId=None, InternetServiceSrcName=None, Ippool=None, IpsSensor=None, Logtraffic=None, LogtrafficStart=None, Name=None, Nat=None, PackageName=None, PerIpShaper=None, Poolname=None, ProfileGroup=None, ProfileProtocolOptions=None, ProfileType=None, Rsso=None, Schedule=None, Service=None, Srcaddr=None, Srcintf=None, TrafficShaper=None, TrafficShaperReverse=None, Users=None, UtmStatus=None, VpnTunnel=None, WafProfile=None, WebfilterProfile=None)
