...
TYPE_NAME = "TF::FortiOS::ReportStyle"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_report_style/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_report_style',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_report_style/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_report_style/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Align=None, BgColor=None, BorderBottom=None, BorderLeft=None, BorderRight=None, BorderTop=None, ColumnGap=None, ColumnSpan=None, FgColor=None, FontFamily=None, FontSize=None, FontStyle=None, FontWeight=None, Height=None, Id=None, LineHeight=None, MarginBottom=None, MarginLeft=None, MarginRight=None, MarginTop=None, Name=None, Options=None, PaddingBottom=None, PaddingLeft=None, PaddingRight=None, PaddingTop=None, Vdomparam=None, Width=None)
