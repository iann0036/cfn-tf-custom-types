...
TYPE_NAME = "TF::FortiOS::SystemDdns"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_ddns/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_ddns',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_ddns/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_ddns/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BoundIp=None, ClearText=None, DdnsAuth=None, DdnsDomain=None, DdnsKey=None, DdnsKeyname=None, DdnsPassword=None, DdnsServer=None, DdnsServerIp=None, DdnsSn=None, DdnsTtl=None, DdnsUsername=None, DdnsZone=None, Ddnsid=None, DynamicSortSubtable=None, Id=None, SslCertificate=None, UpdateInterval=None, UsePublicIp=None, Vdomparam=None, MonitorInterface=None)
