...
TYPE_NAME = "TF::FortiOS::SystemLinkmonitor"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_linkmonitor/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_system_linkmonitor',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_system_linkmonitor/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_system_linkmonitor/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddrMode=None, DynamicSortSubtable=None, Failtime=None, GatewayIp=None, GatewayIp6=None, HaPriority=None, HttpAgent=None, HttpGet=None, HttpMatch=None, Id=None, Interval=None, Name=None, PacketSize=None, Password=None, Port=None, ProbeCount=None, ProbeTimeout=None, Protocol=None, Recoverytime=None, SecurityMode=None, SourceIp=None, SourceIp6=None, Srcintf=None, Status=None, UpdateCascadeInterface=None, UpdateStaticRoute=None, Vdomparam=None, Server=None)
