...
TYPE_NAME = "TF::FortiOS::WebproxyGlobal"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_webproxy_global/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'fortios_webproxy_global',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/fortios_webproxy_global/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/fortios_webproxy_global/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DynamicSortSubtable=None, FastPolicyMatch=None, ForwardProxyAuth=None, ForwardServerAffinityTimeout=None, Id=None, LearnClientIp=None, LearnClientIpFromHeader=None, MaxMessageLength=None, MaxRequestLength=None, MaxWafBodyCacheLength=None, ProxyFqdn=None, SslCaCert=None, SslCert=None, StrictWebCheck=None, TunnelNonHttp=None, UnknownHttpVersion=None, Vdomparam=None, WebproxyProfile=None, LearnClientIpSrcaddr=None, LearnClientIpSrcaddr6=None)
