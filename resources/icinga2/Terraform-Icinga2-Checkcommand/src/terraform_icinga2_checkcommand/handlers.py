...
def create_handler(
    ...
    try:
        ...
        tf['resource']['icinga2_checkcommand'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("icinga2"))['SecretString']
            tf["provider"]["icinga2"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['icinga2_checkcommand'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
