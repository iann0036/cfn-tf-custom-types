...
TYPE_NAME = "TF::CloudEOS::VpcConfig"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_vpc_config/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'cloudeos_vpc_config',
                'returnValues': ["Id", "PeerRgName", "PeerVnetId", "PeerVnetName", "PeerVpcCidr", "PeerVpcId", "Peervpcidr", "TfId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_vpc_config/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudeos_vpc_config/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClosId=None, ClosName=None, CloudProvider=None, Cnps=None, Id=None, PeerRgName=None, PeerVnetId=None, PeerVnetName=None, PeerVpcCidr=None, PeerVpcId=None, Peervpcidr=None, Region=None, RgName=None, Role=None, Tags=None, TfId=None, TopologyId=None, TopologyName=None, VnetName=None, WanId=None, WanName=None, Timeouts=None)
