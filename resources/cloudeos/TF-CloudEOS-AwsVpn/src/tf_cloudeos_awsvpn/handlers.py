...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_aws_vpn/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'cloudeos_aws_vpn',
                'returnValues': ["Id", "TfId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_aws_vpn/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudeos_aws_vpn/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CgwId=None, Cnps=None, Id=None, RouterId=None, TfId=None, TgwId=None, Tunnel1AwsEndpointIp=None, Tunnel1AwsOverlayIp=None, Tunnel1BgpAsn=None, Tunnel1BgpHoldtime=None, Tunnel1PresharedKey=None, Tunnel1RouterOverlayIp=None, Tunnel2AwsEndpointIp=None, Tunnel2AwsOverlayIp=None, Tunnel2BgpAsn=None, Tunnel2BgpHoldtime=None, Tunnel2PresharedKey=None, Tunnel2RouterOverlayIp=None, VpcId=None, VpnConnectionId=None, VpnGatewayId=None, VpnTgwAttachmentId=None)
