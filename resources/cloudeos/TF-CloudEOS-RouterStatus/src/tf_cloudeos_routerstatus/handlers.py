...
TYPE_NAME = "TF::CloudEOS::RouterStatus"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_router_status/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'aristanetworks/cloudeos',
                'providerTypeName': 'cloudeos',
                'terraformTypeName': 'cloudeos_router_status',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_router_status/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudeos_router_status/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilitySetId=None, AvailabilityZone=None, CloudProvider=None, Cnps=None, CvContainer=None, DeploymentStatus=None, HaName=None, Id=None, InstanceId=None, InstanceType=None, InternalRtTableIds=None, IntfId=None, IntfName=None, IntfPrivateIp=None, IntfSubnetId=None, IntfType=None, IsRr=None, PrimaryNetworkInterfaceId=None, PrivateRtTableIds=None, PublicIp=None, PublicRtTableIds=None, Region=None, RgLocation=None, RgName=None, RouterBgpAsn=None, RoutingResourceInfo=None, Tags=None, TfId=None, VpcId=None, Timeouts=None)
