...
TYPE_NAME = "TF::CloudEOS::RouterConfig"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_router_config/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'cloudeos_router_config',
                'returnValues': ["BootstrapCfg", "HaRtrId", "Id", "InternalRtTableId", "PeerRoutetableId", "Peerroutetableid1", "PrivateRtTableId", "PublicRtTableId", "TfId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudeos_router_config/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudeos_router_config/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Ami=None, AvailabilityZone=None, BootstrapCfg=None, CloudProvider=None, Cnps=None, HaRtrId=None, Id=None, InternalRtTableId=None, IntfName=None, IntfPrivateIp=None, IntfType=None, IsRr=None, KeyName=None, PeerRoutetableId=None, Peerroutetableid1=None, PrivateRtTableId=None, PublicRtTableId=None, Region=None, Role=None, Tags=None, TfId=None, TopologyName=None, VpcId=None, Timeouts=None)
