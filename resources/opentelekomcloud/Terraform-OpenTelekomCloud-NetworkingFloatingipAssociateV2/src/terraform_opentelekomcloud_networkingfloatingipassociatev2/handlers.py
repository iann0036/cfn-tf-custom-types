...
def create_handler(
    ...
    try:
        ...
        tf['resource']['opentelekomcloud_networking_floatingip_associate_v2'][request.logicalResourceIdentifier] = {}

        if "opentelekomcloud" == "aws":
            creds = session.credentials
            tf["provider"]["opentelekomcloud"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("opentelekomcloud"))['SecretString']
            tf["provider"]["opentelekomcloud"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['opentelekomcloud_networking_floatingip_associate_v2'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
