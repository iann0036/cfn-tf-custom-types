...
def create_handler(
    ...
    try:
        ...
        tf['resource']['powerdns_record'][request.logicalResourceIdentifier] = {}

        if "powerdns" == "aws":
            creds = session.credentials
            tf["provider"]["powerdns"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("powerdns"))['SecretString']
            tf["provider"]["powerdns"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['powerdns_record'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
