...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/akamai_gtm_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'akamai/akamai',
                'providerTypeName': 'akamai',
                'terraformTypeName': 'akamai_gtm_domain',
                'returnValues': ["DefaultHealthMax", "DefaultHealthMultiplier", "DefaultHealthThreshold", "DefaultMaxUnreachablePenalty", "DefaultUnreachableThreshold", "Id", "MapUpdateInterval", "MaxProperties", "MaxResources", "MaxTestTimeout", "MaxTtl", "MinPingableRegionFraction", "MinTestInterval", "MinTtl", "PingInterval", "PingPacketSize", "RoundRobinPrefix", "ServermonitorLivenessCount", "ServermonitorLoadCount", "ServermonitorPool"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/akamai_gtm_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/akamai_gtm_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CnameCoalescingEnabled=None, Comment=None, Contract=None, DefaultErrorPenalty=None, DefaultHealthMax=None, DefaultHealthMultiplier=None, DefaultHealthThreshold=None, DefaultMaxUnreachablePenalty=None, DefaultSslClientCertificate=None, DefaultSslClientPrivateKey=None, DefaultTimeoutPenalty=None, DefaultUnreachableThreshold=None, EmailNotificationList=None, EndUserMappingEnabled=None, Group=None, Id=None, LoadFeedback=None, LoadImbalancePercentage=None, MapUpdateInterval=None, MaxProperties=None, MaxResources=None, MaxTestTimeout=None, MaxTtl=None, MinPingableRegionFraction=None, MinTestInterval=None, MinTtl=None, Name=None, PingInterval=None, PingPacketSize=None, RoundRobinPrefix=None, ServermonitorLivenessCount=None, ServermonitorLoadCount=None, ServermonitorPool=None, Type=None, WaitOnComplete=None)
