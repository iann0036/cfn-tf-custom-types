...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/akamai_iam_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'akamai_iam_user',
                'returnValues': ["EmailUpdatePending", "Id", "IsLocked", "LastLogin", "PasswordExpiredAfter", "TfaConfigured", "UserName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/akamai_iam_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/akamai_iam_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, AuthGrantsJson=None, City=None, ContactType=None, Country=None, Email=None, EmailUpdatePending=None, EnableTfa=None, FirstName=None, Id=None, IsLocked=None, JobTitle=None, LastLogin=None, LastName=None, MobilePhone=None, PasswordExpiredAfter=None, Phone=None, PreferredLanguage=None, SecondaryEmail=None, SessionTimeout=None, State=None, TfaConfigured=None, TimeZone=None, UserName=None, ZipCode=None)
