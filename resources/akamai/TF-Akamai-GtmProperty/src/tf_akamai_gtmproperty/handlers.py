...
TYPE_NAME = "TF::Akamai::GtmProperty"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/akamai_gtm_property/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'akamai_gtm_property',
                'returnValues': ["Id", "WeightedHashBitsForIpv4", "WeightedHashBitsForIpv6"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/akamai_gtm_property/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/akamai_gtm_property/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BackupCname=None, BackupIp=None, BalanceByDownloadScore=None, Cname=None, Comments=None, Domain=None, DynamicTtl=None, FailbackDelay=None, FailoverDelay=None, GhostDemandReporting=None, HandoutLimit=None, HandoutMode=None, HealthMax=None, HealthMultiplier=None, HealthThreshold=None, Id=None, Ipv6=None, LoadImbalancePercentage=None, MapName=None, MaxUnreachablePenalty=None, MinLiveFraction=None, Name=None, ScoreAggregationType=None, StaticTtl=None, StickinessBonusConstant=None, StickinessBonusPercentage=None, Type=None, UnreachableThreshold=None, UseComputedTargets=None, WaitOnComplete=None, WeightedHashBitsForIpv4=None, WeightedHashBitsForIpv6=None, LivenessTest=None, StaticRrSet=None, TrafficTarget=None)
