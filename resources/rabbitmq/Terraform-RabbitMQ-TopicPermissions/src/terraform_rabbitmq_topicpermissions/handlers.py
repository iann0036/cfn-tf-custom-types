...
def create_handler(
    ...
    try:
        ...
        tf['resource']['rabbitmq_topic_permissions'][request.logicalResourceIdentifier] = {}

        if "rabbitmq" == "aws":
            creds = session.credentials
            tf["provider"]["rabbitmq"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("rabbitmq"))['SecretString']
            tf["provider"]["rabbitmq"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['rabbitmq_topic_permissions'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
