...
def create_handler(
    ...
    try:
        ...
        tf['resource']['cloudstack_vpn_customer_gateway'][request.logicalResourceIdentifier] = {}

        if "cloudstack" == "aws":
            creds = session.credentials
            tf["provider"]["cloudstack"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("cloudstack"))['SecretString']
            tf["provider"]["cloudstack"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['cloudstack_vpn_customer_gateway'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
