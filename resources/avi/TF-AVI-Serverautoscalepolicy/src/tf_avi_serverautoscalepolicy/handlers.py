...
TYPE_NAME = "TF::AVI::Serverautoscalepolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_serverautoscalepolicy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'avi_serverautoscalepolicy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_serverautoscalepolicy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/avi_serverautoscalepolicy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DelayForServerGarbageCollection=None, Description=None, Id=None, IntelligentAutoscale=None, IntelligentScaleinMargin=None, IntelligentScaleoutMargin=None, MaxScaleinAdjustmentStep=None, MaxScaleoutAdjustmentStep=None, MaxSize=None, MinSize=None, Name=None, ScaleinAlertconfigRefs=None, ScaleinCooldown=None, ScaleoutAlertconfigRefs=None, ScaleoutCooldown=None, TenantRef=None, UsePredictedLoad=None, Uuid=None, Markers=None)
