...
class ServiceStatusDefinition(BaseModel):
    ...
    UpstreamSyncTimestamp: Optional[Sequence["_UpstreamSyncTimestampDefinition"]]
    ...
    def _deserialize(
        ...
        return cls(
            ...
            UpstreamSyncTimestamp=deserialize_list(json_data.get("UpstreamSyncTimestamp"), UpstreamSyncTimestampDefinition),
        )
...
@dataclass
class UpstreamSyncTimestampDefinition(BaseModel):
    Secs: Optional[float]
    Usecs: Optional[float]

    @classmethod
    def _deserialize(
        cls: Type["_UpstreamSyncTimestampDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_UpstreamSyncTimestampDefinition"]:
        if not json_data:
            return None
        return cls(
            Secs=json_data.get("Secs"),
...
# work around possible type aliasing issues when variable has same name as a model
_UpstreamSyncTimestampDefinition = UpstreamSyncTimestampDefinition
