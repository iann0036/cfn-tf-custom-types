...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_gslb/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_gslb/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/avi_gslb/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AsyncInterval=None, ClearOnMaxRetries=None, Description=None, EnableConfigByMembers=None, ErrorResyncInterval=None, Id=None, IsFederated=None, LeaderClusterUuid=None, MaintenanceMode=None, Name=None, SendInterval=None, SendIntervalPriorToMaintenanceMode=None, TenantRef=None, TenantScoped=None, Uuid=None, ViewId=None, ClientIpAddrGroup=None, DnsConfigs=None, ReplicationPolicy=None, Sites=None, ThirdPartySites=None)
