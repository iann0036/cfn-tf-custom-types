...
TYPE_NAME = "TF::AVI::Systemconfiguration"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_systemconfiguration/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'avi_systemconfiguration',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_systemconfiguration/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/avi_systemconfiguration/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CommonCriteriaMode=None, DefaultLicenseTier=None, DnsVirtualserviceRefs=None, DockerMode=None, EnableCors=None, FipsMode=None, Id=None, SshCiphers=None, SshHmacs=None, Uuid=None, WelcomeWorkflowComplete=None, AdminAuthConfiguration=None, DnsConfiguration=None, EmailConfiguration=None, GlobalTenantConfig=None, LinuxConfiguration=None, MgmtIpAccessControl=None, NtpConfiguration=None, PortalConfiguration=None, ProxyConfiguration=None, SecureChannelConfiguration=None, SnmpConfiguration=None)
