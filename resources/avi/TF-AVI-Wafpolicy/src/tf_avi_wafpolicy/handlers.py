...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_wafpolicy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'avi_wafpolicy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_wafpolicy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/avi_wafpolicy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowModeDelegation=None, CreatedBy=None, Description=None, EnableAppLearning=None, EnableAutoRuleUpdates=None, EnableRegexLearning=None, FailureMode=None, Id=None, MinConfidence=None, Mode=None, Name=None, ParanoiaLevel=None, TenantRef=None, Uuid=None, WafCrsRef=None, WafProfileRef=None, Allowlist=None, ApplicationSignatures=None, ConfidenceOverride=None, CrsOverrides=None, LearningParams=None, Markers=None, PositiveSecurityModel=None, PostCrsGroups=None, PreCrsGroups=None, ResolvedCrsGroups=None)
