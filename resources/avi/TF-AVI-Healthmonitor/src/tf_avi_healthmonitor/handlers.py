...
TYPE_NAME = "TF::AVI::Healthmonitor"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_healthmonitor/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'avi_healthmonitor',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_healthmonitor/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/avi_healthmonitor/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowDuplicateMonitors=None, Description=None, DisableQuickstart=None, FailedChecks=None, Id=None, IsFederated=None, MonitorPort=None, Name=None, ReceiveTimeout=None, SendInterval=None, SuccessfulChecks=None, TenantRef=None, Type=None, Uuid=None, Authentication=None, DnsMonitor=None, ExternalMonitor=None, HttpMonitor=None, HttpsMonitor=None, ImapMonitor=None, ImapsMonitor=None, Markers=None, Pop3Monitor=None, Pop3sMonitor=None, RadiusMonitor=None, SipMonitor=None, SmtpMonitor=None, SmtpsMonitor=None, TcpMonitor=None, UdpMonitor=None)
