...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/avi_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/avi_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoscalingGroupName=None, AvailabilityZone=None, Description=None, Enabled=None, ExternalOrchestrationId=None, ExternalUuid=None, Hostname=None, Id=None, Ip=None, MacAddress=None, NwRef=None, PoolRef=None, Port=None, PrstHdrVal=None, Ratio=None, ResolveServerByDns=None, RewriteHostHeader=None, ServerNode=None, Static=None, Type=None, VerifyNetwork=None, VmRef=None, DiscoveredNetworks=None, Location=None)
