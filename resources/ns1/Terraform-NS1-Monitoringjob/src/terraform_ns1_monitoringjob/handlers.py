...
def create_handler(
    ...
    try:
        ...
        tf['resource']['ns1_monitoringjob'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("ns1"))['SecretString']
            tf["provider"]["ns1"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['ns1_monitoringjob'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
