...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ns1_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'ns1-terraform/ns1',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ns1_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ns1_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountManageAccountSettings=None, AccountManageApikeys=None, AccountManageIpWhitelist=None, AccountManagePaymentMethods=None, AccountManagePlan=None, AccountManageTeams=None, AccountManageUsers=None, AccountViewActivityLog=None, AccountViewInvoices=None, DataManageDatafeeds=None, DataManageDatasources=None, DataPushToDatafeeds=None, DhcpManageDhcp=None, DhcpViewDhcp=None, DnsManageZones=None, DnsViewZones=None, DnsZonesAllow=None, DnsZonesAllowByDefault=None, DnsZonesDeny=None, Email=None, Id=None, IpWhitelist=None, IpWhitelistStrict=None, IpamManageIpam=None, IpamViewIpam=None, MonitoringManageJobs=None, MonitoringManageLists=None, MonitoringViewJobs=None, Name=None, Notify=None, SecurityManageActiveDirectory=None, SecurityManageGlobal2fa=None, Teams=None, Username=None)
