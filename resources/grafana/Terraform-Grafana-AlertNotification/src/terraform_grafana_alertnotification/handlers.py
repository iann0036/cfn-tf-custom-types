...
def create_handler(
    ...
    try:
        ...
        tf['resource']['grafana_alert_notification'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("grafana"))['SecretString']
            tf["provider"]["grafana"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['grafana_alert_notification'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
