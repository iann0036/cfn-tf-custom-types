...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/helm_release/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/helm',
                'providerTypeName': 'helm',
                'terraformTypeName': 'helm_release',
                'returnValues': ["Id", "Manifest", "Metadata", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/helm_release/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/helm_release/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Atomic=None, Chart=None, CleanupOnFail=None, CreateNamespace=None, DependencyUpdate=None, Description=None, Devel=None, DisableCrdHooks=None, DisableOpenapiValidation=None, DisableWebhooks=None, ForceUpdate=None, Id=None, Keyring=None, Lint=None, Manifest=None, MaxHistory=None, Metadata=None, Name=None, Namespace=None, RecreatePods=None, RenderSubchartNotes=None, Replace=None, Repository=None, RepositoryCaFile=None, RepositoryCertFile=None, RepositoryKeyFile=None, RepositoryPassword=None, RepositoryUsername=None, ResetValues=None, ReuseValues=None, SkipCrds=None, Status=None, Timeout=None, Values=None, Verify=None, Version=None, Wait=None, WaitForJobs=None, Postrender=None, Set=None, SetSensitive=None)
