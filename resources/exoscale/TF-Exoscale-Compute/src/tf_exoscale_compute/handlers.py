...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/exoscale_compute/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'exoscale/exoscale',
                'providerTypeName': 'exoscale',
                'terraformTypeName': 'exoscale_compute',
                'returnValues': ["Gateway", "Id", "Ip6Address", "Ip6Cidr", "IpAddress", "Name", "Password", "UserDataBase64", "Username"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/exoscale_compute/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/exoscale_compute/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AffinityGroupIds=None, AffinityGroups=None, DiskSize=None, DisplayName=None, Gateway=None, Hostname=None, Id=None, Ip4=None, Ip6=None, Ip6Address=None, Ip6Cidr=None, IpAddress=None, KeyPair=None, Keyboard=None, Name=None, Password=None, ReverseDns=None, SecurityGroupIds=None, SecurityGroups=None, Size=None, State=None, Tags=None, Template=None, TemplateId=None, UserData=None, UserDataBase64=None, Username=None, Zone=None, Timeouts=None)
