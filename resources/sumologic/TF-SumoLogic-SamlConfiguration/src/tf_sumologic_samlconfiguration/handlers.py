...
TYPE_NAME = "TF::SumoLogic::SamlConfiguration"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/sumologic_saml_configuration/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'sumologic_saml_configuration',
                'returnValues': ["Certificate", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/sumologic_saml_configuration/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/sumologic_saml_configuration/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthnRequestUrl=None, Certificate=None, ConfigurationName=None, DebugMode=None, DisableRequestedAuthnContext=None, EmailAttribute=None, Id=None, IsRedirectBinding=None, Issuer=None, LogoutEnabled=None, LogoutUrl=None, RolesAttribute=None, SignAuthnRequest=None, SpInitiatedLoginEnabled=None, SpInitiatedLoginPath=None, X509cert1=None, X509cert2=None, X509cert3=None, OnDemandProvisioningEnabled=None)
