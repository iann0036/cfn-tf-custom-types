...
TYPE_NAME = "TF::SumoLogic::CloudsyslogSource"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/sumologic_cloudsyslog_source/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'sumologic_cloudsyslog_source',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/sumologic_cloudsyslog_source/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/sumologic_cloudsyslog_source/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutomaticDateParsing=None, Category=None, CollectorId=None, ContentType=None, CutoffRelativeTime=None, CutoffTimestamp=None, Description=None, Fields=None, ForceTimezone=None, HostName=None, Id=None, ManualPrefixRegexp=None, MultilineProcessingEnabled=None, Name=None, Timezone=None, Token=None, UseAutolineMatching=None, DefaultDateFormats=None, Filters=None)
