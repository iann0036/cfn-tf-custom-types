...
TYPE_NAME = "TF::Boundary::AuthMethodOidc"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/boundary_auth_method_oidc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/boundary',
                'providerTypeName': 'boundary',
                'terraformTypeName': 'boundary_auth_method_oidc',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/boundary_auth_method_oidc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/boundary_auth_method_oidc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowedAudiences=None, ApiUrlPrefix=None, CallbackUrl=None, ClientId=None, ClientSecret=None, ClientSecretHmac=None, Description=None, DisableDiscoveredConfigValidation=None, Id=None, IdpCaCerts=None, IsPrimaryForScope=None, Issuer=None, MaxAge=None, Name=None, ScopeId=None, SigningAlgorithms=None, State=None, Type=None)
