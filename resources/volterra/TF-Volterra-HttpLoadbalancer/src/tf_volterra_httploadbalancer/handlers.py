...
TYPE_NAME = "TF::Volterra::HttpLoadbalancer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_http_loadbalancer/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'volterra_http_loadbalancer',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_http_loadbalancer/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/volterra_http_loadbalancer/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddLocation=None, AdvertiseOnPublicDefaultVip=None, Annotations=None, Description=None, Disable=None, DisableRateLimit=None, DisableWaf=None, DoNotAdvertise=None, Domains=None, Id=None, Labels=None, LeastActive=None, Name=None, Namespace=None, NoChallenge=None, NoServicePolicies=None, Random=None, RoundRobin=None, ServicePoliciesFromNamespace=None, SourceIpStickiness=None, ActiveServicePolicies=None, AdvertiseCustom=None, AdvertiseOnPublic=None, BlockedClients=None, CaptchaChallenge=None, CookieStickiness=None, CorsPolicy=None, DefaultRoutePools=None, Http=None, Https=None, HttpsAutoCert=None, JsChallenge=None, MaliciousUserMitigation=None, MoreOption=None, PolicyBasedChallenge=None, RateLimit=None, RingHash=None, Routes=None, TrustedClients=None, UserIdentification=None, Waf=None, WafExclusionRules=None, WafRule=None)
