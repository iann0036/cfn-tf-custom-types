...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_fleet/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'volterra_fleet',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_fleet/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/volterra_fleet/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowAllUsb=None, Annotations=None, DefaultConfig=None, DefaultStorageClass=None, DenyAllUsb=None, Description=None, Disable=None, DisableGpu=None, EnableDefaultFleetConfigDownload=None, EnableGpu=None, FleetLabel=None, Id=None, Labels=None, LogsStreamingDisabled=None, Name=None, Namespace=None, NoBondDevices=None, NoDcClusterGroup=None, NoStorageDevice=None, NoStorageInterfaces=None, NoStorageStaticRoutes=None, OperatingSystemVersion=None, VolterraSoftwareVersion=None, BondDeviceList=None, DcClusterGroup=None, DcClusterGroupInside=None, DeviceList=None, InsideVirtualNetwork=None, InterfaceList=None, LogReceiver=None, NetworkConnectors=None, NetworkFirewall=None, OutsideVirtualNetwork=None, StorageClassList=None, StorageDeviceList=None, StorageInterfaceList=None, StorageStaticRoutes=None, UsbPolicy=None)
