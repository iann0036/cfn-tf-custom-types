...
TYPE_NAME = "TF::Volterra::AzureVnetSite"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_azure_vnet_site/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'volterra_azure_vnet_site',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_azure_vnet_site/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/volterra_azure_vnet_site/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, Annotations=None, Assisted=None, AzureRegion=None, Description=None, Disable=None, DiskSize=None, Id=None, Labels=None, LogsStreamingDisabled=None, MachineType=None, Name=None, Namespace=None, NodesPerAz=None, ResourceGroup=None, SshKey=None, AzureCred=None, Coordinates=None, IngressEgressGw=None, IngressGw=None, LogReceiver=None, Os=None, Sw=None, Vnet=None, VoltstackCluster=None)
