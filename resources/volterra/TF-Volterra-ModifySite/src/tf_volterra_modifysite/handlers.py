...
TYPE_NAME = "TF::Volterra::ModifySite"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_modify_site/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'volterra_modify_site',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_modify_site/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/volterra_modify_site/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, Annotations=None, BgpPeerAddress=None, BgpRouterId=None, Description=None, DesiredPoolCount=None, Disable=None, Id=None, InsideNameserver=None, InsideVip=None, Labels=None, Name=None, Namespace=None, OperatingSystemVersion=None, OutsideNameserver=None, OutsideVip=None, Retry=None, SiteToSiteNetworkType=None, SiteToSiteTunnelIp=None, TunnelDeadTimeout=None, TunnelType=None, VipVrrpMode=None, VolterraSoftwareOveride=None, VolterraSoftwareVersion=None, WaitTime=None, Coordinates=None)
