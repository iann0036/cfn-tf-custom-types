...
TYPE_NAME = "TF::Volterra::VirtualHost"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_virtual_host/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'volterra_virtual_host',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/volterra_virtual_host/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/volterra_virtual_host/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddLocation=None, Annotations=None, AppendServerName=None, CustomErrors=None, DefaultHeader=None, Description=None, Disable=None, DisableDefaultErrorPages=None, DisableDnsResolve=None, Domains=None, Id=None, IdleTimeout=None, Labels=None, MaxRequestHeaderSize=None, Name=None, Namespace=None, NoAuthentication=None, NoChallenge=None, PassThrough=None, Proxy=None, RequestHeadersToRemove=None, ResponseHeadersToRemove=None, ServerName=None, AdvertisePolicies=None, Authentication=None, BufferPolicy=None, CaptchaChallenge=None, CompressionParams=None, CorsPolicy=None, DynamicReverseProxy=None, JsChallenge=None, RateLimiter=None, RateLimiterAllowedPrefixes=None, RequestHeadersToAdd=None, ResponseHeadersToAdd=None, RetryPolicy=None, Routes=None, TemporaryUserBlocking=None, TlsParameters=None, UserIdentification=None, WafType=None)
