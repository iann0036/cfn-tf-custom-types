...
def create_handler(
    ...
    try:
        ...
        tf['resource']['huaweicloud_vpc_peering_connection_accepter_v2'][request.logicalResourceIdentifier] = {}

        if "huaweicloud" == "aws":
            creds = session.credentials
            tf["provider"]["huaweicloud"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("huaweicloud"))['SecretString']
            tf["provider"]["huaweicloud"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['huaweicloud_vpc_peering_connection_accepter_v2'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
