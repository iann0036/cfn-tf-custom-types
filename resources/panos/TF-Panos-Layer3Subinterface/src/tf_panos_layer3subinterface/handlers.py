...
TYPE_NAME = "TF::Panos::Layer3Subinterface"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_layer3_subinterface/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_layer3_subinterface/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_layer3_subinterface/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdjustTcpMss=None, Comment=None, CreateDhcpDefaultRoute=None, DecryptForward=None, DhcpDefaultRouteMetric=None, DhcpSendHostnameEnable=None, DhcpSendHostnameValue=None, EnableDhcp=None, Id=None, InterfaceType=None, Ipv4MssAdjust=None, Ipv6Enabled=None, Ipv6InterfaceId=None, Ipv6MssAdjust=None, ManagementProfile=None, Mtu=None, Name=None, NetflowProfile=None, ParentInterface=None, StaticIps=None, Tag=None, Vsys=None)
