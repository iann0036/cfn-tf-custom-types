...
TYPE_NAME = "TF::Panos::PanoramaBgpAggregate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_bgp_aggregate/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_bgp_aggregate/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_panorama_bgp_aggregate/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AsPathLimit=None, AsPathType=None, AsPathValue=None, AsSet=None, CommunityType=None, CommunityValue=None, Enable=None, ExtendedCommunityType=None, ExtendedCommunityValue=None, Id=None, LocalPreference=None, Med=None, Name=None, NextHop=None, Origin=None, Prefix=None, Summary=None, Template=None, TemplateStack=None, VirtualRouter=None, Weight=None)
