...
TYPE_NAME = "TF::Panos::PanoramaBgp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_bgp/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_bgp/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_panorama_bgp/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AggregateMed=None, AllowRedistributeDefaultRoute=None, AlwaysCompareMed=None, AsFormat=None, AsNumber=None, BfdProfile=None, ConfederationMemberAs=None, DefaultLocalPreference=None, DeterministicMedComparison=None, EcmpMultiAs=None, Enable=None, EnableGracefulRestart=None, EnforceFirstAs=None, Id=None, InstallRoute=None, LocalRestartTime=None, MaxPeerRestartTime=None, ReflectorClusterId=None, RejectDefaultRoute=None, RouterId=None, StaleRouteTime=None, Template=None, TemplateStack=None, VirtualRouter=None)
