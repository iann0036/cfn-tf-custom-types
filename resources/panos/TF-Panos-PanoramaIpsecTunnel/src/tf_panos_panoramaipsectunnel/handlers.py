...
TYPE_NAME = "TF::Panos::PanoramaIpsecTunnel"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_ipsec_tunnel/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_ipsec_tunnel/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_panorama_ipsec_tunnel/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AkIkeGateway=None, AkIpsecCryptoProfile=None, AntiReplay=None, CopyFlowLabel=None, CopyTos=None, Disabled=None, EnableIpv6=None, EnableTunnelMonitor=None, GpsCertificateProfile=None, GpsInterface=None, GpsInterfaceFloatingIpIpv4=None, GpsInterfaceFloatingIpIpv6=None, GpsInterfaceIpIpv4=None, GpsInterfaceIpIpv6=None, GpsLocalCertificate=None, GpsPortalAddress=None, GpsPreferIpv6=None, GpsPublishConnectedRoutes=None, GpsPublishRoutes=None, Id=None, MkAuthKey=None, MkAuthKeyEnc=None, MkAuthType=None, MkEspEncryptionKey=None, MkEspEncryptionKeyEnc=None, MkEspEncryptionType=None, MkInterface=None, MkLocalAddressFloatingIp=None, MkLocalAddressIp=None, MkLocalSpi=None, MkProtocol=None, MkRemoteAddress=None, MkRemoteSpi=None, Name=None, Template=None, TunnelInterface=None, TunnelMonitorDestinationIp=None, TunnelMonitorProfile=None, TunnelMonitorProxyId=None, TunnelMonitorSourceIp=None, Type=None)
