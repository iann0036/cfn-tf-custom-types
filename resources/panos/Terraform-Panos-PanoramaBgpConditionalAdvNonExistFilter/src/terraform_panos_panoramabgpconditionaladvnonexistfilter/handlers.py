...
def create_handler(
    ...
    try:
        ...
        tf['resource']['panos_panorama_bgp_conditional_adv_non_exist_filter'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("panos"))['SecretString']
            tf["provider"]["panos"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['panos_panorama_bgp_conditional_adv_non_exist_filter'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
