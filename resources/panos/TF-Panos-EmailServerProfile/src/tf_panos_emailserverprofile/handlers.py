...
TYPE_NAME = "TF::Panos::EmailServerProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_email_server_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_email_server_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_email_server_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthFormat=None, ConfigFormat=None, DataFormat=None, EscapeCharacter=None, EscapedCharacters=None, GtpFormat=None, HipMatchFormat=None, Id=None, IptagFormat=None, Name=None, SctpFormat=None, SystemFormat=None, ThreatFormat=None, TrafficFormat=None, TunnelFormat=None, UrlFormat=None, UserIdFormat=None, Vsys=None, WildfireFormat=None, EmailServer=None)
