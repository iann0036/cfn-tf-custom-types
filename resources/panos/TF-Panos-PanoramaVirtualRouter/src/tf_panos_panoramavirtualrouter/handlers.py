...
TYPE_NAME = "TF::Panos::PanoramaVirtualRouter"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_virtual_router/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_panorama_virtual_router/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_panorama_virtual_router/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EbgpDist=None, EcmpHashSeed=None, EcmpHashSourceOnly=None, EcmpHashUsePort=None, EcmpLoadBalanceMethod=None, EcmpMaxPath=None, EcmpStrictSourcePath=None, EcmpSymmetricReturn=None, EcmpWeightedRoundRobinInterfaces=None, EnableEcmp=None, IbgpDist=None, Id=None, Interfaces=None, Name=None, OspfExtDist=None, OspfIntDist=None, Ospfv3ExtDist=None, Ospfv3IntDist=None, RipDist=None, StaticDist=None, StaticIpv6Dist=None, Template=None, Vsys=None)
