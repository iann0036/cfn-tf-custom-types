...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_bgp_peer/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_bgp_peer/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_bgp_peer/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddressFamilyType=None, AllowIncomingConnections=None, AllowOutgoingConnections=None, AuthProfile=None, BfdProfile=None, BgpPeerGroup=None, Enable=None, EnableMpBgp=None, EnableSenderSideLoopDetection=None, HoldTime=None, Id=None, IdleHoldTime=None, IncomingConnectionsRemotePort=None, KeepAliveInterval=None, LocalAddressInterface=None, LocalAddressIp=None, MaxPrefixes=None, MinRouteAdvertisementInterval=None, MultiHop=None, Name=None, OpenDelayTime=None, OutgoingConnectionsLocalPort=None, PeerAddressIp=None, PeerAs=None, PeeringType=None, ReflectorClient=None, SubsequentAddressFamilyMulticast=None, SubsequentAddressFamilyUnicast=None, VirtualRouter=None)
