...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_ike_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/panos_ike_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/panos_ike_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuthType=None, CertBaseUrl=None, CertEnableHashAndUrl=None, CertEnableStrictValidation=None, CertPermitPayloadMismatch=None, CertProfile=None, CertUseManagementAsSource=None, DeadPeerDetectionInterval=None, DeadPeerDetectionRetry=None, Disabled=None, EnableDeadPeerDetection=None, EnableFragmentation=None, EnableIpv6=None, EnableLivenessCheck=None, EnableNatTraversal=None, EnablePassiveMode=None, Id=None, Ikev1CryptoProfile=None, Ikev1ExchangeMode=None, Ikev2CookieValidation=None, Ikev2CryptoProfile=None, Interface=None, LivenessCheckInterval=None, LocalCert=None, LocalIdType=None, LocalIdValue=None, LocalIpAddressType=None, LocalIpAddressValue=None, Name=None, NatTraversalEnableUdpChecksum=None, NatTraversalKeepAlive=None, PeerIdCheck=None, PeerIdType=None, PeerIdValue=None, PeerIpType=None, PeerIpValue=None, PreSharedKey=None, PreSharedKeyEnc=None, Version=None)
