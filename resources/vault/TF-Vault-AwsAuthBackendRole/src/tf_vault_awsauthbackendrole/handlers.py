...
TYPE_NAME = "TF::Vault::AwsAuthBackendRole"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_aws_auth_backend_role/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/vault',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_aws_auth_backend_role/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vault_aws_auth_backend_role/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowInstanceMigration=None, AuthType=None, Backend=None, BoundAccountId=None, BoundAccountIds=None, BoundAmiId=None, BoundAmiIds=None, BoundEc2InstanceId=None, BoundEc2InstanceIds=None, BoundIamInstanceProfileArn=None, BoundIamInstanceProfileArns=None, BoundIamPrincipalArn=None, BoundIamPrincipalArns=None, BoundIamRoleArn=None, BoundIamRoleArns=None, BoundRegion=None, BoundRegions=None, BoundSubnetId=None, BoundSubnetIds=None, BoundVpcId=None, BoundVpcIds=None, DisallowReauthentication=None, Id=None, InferredAwsRegion=None, InferredEntityType=None, MaxTtl=None, Period=None, Policies=None, ResolveAwsUniqueIds=None, Role=None, RoleTag=None, TokenBoundCidrs=None, TokenExplicitMaxTtl=None, TokenMaxTtl=None, TokenNoDefaultPolicy=None, TokenNumUses=None, TokenPeriod=None, TokenPolicies=None, TokenTtl=None, TokenType=None, Ttl=None)
