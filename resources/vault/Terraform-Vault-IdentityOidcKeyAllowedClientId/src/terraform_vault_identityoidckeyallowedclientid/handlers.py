...
def create_handler(
    ...
    try:
        ...
        tf['resource']['vault_identity_oidc_key_allowed_client_id'][request.logicalResourceIdentifier] = {}
        ...
        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['vault_identity_oidc_key_allowed_client_id'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
