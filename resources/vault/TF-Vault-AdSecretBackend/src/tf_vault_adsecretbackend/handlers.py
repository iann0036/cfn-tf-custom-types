...
TYPE_NAME = "TF::Vault::AdSecretBackend"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_ad_secret_backend/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vault_ad_secret_backend',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_ad_secret_backend/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vault_ad_secret_backend/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AnonymousGroupSearch=None, Backend=None, Binddn=None, Bindpass=None, CaseSensitiveNames=None, Certificate=None, ClientTlsCert=None, ClientTlsKey=None, DefaultLeaseTtlSeconds=None, DenyNullBind=None, Description=None, Discoverdn=None, Formatter=None, Groupattr=None, Groupdn=None, Groupfilter=None, Id=None, InsecureTls=None, LastRotationTolerance=None, Length=None, Local=None, MaxLeaseTtlSeconds=None, MaxTtl=None, PasswordPolicy=None, RequestTimeout=None, Starttls=None, TlsMaxVersion=None, TlsMinVersion=None, Ttl=None, Upndomain=None, Url=None, UsePre111GroupCnBehavior=None, UseTokenGroups=None, Userattr=None, Userdn=None)
