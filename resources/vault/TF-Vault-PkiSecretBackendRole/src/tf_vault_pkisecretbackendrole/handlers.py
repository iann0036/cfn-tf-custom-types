...
TYPE_NAME = "TF::Vault::PkiSecretBackendRole"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_pki_secret_backend_role/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_pki_secret_backend_role/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vault_pki_secret_backend_role/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowAnyName=None, AllowBareDomains=None, AllowGlobDomains=None, AllowIpSans=None, AllowLocalhost=None, AllowSubdomains=None, AllowedDomains=None, AllowedDomainsTemplate=None, AllowedOtherSans=None, AllowedUriSans=None, Backend=None, BasicConstraintsValidForNonCa=None, ClientFlag=None, CodeSigningFlag=None, Country=None, EmailProtectionFlag=None, EnforceHostnames=None, ExtKeyUsage=None, GenerateLease=None, Id=None, KeyBits=None, KeyType=None, KeyUsage=None, Locality=None, MaxTtl=None, Name=None, NoStore=None, NotBeforeDuration=None, Organization=None, Ou=None, PolicyIdentifiers=None, PostalCode=None, Province=None, RequireCn=None, ServerFlag=None, StreetAddress=None, Ttl=None, UseCsrCommonName=None, UseCsrSans=None)
