...
TYPE_NAME = "TF::Vault::PkiSecretBackendRootSignIntermediate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_pki_secret_backend_root_sign_intermediate/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_pki_secret_backend_root_sign_intermediate/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vault_pki_secret_backend_root_sign_intermediate/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AltNames=None, Backend=None, CaChain=None, Certificate=None, CommonName=None, Country=None, Csr=None, ExcludeCnFromSans=None, Format=None, Id=None, IpSans=None, IssuingCa=None, Locality=None, MaxPathLength=None, Organization=None, OtherSans=None, Ou=None, PermittedDnsDomains=None, PostalCode=None, Province=None, Serial=None, StreetAddress=None, Ttl=None, UriSans=None, UseCsrValues=None)
