...
def create_handler(
    ...
    try:
        ...
        tf['resource']['vault_pki_secret_backend_intermediate_cert_request'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("vault"))['SecretString']
            tf["provider"]["vault"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['vault_pki_secret_backend_intermediate_cert_request'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
