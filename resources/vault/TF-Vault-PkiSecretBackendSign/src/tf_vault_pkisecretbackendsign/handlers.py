...
TYPE_NAME = "TF::Vault::PkiSecretBackendSign"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_pki_secret_backend_sign/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_pki_secret_backend_sign/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vault_pki_secret_backend_sign/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AltNames=None, AutoRenew=None, Backend=None, CaChain=None, Certificate=None, CommonName=None, Csr=None, ExcludeCnFromSans=None, Expiration=None, Format=None, Id=None, IpSans=None, IssuingCa=None, MinSecondsRemaining=None, Name=None, OtherSans=None, Serial=None, Ttl=None, UriSans=None)
