...
TYPE_NAME = "TF::Vault::TokenAuthBackendRole"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_token_auth_backend_role/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vault_token_auth_backend_role/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vault_token_auth_backend_role/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowedPolicies=None, BoundCidrs=None, DisallowedPolicies=None, ExplicitMaxTtl=None, Id=None, Orphan=None, PathSuffix=None, Period=None, Renewable=None, RoleName=None, TokenBoundCidrs=None, TokenExplicitMaxTtl=None, TokenMaxTtl=None, TokenNoDefaultPolicy=None, TokenNumUses=None, TokenPeriod=None, TokenPolicies=None, TokenTtl=None, TokenType=None)
