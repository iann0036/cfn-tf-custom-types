...
TYPE_NAME = "TF::LaunchDarkly::Environment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/launchdarkly_environment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'launchdarkly/launchdarkly',
                'providerTypeName': 'launchdarkly',
                'terraformTypeName': 'launchdarkly_environment',
                'returnValues': ["ApiKey", "ClientSideId", "Id", "MobileKey"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/launchdarkly_environment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/launchdarkly_environment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiKey=None, ClientSideId=None, Color=None, ConfirmChanges=None, DefaultTrackEvents=None, DefaultTtl=None, Id=None, Key=None, MobileKey=None, Name=None, ProjectKey=None, RequireComments=None, SecureMode=None, Tags=None)
