...
TYPE_NAME = "TF::MSO::SchemaSiteAnpEpgDomain"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/mso_schema_site_anp_epg_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'CiscoDevNet/mso',
                'providerTypeName': 'mso',
                'terraformTypeName': 'mso_schema_site_anp_epg_domain',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/mso_schema_site_anp_epg_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/mso_schema_site_anp_epg_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowMicroSegmentation=None, AnpName=None, DeployImmediacy=None, Dn=None, DomainType=None, EnhancedLagPolicyDn=None, EnhancedLagPolicyName=None, EpgName=None, Id=None, MicroSegVlan=None, MicroSegVlanType=None, PortEncapVlan=None, PortEncapVlanType=None, ResolutionImmediacy=None, SchemaId=None, SiteId=None, SwitchType=None, SwitchingMode=None, TemplateName=None, VlanEncapMode=None)
