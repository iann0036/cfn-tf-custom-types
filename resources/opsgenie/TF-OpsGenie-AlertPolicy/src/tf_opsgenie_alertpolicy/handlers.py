...
TYPE_NAME = "TF::OpsGenie::AlertPolicy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/opsgenie_alert_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'opsgenie/opsgenie',
                'providerTypeName': 'opsgenie',
                'terraformTypeName': 'opsgenie_alert_policy',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/opsgenie_alert_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/opsgenie_alert_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Actions=None, AlertDescription=None, Alias=None, ContinuePolicy=None, Enabled=None, Entity=None, Id=None, IgnoreOriginalActions=None, IgnoreOriginalDetails=None, IgnoreOriginalResponders=None, IgnoreOriginalTags=None, Message=None, Name=None, PolicyDescription=None, Priority=None, Source=None, Tags=None, TeamId=None, Filter=None, Responders=None, TimeRestriction=None)
