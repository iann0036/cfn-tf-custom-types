...
def create_handler(
    ...
    try:
        ...
        tf['resource']['opsgenie_notification_policy'][request.logicalResourceIdentifier] = {}

        if "opsgenie" == "aws":
            creds = session.credentials
            tf["provider"]["opsgenie"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("opsgenie"))['SecretString']
            tf["provider"]["opsgenie"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['opsgenie_notification_policy'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
