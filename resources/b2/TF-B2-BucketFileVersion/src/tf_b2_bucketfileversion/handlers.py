...
TYPE_NAME = "TF::B2::BucketFileVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/b2_bucket_file_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'Backblaze/b2',
                'providerTypeName': 'b2',
                'terraformTypeName': 'b2_bucket_file_version',
                'returnValues': ["Action", "ContentMd5", "ContentSha1", "FileId", "Id", "Size", "UploadTimestamp"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/b2_bucket_file_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/b2_bucket_file_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, BucketId=None, ContentMd5=None, ContentSha1=None, ContentType=None, FileId=None, FileInfo=None, FileName=None, Id=None, Size=None, Source=None, UploadTimestamp=None, ServerSideEncryption=None)
