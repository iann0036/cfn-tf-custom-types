...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_net_ike_peer/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'bigip_net_ike_peer',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_net_ike_peer/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/bigip_net_ike_peer/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppService=None, CaCertFile=None, CrlFile=None, Description=None, DpdDelay=None, GeneratePolicy=None, Id=None, Lifetime=None, Mode=None, MyCertFile=None, MyCertKeyFile=None, MyCertKeyPassphrase=None, MyIdType=None, MyIdValue=None, Name=None, NatTraversal=None, Passive=None, PeersCertFile=None, PeersCertType=None, PeersIdType=None, PeersIdValue=None, Phase1AuthMethod=None, Phase1EncryptAlgorithm=None, Phase1HashAlgorithm=None, Phase1PerfectForwardSecrecy=None, PresharedKey=None, PresharedKeyEncrypted=None, Prf=None, ProxySupport=None, RemoteAddress=None, ReplayWindowSize=None, State=None, TrafficSelector=None, VerifyCert=None, Version=None)
