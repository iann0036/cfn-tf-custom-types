...
def create_handler(
    ...
    try:
        ...
        tf['resource']['bigip_ltm_persistence_profile_dstaddr'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("bigip"))['SecretString']
            tf["provider"]["bigip"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['bigip_ltm_persistence_profile_dstaddr'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
