...
TYPE_NAME = "TF::BIGIP::LtmVirtualServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_ltm_virtual_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_ltm_virtual_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/bigip_ltm_virtual_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClientProfiles=None, DefaultPersistenceProfile=None, Description=None, Destination=None, FallbackPersistenceProfile=None, Id=None, IpProtocol=None, Irules=None, Mask=None, Name=None, PersistenceProfiles=None, Policies=None, Pool=None, Port=None, Profiles=None, ServerProfiles=None, Snatpool=None, Source=None, SourceAddressTranslation=None, State=None, TranslateAddress=None, TranslatePort=None, Vlans=None, VlansEnabled=None)
