...
TYPE_NAME = "TF::BIGIP::LtmProfileHttp2"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_ltm_profile_http2/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_ltm_profile_http2/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/bigip_ltm_profile_http2/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActivationModes=None, ConcurrentStreamsPerConnection=None, ConnectionIdleTimeout=None, DefaultsFrom=None, EnforceTlsRequirements=None, FrameSize=None, HeaderTableSize=None, Id=None, IncludeContentLength=None, InsertHeader=None, InsertHeaderName=None, Name=None, ReceiveWindow=None, WriteSize=None)
