...
TYPE_NAME = "TF::BIGIP::LtmProfileHttp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_ltm_profile_http/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/bigip_ltm_profile_http/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/bigip_ltm_profile_http/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcceptXff=None, AppService=None, BasicAuthRealm=None, DefaultsFrom=None, Description=None, EncryptCookieSecret=None, EncryptCookies=None, FallbackHost=None, FallbackStatusCodes=None, HeadErase=None, HeadInsert=None, Id=None, InsertXforwardedFor=None, LwsSeparator=None, Name=None, OneconnectTransformations=None, ProxyType=None, RedirectRewrite=None, RequestChunking=None, ResponseChunking=None, ResponseHeadersPermitted=None, ServerAgentName=None, TmPartition=None, ViaHostName=None, ViaRequest=None, ViaResponse=None, XffAlternativeNames=None)
