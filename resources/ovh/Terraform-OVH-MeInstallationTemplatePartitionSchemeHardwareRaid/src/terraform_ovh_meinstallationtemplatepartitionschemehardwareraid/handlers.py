...
def create_handler(
    ...
    try:
        ...
        tf['resource']['ovh_me_installation_template_partition_scheme_hardware_raid'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("ovh"))['SecretString']
            tf["provider"]["ovh"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['ovh_me_installation_template_partition_scheme_hardware_raid'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
