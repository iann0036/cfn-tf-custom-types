...
TYPE_NAME = "TF::OVH::CloudProjectKube"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ovh_cloud_project_kube/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ovh_cloud_project_kube',
                'returnValues': ["ControlPlaneIsUpToDate", "Id", "IsUpToDate", "Kubeconfig", "NextUpgradeVersions", "NodesUrl", "Status", "UpdatePolicy", "Url"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ovh_cloud_project_kube/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ovh_cloud_project_kube/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ControlPlaneIsUpToDate=None, Id=None, IsUpToDate=None, Kubeconfig=None, Name=None, NextUpgradeVersions=None, NodesUrl=None, PrivateNetworkId=None, Region=None, ServiceName=None, Status=None, UpdatePolicy=None, Url=None, Version=None)
