...
TYPE_NAME = "TF::OVH::CloudProjectKubeNodepool"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ovh_cloud_project_kube_nodepool/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ovh_cloud_project_kube_nodepool',
                'returnValues': ["AvailableNodes", "CreatedAt", "CurrentNodes", "Flavor", "Id", "ProjectId", "SizeStatus", "Status", "UpToDateNodes", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ovh_cloud_project_kube_nodepool/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ovh_cloud_project_kube_nodepool/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AntiAffinity=None, Autoscale=None, AvailableNodes=None, CreatedAt=None, CurrentNodes=None, DesiredNodes=None, Flavor=None, FlavorName=None, Id=None, KubeId=None, MaxNodes=None, MinNodes=None, MonthlyBilled=None, Name=None, ProjectId=None, ServiceName=None, SizeStatus=None, Status=None, UpToDateNodes=None, UpdatedAt=None)
