...
TYPE_NAME = "TF::OVH::MeInstallationTemplate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ovh_me_installation_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'ovh/ovh',
                'providerTypeName': 'ovh',
                'terraformTypeName': 'ovh_me_installation_template',
                'returnValues': ["AvailableLanguages", "Beta", "BitFormat", "Category", "Deprecated", "Description", "Distribution", "Family", "Filesystems", "HardRaidConfiguration", "Id", "LastModification", "LvmReady", "SupportsDistributionKernel", "SupportsGptLabel", "SupportsRtm", "SupportsSqlServer", "SupportsUefi"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ovh_me_installation_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ovh_me_installation_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailableLanguages=None, BaseTemplateName=None, Beta=None, BitFormat=None, Category=None, DefaultLanguage=None, Deprecated=None, Description=None, Distribution=None, Family=None, Filesystems=None, HardRaidConfiguration=None, Id=None, LastModification=None, LvmReady=None, RemoveDefaultPartitionSchemes=None, SupportsDistributionKernel=None, SupportsGptLabel=None, SupportsRtm=None, SupportsSqlServer=None, SupportsUefi=None, TemplateName=None, Customization=None)
