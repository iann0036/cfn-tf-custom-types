...
TYPE_NAME = "TF::AzureDevOps::ServiceendpointAzurecr"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azuredevops_serviceendpoint_azurecr/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'microsoft/azuredevops',
                'providerTypeName': 'azuredevops',
                'terraformTypeName': 'azuredevops_serviceendpoint_azurecr',
                'returnValues': ["AppObjectId", "AzSpnRoleAssignmentId", "AzSpnRolePermissions", "Id", "ServicePrincipalId", "SpnObjectId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azuredevops_serviceendpoint_azurecr/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azuredevops_serviceendpoint_azurecr/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppObjectId=None, Authorization=None, AzSpnRoleAssignmentId=None, AzSpnRolePermissions=None, AzurecrName=None, AzurecrSpnTenantid=None, AzurecrSubscriptionId=None, AzurecrSubscriptionName=None, Description=None, Id=None, ProjectId=None, ResourceGroup=None, ServiceEndpointName=None, ServicePrincipalId=None, SpnObjectId=None, Timeouts=None)
