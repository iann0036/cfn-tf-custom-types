...
TYPE_NAME = "TF::Nutanix::VirtualMachine"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nutanix_virtual_machine/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nutanix_virtual_machine/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nutanix_virtual_machine/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiVersion=None, AvailabilityZoneReference=None, BootDeviceDiskAddress=None, BootDeviceMacAddress=None, BootDeviceOrderList=None, BootType=None, CloudInitCdromUuid=None, ClusterName=None, ClusterUuid=None, Description=None, EnableScriptExec=None, GuestCustomizationCloudInitCustomKeyValues=None, GuestCustomizationCloudInitMetaData=None, GuestCustomizationCloudInitUserData=None, GuestCustomizationIsOverridable=None, GuestCustomizationSysprep=None, GuestCustomizationSysprepCustomKeyValues=None, GuestOsId=None, HardwareClockTimezone=None, HostReference=None, HypervisorType=None, Id=None, MachineType=None, MemorySizeMib=None, Metadata=None, Name=None, NgtCredentials=None, NgtEnabledCapabilityList=None, NicListStatus=None, NumSockets=None, NumVcpusPerSocket=None, NumVnumaNodes=None, NutanixGuestTools=None, OwnerReference=None, ParentReference=None, PowerState=None, PowerStateMechanism=None, ProjectReference=None, ShouldFailOnScriptFailure=None, State=None, UseHotAdd=None, VgaConsoleEnabled=None, Categories=None, DiskList=None, GpuList=None, NicList=None, SerialPortList=None)
