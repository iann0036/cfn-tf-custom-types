...
def create_handler(
    ...
    try:
        ...
        tf['resource']['nutanix_network_security_rule'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("nutanix"))['SecretString']
            tf["provider"]["nutanix"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['nutanix_network_security_rule'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
