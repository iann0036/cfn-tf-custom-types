...
TYPE_NAME = "TF::Nutanix::NetworkSecurityRule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nutanix_network_security_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'nutanix/nutanix',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nutanix_network_security_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nutanix_network_security_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdRuleAction=None, AdRuleTargetGroupDefaultInternalPolicy=None, AdRuleTargetGroupFilterKindList=None, AdRuleTargetGroupFilterType=None, AdRuleTargetGroupPeerSpecificationType=None, AllowIpv6Traffic=None, ApiVersion=None, AppRuleAction=None, AppRuleTargetGroupDefaultInternalPolicy=None, AppRuleTargetGroupFilterKindList=None, AppRuleTargetGroupFilterType=None, AppRuleTargetGroupPeerSpecificationType=None, Description=None, Id=None, IsPolicyHitlogEnabled=None, IsolationRuleAction=None, IsolationRuleFirstEntityFilterKindList=None, IsolationRuleFirstEntityFilterType=None, IsolationRuleSecondEntityFilterKindList=None, IsolationRuleSecondEntityFilterType=None, Metadata=None, Name=None, OwnerReference=None, ProjectReference=None, AdRuleInboundAllowList=None, AdRuleOutboundAllowList=None, AdRuleTargetGroupFilterParams=None, AppRuleInboundAllowList=None, AppRuleOutboundAllowList=None, AppRuleTargetGroupFilterParams=None, Categories=None, IsolationRuleFirstEntityFilterParams=None, IsolationRuleSecondEntityFilterParams=None)
