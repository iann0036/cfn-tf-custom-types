...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_deployment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_deployment',
                'returnValues': ["CreatedAt", "CreatedBy", "Expense", "Id", "InputsIncludingDefaults", "LastRequest", "LastUpdatedAt", "LastUpdatedBy", "OrgId", "Project", "Resources", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_deployment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_deployment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BlueprintContent=None, BlueprintId=None, BlueprintVersion=None, CatalogItemId=None, CatalogItemVersion=None, CreatedAt=None, CreatedBy=None, Description=None, ExpandLastRequest=None, ExpandProject=None, ExpandResources=None, Expense=None, Id=None, Inputs=None, InputsIncludingDefaults=None, LastRequest=None, LastUpdatedAt=None, LastUpdatedBy=None, LeaseExpireAt=None, Name=None, OrgId=None, Owner=None, Project=None, ProjectId=None, Resources=None, Status=None, Timeouts=None)
