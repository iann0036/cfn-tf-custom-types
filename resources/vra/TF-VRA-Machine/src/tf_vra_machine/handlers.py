...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_machine/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_machine',
                'returnValues': ["Address", "CreatedAt", "DisksList", "ExternalId", "ExternalRegionId", "ExternalZoneId", "Id", "Links", "OrganizationId", "Owner", "PowerState", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_machine/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_machine/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, CreatedAt=None, CustomProperties=None, DeploymentId=None, Description=None, DisksList=None, ExternalId=None, ExternalRegionId=None, ExternalZoneId=None, Flavor=None, Id=None, Image=None, ImageRef=None, Links=None, Name=None, OrganizationId=None, Owner=None, PowerState=None, ProjectId=None, UpdatedAt=None, BootConfig=None, Constraints=None, Disks=None, ImageDiskConstraints=None, Nics=None, Tags=None, Timeouts=None)
