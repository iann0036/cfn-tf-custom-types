...
TYPE_NAME = "TF::VRA::CatalogSourceBlueprint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_catalog_source_blueprint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_catalog_source_blueprint',
                'returnValues': ["CreatedAt", "CreatedBy", "Global", "Id", "ItemsFound", "ItemsImported", "LastImportCompletedAt", "LastImportErrors", "LastImportStartedAt", "LastUpdatedBy", "TypeId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_catalog_source_blueprint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_catalog_source_blueprint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Config=None, CreatedAt=None, CreatedBy=None, Description=None, Global=None, Id=None, ItemsFound=None, ItemsImported=None, LastImportCompletedAt=None, LastImportErrors=None, LastImportStartedAt=None, LastUpdatedBy=None, Name=None, ProjectId=None, TypeId=None)
