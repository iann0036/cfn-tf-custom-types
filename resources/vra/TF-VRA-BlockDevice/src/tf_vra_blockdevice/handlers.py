...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_block_device/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_block_device',
                'returnValues': ["CreatedAt", "ExternalId", "ExternalRegionId", "ExternalZoneId", "Id", "Links", "OrgId", "Owner", "Snapshots", "Status", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_block_device/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_block_device/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CapacityInGb=None, CreatedAt=None, CustomProperties=None, DeploymentId=None, Description=None, DiskContentBase64=None, Encrypted=None, ExpandSnapshots=None, ExternalId=None, ExternalRegionId=None, ExternalZoneId=None, Id=None, Links=None, Name=None, OrgId=None, Owner=None, Persistent=None, ProjectId=None, Purge=None, Snapshots=None, SourceReference=None, Status=None, UpdatedAt=None, Constraints=None, Tags=None, Timeouts=None)
