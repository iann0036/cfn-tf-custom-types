...
TYPE_NAME = "TF::VRA::CloudAccountVsphere"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_cloud_account_vsphere/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_cloud_account_vsphere',
                'returnValues': ["CreatedAt", "CustomProperties", "Id", "Links", "OrgId", "Owner", "RegionIds", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_cloud_account_vsphere/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_cloud_account_vsphere/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcceptSelfSignedCert=None, AssociatedCloudAccountIds=None, CreatedAt=None, CustomProperties=None, Dcid=None, Description=None, Hostname=None, Id=None, Links=None, Name=None, OrgId=None, Owner=None, Password=None, RegionIds=None, Regions=None, UpdatedAt=None, Username=None, Tags=None)
