...
TYPE_NAME = "TF::VRA::FabricNetworkVsphere"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_fabric_network_vsphere/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_fabric_network_vsphere',
                'returnValues': ["CloudAccountIds", "CreatedAt", "CustomProperties", "ExternalId", "ExternalRegionId", "Id", "Links", "Name", "OrgId", "OrganizationId", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_fabric_network_vsphere/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_fabric_network_vsphere/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Cidr=None, CloudAccountIds=None, CreatedAt=None, CustomProperties=None, DefaultGateway=None, DefaultIpv6Gateway=None, DnsSearchDomains=None, DnsServerAddresses=None, Domain=None, ExternalId=None, ExternalRegionId=None, Id=None, Ipv6Cidr=None, IsDefault=None, IsPublic=None, Links=None, Name=None, OrgId=None, OrganizationId=None, UpdatedAt=None, Tags=None)
