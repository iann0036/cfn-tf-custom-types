...
TYPE_NAME = "TF::VRA::StorageProfileVsphere"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_storage_profile_vsphere/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_storage_profile_vsphere',
                'returnValues': ["CloudAccountId", "CreatedAt", "ExternalRegionId", "Id", "Links", "OrgId", "Owner", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_storage_profile_vsphere/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_storage_profile_vsphere/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CloudAccountId=None, CreatedAt=None, DatastoreId=None, DefaultItem=None, Description=None, DiskMode=None, DiskType=None, ExternalRegionId=None, Id=None, LimitIops=None, Links=None, Name=None, OrgId=None, Owner=None, ProvisioningType=None, RegionId=None, Shares=None, SharesLevel=None, StoragePolicyId=None, SupportsEncryption=None, UpdatedAt=None, Tags=None)
