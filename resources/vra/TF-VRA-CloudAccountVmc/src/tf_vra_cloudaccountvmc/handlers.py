...
TYPE_NAME = "TF::VRA::CloudAccountVmc"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_cloud_account_vmc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_cloud_account_vmc',
                'returnValues': ["CreatedAt", "Id", "Links", "OrgId", "Owner", "RegionIds", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_cloud_account_vmc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_cloud_account_vmc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcceptSelfSignedCert=None, ApiToken=None, CreatedAt=None, DcId=None, Description=None, Id=None, Links=None, Name=None, NsxHostname=None, OrgId=None, Owner=None, RegionIds=None, Regions=None, SddcName=None, UpdatedAt=None, VcenterHostname=None, VcenterPassword=None, VcenterUsername=None, Tags=None)
