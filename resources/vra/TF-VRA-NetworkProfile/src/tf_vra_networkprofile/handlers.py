...
TYPE_NAME = "TF::VRA::NetworkProfile"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_network_profile/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_network_profile',
                'returnValues': ["CloudAccountId", "CreatedAt", "ExternalRegionId", "Id", "Links", "OrgId", "OrganizationId", "Owner", "UpdatedAt"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_network_profile/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_network_profile/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CloudAccountId=None, CreatedAt=None, CustomProperties=None, Description=None, ExternalRegionId=None, FabricNetworkIds=None, Id=None, IsolatedExternalFabricNetworkId=None, IsolatedNetworkCidrPrefix=None, IsolatedNetworkDomainCidr=None, IsolatedNetworkDomainId=None, IsolationType=None, Links=None, Name=None, OrgId=None, OrganizationId=None, Owner=None, RegionId=None, SecurityGroupIds=None, UpdatedAt=None, Tags=None)
