...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_blueprint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'vmware/vra',
                'providerTypeName': 'vra',
                'terraformTypeName': 'vra_blueprint',
                'returnValues': ["ContentSourceId", "ContentSourcePath", "ContentSourceSyncAt", "ContentSourceSyncMessages", "ContentSourceSyncStatus", "ContentSourceType", "CreatedAt", "CreatedBy", "Id", "OrgId", "ProjectName", "SelfLink", "Status", "TotalReleasedVersions", "TotalVersions", "UpdatedAt", "UpdatedBy", "Valid", "ValidationMessages"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_blueprint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_blueprint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Content=None, ContentSourceId=None, ContentSourcePath=None, ContentSourceSyncAt=None, ContentSourceSyncMessages=None, ContentSourceSyncStatus=None, ContentSourceType=None, CreatedAt=None, CreatedBy=None, Description=None, Id=None, Name=None, OrgId=None, ProjectId=None, ProjectName=None, RequestScopeOrg=None, SelfLink=None, Status=None, TotalReleasedVersions=None, TotalVersions=None, UpdatedAt=None, UpdatedBy=None, Valid=None, ValidationMessages=None)
