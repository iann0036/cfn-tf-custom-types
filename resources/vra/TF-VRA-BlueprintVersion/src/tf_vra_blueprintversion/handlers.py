...
TYPE_NAME = "TF::VRA::BlueprintVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_blueprint_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vra_blueprint_version',
                'returnValues': ["BlueprintDescription", "Content", "CreatedAt", "CreatedBy", "Id", "Name", "OrgId", "ProjectId", "ProjectName", "Status", "UpdatedAt", "UpdatedBy", "Valid"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vra_blueprint_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vra_blueprint_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BlueprintDescription=None, BlueprintId=None, ChangeLog=None, Content=None, CreatedAt=None, CreatedBy=None, Description=None, Id=None, Name=None, OrgId=None, ProjectId=None, ProjectName=None, Release=None, Status=None, UpdatedAt=None, UpdatedBy=None, Valid=None, Version=None)
