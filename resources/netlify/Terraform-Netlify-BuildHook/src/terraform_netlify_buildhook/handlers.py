...
def create_handler(
    ...
    try:
        ...
        tf['resource']['netlify_build_hook'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("netlify"))['SecretString']
            tf["provider"]["netlify"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['netlify_build_hook'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
