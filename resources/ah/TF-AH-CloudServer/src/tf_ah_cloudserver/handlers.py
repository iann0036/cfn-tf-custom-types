...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ah_cloud_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'advancedhosting/ah',
                'providerTypeName': 'ah',
                'terraformTypeName': 'ah_cloud_server',
                'returnValues': ["CreatedAt", "Disk", "Id", "Ips", "Ram", "State", "Vcpu"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ah_cloud_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ah_cloud_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Backups=None, CreatePublicIpAddress=None, CreatedAt=None, Datacenter=None, Disk=None, Id=None, Image=None, Ips=None, Name=None, Product=None, Ram=None, SshKeys=None, State=None, UsePassword=None, Vcpu=None)
