...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/dcnm_network/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'dcnm_network',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/dcnm_network/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/dcnm_network/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ArpSuppFlag=None, Deploy=None, DeployTimeout=None, Description=None, Dhcp1=None, Dhcp2=None, DhcpVrf=None, DisplayName=None, ExtensionTemplate=None, FabricName=None, Id=None, Ipv4Gateway=None, Ipv6Gateway=None, IrEnableFlag=None, L2OnlyFlag=None, L3GatewayFlag=None, LoopbackId=None, McastGroup=None, Mtu=None, Name=None, NetworkId=None, RtBothFlag=None, SecondaryGw1=None, SecondaryGw2=None, ServiceTemplate=None, Source=None, Tag=None, Template=None, TrmEnableFlag=None, VlanId=None, VlanName=None, VrfName=None, Attachments=None)
