...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/dcnm_vrf/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/dcnm_vrf/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/dcnm_vrf/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdvertiseDefaultRoute=None, AdvertiseHostRoute=None, Deploy=None, DeployTimeout=None, Description=None, ExtensionTemplate=None, FabricName=None, Id=None, IntfDescription=None, Ipv6LinkLocalFlag=None, LoopbackId=None, MaxBgpPath=None, MaxIbgpPath=None, Mtu=None, MutlicastAddress=None, MutlicastGroup=None, Name=None, RpAddress=None, RpExternalFlag=None, SegmentId=None, ServiceTemplate=None, Source=None, StaticDefaultRoute=None, Tag=None, Template=None, TrmBgwMsiteFlag=None, TrmEnable=None, VlanId=None, VlanName=None, Attachments=None)
