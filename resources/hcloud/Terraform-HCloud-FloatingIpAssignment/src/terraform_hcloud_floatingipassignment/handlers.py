...
def create_handler(
    ...
    try:
        ...
        tf['resource']['hcloud_floating_ip_assignment'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("hcloud"))['SecretString']
            tf["provider"]["hcloud"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['hcloud_floating_ip_assignment'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
