...
TYPE_NAME = "TF::AzureRM::EventgridEventSubscription"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_eventgrid_event_subscription/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_eventgrid_event_subscription/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_eventgrid_event_subscription/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EventDeliverySchema=None, EventhubEndpointId=None, ExpirationTimeUtc=None, HybridConnectionEndpointId=None, Id=None, IncludedEventTypes=None, Labels=None, Name=None, Scope=None, ServiceBusQueueEndpointId=None, ServiceBusTopicEndpointId=None, TopicName=None, AdvancedFilter=None, AzureFunctionEndpoint=None, EventhubEndpoint=None, HybridConnectionEndpoint=None, RetryPolicy=None, StorageBlobDeadLetterDestination=None, StorageQueueEndpoint=None, SubjectFilter=None, Timeouts=None, WebhookEndpoint=None)
