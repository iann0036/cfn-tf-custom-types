...
TYPE_NAME = "TF::AzureRM::EventhubNamespace"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_eventhub_namespace/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["DefaultPrimaryConnectionString", "DefaultPrimaryConnectionStringAlias", "DefaultPrimaryKey", "DefaultSecondaryConnectionString", "DefaultSecondaryConnectionStringAlias", "DefaultSecondaryKey", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_eventhub_namespace/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_eventhub_namespace/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoInflateEnabled=None, Capacity=None, DedicatedClusterId=None, DefaultPrimaryConnectionString=None, DefaultPrimaryConnectionStringAlias=None, DefaultPrimaryKey=None, DefaultSecondaryConnectionString=None, DefaultSecondaryConnectionStringAlias=None, DefaultSecondaryKey=None, Id=None, Location=None, MaximumThroughputUnits=None, Name=None, NetworkRulesets=None, ResourceGroupName=None, Sku=None, Tags=None, ZoneRedundant=None, Identity=None, Timeouts=None)
