...
TYPE_NAME = "TF::AzureRM::ServicebusSubscription"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_servicebus_subscription/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_servicebus_subscription/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_servicebus_subscription/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoDeleteOnIdle=None, DeadLetteringOnFilterEvaluationError=None, DeadLetteringOnMessageExpiration=None, DefaultMessageTtl=None, EnableBatchedOperations=None, ForwardDeadLetteredMessagesTo=None, ForwardTo=None, Id=None, LockDuration=None, MaxDeliveryCount=None, Name=None, NamespaceName=None, RequiresSession=None, ResourceGroupName=None, Status=None, TopicName=None, Timeouts=None)
