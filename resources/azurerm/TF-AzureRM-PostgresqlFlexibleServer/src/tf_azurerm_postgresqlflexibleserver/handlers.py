...
TYPE_NAME = "TF::AzureRM::PostgresqlFlexibleServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_postgresql_flexible_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_postgresql_flexible_server',
                'returnValues': ["CmkEnabled", "Fqdn", "Id", "PublicNetworkAccessEnabled"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_postgresql_flexible_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_postgresql_flexible_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdministratorLogin=None, AdministratorPassword=None, BackupRetentionDays=None, CmkEnabled=None, CreateMode=None, DelegatedSubnetId=None, Fqdn=None, Id=None, Location=None, Name=None, PointInTimeRestoreTimeInUtc=None, PublicNetworkAccessEnabled=None, ResourceGroupName=None, SkuName=None, SourceServerId=None, StorageMb=None, Tags=None, Version=None, Zone=None, MaintenanceWindow=None, Timeouts=None)
