...
TYPE_NAME = "TF::AzureRM::KeyVaultManagedHardwareSecurityModule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_key_vault_managed_hardware_security_module/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_key_vault_managed_hardware_security_module',
                'returnValues': ["HsmUri", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_key_vault_managed_hardware_security_module/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_key_vault_managed_hardware_security_module/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminObjectIds=None, HsmUri=None, Id=None, Location=None, Name=None, PurgeProtectionEnabled=None, ResourceGroupName=None, SkuName=None, SoftDeleteRetentionDays=None, Tags=None, TenantId=None, Timeouts=None)
