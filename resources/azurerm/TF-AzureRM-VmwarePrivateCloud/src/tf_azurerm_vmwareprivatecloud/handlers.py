...
TYPE_NAME = "TF::AzureRM::VmwarePrivateCloud"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_vmware_private_cloud/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_vmware_private_cloud',
                'returnValues': ["Circuit", "HcxCloudManagerEndpoint", "Id", "ManagementSubnetCidr", "NsxtCertificateThumbprint", "NsxtManagerEndpoint", "ProvisioningSubnetCidr", "VcenterCertificateThumbprint", "VcsaEndpoint", "VmotionSubnetCidr"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_vmware_private_cloud/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_vmware_private_cloud/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Circuit=None, HcxCloudManagerEndpoint=None, Id=None, InternetConnectionEnabled=None, Location=None, ManagementSubnetCidr=None, Name=None, NetworkSubnetCidr=None, NsxtCertificateThumbprint=None, NsxtManagerEndpoint=None, NsxtPassword=None, ProvisioningSubnetCidr=None, ResourceGroupName=None, SkuName=None, Tags=None, VcenterCertificateThumbprint=None, VcenterPassword=None, VcsaEndpoint=None, VmotionSubnetCidr=None, ManagementCluster=None, Timeouts=None)
