...
TYPE_NAME = "TF::AzureRM::ServicebusQueue"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_servicebus_queue/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_servicebus_queue/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_servicebus_queue/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoDeleteOnIdle=None, DeadLetteringOnMessageExpiration=None, DefaultMessageTtl=None, DuplicateDetectionHistoryTimeWindow=None, EnableBatchedOperations=None, EnableExpress=None, EnablePartitioning=None, ForwardDeadLetteredMessagesTo=None, ForwardTo=None, Id=None, LockDuration=None, MaxDeliveryCount=None, MaxSizeInMegabytes=None, Name=None, NamespaceName=None, RequiresDuplicateDetection=None, RequiresSession=None, ResourceGroupName=None, Status=None, Timeouts=None)
