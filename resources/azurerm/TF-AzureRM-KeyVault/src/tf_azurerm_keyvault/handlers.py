...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_key_vault/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_key_vault/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_key_vault/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessPolicy=None, EnableRbacAuthorization=None, EnabledForDeployment=None, EnabledForDiskEncryption=None, EnabledForTemplateDeployment=None, Id=None, Location=None, Name=None, PurgeProtectionEnabled=None, ResourceGroupName=None, SkuName=None, SoftDeleteEnabled=None, SoftDeleteRetentionDays=None, Tags=None, TenantId=None, VaultUri=None, Contact=None, NetworkAcls=None, Timeouts=None)
