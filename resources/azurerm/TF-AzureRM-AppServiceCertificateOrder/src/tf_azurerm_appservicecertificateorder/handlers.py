...
TYPE_NAME = "TF::AzureRM::AppServiceCertificateOrder"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_app_service_certificate_order/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_app_service_certificate_order/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_app_service_certificate_order/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppServiceCertificateNotRenewableReasons=None, AutoRenew=None, Certificates=None, Csr=None, DistinguishedName=None, DomainVerificationToken=None, ExpirationTime=None, Id=None, IntermediateThumbprint=None, IsPrivateKeyExternal=None, KeySize=None, Location=None, Name=None, ProductType=None, ResourceGroupName=None, RootThumbprint=None, SignedCertificateThumbprint=None, Status=None, Tags=None, ValidityInYears=None, Timeouts=None)
