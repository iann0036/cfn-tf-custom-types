...
TYPE_NAME = "TF::AzureRM::MssqlDatabase"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_mssql_database/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_mssql_database/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_mssql_database/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoPauseDelayInMinutes=None, Collation=None, CreateMode=None, CreationSourceDatabaseId=None, ElasticPoolId=None, ExtendedAuditingPolicy=None, GeoBackupEnabled=None, Id=None, LicenseType=None, MaxSizeGb=None, MinCapacity=None, Name=None, ReadReplicaCount=None, ReadScale=None, RecoverDatabaseId=None, RestoreDroppedDatabaseId=None, RestorePointInTime=None, SampleName=None, ServerId=None, SkuName=None, StorageAccountType=None, Tags=None, ZoneRedundant=None, LongTermRetentionPolicy=None, ShortTermRetentionPolicy=None, ThreatDetectionPolicy=None, Timeouts=None)
