...
TYPE_NAME = "TF::AzureRM::LogicAppWorkflow"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_logic_app_workflow/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_logic_app_workflow/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_logic_app_workflow/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessEndpoint=None, ConnectorEndpointIpAddresses=None, ConnectorOutboundIpAddresses=None, Id=None, IntegrationServiceEnvironmentId=None, Location=None, LogicAppIntegrationAccountId=None, Name=None, Parameters=None, ResourceGroupName=None, Tags=None, WorkflowEndpointIpAddresses=None, WorkflowOutboundIpAddresses=None, WorkflowSchema=None, WorkflowVersion=None, Timeouts=None)
