...
TYPE_NAME = "TF::AzureRM::SignalrService"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_signalr_service/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_signalr_service/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_signalr_service/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Hostname=None, Id=None, IpAddress=None, Location=None, Name=None, PrimaryAccessKey=None, PrimaryConnectionString=None, PublicPort=None, ResourceGroupName=None, SecondaryAccessKey=None, SecondaryConnectionString=None, ServerPort=None, Tags=None, Cors=None, Features=None, Sku=None, Timeouts=None, UpstreamEndpoint=None)
