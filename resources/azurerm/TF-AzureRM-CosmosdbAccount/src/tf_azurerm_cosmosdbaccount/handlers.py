...
TYPE_NAME = "TF::AzureRM::CosmosdbAccount"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_cosmosdb_account/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ConnectionStrings", "Endpoint", "Id", "PrimaryKey", "PrimaryMasterKey", "PrimaryReadonlyKey", "PrimaryReadonlyMasterKey", "ReadEndpoints", "SecondaryKey", "SecondaryMasterKey", "SecondaryReadonlyKey", "SecondaryReadonlyMasterKey", "WriteEndpoints"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_cosmosdb_account/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_cosmosdb_account/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessKeyMetadataWritesEnabled=None, AnalyticalStorageEnabled=None, ConnectionStrings=None, EnableAutomaticFailover=None, EnableFreeTier=None, EnableMultipleWriteLocations=None, Endpoint=None, Id=None, IpRangeFilter=None, IsVirtualNetworkFilterEnabled=None, KeyVaultKeyId=None, Kind=None, Location=None, MongoServerVersion=None, Name=None, NetworkAclBypassForAzureServices=None, NetworkAclBypassIds=None, OfferType=None, PrimaryKey=None, PrimaryMasterKey=None, PrimaryReadonlyKey=None, PrimaryReadonlyMasterKey=None, PublicNetworkAccessEnabled=None, ReadEndpoints=None, ResourceGroupName=None, SecondaryKey=None, SecondaryMasterKey=None, SecondaryReadonlyKey=None, SecondaryReadonlyMasterKey=None, Tags=None, WriteEndpoints=None, Backup=None, Capabilities=None, ConsistencyPolicy=None, CorsRule=None, GeoLocation=None, Identity=None, Timeouts=None, VirtualNetworkRule=None)
