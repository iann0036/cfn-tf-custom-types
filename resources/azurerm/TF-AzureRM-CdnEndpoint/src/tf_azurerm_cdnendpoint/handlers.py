...
TYPE_NAME = "TF::AzureRM::CdnEndpoint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_cdn_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_cdn_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_cdn_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ContentTypesToCompress=None, HostName=None, Id=None, IsCompressionEnabled=None, IsHttpAllowed=None, IsHttpsAllowed=None, Location=None, Name=None, OptimizationType=None, OriginHostHeader=None, OriginPath=None, ProbePath=None, ProfileName=None, QuerystringCachingBehaviour=None, ResourceGroupName=None, Tags=None, DeliveryRule=None, GeoFilter=None, GlobalDeliveryRule=None, Origin=None, Timeouts=None)
