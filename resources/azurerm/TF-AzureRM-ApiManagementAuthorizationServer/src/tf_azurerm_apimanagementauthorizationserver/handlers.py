...
TYPE_NAME = "TF::AzureRM::ApiManagementAuthorizationServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_api_management_authorization_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_api_management_authorization_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_api_management_authorization_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiManagementName=None, AuthorizationEndpoint=None, AuthorizationMethods=None, BearerTokenSendingMethods=None, ClientAuthenticationMethod=None, ClientId=None, ClientRegistrationEndpoint=None, ClientSecret=None, DefaultScope=None, Description=None, DisplayName=None, GrantTypes=None, Id=None, Name=None, ResourceGroupName=None, ResourceOwnerPassword=None, ResourceOwnerUsername=None, SupportState=None, TokenEndpoint=None, Timeouts=None, TokenBodyParameter=None)
