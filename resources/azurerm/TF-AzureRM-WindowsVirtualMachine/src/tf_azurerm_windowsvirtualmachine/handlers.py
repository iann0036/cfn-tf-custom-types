...
TYPE_NAME = "TF::AzureRM::WindowsVirtualMachine"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_windows_virtual_machine/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_windows_virtual_machine/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_windows_virtual_machine/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminPassword=None, AdminUsername=None, AllowExtensionOperations=None, AvailabilitySetId=None, ComputerName=None, CustomData=None, DedicatedHostId=None, EnableAutomaticUpdates=None, EncryptionAtHostEnabled=None, EvictionPolicy=None, ExtensionsTimeBudget=None, Id=None, LicenseType=None, Location=None, MaxBidPrice=None, Name=None, NetworkInterfaceIds=None, PatchMode=None, PlatformFaultDomain=None, Priority=None, PrivateIpAddress=None, PrivateIpAddresses=None, ProvisionVmAgent=None, ProximityPlacementGroupId=None, PublicIpAddress=None, PublicIpAddresses=None, ResourceGroupName=None, Size=None, SourceImageId=None, Tags=None, Timezone=None, VirtualMachineId=None, VirtualMachineScaleSetId=None, Zone=None, AdditionalCapabilities=None, AdditionalUnattendContent=None, BootDiagnostics=None, Identity=None, OsDisk=None, Plan=None, Secret=None, SourceImageReference=None, Timeouts=None, WinrmListener=None)
