...
TYPE_NAME = "TF::AzureRM::FunctionAppSlot"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_function_app_slot/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_function_app_slot',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_function_app_slot/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_function_app_slot/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppServicePlanId=None, AppSettings=None, ClientAffinityEnabled=None, DailyMemoryTimeQuota=None, DefaultHostname=None, EnableBuiltinLogging=None, Enabled=None, FunctionAppName=None, HttpsOnly=None, Id=None, Kind=None, Location=None, Name=None, OsType=None, OutboundIpAddresses=None, PossibleOutboundIpAddresses=None, ResourceGroupName=None, SiteCredential=None, StorageAccountAccessKey=None, StorageAccountName=None, Tags=None, Version=None, AuthSettings=None, ConnectionString=None, Identity=None, SiteConfig=None, Timeouts=None)
