...
TYPE_NAME = "TF::AzureRM::KubernetesCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_kubernetes_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_kubernetes_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_kubernetes_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiServerAuthorizedIpRanges=None, AutomaticChannelUpgrade=None, DiskEncryptionSetId=None, DnsPrefix=None, DnsPrefixPrivateCluster=None, EnablePodSecurityPolicy=None, Fqdn=None, Id=None, KubeAdminConfig=None, KubeAdminConfigRaw=None, KubeConfig=None, KubeConfigRaw=None, KubernetesVersion=None, Location=None, Name=None, NodeResourceGroup=None, PrivateClusterEnabled=None, PrivateDnsZoneId=None, PrivateFqdn=None, PrivateLinkEnabled=None, ResourceGroupName=None, SkuTier=None, Tags=None, AddonProfile=None, AutoScalerProfile=None, DefaultNodePool=None, Identity=None, KubeletIdentity=None, LinuxProfile=None, NetworkProfile=None, RoleBasedAccessControl=None, ServicePrincipal=None, Timeouts=None, WindowsProfile=None)
