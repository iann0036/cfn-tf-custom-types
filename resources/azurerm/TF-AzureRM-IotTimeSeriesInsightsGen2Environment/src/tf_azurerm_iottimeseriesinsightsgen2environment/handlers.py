...
TYPE_NAME = "TF::AzureRM::IotTimeSeriesInsightsGen2Environment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_iot_time_series_insights_gen2_environment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_iot_time_series_insights_gen2_environment',
                'returnValues': ["DataAccessFqdn", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_iot_time_series_insights_gen2_environment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_iot_time_series_insights_gen2_environment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DataAccessFqdn=None, Id=None, IdProperties=None, Location=None, Name=None, ResourceGroupName=None, SkuName=None, Tags=None, WarmStoreDataRetentionTime=None, Storage=None, Timeouts=None)
