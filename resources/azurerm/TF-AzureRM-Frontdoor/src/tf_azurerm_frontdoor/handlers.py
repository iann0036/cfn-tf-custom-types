...
TYPE_NAME = "TF::AzureRM::Frontdoor"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_frontdoor/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["BackendPoolHealthProbes", "BackendPoolLoadBalancingSettings", "BackendPools", "Cname", "ExplicitResourceOrder", "FrontendEndpoints", "HeaderFrontdoorId", "Id", "RoutingRules"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_frontdoor/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_frontdoor/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BackendPoolHealthProbes=None, BackendPoolLoadBalancingSettings=None, BackendPools=None, BackendPoolsSendReceiveTimeoutSeconds=None, Cname=None, EnforceBackendPoolsCertificateNameCheck=None, ExplicitResourceOrder=None, FriendlyName=None, FrontendEndpoints=None, HeaderFrontdoorId=None, Id=None, LoadBalancerEnabled=None, Location=None, Name=None, ResourceGroupName=None, RoutingRules=None, Tags=None, BackendPool=None, BackendPoolHealthProbe=None, BackendPoolLoadBalancing=None, FrontendEndpoint=None, RoutingRule=None, Timeouts=None)
