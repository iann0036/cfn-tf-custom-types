# DO NOT modify this file by hand, changes will be overwritten
import sys
from dataclasses import dataclass
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

from cloudformation_cli_python_lib.interface import (
    BaseModel,
    BaseResourceHandlerRequest,
)
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class ResourceHandlerRequest(BaseResourceHandlerRequest):
    # pylint: disable=invalid-name
    desiredResourceState: Optional["ResourceModel"]
    previousResourceState: Optional["ResourceModel"]


@dataclass
class ResourceModel(BaseModel):
    tfcfnid: Optional[str]
    Fqdn: Optional[str]
    HttpsOnly: Optional[bool]
    Id: Optional[str]
    IsPublic: Optional[bool]
    Name: Optional[str]
    ResourceGroupName: Optional[str]
    ServiceName: Optional[str]
    TlsEnabled: Optional[bool]
    Url: Optional[str]
    Identity: Optional[Sequence["_IdentityDefinition"]]
    PersistentDisk: Optional[Sequence["_PersistentDiskDefinition"]]
    Timeouts: Optional["_TimeoutsDefinition"]

    @classmethod
    def _deserialize(
        cls: Type["_ResourceModel"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_ResourceModel"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            tfcfnid=json_data.get("tfcfnid"),
            Fqdn=json_data.get("Fqdn"),
            HttpsOnly=json_data.get("HttpsOnly"),
            Id=json_data.get("Id"),
            IsPublic=json_data.get("IsPublic"),
            Name=json_data.get("Name"),
            ResourceGroupName=json_data.get("ResourceGroupName"),
            ServiceName=json_data.get("ServiceName"),
            TlsEnabled=json_data.get("TlsEnabled"),
            Url=json_data.get("Url"),
            Identity=deserialize_list(json_data.get("Identity"), IdentityDefinition),
            PersistentDisk=deserialize_list(json_data.get("PersistentDisk"), PersistentDiskDefinition),
            Timeouts=TimeoutsDefinition._deserialize(json_data.get("Timeouts")),
        )


# work around possible type aliasing issues when variable has same name as a model
_ResourceModel = ResourceModel


@dataclass
class IdentityDefinition(BaseModel):
    Type: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_IdentityDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_IdentityDefinition"]:
        if not json_data:
            return None
        return cls(
            Type=json_data.get("Type"),
        )


# work around possible type aliasing issues when variable has same name as a model
_IdentityDefinition = IdentityDefinition


@dataclass
class PersistentDiskDefinition(BaseModel):
    MountPath: Optional[str]
    SizeInGb: Optional[float]

    @classmethod
    def _deserialize(
        cls: Type["_PersistentDiskDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_PersistentDiskDefinition"]:
        if not json_data:
            return None
        return cls(
            MountPath=json_data.get("MountPath"),
            SizeInGb=json_data.get("SizeInGb"),
        )


# work around possible type aliasing issues when variable has same name as a model
_PersistentDiskDefinition = PersistentDiskDefinition


@dataclass
class TimeoutsDefinition(BaseModel):
    Create: Optional[str]
    Delete: Optional[str]
    Read: Optional[str]
    Update: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_TimeoutsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_TimeoutsDefinition"]:
        if not json_data:
            return None
        return cls(
            Create=json_data.get("Create"),
            Delete=json_data.get("Delete"),
            Read=json_data.get("Read"),
            Update=json_data.get("Update"),
        )


# work around possible type aliasing issues when variable has same name as a model
_TimeoutsDefinition = TimeoutsDefinition


