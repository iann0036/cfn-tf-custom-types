...
TYPE_NAME = "TF::AzureRM::ServiceFabricCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_service_fabric_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_service_fabric_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_service_fabric_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AddOnFeatures=None, ClusterCodeVersion=None, ClusterEndpoint=None, Id=None, Location=None, ManagementEndpoint=None, Name=None, ReliabilityLevel=None, ResourceGroupName=None, Tags=None, UpgradeMode=None, VmImage=None, AzureActiveDirectory=None, Certificate=None, CertificateCommonNames=None, ClientCertificateCommonName=None, ClientCertificateThumbprint=None, DiagnosticsConfig=None, FabricSettings=None, NodeType=None, ReverseProxyCertificate=None, ReverseProxyCertificateCommonNames=None, Timeouts=None, UpgradePolicy=None)
