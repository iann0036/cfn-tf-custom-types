...
TYPE_NAME = "TF::AzureRM::ApplicationGateway"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_application_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_application_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_application_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EnableHttp2=None, FirewallPolicyId=None, Id=None, Location=None, Name=None, ResourceGroupName=None, Tags=None, Zones=None, AuthenticationCertificate=None, AutoscaleConfiguration=None, BackendAddressPool=None, BackendHttpSettings=None, CustomErrorConfiguration=None, FrontendIpConfiguration=None, FrontendPort=None, GatewayIpConfiguration=None, HttpListener=None, Identity=None, Probe=None, RedirectConfiguration=None, RequestRoutingRule=None, RewriteRuleSet=None, Sku=None, SslCertificate=None, SslPolicy=None, Timeouts=None, TrustedRootCertificate=None, UrlPathMap=None, WafConfiguration=None)
