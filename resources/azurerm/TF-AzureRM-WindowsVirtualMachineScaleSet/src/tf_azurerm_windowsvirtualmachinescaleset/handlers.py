...
TYPE_NAME = "TF::AzureRM::WindowsVirtualMachineScaleSet"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_windows_virtual_machine_scale_set/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_windows_virtual_machine_scale_set/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_windows_virtual_machine_scale_set/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminPassword=None, AdminUsername=None, ComputerNamePrefix=None, CustomData=None, DoNotRunExtensionsOnOverprovisionedMachines=None, EnableAutomaticUpdates=None, EncryptionAtHostEnabled=None, EvictionPolicy=None, ExtensionsTimeBudget=None, HealthProbeId=None, Id=None, Instances=None, LicenseType=None, Location=None, MaxBidPrice=None, Name=None, Overprovision=None, PlatformFaultDomainCount=None, Priority=None, ProvisionVmAgent=None, ProximityPlacementGroupId=None, ResourceGroupName=None, ScaleInPolicy=None, SinglePlacementGroup=None, Sku=None, SourceImageId=None, Tags=None, Timezone=None, UniqueId=None, UpgradeMode=None, ZoneBalance=None, Zones=None, AdditionalCapabilities=None, AdditionalUnattendContent=None, AutomaticInstanceRepair=None, AutomaticOsUpgradePolicy=None, BootDiagnostics=None, DataDisk=None, Extension=None, Identity=None, NetworkInterface=None, OsDisk=None, Plan=None, RollingUpgradePolicy=None, Secret=None, SourceImageReference=None, TerminateNotification=None, Timeouts=None, WinrmListener=None)
