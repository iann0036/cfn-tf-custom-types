...
TYPE_NAME = "TF::AzureRM::TrafficManagerEndpoint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_traffic_manager_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_traffic_manager_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_traffic_manager_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EndpointLocation=None, EndpointMonitorStatus=None, EndpointStatus=None, GeoMappings=None, Id=None, MinChildEndpoints=None, Name=None, Priority=None, ProfileName=None, ResourceGroupName=None, Target=None, TargetResourceId=None, Type=None, Weight=None, CustomHeader=None, Subnet=None, Timeouts=None)
