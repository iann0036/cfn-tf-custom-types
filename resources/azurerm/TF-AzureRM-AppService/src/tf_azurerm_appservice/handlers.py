...
TYPE_NAME = "TF::AzureRM::AppService"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_app_service/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CustomDomainVerificationId", "DefaultSiteHostname", "Id", "OutboundIpAddressList", "OutboundIpAddresses", "PossibleOutboundIpAddressList", "PossibleOutboundIpAddresses", "SiteCredential"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_app_service/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_app_service/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppServicePlanId=None, AppSettings=None, ClientAffinityEnabled=None, ClientCertEnabled=None, CustomDomainVerificationId=None, DefaultSiteHostname=None, Enabled=None, HttpsOnly=None, Id=None, Location=None, Name=None, OutboundIpAddressList=None, OutboundIpAddresses=None, PossibleOutboundIpAddressList=None, PossibleOutboundIpAddresses=None, ResourceGroupName=None, SiteCredential=None, Tags=None, AuthSettings=None, Backup=None, ConnectionString=None, Identity=None, Logs=None, SiteConfig=None, SourceControl=None, StorageAccount=None, Timeouts=None)
