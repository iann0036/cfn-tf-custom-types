...
TYPE_NAME = "TF::AzureRM::ApiManagement"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_api_management/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["DeveloperPortalUrl", "GatewayRegionalUrl", "GatewayUrl", "Id", "ManagementApiUrl", "PortalUrl", "PrivateIpAddresses", "PublicIpAddresses", "ScmUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_api_management/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_api_management/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClientCertificateEnabled=None, DeveloperPortalUrl=None, GatewayDisabled=None, GatewayRegionalUrl=None, GatewayUrl=None, Id=None, Location=None, ManagementApiUrl=None, MinApiVersion=None, Name=None, NotificationSenderEmail=None, Policy=None, PortalUrl=None, PrivateIpAddresses=None, PublicIpAddresses=None, PublisherEmail=None, PublisherName=None, ResourceGroupName=None, ScmUrl=None, SkuName=None, Tags=None, VirtualNetworkType=None, Zones=None, AdditionalLocation=None, Certificate=None, HostnameConfiguration=None, Identity=None, Protocols=None, Security=None, SignIn=None, SignUp=None, TenantAccess=None, Timeouts=None, VirtualNetworkConfiguration=None)
