...
TYPE_NAME = "TF::AzureRM::SentinelAlertRuleScheduled"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_sentinel_alert_rule_scheduled/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_sentinel_alert_rule_scheduled',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_sentinel_alert_rule_scheduled/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_sentinel_alert_rule_scheduled/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AlertRuleTemplateGuid=None, Description=None, DisplayName=None, Enabled=None, Id=None, LogAnalyticsWorkspaceId=None, Name=None, Query=None, QueryFrequency=None, QueryPeriod=None, Severity=None, SuppressionDuration=None, SuppressionEnabled=None, Tactics=None, TriggerOperator=None, TriggerThreshold=None, EventGrouping=None, IncidentConfiguration=None, Timeouts=None)
