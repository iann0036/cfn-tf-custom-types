...
TYPE_NAME = "TF::AzureRM::ContainerGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_container_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_container_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_container_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DnsNameLabel=None, ExposedPort=None, Fqdn=None, Id=None, IpAddress=None, IpAddressType=None, Location=None, Name=None, NetworkProfileId=None, OsType=None, ResourceGroupName=None, RestartPolicy=None, Tags=None, Container=None, Diagnostics=None, DnsConfig=None, Identity=None, ImageRegistryCredential=None, Timeouts=None)
