...
TYPE_NAME = "TF::AzureRM::KubernetesClusterNodePool"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_kubernetes_cluster_node_pool/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_kubernetes_cluster_node_pool/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_kubernetes_cluster_node_pool/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZones=None, EnableAutoScaling=None, EnableHostEncryption=None, EnableNodePublicIp=None, EvictionPolicy=None, Id=None, KubernetesClusterId=None, MaxCount=None, MaxPods=None, MinCount=None, Mode=None, Name=None, NodeCount=None, NodeLabels=None, NodePublicIpPrefixId=None, NodeTaints=None, OrchestratorVersion=None, OsDiskSizeGb=None, OsDiskType=None, OsType=None, Priority=None, ProximityPlacementGroupId=None, SpotMaxPrice=None, Tags=None, VmSize=None, VnetSubnetId=None, KubeletConfig=None, LinuxOsConfig=None, Timeouts=None, UpgradeSettings=None)
