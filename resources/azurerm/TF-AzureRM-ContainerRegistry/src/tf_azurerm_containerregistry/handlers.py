...
TYPE_NAME = "TF::AzureRM::ContainerRegistry"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_container_registry/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_container_registry/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_container_registry/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminEnabled=None, AdminPassword=None, AdminUsername=None, Encryption=None, GeoreplicationLocations=None, Georeplications=None, Id=None, Location=None, LoginServer=None, Name=None, NetworkRuleSet=None, PublicNetworkAccessEnabled=None, QuarantinePolicyEnabled=None, ResourceGroupName=None, RetentionPolicy=None, Sku=None, StorageAccountId=None, Tags=None, TrustPolicy=None, ZoneRedundancyEnabled=None, Identity=None, Timeouts=None)
