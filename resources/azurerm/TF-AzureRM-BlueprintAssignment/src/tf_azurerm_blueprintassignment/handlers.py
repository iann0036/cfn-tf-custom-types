...
TYPE_NAME = "TF::AzureRM::BlueprintAssignment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_blueprint_assignment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'azurerm_blueprint_assignment',
                'returnValues': ["BlueprintName", "Description", "DisplayName", "Id", "Type"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azurerm_blueprint_assignment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azurerm_blueprint_assignment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BlueprintName=None, Description=None, DisplayName=None, Id=None, Location=None, LockExcludePrincipals=None, LockMode=None, Name=None, ParameterValues=None, ResourceGroups=None, TargetSubscriptionId=None, Type=None, VersionId=None, Identity=None, Timeouts=None)
