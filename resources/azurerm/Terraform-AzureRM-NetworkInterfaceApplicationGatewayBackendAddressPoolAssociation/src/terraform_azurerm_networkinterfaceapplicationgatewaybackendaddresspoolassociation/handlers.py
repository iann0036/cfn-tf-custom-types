...
def create_handler(
    ...
    try:
        ...
        tf['resource']['azurerm_network_interface_application_gateway_backend_address_pool_association'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("azurerm"))['SecretString']
            tf["provider"]["azurerm"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['azurerm_network_interface_application_gateway_backend_address_pool_association'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
