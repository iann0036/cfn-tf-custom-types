...
TYPE_NAME = "TF::PrismaCloud::Policy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/prismacloud_policy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'prismacloud_policy',
                'returnValues': ["CreatedBy", "CreatedOn", "Id", "LastModifiedBy", "LastModifiedOn", "OpenAlertsCount", "Owner", "PolicyId", "PolicyMode", "RuleLastModifiedOn"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/prismacloud_policy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/prismacloud_policy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CloudType=None, CreatedBy=None, CreatedOn=None, Deleted=None, Description=None, Enabled=None, Id=None, Labels=None, LastModifiedBy=None, LastModifiedOn=None, Name=None, OpenAlertsCount=None, Overridden=None, Owner=None, PolicyId=None, PolicyMode=None, PolicyType=None, Recommendation=None, Remediable=None, RestrictAlertDismissal=None, RuleLastModifiedOn=None, Severity=None, SystemDefault=None, ComplianceMetadata=None, Remediation=None, Rule=None, Timeouts=None)
