...
TYPE_NAME = "TF::PrismaCloud::ComplianceStandardRequirementSection"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/prismacloud_compliance_standard_requirement_section/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'PaloAltoNetworks/prismacloud',
                'providerTypeName': 'prismacloud',
                'terraformTypeName': 'prismacloud_compliance_standard_requirement_section',
                'returnValues': ["AssociatedPolicyIds", "CreatedBy", "CreatedOn", "CsrsId", "Id", "LastModifiedBy", "LastModifiedOn", "PoliciesAssignedCount", "RequirementName", "StandardName", "SystemDefault"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/prismacloud_compliance_standard_requirement_section/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/prismacloud_compliance_standard_requirement_section/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AssociatedPolicyIds=None, CreatedBy=None, CreatedOn=None, CsrId=None, CsrsId=None, Description=None, Id=None, Label=None, LastModifiedBy=None, LastModifiedOn=None, PoliciesAssignedCount=None, RequirementName=None, SectionId=None, StandardName=None, SystemDefault=None, ViewOrder=None)
