...
TYPE_NAME = "TF::PrismaCloud::AlertRule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/prismacloud_alert_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'prismacloud_alert_rule',
                'returnValues': ["Id", "LastModifiedBy", "LastModifiedOn", "NotificationChannels", "OpenAlertsCount", "Owner", "PolicyScanConfigId", "ReadOnly"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/prismacloud_alert_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/prismacloud_alert_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowAutoRemediate=None, DelayNotificationMs=None, Description=None, Enabled=None, ExcludedPolicies=None, Id=None, LastModifiedBy=None, LastModifiedOn=None, Name=None, NotificationChannels=None, NotifyOnDismissed=None, NotifyOnOpen=None, NotifyOnResolved=None, NotifyOnSnoozed=None, OpenAlertsCount=None, Owner=None, Policies=None, PolicyLabels=None, PolicyScanConfigId=None, ReadOnly=None, ScanAll=None, NotificationConfig=None, Target=None)
