...
def create_handler(
    ...
    try:
        ...
        tf['resource']['pagerduty_user_notification_rule'][request.logicalResourceIdentifier] = {}

        if "pagerduty" == "aws":
            creds = session.credentials
            tf["provider"]["pagerduty"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("pagerduty"))['SecretString']
            tf["provider"]["pagerduty"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['pagerduty_user_notification_rule'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
