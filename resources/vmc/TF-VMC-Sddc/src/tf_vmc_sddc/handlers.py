...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vmc_sddc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'vmware/vmc',
                'providerTypeName': 'vmc',
                'terraformTypeName': 'vmc_sddc',
                'returnValues': ["AvailabilityZones", "CloudPassword", "CloudUsername", "ClusterId", "ClusterInfo", "Id", "NsxtReverseProxyUrl", "SddcSize", "SddcState", "VcUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vmc_sddc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vmc_sddc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZones=None, CloudPassword=None, CloudUsername=None, ClusterId=None, ClusterInfo=None, DelayAccountLink=None, DeploymentType=None, EdrsPolicyType=None, EnableEdrs=None, HostInstanceType=None, Id=None, IntranetMtuUplink=None, MaxHosts=None, MinHosts=None, NsxtReverseProxyUrl=None, NumHost=None, ProviderType=None, Region=None, SddcName=None, SddcSize=None, SddcState=None, SddcTemplateId=None, SddcType=None, Size=None, SkipCreatingVxlan=None, SsoDomain=None, StorageCapacity=None, VcUrl=None, VpcCidr=None, VxlanSubnet=None, AccountLinkSddcConfig=None, MicrosoftLicensingConfig=None, Timeouts=None)
