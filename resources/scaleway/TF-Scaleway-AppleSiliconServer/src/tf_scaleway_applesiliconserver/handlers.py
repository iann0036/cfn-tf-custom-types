...
TYPE_NAME = "TF::Scaleway::AppleSiliconServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_apple_silicon_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_apple_silicon_server',
                'returnValues': ["CreatedAt", "DeletableAt", "Id", "Ip", "OrganizationId", "UpdatedAt", "VncUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_apple_silicon_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/scaleway_apple_silicon_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreatedAt=None, DeletableAt=None, Id=None, Ip=None, Name=None, OrganizationId=None, ProjectId=None, Type=None, UpdatedAt=None, VncUrl=None, Zone=None, Timeouts=None)
