...
TYPE_NAME = "TF::Scaleway::InstanceServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_instance_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'scaleway/scaleway',
                'providerTypeName': 'scaleway',
                'terraformTypeName': 'scaleway_instance_server',
                'returnValues': ["Id", "Ipv6Address", "Ipv6Gateway", "Ipv6PrefixLength", "OrganizationId", "PlacementGroupPolicyRespected", "PrivateIp", "PublicIp"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_instance_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/scaleway_instance_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdditionalVolumeIds=None, BootType=None, BootscriptId=None, CloudInit=None, EnableDynamicIp=None, EnableIpv6=None, Id=None, Image=None, IpId=None, Ipv6Address=None, Ipv6Gateway=None, Ipv6PrefixLength=None, Name=None, OrganizationId=None, PlacementGroupId=None, PlacementGroupPolicyRespected=None, PrivateIp=None, ProjectId=None, PublicIp=None, SecurityGroupId=None, State=None, Tags=None, Type=None, UserData=None, Zone=None, RootVolume=None, Timeouts=None)
