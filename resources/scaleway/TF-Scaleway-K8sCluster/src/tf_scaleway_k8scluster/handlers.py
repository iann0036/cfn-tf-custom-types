...
TYPE_NAME = "TF::Scaleway::K8sCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_k8s_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_k8s_cluster',
                'returnValues': ["ApiserverUrl", "CreatedAt", "Id", "Kubeconfig", "OrganizationId", "Status", "UpdatedAt", "UpgradeAvailable", "WildcardDns"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_k8s_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/scaleway_k8s_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdmissionPlugins=None, ApiserverCertSans=None, ApiserverUrl=None, Cni=None, CreatedAt=None, DeleteAdditionalResources=None, Description=None, FeatureGates=None, Id=None, Kubeconfig=None, Name=None, OrganizationId=None, ProjectId=None, Region=None, Status=None, Tags=None, UpdatedAt=None, UpgradeAvailable=None, Version=None, WildcardDns=None, AutoUpgrade=None, AutoscalerConfig=None, OpenIdConnectConfig=None, Timeouts=None)
