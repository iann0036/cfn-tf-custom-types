...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_k8s_pool/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_k8s_pool',
                'returnValues': ["CreatedAt", "CurrentSize", "Id", "Nodes", "Status", "UpdatedAt", "Version"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_k8s_pool/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/scaleway_k8s_pool/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Autohealing=None, Autoscaling=None, ClusterId=None, ContainerRuntime=None, CreatedAt=None, CurrentSize=None, Id=None, KubeletArgs=None, MaxSize=None, MinSize=None, Name=None, NodeType=None, Nodes=None, PlacementGroupId=None, Region=None, Size=None, Status=None, Tags=None, UpdatedAt=None, Version=None, WaitForPoolReady=None, Zone=None, Timeouts=None, UpgradePolicy=None)
