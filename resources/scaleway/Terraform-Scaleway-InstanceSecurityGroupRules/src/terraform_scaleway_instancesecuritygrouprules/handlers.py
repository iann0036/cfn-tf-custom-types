...
def create_handler(
    ...
    try:
        ...
        tf['resource']['scaleway_instance_security_group_rules'][request.logicalResourceIdentifier] = {}

        if "scaleway" == "aws":
            creds = session.credentials
            tf["provider"]["scaleway"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("scaleway"))['SecretString']
            tf["provider"]["scaleway"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['scaleway_instance_security_group_rules'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
