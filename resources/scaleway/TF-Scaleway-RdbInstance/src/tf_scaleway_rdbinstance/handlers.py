...
TYPE_NAME = "TF::Scaleway::RdbInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_rdb_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'scaleway_rdb_instance',
                'returnValues': ["Certificate", "EndpointIp", "EndpointPort", "Id", "OrganizationId", "ReadReplicas"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/scaleway_rdb_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/scaleway_rdb_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Certificate=None, DisableBackup=None, EndpointIp=None, EndpointPort=None, Engine=None, Id=None, IsHaCluster=None, Name=None, NodeType=None, OrganizationId=None, Password=None, ProjectId=None, ReadReplicas=None, Region=None, Settings=None, Tags=None, UserName=None, VolumeSizeInGb=None, VolumeType=None, Timeouts=None)
