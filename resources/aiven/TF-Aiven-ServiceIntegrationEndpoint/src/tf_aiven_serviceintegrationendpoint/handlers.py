...
TYPE_NAME = "TF::Aiven::ServiceIntegrationEndpoint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aiven_service_integration_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'aiven/aiven',
                'providerTypeName': 'aiven',
                'terraformTypeName': 'aiven_service_integration_endpoint',
                'returnValues': ["EndpointConfig", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aiven_service_integration_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aiven_service_integration_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), EndpointConfig=None, EndpointName=None, EndpointType=None, Id=None, Project=None, DatadogUserConfig=None, ExternalAwsCloudwatchLogsUserConfig=None, ExternalAwsCloudwatchMetricsUserConfig=None, ExternalElasticsearchLogsUserConfig=None, ExternalGoogleCloudLoggingUserConfig=None, ExternalKafkaUserConfig=None, ExternalSchemaRegistryUserConfig=None, JolokiaUserConfig=None, PrometheusUserConfig=None, RsyslogUserConfig=None, SignalfxUserConfig=None)
