...
TYPE_NAME = "TF::Aiven::KafkaMirrormaker"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aiven_kafka_mirrormaker/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aiven_kafka_mirrormaker',
                'returnValues': ["Components", "Id", "ServiceHost", "ServicePassword", "ServicePort", "ServiceType", "ServiceUri", "ServiceUsername", "State"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aiven_kafka_mirrormaker/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aiven_kafka_mirrormaker/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CloudName=None, Components=None, Id=None, MaintenanceWindowDow=None, MaintenanceWindowTime=None, Plan=None, Project=None, ProjectVpcId=None, ServiceHost=None, ServiceName=None, ServicePassword=None, ServicePort=None, ServiceType=None, ServiceUri=None, ServiceUsername=None, State=None, TerminationProtection=None, KafkaMirrormaker=None, KafkaMirrormakerUserConfig=None, ServiceIntegrations=None, Timeouts=None)
