...
TYPE_NAME = "TF::Wavefront::CloudIntegrationCloudwatch"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/wavefront_cloud_integration_cloudwatch/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'wavefront_cloud_integration_cloudwatch',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/wavefront_cloud_integration_cloudwatch/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/wavefront_cloud_integration_cloudwatch/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdditionalTags=None, ExternalId=None, ForceSave=None, Id=None, InstanceSelectionTags=None, MetricFilterRegex=None, Name=None, Namespaces=None, PointTagFilterRegex=None, RoleArn=None, Service=None, ServiceRefreshRateInMinutes=None, VolumeSelectionTags=None)
