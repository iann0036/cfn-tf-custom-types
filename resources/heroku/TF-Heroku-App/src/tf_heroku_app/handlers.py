...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/heroku_app/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'heroku/heroku',
                'providerTypeName': 'heroku',
                'terraformTypeName': 'heroku_app',
                'returnValues': ["AllConfigVars", "GitUrl", "HerokuHostname", "Id", "Uuid", "WebUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/heroku_app/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/heroku_app/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Acm=None, AllConfigVars=None, Buildpacks=None, ConfigVars=None, GitUrl=None, HerokuHostname=None, Id=None, InternalRouting=None, Name=None, Region=None, SensitiveConfigVars=None, Space=None, Stack=None, Uuid=None, WebUrl=None, Organization=None)
