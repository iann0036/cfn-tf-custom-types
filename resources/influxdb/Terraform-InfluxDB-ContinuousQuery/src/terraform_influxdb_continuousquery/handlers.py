...
def create_handler(
    ...
    try:
        ...
        tf['resource']['influxdb_continuous_query'][request.logicalResourceIdentifier] = {}

        if "influxdb" == "aws":
            creds = session.credentials
            tf["provider"]["influxdb"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("influxdb"))['SecretString']
            tf["provider"]["influxdb"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['influxdb_continuous_query'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
