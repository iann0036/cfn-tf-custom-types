...
TYPE_NAME = "TF::Thunder::SlbTemplateDiameter"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_diameter/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_slb_template_diameter',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_diameter/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_slb_template_diameter/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvpCode=None, AvpString=None, CustomizeCea=None, DwrTime=None, DwrUpRetry=None, ForwardToLatestServer=None, ForwardUnknownSessionId=None, Id=None, IdleTimeout=None, LoadBalanceOnSessionId=None, MultipleOriginHost=None, Name=None, OriginRealm=None, ProductName=None, ServiceGroupName=None, SessionAge=None, TerminateOnCcaT=None, UserTag=None, Uuid=None, VendorId=None, AvpList=None, MessageCodeList=None, OriginHost=None)
