...
TYPE_NAME = "TF::Thunder::VirtualServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_virtual_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_virtual_server',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_virtual_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_virtual_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AclId=None, AclIdShared=None, AclName=None, AclNameShared=None, ArpDisable=None, Description=None, DisableVipAdv=None, EnableDisableAction=None, Ethernet=None, ExtendedStats=None, HaDynamic=None, Id=None, IpAddress=None, Ipv6Acl=None, Ipv6AclShared=None, Name=None, Netmask=None, RedistributeRouteMap=None, RedistributionFlagged=None, SharedPartitionPolicyTemplate=None, StatsDataAction=None, TemplateLogging=None, TemplatePolicy=None, TemplatePolicyAcl=None, TemplatePolicyAclShared=None, TemplatePolicyAddress=None, TemplatePolicyShared=None, TemplateScaleout=None, TemplateVirtualServer=None, UseIfIp=None, UserTag=None, Uuid=None, Vrid=None, MigrateVip=None, PortList=None)
