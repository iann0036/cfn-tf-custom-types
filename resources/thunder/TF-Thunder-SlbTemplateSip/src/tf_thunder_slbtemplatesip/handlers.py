...
TYPE_NAME = "TF::Thunder::SlbTemplateSip"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_sip/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_slb_template_sip',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_sip/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_slb_template_sip/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AclId=None, AclNameValue=None, AlgDestNat=None, AlgSourceNat=None, CallIdPersistDisable=None, ClientKeepAlive=None, DialogAware=None, DropWhenClientFail=None, DropWhenServerFail=None, FailedClientSelection=None, FailedClientSelectionMessage=None, FailedServerSelection=None, FailedServerSelectionMessage=None, Id=None, InsertClientIp=None, Interval=None, KeepServerIpIfMatchAcl=None, Name=None, PstnGw=None, ServerKeepAlive=None, ServerSelectionPerRequest=None, ServiceGroup=None, SmpCallIdRtpSession=None, Timeout=None, UserTag=None, Uuid=None, ClientRequestHeader=None, ClientResponseHeader=None, ExcludeTranslation=None, ServerRequestHeader=None, ServerResponseHeader=None)
