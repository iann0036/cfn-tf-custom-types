...
TYPE_NAME = "TF::Thunder::InterfaceVeIp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_interface_ve_ip/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_interface_ve_ip',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_interface_ve_ip/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_interface_ve_ip/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowPromiscuousVip=None, Client=None, Dhcp=None, GenerateMembershipQuery=None, Id=None, Ifnum=None, Inside=None, MaxRespTime=None, Outside=None, QueryInterval=None, Server=None, SlbPartitionRedirect=None, TtlIgnore=None, Uuid=None, AddressList=None, HelperAddressList=None, Ospf=None, Rip=None, Router=None, StatefulFirewall=None)
