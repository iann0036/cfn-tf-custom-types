...
TYPE_NAME = "TF::Thunder::SlbTemplateReqmodIcap"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_reqmod_icap/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_slb_template_reqmod_icap',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_reqmod_icap/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_slb_template_reqmod_icap/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, AllowedHttpMethods=None, Cylance=None, DisableHttpServerReset=None, FailClose=None, Id=None, IncludeProtocolInUri=None, LogOnlyAllowedMethod=None, Logging=None, MinPayloadSize=None, Name=None, Preview=None, ServerSsl=None, ServiceGroup=None, ServiceUrl=None, SharedPartitionPersistSourceIpTemplate=None, SharedPartitionTcpProxyTemplate=None, SourceIp=None, TcpProxy=None, TemplatePersistSourceIpShared=None, TemplateTcpProxyShared=None, UserTag=None, Uuid=None, BypassIpCfg=None)
