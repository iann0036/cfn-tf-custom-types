...
TYPE_NAME = "TF::Thunder::SnmpServerEnableTrapsSlb"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_snmp_server_enable_traps_slb/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_snmp_server_enable_traps_slb',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_snmp_server_enable_traps_slb/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_snmp_server_enable_traps_slb/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), All=None, ApplicationBufferLimit=None, BwRateLimitExceed=None, BwRateLimitResume=None, GatewayDown=None, GatewayUp=None, Id=None, ServerConnLimit=None, ServerConnResume=None, ServerDisabled=None, ServerDown=None, ServerSelectionFailure=None, ServerUp=None, ServiceConnLimit=None, ServiceConnResume=None, ServiceDown=None, ServiceGroupDown=None, ServiceGroupMemberDown=None, ServiceGroupMemberUp=None, ServiceGroupUp=None, ServiceUp=None, Uuid=None, VipConnlimit=None, VipConnratelimit=None, VipDown=None, VipPortConnlimit=None, VipPortConnratelimit=None, VipPortDown=None, VipPortUp=None, VipUp=None)
