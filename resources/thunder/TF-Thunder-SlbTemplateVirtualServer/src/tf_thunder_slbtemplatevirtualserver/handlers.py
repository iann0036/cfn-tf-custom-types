...
TYPE_NAME = "TF::Thunder::SlbTemplateVirtualServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_virtual_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_slb_template_virtual_server',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_virtual_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_slb_template_virtual_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ConnLimit=None, ConnLimitNoLogging=None, ConnLimitReset=None, ConnRateLimit=None, ConnRateLimitNoLogging=None, ConnRateLimitReset=None, IcmpLockup=None, IcmpLockupPeriod=None, IcmpRateLimit=None, Icmpv6Lockup=None, Icmpv6LockupPeriod=None, Icmpv6RateLimit=None, Id=None, Name=None, RateInterval=None, SubnetGratuitousArp=None, TcpStackTfoActiveConnLimit=None, TcpStackTfoBackoffTime=None, TcpStackTfoCookieTimeLimit=None, UserTag=None, Uuid=None)
