...
TYPE_NAME = "TF::Thunder::SlbTemplateServerSsl"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_server_ssl/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_slb_template_server_ssl',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_template_server_ssl/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_slb_template_server_ssl/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AlertType=None, Cert=None, CertSharedStr=None, CipherTemplate=None, CloseNotify=None, Dgversion=None, DhType=None, EnableTlsAlertLogging=None, Encrypted=None, ForwardProxyEnable=None, HandshakeLoggingEnable=None, Id=None, Key=None, KeySharedEncrypted=None, KeySharedPassphrase=None, KeySharedStr=None, Name=None, OcspStapling=None, Passphrase=None, RenegotiationDisable=None, SessionCacheSize=None, SessionCacheTimeout=None, SessionTicketEnable=None, SharedPartitionCipherTemplate=None, SsliLogging=None, Sslilogging=None, TemplateCipherShared=None, UseClientSni=None, UserTag=None, Uuid=None, Version=None, CaCerts=None, CipherWithoutPrioList=None, CrlCerts=None, EcList=None, ServerCertificateError=None)
