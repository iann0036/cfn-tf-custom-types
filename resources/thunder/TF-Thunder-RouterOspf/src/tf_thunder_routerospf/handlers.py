...
TYPE_NAME = "TF::Thunder::RouterOspf"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_router_ospf/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_router_ospf',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_router_ospf/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_router_ospf/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, AsNumber=None, AutoCostReferenceBandwidth=None, BfdAllInterfaces=None, DefaultMetric=None, Id=None, MaxConcurrentDd=None, MaximumArea=None, ProcessId=None, Rfc1583Compatible=None, Sequence=None, UserTag=None, Uuid=None, AreaList=None, DefaultInformation=None, Distance=None, DistributeInternalList=None, DistributeLists=None, HaStandbyExtraCost=None, HostList=None, LogAdjacencyChangesCfg=None, NeighborList=None, NetworkList=None, Ospf1=None, Overflow=None, PassiveInterface=None, Redistribute=None, RouterId=None, SummaryAddressList=None, Timers=None)
