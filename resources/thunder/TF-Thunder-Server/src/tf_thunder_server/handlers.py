...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_server',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, ConnLimit=None, ConnResume=None, ExtendedStats=None, ExternalIp=None, FqdnName=None, HealthCheck=None, HealthCheckDisable=None, HealthCheckShared=None, Host=None, Id=None, Ipv6=None, Name=None, NoLogging=None, ResolveAs=None, ServerIpv6Addr=None, SharedPartitionHealthCheck=None, SlowStart=None, SpoofingCache=None, StatsDataAction=None, TemplateServer=None, UserTag=None, Uuid=None, Weight=None, AlternateServer=None, PortList=None, SamplingEnable=None)
