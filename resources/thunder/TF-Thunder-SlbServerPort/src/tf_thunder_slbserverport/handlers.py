...
TYPE_NAME = "TF::Thunder::SlbServerPort"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_server_port/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'thunder_slb_server_port',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/thunder_slb_server_port/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/thunder_slb_server_port/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, ConnLimit=None, ConnResume=None, ExtendedStats=None, FollowPortProtocol=None, HealthCheck=None, HealthCheckDisable=None, HealthCheckFollowPort=None, Id=None, Name=None, NoLogging=None, NoSsl=None, PortNumber=None, Protocol=None, Range=None, RportHealthCheckShared=None, SharedPartitionPortTemplate=None, SharedRportHealthCheck=None, StatsDataAction=None, SupportHttp2=None, TemplatePort=None, TemplatePortShared=None, TemplateServerSsl=None, UserTag=None, Uuid=None, Weight=None, AlternatePort=None, AuthCfg=None, SamplingEnable=None)
