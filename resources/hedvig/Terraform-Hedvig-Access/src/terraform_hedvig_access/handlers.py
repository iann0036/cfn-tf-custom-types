...
def create_handler(
    ...
    try:
        ...
        tf['resource']['hedvig_access'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("hedvig"))['SecretString']
            tf["provider"]["hedvig"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['hedvig_access'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
