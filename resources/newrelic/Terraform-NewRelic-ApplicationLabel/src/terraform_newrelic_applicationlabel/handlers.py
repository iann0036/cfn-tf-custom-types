...
def create_handler(
    ...
    try:
        ...
        tf = {
            ...
            "resource": {
                "newrelic_application_label": {}
            }
        }
        tf['resource']['newrelic_application_label'][request.logicalResourceIdentifier] = {}

        if "newrelic" == "aws":
            creds = session.credentials
            tf["provider"]["newrelic"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("newrelic"))['SecretString']
            tf["provider"]["newrelic"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['newrelic_application_label'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
