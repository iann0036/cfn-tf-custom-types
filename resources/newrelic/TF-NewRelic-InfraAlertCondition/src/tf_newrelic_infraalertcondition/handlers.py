...
TYPE_NAME = "TF::NewRelic::InfraAlertCondition"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/newrelic_infra_alert_condition/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/newrelic_infra_alert_condition/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/newrelic_infra_alert_condition/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Comparison=None, CreatedAt=None, Description=None, Enabled=None, Event=None, Id=None, IntegrationProvider=None, Name=None, PolicyId=None, ProcessWhere=None, RunbookUrl=None, Select=None, Type=None, UpdatedAt=None, ViolationCloseTimer=None, Where=None, Critical=None, Warning=None)
