...
TYPE_NAME = "TF::RedisCloud::SubscriptionPeering"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/rediscloud_subscription_peering/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'RedisLabs/rediscloud',
                'providerTypeName': 'rediscloud',
                'terraformTypeName': 'rediscloud_subscription_peering',
                'returnValues': ["AwsPeeringId", "GcpPeeringId", "GcpRedisNetworkName", "GcpRedisProjectId", "Id", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/rediscloud_subscription_peering/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/rediscloud_subscription_peering/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AwsAccountId=None, AwsPeeringId=None, GcpNetworkName=None, GcpPeeringId=None, GcpProjectId=None, GcpRedisNetworkName=None, GcpRedisProjectId=None, Id=None, ProviderName=None, Region=None, Status=None, SubscriptionId=None, VpcCidr=None, VpcId=None, Timeouts=None)
