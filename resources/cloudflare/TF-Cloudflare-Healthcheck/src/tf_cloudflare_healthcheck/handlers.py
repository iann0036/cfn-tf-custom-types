...
TYPE_NAME = "TF::Cloudflare::Healthcheck"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudflare_healthcheck/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'cloudflare/cloudflare',
                'providerTypeName': 'cloudflare',
                'terraformTypeName': 'cloudflare_healthcheck',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudflare_healthcheck/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudflare_healthcheck/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, AllowInsecure=None, CheckRegions=None, ConsecutiveFails=None, ConsecutiveSuccesses=None, CreatedOn=None, Description=None, ExpectedBody=None, ExpectedCodes=None, FollowRedirects=None, Id=None, Interval=None, Method=None, ModifiedOn=None, Name=None, NotificationEmailAddresses=None, NotificationSuspended=None, Path=None, Port=None, Retries=None, Suspended=None, Timeout=None, Type=None, ZoneId=None, Header=None, Timeouts=None)
