...
def create_handler(
    ...
    try:
        ...
        tf['resource']['cloudflare_access_identity_provider'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("cloudflare"))['SecretString']
            tf["provider"]["cloudflare"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['cloudflare_access_identity_provider'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
