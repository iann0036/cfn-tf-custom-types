...
TYPE_NAME = "TF::Cloudsmith::Repository"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudsmith_repository/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'cloudsmith-io/cloudsmith',
                'providerTypeName': 'cloudsmith',
                'terraformTypeName': 'cloudsmith_repository',
                'returnValues': ["CdnUrl", "CreatedAt", "DeletedAt", "Id", "NamespaceUrl", "SelfHtmlUrl", "SelfUrl", "SlugPerm"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudsmith_repository/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudsmith_repository/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CdnUrl=None, CreatedAt=None, DeletedAt=None, Description=None, Id=None, IndexFiles=None, Name=None, Namespace=None, NamespaceUrl=None, RepositoryType=None, SelfHtmlUrl=None, SelfUrl=None, Slug=None, SlugPerm=None, StorageRegion=None, WaitForDeletion=None)
