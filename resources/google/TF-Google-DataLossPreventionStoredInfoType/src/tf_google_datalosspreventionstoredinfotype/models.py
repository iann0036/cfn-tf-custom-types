# DO NOT modify this file by hand, changes will be overwritten
import sys
from dataclasses import dataclass
from inspect import getmembers, isclass
from typing import (
    AbstractSet,
    Any,
    Generic,
    Mapping,
    MutableMapping,
    Optional,
    Sequence,
    Type,
    TypeVar,
)

from cloudformation_cli_python_lib.interface import (
    BaseModel,
    BaseResourceHandlerRequest,
)
from cloudformation_cli_python_lib.recast import recast_object
from cloudformation_cli_python_lib.utils import deserialize_list

T = TypeVar("T")


def set_or_none(value: Optional[Sequence[T]]) -> Optional[AbstractSet[T]]:
    if value:
        return set(value)
    return None


@dataclass
class ResourceHandlerRequest(BaseResourceHandlerRequest):
    # pylint: disable=invalid-name
    desiredResourceState: Optional["ResourceModel"]
    previousResourceState: Optional["ResourceModel"]


@dataclass
class ResourceModel(BaseModel):
    tfcfnid: Optional[str]
    Description: Optional[str]
    DisplayName: Optional[str]
    Id: Optional[str]
    Name: Optional[str]
    Parent: Optional[str]
    Dictionary: Optional[Sequence["_DictionaryDefinition"]]
    LargeCustomDictionary: Optional[Sequence["_LargeCustomDictionaryDefinition"]]
    Regex: Optional[Sequence["_RegexDefinition"]]
    Timeouts: Optional["_TimeoutsDefinition"]

    @classmethod
    def _deserialize(
        cls: Type["_ResourceModel"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_ResourceModel"]:
        if not json_data:
            return None
        dataclasses = {n: o for n, o in getmembers(sys.modules[__name__]) if isclass(o)}
        recast_object(cls, json_data, dataclasses)
        return cls(
            tfcfnid=json_data.get("tfcfnid"),
            Description=json_data.get("Description"),
            DisplayName=json_data.get("DisplayName"),
            Id=json_data.get("Id"),
            Name=json_data.get("Name"),
            Parent=json_data.get("Parent"),
            Dictionary=deserialize_list(json_data.get("Dictionary"), DictionaryDefinition),
            LargeCustomDictionary=deserialize_list(json_data.get("LargeCustomDictionary"), LargeCustomDictionaryDefinition),
            Regex=deserialize_list(json_data.get("Regex"), RegexDefinition),
            Timeouts=TimeoutsDefinition._deserialize(json_data.get("Timeouts")),
        )


# work around possible type aliasing issues when variable has same name as a model
_ResourceModel = ResourceModel


@dataclass
class DictionaryDefinition(BaseModel):
    CloudStoragePath: Optional[Sequence["_CloudStoragePathDefinition"]]
    WordList: Optional[Sequence["_WordListDefinition"]]

    @classmethod
    def _deserialize(
        cls: Type["_DictionaryDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_DictionaryDefinition"]:
        if not json_data:
            return None
        return cls(
            CloudStoragePath=deserialize_list(json_data.get("CloudStoragePath"), CloudStoragePathDefinition),
            WordList=deserialize_list(json_data.get("WordList"), WordListDefinition),
        )


# work around possible type aliasing issues when variable has same name as a model
_DictionaryDefinition = DictionaryDefinition


@dataclass
class CloudStoragePathDefinition(BaseModel):
    Path: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_CloudStoragePathDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_CloudStoragePathDefinition"]:
        if not json_data:
            return None
        return cls(
            Path=json_data.get("Path"),
        )


# work around possible type aliasing issues when variable has same name as a model
_CloudStoragePathDefinition = CloudStoragePathDefinition


@dataclass
class WordListDefinition(BaseModel):
    Words: Optional[Sequence[str]]

    @classmethod
    def _deserialize(
        cls: Type["_WordListDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_WordListDefinition"]:
        if not json_data:
            return None
        return cls(
            Words=json_data.get("Words"),
        )


# work around possible type aliasing issues when variable has same name as a model
_WordListDefinition = WordListDefinition


@dataclass
class LargeCustomDictionaryDefinition(BaseModel):
    BigQueryField: Optional[Sequence["_BigQueryFieldDefinition"]]
    CloudStorageFileSet: Optional[Sequence["_CloudStorageFileSetDefinition"]]
    OutputPath: Optional[Sequence["_OutputPathDefinition"]]

    @classmethod
    def _deserialize(
        cls: Type["_LargeCustomDictionaryDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_LargeCustomDictionaryDefinition"]:
        if not json_data:
            return None
        return cls(
            BigQueryField=deserialize_list(json_data.get("BigQueryField"), BigQueryFieldDefinition),
            CloudStorageFileSet=deserialize_list(json_data.get("CloudStorageFileSet"), CloudStorageFileSetDefinition),
            OutputPath=deserialize_list(json_data.get("OutputPath"), OutputPathDefinition),
        )


# work around possible type aliasing issues when variable has same name as a model
_LargeCustomDictionaryDefinition = LargeCustomDictionaryDefinition


@dataclass
class BigQueryFieldDefinition(BaseModel):
    Field: Optional[Sequence["_FieldDefinition"]]
    Table: Optional[Sequence["_TableDefinition"]]

    @classmethod
    def _deserialize(
        cls: Type["_BigQueryFieldDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_BigQueryFieldDefinition"]:
        if not json_data:
            return None
        return cls(
            Field=deserialize_list(json_data.get("Field"), FieldDefinition),
            Table=deserialize_list(json_data.get("Table"), TableDefinition),
        )


# work around possible type aliasing issues when variable has same name as a model
_BigQueryFieldDefinition = BigQueryFieldDefinition


@dataclass
class FieldDefinition(BaseModel):
    Name: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_FieldDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_FieldDefinition"]:
        if not json_data:
            return None
        return cls(
            Name=json_data.get("Name"),
        )


# work around possible type aliasing issues when variable has same name as a model
_FieldDefinition = FieldDefinition


@dataclass
class TableDefinition(BaseModel):
    DatasetId: Optional[str]
    ProjectId: Optional[str]
    TableId: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_TableDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_TableDefinition"]:
        if not json_data:
            return None
        return cls(
            DatasetId=json_data.get("DatasetId"),
            ProjectId=json_data.get("ProjectId"),
            TableId=json_data.get("TableId"),
        )


# work around possible type aliasing issues when variable has same name as a model
_TableDefinition = TableDefinition


@dataclass
class CloudStorageFileSetDefinition(BaseModel):
    Url: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_CloudStorageFileSetDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_CloudStorageFileSetDefinition"]:
        if not json_data:
            return None
        return cls(
            Url=json_data.get("Url"),
        )


# work around possible type aliasing issues when variable has same name as a model
_CloudStorageFileSetDefinition = CloudStorageFileSetDefinition


@dataclass
class OutputPathDefinition(BaseModel):
    Path: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_OutputPathDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_OutputPathDefinition"]:
        if not json_data:
            return None
        return cls(
            Path=json_data.get("Path"),
        )


# work around possible type aliasing issues when variable has same name as a model
_OutputPathDefinition = OutputPathDefinition


@dataclass
class RegexDefinition(BaseModel):
    GroupIndexes: Optional[Sequence[float]]
    Pattern: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_RegexDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_RegexDefinition"]:
        if not json_data:
            return None
        return cls(
            GroupIndexes=json_data.get("GroupIndexes"),
            Pattern=json_data.get("Pattern"),
        )


# work around possible type aliasing issues when variable has same name as a model
_RegexDefinition = RegexDefinition


@dataclass
class TimeoutsDefinition(BaseModel):
    Create: Optional[str]
    Delete: Optional[str]
    Update: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_TimeoutsDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_TimeoutsDefinition"]:
        if not json_data:
            return None
        return cls(
            Create=json_data.get("Create"),
            Delete=json_data.get("Delete"),
            Update=json_data.get("Update"),
        )


# work around possible type aliasing issues when variable has same name as a model
_TimeoutsDefinition = TimeoutsDefinition


