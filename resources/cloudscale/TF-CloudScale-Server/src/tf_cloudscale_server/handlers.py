...
TYPE_NAME = "TF::CloudScale::Server"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudscale_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'cloudscale-ch/cloudscale',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/cloudscale_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/cloudscale_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowStoppingForUpdate=None, BulkVolumeSizeGb=None, FlavorSlug=None, Href=None, Id=None, ImageSlug=None, Name=None, Password=None, PrivateIpv4Address=None, PublicIpv4Address=None, PublicIpv6Address=None, ServerGroupIds=None, ServerGroups=None, SshFingerprints=None, SshHostKeys=None, SshKeys=None, Status=None, UseIpv6=None, UsePrivateNetwork=None, UsePublicNetwork=None, UserData=None, VolumeSizeGb=None, Volumes=None, ZoneSlug=None, Interfaces=None)
