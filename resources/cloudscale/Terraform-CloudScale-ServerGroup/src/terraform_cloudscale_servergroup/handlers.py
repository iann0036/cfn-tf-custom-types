...
def create_handler(
    ...
    try:
        ...
        tf['resource']['cloudscale_server_group'][request.logicalResourceIdentifier] = {}

        if "cloudscale" == "aws":
            creds = session.credentials
            tf["provider"]["cloudscale"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("cloudscale"))['SecretString']
            tf["provider"]["cloudscale"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['cloudscale_server_group'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
