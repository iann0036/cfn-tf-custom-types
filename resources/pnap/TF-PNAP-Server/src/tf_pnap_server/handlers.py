...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/pnap_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'phoenixnap/pnap',
                'providerTypeName': 'pnap',
                'terraformTypeName': 'pnap_server',
                'returnValues': ["Cpu", "Id", "PrivateIpAddresses", "PublicIpAddresses", "Ram", "Status", "Storage"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/pnap_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/pnap_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, Cpu=None, Description=None, Hostname=None, Id=None, Location=None, Os=None, PrivateIpAddresses=None, PublicIpAddresses=None, Ram=None, SshKeys=None, Status=None, Storage=None, Type=None, Timeouts=None)
