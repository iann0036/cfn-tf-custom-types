...
TYPE_NAME = "TF::VCD::NetworkDirect"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_network_direct/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ExternalNetworkDns1", "ExternalNetworkDns2", "ExternalNetworkDnsSuffix", "ExternalNetworkGateway", "ExternalNetworkNetmask", "Href", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_network_direct/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vcd_network_direct/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Description=None, ExternalNetwork=None, ExternalNetworkDns1=None, ExternalNetworkDns2=None, ExternalNetworkDnsSuffix=None, ExternalNetworkGateway=None, ExternalNetworkNetmask=None, Href=None, Id=None, Name=None, Org=None, Shared=None, Vdc=None)
