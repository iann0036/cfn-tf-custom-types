...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_org_vdc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_org_vdc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vcd_org_vdc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllocationModel=None, AllowOverCommit=None, CpuGuaranteed=None, CpuSpeed=None, DefaultVmSizingPolicyId=None, DeleteForce=None, DeleteRecursive=None, Description=None, Elasticity=None, EnableFastProvisioning=None, EnableThinProvisioning=None, EnableVmDiscovery=None, Enabled=None, Id=None, IncludeVmMemoryOverhead=None, MemoryGuaranteed=None, Metadata=None, Name=None, NetworkPoolName=None, NetworkQuota=None, NicQuota=None, Org=None, ProviderVdcName=None, VmQuota=None, VmSizingPolicyIds=None, ComputeCapacity=None, StorageProfile=None)
