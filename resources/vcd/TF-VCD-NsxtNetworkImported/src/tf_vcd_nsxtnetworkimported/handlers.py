...
TYPE_NAME = "TF::VCD::NsxtNetworkImported"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_nsxt_network_imported/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vcd_nsxt_network_imported',
                'returnValues': ["Id", "NsxtLogicalSwitchId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_nsxt_network_imported/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vcd_nsxt_network_imported/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Description=None, Dns1=None, Dns2=None, DnsSuffix=None, Gateway=None, Id=None, Name=None, NsxtLogicalSwitchId=None, NsxtLogicalSwitchName=None, Org=None, PrefixLength=None, Vdc=None, StaticIpPool=None)
