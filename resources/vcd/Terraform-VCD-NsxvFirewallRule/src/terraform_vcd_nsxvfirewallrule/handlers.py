...
def create_handler(
    ...
    try:
        ...
        tf['resource']['vcd_nsxv_firewall_rule'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("vcd"))['SecretString']
            tf["provider"]["vcd"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['vcd_nsxv_firewall_rule'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
