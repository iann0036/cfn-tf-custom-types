...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_vapp_vm/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'vmware/vcd',
                'providerTypeName': 'vcd',
                'terraformTypeName': 'vcd_vapp_vm',
                'returnValues': ["Id", "InternalDisk", "VmType"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vcd_vapp_vm/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vcd_vapp_vm/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcceptAllEulas=None, BootImage=None, CatalogName=None, ComputerName=None, CpuCores=None, CpuHotAddEnabled=None, Cpus=None, Description=None, ExposeHardwareVirtualization=None, GuestProperties=None, HardwareVersion=None, Href=None, Id=None, InternalDisk=None, Memory=None, MemoryHotAddEnabled=None, Metadata=None, Name=None, NetworkDhcpWaitSeconds=None, Org=None, OsType=None, PowerOn=None, PreventUpdatePowerOff=None, SizingPolicyId=None, StorageProfile=None, TemplateName=None, VappName=None, Vdc=None, VmNameInTemplate=None, VmType=None, Customization=None, Disk=None, Network=None, OverrideTemplateDisk=None)
