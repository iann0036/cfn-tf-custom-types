...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeRegionDisk"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_region_disk/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_region_disk/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_region_disk/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreationTimestamp=None, Description=None, Id=None, Interface=None, LabelFingerprint=None, Labels=None, LastAttachTimestamp=None, LastDetachTimestamp=None, Name=None, PhysicalBlockSizeBytes=None, Project=None, Region=None, ReplicaZones=None, SelfLink=None, Size=None, Snapshot=None, SourceSnapshotId=None, Type=None, Users=None, DiskEncryptionKey=None, SourceSnapshotEncryptionKey=None, Timeouts=None)
