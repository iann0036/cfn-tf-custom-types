...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeInstanceFromTemplate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_instance_from_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_instance_from_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_instance_from_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowStoppingForUpdate=None, AttachedDisk=None, CanIpForward=None, CpuPlatform=None, CurrentStatus=None, DeletionProtection=None, Description=None, DesiredStatus=None, EnableDisplay=None, GuestAccelerator=None, Hostname=None, Id=None, InstanceId=None, LabelFingerprint=None, Labels=None, MachineType=None, Metadata=None, MetadataFingerprint=None, MetadataStartupScript=None, MinCpuPlatform=None, Name=None, Project=None, ResourcePolicies=None, ScratchDisk=None, SelfLink=None, ServiceAccount=None, SourceInstanceTemplate=None, Tags=None, TagsFingerprint=None, Zone=None, BootDisk=None, ConfidentialInstanceConfig=None, NetworkInterface=None, ReservationAffinity=None, Scheduling=None, ShieldedInstanceConfig=None, Timeouts=None)
