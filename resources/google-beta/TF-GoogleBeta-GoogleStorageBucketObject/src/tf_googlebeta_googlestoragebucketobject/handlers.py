...
TYPE_NAME = "TF::GoogleBeta::GoogleStorageBucketObject"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_storage_bucket_object/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Crc32c", "Id", "Md5hash", "MediaLink", "OutputName", "SelfLink"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_storage_bucket_object/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_storage_bucket_object/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Bucket=None, CacheControl=None, Content=None, ContentDisposition=None, ContentEncoding=None, ContentLanguage=None, ContentType=None, Crc32c=None, DetectMd5hash=None, Id=None, KmsKeyName=None, Md5hash=None, MediaLink=None, Metadata=None, Name=None, OutputName=None, SelfLink=None, Source=None, StorageClass=None)
