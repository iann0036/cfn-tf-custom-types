...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeRegionHealthCheck"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_region_health_check/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_region_health_check/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_region_health_check/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CheckIntervalSec=None, CreationTimestamp=None, Description=None, HealthyThreshold=None, Id=None, Name=None, Project=None, Region=None, SelfLink=None, TimeoutSec=None, Type=None, UnhealthyThreshold=None, GrpcHealthCheck=None, Http2HealthCheck=None, HttpHealthCheck=None, HttpsHealthCheck=None, LogConfig=None, SslHealthCheck=None, TcpHealthCheck=None, Timeouts=None)
