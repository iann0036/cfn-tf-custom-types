...
TYPE_NAME = "TF::GoogleBeta::GoogleRedisInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_redis_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AuthString", "CreateTime", "CurrentLocationId", "Host", "Id", "PersistenceIamIdentity", "Port", "ServerCaCerts"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_redis_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_redis_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AlternativeLocationId=None, AuthEnabled=None, AuthString=None, AuthorizedNetwork=None, ConnectMode=None, CreateTime=None, CurrentLocationId=None, DisplayName=None, Host=None, Id=None, Labels=None, LocationId=None, MemorySizeGb=None, Name=None, PersistenceIamIdentity=None, Port=None, Project=None, RedisConfigs=None, RedisVersion=None, Region=None, ReservedIpRange=None, ServerCaCerts=None, Tier=None, TransitEncryptionMode=None, Timeouts=None)
