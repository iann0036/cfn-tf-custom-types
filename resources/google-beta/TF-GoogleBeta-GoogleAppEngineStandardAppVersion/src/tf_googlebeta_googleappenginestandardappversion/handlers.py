...
TYPE_NAME = "TF::GoogleBeta::GoogleAppEngineStandardAppVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_app_engine_standard_app_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_app_engine_standard_app_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_app_engine_standard_app_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DeleteServiceOnDestroy=None, EnvVariables=None, Id=None, InboundServices=None, InstanceClass=None, Name=None, NoopOnDestroy=None, Project=None, Runtime=None, RuntimeApiVersion=None, Service=None, Threadsafe=None, VersionId=None, AutomaticScaling=None, BasicScaling=None, Deployment=None, Entrypoint=None, Handlers=None, Libraries=None, ManualScaling=None, Timeouts=None, VpcAccessConnector=None)
