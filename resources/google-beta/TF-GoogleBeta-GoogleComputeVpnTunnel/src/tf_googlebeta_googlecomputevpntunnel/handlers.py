...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeVpnTunnel"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_vpn_tunnel/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CreationTimestamp", "DetailedStatus", "Id", "LabelFingerprint", "SelfLink", "SharedSecretHash", "TunnelId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_vpn_tunnel/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_vpn_tunnel/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreationTimestamp=None, Description=None, DetailedStatus=None, Id=None, IkeVersion=None, LabelFingerprint=None, Labels=None, LocalTrafficSelector=None, Name=None, PeerExternalGateway=None, PeerExternalGatewayInterface=None, PeerGcpGateway=None, PeerIp=None, Project=None, Region=None, RemoteTrafficSelector=None, Router=None, SelfLink=None, SharedSecret=None, SharedSecretHash=None, TargetVpnGateway=None, TunnelId=None, VpnGateway=None, VpnGatewayInterface=None, Timeouts=None)
