...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeInterconnectAttachment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_interconnect_attachment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_interconnect_attachment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_interconnect_attachment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdminEnabled=None, Bandwidth=None, CandidateSubnets=None, CloudRouterIpAddress=None, CreationTimestamp=None, CustomerRouterIpAddress=None, Description=None, EdgeAvailabilityDomain=None, Encryption=None, GoogleReferenceId=None, Id=None, Interconnect=None, IpsecInternalAddresses=None, Mtu=None, Name=None, PairingKey=None, PartnerAsn=None, PrivateInterconnectInfo=None, Project=None, Region=None, Router=None, SelfLink=None, State=None, Type=None, VlanTag8021q=None, Timeouts=None)
