...
TYPE_NAME = "TF::GoogleBeta::GooglePrivatecaCertificateAuthority"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_privateca_certificate_authority/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_privateca_certificate_authority',
                'returnValues': ["AccessUrls", "CreateTime", "Id", "Name", "PemCaCertificates", "State", "UpdateTime"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_privateca_certificate_authority/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_privateca_certificate_authority/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessUrls=None, CertificateAuthorityId=None, CreateTime=None, DisableOnDelete=None, GcsBucket=None, Id=None, Labels=None, Lifetime=None, Location=None, Name=None, PemCaCertificates=None, Project=None, State=None, Tier=None, Type=None, UpdateTime=None, Config=None, IssuingOptions=None, KeySpec=None, Timeouts=None)
