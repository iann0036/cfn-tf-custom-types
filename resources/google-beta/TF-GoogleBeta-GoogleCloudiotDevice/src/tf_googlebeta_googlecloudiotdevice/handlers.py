...
TYPE_NAME = "TF::GoogleBeta::GoogleCloudiotDevice"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_cloudiot_device/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_cloudiot_device',
                'returnValues': ["Config", "Id", "LastConfigAckTime", "LastConfigSendTime", "LastErrorStatus", "LastErrorTime", "LastEventTime", "LastHeartbeatTime", "LastStateTime", "NumId", "State"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_cloudiot_device/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_cloudiot_device/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Blocked=None, Config=None, Id=None, LastConfigAckTime=None, LastConfigSendTime=None, LastErrorStatus=None, LastErrorTime=None, LastEventTime=None, LastHeartbeatTime=None, LastStateTime=None, LogLevel=None, Metadata=None, Name=None, NumId=None, Registry=None, State=None, Credentials=None, GatewayConfig=None, Timeouts=None)
