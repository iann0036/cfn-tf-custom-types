...
TYPE_NAME = "TF::GoogleBeta::GoogleSqlDatabaseInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_sql_database_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_sql_database_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_sql_database_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ConnectionName=None, DatabaseVersion=None, DeletionProtection=None, EncryptionKeyName=None, FirstIpAddress=None, Id=None, IpAddress=None, MasterInstanceName=None, Name=None, PrivateIpAddress=None, Project=None, PublicIpAddress=None, Region=None, RootPassword=None, SelfLink=None, ServerCaCert=None, ServiceAccountEmailAddress=None, Clone=None, ReplicaConfiguration=None, RestoreBackupContext=None, Settings=None, Timeouts=None)
