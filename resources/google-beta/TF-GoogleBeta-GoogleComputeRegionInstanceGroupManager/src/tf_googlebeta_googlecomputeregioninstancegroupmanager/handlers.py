...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeRegionInstanceGroupManager"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_region_instance_group_manager/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Fingerprint", "Id", "InstanceGroup", "SelfLink", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_region_instance_group_manager/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_region_instance_group_manager/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BaseInstanceName=None, Description=None, DistributionPolicyTargetShape=None, DistributionPolicyZones=None, Fingerprint=None, Id=None, InstanceGroup=None, Name=None, Project=None, Region=None, SelfLink=None, Status=None, TargetPools=None, TargetSize=None, WaitForInstances=None, WaitForInstancesStatus=None, AutoHealingPolicies=None, NamedPort=None, StatefulDisk=None, Timeouts=None, UpdatePolicy=None, Version=None)
