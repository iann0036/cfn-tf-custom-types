...
TYPE_NAME = "TF::GoogleBeta::GoogleBigqueryDataTransferConfig"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_bigquery_data_transfer_config/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_bigquery_data_transfer_config/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_bigquery_data_transfer_config/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DataRefreshWindowDays=None, DataSourceId=None, DestinationDatasetId=None, Disabled=None, DisplayName=None, Id=None, Location=None, Name=None, NotificationPubsubTopic=None, Params=None, Project=None, Schedule=None, ServiceAccountName=None, EmailPreferences=None, ScheduleOptions=None, SensitiveParams=None, Timeouts=None)
