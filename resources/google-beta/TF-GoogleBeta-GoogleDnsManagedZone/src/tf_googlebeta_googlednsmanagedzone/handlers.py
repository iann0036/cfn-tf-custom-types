...
TYPE_NAME = "TF::GoogleBeta::GoogleDnsManagedZone"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_dns_managed_zone/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_dns_managed_zone/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_dns_managed_zone/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Description=None, DnsName=None, ForceDestroy=None, Id=None, Labels=None, Name=None, NameServers=None, Project=None, ReverseLookup=None, Visibility=None, DnssecConfig=None, ForwardingConfig=None, PeeringConfig=None, PrivateVisibilityConfig=None, ServiceDirectoryConfig=None, Timeouts=None)
