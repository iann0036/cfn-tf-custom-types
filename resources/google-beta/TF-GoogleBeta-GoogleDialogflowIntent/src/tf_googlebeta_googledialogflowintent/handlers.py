...
TYPE_NAME = "TF::GoogleBeta::GoogleDialogflowIntent"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_dialogflow_intent/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_dialogflow_intent',
                'returnValues': ["FollowupIntentInfo", "Id", "Name", "RootFollowupIntentName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_dialogflow_intent/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_dialogflow_intent/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Action=None, DefaultResponsePlatforms=None, DisplayName=None, Events=None, FollowupIntentInfo=None, Id=None, InputContextNames=None, IsFallback=None, MlDisabled=None, Name=None, ParentFollowupIntentName=None, Priority=None, Project=None, ResetContexts=None, RootFollowupIntentName=None, WebhookState=None, Timeouts=None)
