...
TYPE_NAME = "TF::GoogleBeta::GoogleNotebooksInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_notebooks_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_notebooks_instance',
                'returnValues': ["Id", "ProxyUri", "State"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_notebooks_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_notebooks_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BootDiskSizeGb=None, BootDiskType=None, CreateTime=None, CustomGpuDriverPath=None, DataDiskSizeGb=None, DataDiskType=None, DiskEncryption=None, Id=None, InstallGpuDriver=None, InstanceOwners=None, KmsKey=None, Labels=None, Location=None, MachineType=None, Metadata=None, Name=None, Network=None, NoProxyAccess=None, NoPublicIp=None, NoRemoveDataDisk=None, PostStartupScript=None, Project=None, ProxyUri=None, ServiceAccount=None, ServiceAccountScopes=None, State=None, Subnet=None, Tags=None, UpdateTime=None, AcceleratorConfig=None, ContainerImage=None, ShieldedInstanceConfig=None, Timeouts=None, VmImage=None)
