...
TYPE_NAME = "TF::GoogleBeta::GoogleComputeForwardingRule"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_forwarding_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["CreationTimestamp", "Id", "LabelFingerprint", "SelfLink", "ServiceName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_compute_forwarding_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_compute_forwarding_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllPorts=None, AllowGlobalAccess=None, BackendService=None, CreationTimestamp=None, Description=None, Id=None, IpAddress=None, IpProtocol=None, IsMirroringCollector=None, LabelFingerprint=None, Labels=None, LoadBalancingScheme=None, Name=None, Network=None, NetworkTier=None, PortRange=None, Ports=None, Project=None, Region=None, SelfLink=None, ServiceLabel=None, ServiceName=None, Subnetwork=None, Target=None, Timeouts=None)
