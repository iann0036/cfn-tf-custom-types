...
TYPE_NAME = "TF::GoogleBeta::GoogleAppEngineFlexibleAppVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_app_engine_flexible_app_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'google_app_engine_flexible_app_version',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/google_app_engine_flexible_app_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/google_app_engine_flexible_app_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BetaSettings=None, DefaultExpiration=None, DeleteServiceOnDestroy=None, EnvVariables=None, Id=None, InboundServices=None, InstanceClass=None, Name=None, NobuildFilesRegex=None, NoopOnDestroy=None, Project=None, Runtime=None, RuntimeApiVersion=None, RuntimeChannel=None, RuntimeMainExecutablePath=None, Service=None, ServingStatus=None, VersionId=None, ApiConfig=None, AutomaticScaling=None, Deployment=None, EndpointsApiService=None, Entrypoint=None, Handlers=None, LivenessCheck=None, ManualScaling=None, Network=None, ReadinessCheck=None, Resources=None, Timeouts=None, VpcAccessConnector=None)
