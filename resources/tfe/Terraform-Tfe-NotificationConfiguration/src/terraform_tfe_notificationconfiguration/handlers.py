...
def create_handler(
    ...
    try:
        ...
        tf['resource']['tfe_notification_configuration'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("tfe"))['SecretString']
            tf["provider"]["tfe"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['tfe_notification_configuration'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
