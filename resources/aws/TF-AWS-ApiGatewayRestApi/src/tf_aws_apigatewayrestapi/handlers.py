...
TYPE_NAME = "TF::AWS::ApiGatewayRestApi"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_api_gateway_rest_api/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_api_gateway_rest_api/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_api_gateway_rest_api/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiKeySource=None, Arn=None, BinaryMediaTypes=None, Body=None, CreatedDate=None, Description=None, DisableExecuteApiEndpoint=None, ExecutionArn=None, Id=None, MinimumCompressionSize=None, Name=None, Parameters=None, Policy=None, RootResourceId=None, Tags=None, TagsAll=None, EndpointConfiguration=None)
