...
TYPE_NAME = "TF::AWS::EcsTaskDefinition"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ecs_task_definition/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ecs_task_definition/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_ecs_task_definition/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, ContainerDefinitions=None, Cpu=None, ExecutionRoleArn=None, Family=None, Id=None, IpcMode=None, Memory=None, NetworkMode=None, PidMode=None, RequiresCompatibilities=None, Revision=None, Tags=None, TagsAll=None, TaskRoleArn=None, EphemeralStorage=None, InferenceAccelerator=None, PlacementConstraints=None, ProxyConfiguration=None, Volume=None)
