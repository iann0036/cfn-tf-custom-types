...
TYPE_NAME = "TF::AWS::EfsMountTarget"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_efs_mount_target/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AvailabilityZoneId", "AvailabilityZoneName", "DnsName", "FileSystemArn", "Id", "MountTargetDnsName", "NetworkInterfaceId", "OwnerId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_efs_mount_target/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_efs_mount_target/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZoneId=None, AvailabilityZoneName=None, DnsName=None, FileSystemArn=None, FileSystemId=None, Id=None, IpAddress=None, MountTargetDnsName=None, NetworkInterfaceId=None, OwnerId=None, SecurityGroups=None, SubnetId=None)
