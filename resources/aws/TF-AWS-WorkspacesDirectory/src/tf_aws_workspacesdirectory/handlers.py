...
TYPE_NAME = "TF::AWS::WorkspacesDirectory"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_workspaces_directory/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Alias", "CustomerUserName", "DirectoryName", "DirectoryType", "DnsIpAddresses", "IamRoleId", "Id", "RegistrationCode", "WorkspaceSecurityGroupId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_workspaces_directory/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_workspaces_directory/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Alias=None, CustomerUserName=None, DirectoryId=None, DirectoryName=None, DirectoryType=None, DnsIpAddresses=None, IamRoleId=None, Id=None, IpGroupIds=None, RegistrationCode=None, SubnetIds=None, Tags=None, TagsAll=None, WorkspaceSecurityGroupId=None, SelfServicePermissions=None, WorkspaceAccessProperties=None, WorkspaceCreationProperties=None)
