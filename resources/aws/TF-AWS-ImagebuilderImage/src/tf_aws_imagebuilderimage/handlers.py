...
TYPE_NAME = "TF::AWS::ImagebuilderImage"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_imagebuilder_image/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_imagebuilder_image',
                'returnValues': ["Arn", "DateCreated", "Id", "Name", "OsVersion", "OutputResources", "Platform", "Version"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_imagebuilder_image/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_imagebuilder_image/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, DateCreated=None, DistributionConfigurationArn=None, EnhancedImageMetadataEnabled=None, Id=None, ImageRecipeArn=None, InfrastructureConfigurationArn=None, Name=None, OsVersion=None, OutputResources=None, Platform=None, Tags=None, TagsAll=None, Version=None, ImageTestsConfiguration=None, Timeouts=None)
