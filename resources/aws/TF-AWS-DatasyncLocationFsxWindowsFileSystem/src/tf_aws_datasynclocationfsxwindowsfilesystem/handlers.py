...
TYPE_NAME = "TF::AWS::DatasyncLocationFsxWindowsFileSystem"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_datasync_location_fsx_windows_file_system/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_datasync_location_fsx_windows_file_system',
                'returnValues': ["Arn", "CreationTime", "Id", "Uri"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_datasync_location_fsx_windows_file_system/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_datasync_location_fsx_windows_file_system/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, CreationTime=None, Domain=None, FsxFilesystemArn=None, Id=None, Password=None, SecurityGroupArns=None, Subdirectory=None, Tags=None, TagsAll=None, Uri=None, User=None)
