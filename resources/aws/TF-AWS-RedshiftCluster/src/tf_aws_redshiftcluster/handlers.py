...
TYPE_NAME = "TF::AWS::RedshiftCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_redshift_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_redshift_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_redshift_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowVersionUpgrade=None, Arn=None, AutomatedSnapshotRetentionPeriod=None, AvailabilityZone=None, ClusterIdentifier=None, ClusterParameterGroupName=None, ClusterPublicKey=None, ClusterRevisionNumber=None, ClusterSecurityGroups=None, ClusterSubnetGroupName=None, ClusterType=None, ClusterVersion=None, DatabaseName=None, DnsName=None, ElasticIp=None, Encrypted=None, Endpoint=None, EnhancedVpcRouting=None, FinalSnapshotIdentifier=None, IamRoles=None, Id=None, KmsKeyId=None, MasterPassword=None, MasterUsername=None, NodeType=None, NumberOfNodes=None, OwnerAccount=None, Port=None, PreferredMaintenanceWindow=None, PubliclyAccessible=None, SkipFinalSnapshot=None, SnapshotClusterIdentifier=None, SnapshotIdentifier=None, Tags=None, TagsAll=None, VpcSecurityGroupIds=None, Logging=None, SnapshotCopy=None, Timeouts=None)
