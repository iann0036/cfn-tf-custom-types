...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_default_vpc/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_default_vpc/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_default_vpc/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AssignGeneratedIpv6CidrBlock=None, CidrBlock=None, DefaultNetworkAclId=None, DefaultRouteTableId=None, DefaultSecurityGroupId=None, DhcpOptionsId=None, EnableClassiclink=None, EnableClassiclinkDnsSupport=None, EnableDnsHostnames=None, EnableDnsSupport=None, Id=None, InstanceTenancy=None, Ipv6AssociationId=None, Ipv6CidrBlock=None, MainRouteTableId=None, OwnerId=None, Tags=None, TagsAll=None)
