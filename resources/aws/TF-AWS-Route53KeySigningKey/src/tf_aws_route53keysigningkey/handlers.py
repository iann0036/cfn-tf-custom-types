...
TYPE_NAME = "TF::AWS::Route53KeySigningKey"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_route53_key_signing_key/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_route53_key_signing_key',
                'returnValues': ["DigestAlgorithmMnemonic", "DigestAlgorithmType", "DigestValue", "DnskeyRecord", "DsRecord", "Flag", "Id", "KeyTag", "PublicKey", "SigningAlgorithmMnemonic", "SigningAlgorithmType"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_route53_key_signing_key/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_route53_key_signing_key/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), DigestAlgorithmMnemonic=None, DigestAlgorithmType=None, DigestValue=None, DnskeyRecord=None, DsRecord=None, Flag=None, HostedZoneId=None, Id=None, KeyManagementServiceArn=None, KeyTag=None, Name=None, PublicKey=None, SigningAlgorithmMnemonic=None, SigningAlgorithmType=None, Status=None)
