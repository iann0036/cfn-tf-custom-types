...
TYPE_NAME = "TF::AWS::SagemakerDomain"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_sagemaker_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_sagemaker_domain',
                'returnValues': ["Arn", "HomeEfsFileSystemId", "Id", "SingleSignOnManagedApplicationInstanceId", "Url"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_sagemaker_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_sagemaker_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppNetworkAccessType=None, Arn=None, AuthMode=None, DomainName=None, HomeEfsFileSystemId=None, Id=None, KmsKeyId=None, SingleSignOnManagedApplicationInstanceId=None, SubnetIds=None, Tags=None, TagsAll=None, Url=None, VpcId=None, DefaultUserSettings=None)
