...
TYPE_NAME = "TF::AWS::AutoscalingGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_autoscaling_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_autoscaling_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_autoscaling_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AvailabilityZones=None, CapacityRebalance=None, DefaultCooldown=None, DesiredCapacity=None, EnabledMetrics=None, ForceDelete=None, ForceDeleteWarmPool=None, HealthCheckGracePeriod=None, HealthCheckType=None, Id=None, LaunchConfiguration=None, LoadBalancers=None, MaxInstanceLifetime=None, MaxSize=None, MetricsGranularity=None, MinElbCapacity=None, MinSize=None, Name=None, NamePrefix=None, PlacementGroup=None, ProtectFromScaleIn=None, ServiceLinkedRoleArn=None, SuspendedProcesses=None, Tags=None, TargetGroupArns=None, TerminationPolicies=None, VpcZoneIdentifier=None, WaitForCapacityTimeout=None, WaitForElbCapacity=None, InitialLifecycleHook=None, InstanceRefresh=None, LaunchTemplate=None, MixedInstancesPolicy=None, Tag=None, Timeouts=None, WarmPool=None)
