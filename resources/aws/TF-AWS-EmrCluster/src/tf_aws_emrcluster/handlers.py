...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_emr_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_emr_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_emr_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AdditionalInfo=None, Applications=None, Arn=None, AutoscalingRole=None, ClusterState=None, Configurations=None, ConfigurationsJson=None, CustomAmiId=None, EbsRootVolumeSize=None, Id=None, KeepJobFlowAliveWhenNoSteps=None, LogUri=None, MasterPublicDns=None, Name=None, ReleaseLabel=None, ScaleDownBehavior=None, SecurityConfiguration=None, ServiceRole=None, Step=None, StepConcurrencyLevel=None, Tags=None, TagsAll=None, TerminationProtection=None, VisibleToAllUsers=None, BootstrapAction=None, CoreInstanceFleet=None, CoreInstanceGroup=None, Ec2Attributes=None, KerberosAttributes=None, MasterInstanceFleet=None, MasterInstanceGroup=None)
