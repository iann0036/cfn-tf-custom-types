...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_eip/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["AllocationId", "AssociationId", "CarrierIp", "CustomerOwnedIp", "Domain", "Id", "PrivateDns", "PrivateIp", "PublicDns", "PublicIp"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_eip/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_eip/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Address=None, AllocationId=None, AssociateWithPrivateIp=None, AssociationId=None, CarrierIp=None, CustomerOwnedIp=None, CustomerOwnedIpv4Pool=None, Domain=None, Id=None, Instance=None, NetworkBorderGroup=None, NetworkInterface=None, PrivateDns=None, PrivateIp=None, PublicDns=None, PublicIp=None, PublicIpv4Pool=None, Tags=None, TagsAll=None, Vpc=None, Timeouts=None)
