...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_s3_bucket/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["BucketDomainName", "BucketRegionalDomainName", "Id", "Region"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_s3_bucket/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_s3_bucket/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccelerationStatus=None, Acl=None, Arn=None, Bucket=None, BucketDomainName=None, BucketPrefix=None, BucketRegionalDomainName=None, ForceDestroy=None, HostedZoneId=None, Id=None, Policy=None, Region=None, RequestPayer=None, Tags=None, TagsAll=None, WebsiteDomain=None, WebsiteEndpoint=None, CorsRule=None, Grant=None, LifecycleRule=None, Logging=None, ObjectLockConfiguration=None, ReplicationConfiguration=None, ServerSideEncryptionConfiguration=None, Versioning=None, Website=None)
