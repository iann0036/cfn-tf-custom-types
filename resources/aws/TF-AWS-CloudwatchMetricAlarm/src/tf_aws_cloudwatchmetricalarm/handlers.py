...
TYPE_NAME = "TF::AWS::CloudwatchMetricAlarm"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_cloudwatch_metric_alarm/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_cloudwatch_metric_alarm/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_cloudwatch_metric_alarm/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActionsEnabled=None, AlarmActions=None, AlarmDescription=None, AlarmName=None, Arn=None, ComparisonOperator=None, DatapointsToAlarm=None, Dimensions=None, EvaluateLowSampleCountPercentiles=None, EvaluationPeriods=None, ExtendedStatistic=None, Id=None, InsufficientDataActions=None, MetricName=None, Namespace=None, OkActions=None, Period=None, Statistic=None, Tags=None, TagsAll=None, Threshold=None, ThresholdMetricId=None, TreatMissingData=None, Unit=None, MetricQuery=None)
