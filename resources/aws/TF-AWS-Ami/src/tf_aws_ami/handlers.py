...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ami/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "Hypervisor", "Id", "ImageOwnerAlias", "ImageType", "ManageEbsSnapshots", "OwnerId", "Platform", "PlatformDetails", "Public", "RootSnapshotId", "UsageOperation"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ami/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_ami/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Architecture=None, Arn=None, Description=None, EnaSupport=None, Hypervisor=None, Id=None, ImageLocation=None, ImageOwnerAlias=None, ImageType=None, KernelId=None, ManageEbsSnapshots=None, Name=None, OwnerId=None, Platform=None, PlatformDetails=None, Public=None, RamdiskId=None, RootDeviceName=None, RootSnapshotId=None, SriovNetSupport=None, Tags=None, TagsAll=None, UsageOperation=None, VirtualizationType=None, EbsBlockDevice=None, EphemeralBlockDevice=None, Timeouts=None)
