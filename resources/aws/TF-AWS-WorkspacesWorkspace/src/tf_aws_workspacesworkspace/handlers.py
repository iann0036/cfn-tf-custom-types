...
TYPE_NAME = "TF::AWS::WorkspacesWorkspace"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_workspaces_workspace/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_workspaces_workspace',
                'returnValues': ["ComputerName", "Id", "IpAddress", "State"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_workspaces_workspace/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_workspaces_workspace/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BundleId=None, ComputerName=None, DirectoryId=None, Id=None, IpAddress=None, RootVolumeEncryptionEnabled=None, State=None, Tags=None, TagsAll=None, UserName=None, UserVolumeEncryptionEnabled=None, VolumeEncryptionKey=None, Timeouts=None, WorkspaceProperties=None)
