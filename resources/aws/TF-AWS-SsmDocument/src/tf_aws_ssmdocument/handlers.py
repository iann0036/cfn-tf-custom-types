...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ssm_document/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CreatedDate", "DefaultVersion", "Description", "DocumentVersion", "Hash", "HashType", "Id", "LatestVersion", "Owner", "Parameter", "PlatformTypes", "SchemaVersion", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ssm_document/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_ssm_document/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, Content=None, CreatedDate=None, DefaultVersion=None, Description=None, DocumentFormat=None, DocumentType=None, DocumentVersion=None, Hash=None, HashType=None, Id=None, LatestVersion=None, Name=None, Owner=None, Parameter=None, Permissions=None, PlatformTypes=None, SchemaVersion=None, Status=None, Tags=None, TagsAll=None, TargetType=None, VersionName=None, AttachmentsSource=None)
