...
TYPE_NAME = "TF::AWS::ElasticacheGlobalReplicationGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticache_global_replication_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_elasticache_global_replication_group',
                'returnValues': ["ActualEngineVersion", "Arn", "AtRestEncryptionEnabled", "AuthTokenEnabled", "CacheNodeType", "ClusterEnabled", "Engine", "EngineVersionActual", "GlobalReplicationGroupId", "Id", "TransitEncryptionEnabled"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticache_global_replication_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_elasticache_global_replication_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActualEngineVersion=None, Arn=None, AtRestEncryptionEnabled=None, AuthTokenEnabled=None, CacheNodeType=None, ClusterEnabled=None, Engine=None, EngineVersionActual=None, GlobalReplicationGroupDescription=None, GlobalReplicationGroupId=None, GlobalReplicationGroupIdSuffix=None, Id=None, PrimaryReplicationGroupId=None, TransitEncryptionEnabled=None)
