...
TYPE_NAME = "TF::AWS::SignerSigningJob"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_signer_signing_job/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_signer_signing_job',
                'returnValues': ["CompletedAt", "CreatedAt", "Id", "JobId", "JobInvoker", "JobOwner", "PlatformDisplayName", "PlatformId", "ProfileVersion", "RequestedBy", "RevocationRecord", "SignatureExpiresAt", "SignedObject", "Status", "StatusReason"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_signer_signing_job/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_signer_signing_job/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CompletedAt=None, CreatedAt=None, Id=None, IgnoreSigningJobFailure=None, JobId=None, JobInvoker=None, JobOwner=None, PlatformDisplayName=None, PlatformId=None, ProfileName=None, ProfileVersion=None, RequestedBy=None, RevocationRecord=None, SignatureExpiresAt=None, SignedObject=None, Status=None, StatusReason=None, Destination=None, Source=None)
