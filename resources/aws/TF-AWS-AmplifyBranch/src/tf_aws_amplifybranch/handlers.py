...
TYPE_NAME = "TF::AWS::AmplifyBranch"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_amplify_branch/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_amplify_branch',
                'returnValues': ["Arn", "AssociatedResources", "CustomDomains", "DestinationBranch", "Id", "SourceBranch"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_amplify_branch/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_amplify_branch/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppId=None, Arn=None, AssociatedResources=None, BackendEnvironmentArn=None, BasicAuthCredentials=None, BranchName=None, CustomDomains=None, Description=None, DestinationBranch=None, DisplayName=None, EnableAutoBuild=None, EnableBasicAuth=None, EnableNotification=None, EnablePerformanceMode=None, EnablePullRequestPreview=None, EnvironmentVariables=None, Framework=None, Id=None, PullRequestEnvironmentName=None, SourceBranch=None, Stage=None, Tags=None, TagsAll=None, Ttl=None)
