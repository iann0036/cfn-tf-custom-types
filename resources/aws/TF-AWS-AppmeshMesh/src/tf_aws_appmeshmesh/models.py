...
class SpecDefinition(BaseModel):
    EgressFilter: Optional[Sequence["_EgressFilterDefinition"]]
    ...
    def _deserialize(
        ...
        if not json_data:
            return None
        return cls(
            EgressFilter=deserialize_list(json_data.get("EgressFilter"), EgressFilterDefinition),
        )
...
@dataclass
class EgressFilterDefinition(BaseModel):
    Type: Optional[str]

    @classmethod
    def _deserialize(
        cls: Type["_EgressFilterDefinition"],
        json_data: Optional[Mapping[str, Any]],
    ) -> Optional["_EgressFilterDefinition"]:
        if not json_data:
            return None
        return cls(
...
# work around possible type aliasing issues when variable has same name as a model
_EgressFilterDefinition = EgressFilterDefinition
