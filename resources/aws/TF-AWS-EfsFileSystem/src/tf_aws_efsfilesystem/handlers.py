...
TYPE_NAME = "TF::AWS::EfsFileSystem"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_efs_file_system/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "AvailabilityZoneId", "DnsName", "Id", "NumberOfMountTargets", "OwnerId", "SizeInBytes"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_efs_file_system/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_efs_file_system/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AvailabilityZoneId=None, AvailabilityZoneName=None, CreationToken=None, DnsName=None, Encrypted=None, Id=None, KmsKeyId=None, NumberOfMountTargets=None, OwnerId=None, PerformanceMode=None, ProvisionedThroughputInMibps=None, SizeInBytes=None, Tags=None, TagsAll=None, ThroughputMode=None, LifecyclePolicy=None)
