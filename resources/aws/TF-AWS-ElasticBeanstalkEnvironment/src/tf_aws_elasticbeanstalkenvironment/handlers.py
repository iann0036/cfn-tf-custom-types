...
TYPE_NAME = "TF::AWS::ElasticBeanstalkEnvironment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elastic_beanstalk_environment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elastic_beanstalk_environment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_elastic_beanstalk_environment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllSettings=None, Application=None, Arn=None, AutoscalingGroups=None, Cname=None, CnamePrefix=None, Description=None, EndpointUrl=None, Id=None, Instances=None, LaunchConfigurations=None, LoadBalancers=None, Name=None, PlatformArn=None, PollInterval=None, Queues=None, SolutionStackName=None, Tags=None, TagsAll=None, TemplateName=None, Tier=None, Triggers=None, VersionLabel=None, WaitForReadyTimeout=None, Setting=None)
