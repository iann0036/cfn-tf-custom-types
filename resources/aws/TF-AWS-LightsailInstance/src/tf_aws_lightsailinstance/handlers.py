...
TYPE_NAME = "TF::AWS::LightsailInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lightsail_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CpuCount", "CreatedAt", "Id", "Ipv6Address", "Ipv6Addresses", "IsStaticIp", "PrivateIpAddress", "PublicIpAddress", "RamSize", "Username"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lightsail_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_lightsail_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AvailabilityZone=None, BlueprintId=None, BundleId=None, CpuCount=None, CreatedAt=None, Id=None, Ipv6Address=None, Ipv6Addresses=None, IsStaticIp=None, KeyPairName=None, Name=None, PrivateIpAddress=None, PublicIpAddress=None, RamSize=None, Tags=None, TagsAll=None, UserData=None, Username=None)
