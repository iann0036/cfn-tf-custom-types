...
TYPE_NAME = "TF::AWS::Macie2ClassificationJob"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_macie2_classification_job/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_macie2_classification_job',
                'returnValues': ["CreatedAt", "Id", "JobArn", "JobId", "UserPausedDetails"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_macie2_classification_job/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_macie2_classification_job/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreatedAt=None, CustomDataIdentifierIds=None, Description=None, Id=None, InitialRun=None, JobArn=None, JobId=None, JobStatus=None, JobType=None, Name=None, NamePrefix=None, SamplingPercentage=None, Tags=None, TagsAll=None, UserPausedDetails=None, S3JobDefinition=None, ScheduleFrequency=None)
