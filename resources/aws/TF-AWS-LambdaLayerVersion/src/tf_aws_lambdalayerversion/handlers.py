...
TYPE_NAME = "TF::AWS::LambdaLayerVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lambda_layer_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CreatedDate", "Id", "LayerArn", "SigningJobArn", "SigningProfileVersionArn", "SourceCodeSize", "Version"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lambda_layer_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_lambda_layer_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, CompatibleRuntimes=None, CreatedDate=None, Description=None, Filename=None, Id=None, LayerArn=None, LayerName=None, LicenseInfo=None, S3Bucket=None, S3Key=None, S3ObjectVersion=None, SigningJobArn=None, SigningProfileVersionArn=None, SourceCodeHash=None, SourceCodeSize=None, Version=None)
