...
TYPE_NAME = "TF::AWS::CloudfrontDistribution"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_cloudfront_distribution/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CallerReference", "DomainName", "Etag", "HostedZoneId", "Id", "InProgressValidationBatches", "LastModifiedTime", "Status", "TrustedKeyGroups", "TrustedSigners"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_cloudfront_distribution/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_cloudfront_distribution/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Aliases=None, Arn=None, CallerReference=None, Comment=None, DefaultRootObject=None, DomainName=None, Enabled=None, Etag=None, HostedZoneId=None, HttpVersion=None, Id=None, InProgressValidationBatches=None, IsIpv6Enabled=None, LastModifiedTime=None, PriceClass=None, RetainOnDelete=None, Status=None, Tags=None, TagsAll=None, TrustedKeyGroups=None, TrustedSigners=None, WaitForDeployment=None, WebAclId=None, CustomErrorResponse=None, DefaultCacheBehavior=None, LoggingConfig=None, OrderedCacheBehavior=None, Origin=None, OriginGroup=None, Restrictions=None, ViewerCertificate=None)
