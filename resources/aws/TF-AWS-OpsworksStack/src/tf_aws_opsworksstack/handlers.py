...
TYPE_NAME = "TF::AWS::OpsworksStack"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_opsworks_stack/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_opsworks_stack/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_opsworks_stack/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AgentVersion=None, Arn=None, BerkshelfVersion=None, Color=None, ConfigurationManagerName=None, ConfigurationManagerVersion=None, CustomJson=None, DefaultAvailabilityZone=None, DefaultInstanceProfileArn=None, DefaultOs=None, DefaultRootDeviceType=None, DefaultSshKeyName=None, DefaultSubnetId=None, HostnameTheme=None, Id=None, ManageBerkshelf=None, Name=None, Region=None, ServiceRoleArn=None, StackEndpoint=None, Tags=None, TagsAll=None, UseCustomCookbooks=None, UseOpsworksSecurityGroups=None, VpcId=None, CustomCookbooksSource=None)
