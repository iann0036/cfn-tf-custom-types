...
TYPE_NAME = "TF::AWS::Apigatewayv2Api"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_apigatewayv2_api/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_apigatewayv2_api/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_apigatewayv2_api/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiEndpoint=None, ApiKeySelectionExpression=None, Arn=None, Body=None, CredentialsArn=None, Description=None, DisableExecuteApiEndpoint=None, ExecutionArn=None, FailOnWarnings=None, Id=None, Name=None, ProtocolType=None, RouteKey=None, RouteSelectionExpression=None, Tags=None, TagsAll=None, Target=None, Version=None, CorsConfiguration=None)
