...
TYPE_NAME = "TF::AWS::OpsworksJavaAppLayer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_opsworks_java_app_layer/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_opsworks_java_app_layer/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_opsworks_java_app_layer/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppServer=None, AppServerVersion=None, Arn=None, AutoAssignElasticIps=None, AutoAssignPublicIps=None, AutoHealing=None, CustomConfigureRecipes=None, CustomDeployRecipes=None, CustomInstanceProfileArn=None, CustomJson=None, CustomSecurityGroupIds=None, CustomSetupRecipes=None, CustomShutdownRecipes=None, CustomUndeployRecipes=None, DrainElbOnShutdown=None, ElasticLoadBalancer=None, Id=None, InstallUpdatesOnBoot=None, InstanceShutdownTimeout=None, JvmOptions=None, JvmType=None, JvmVersion=None, Name=None, StackId=None, SystemPackages=None, Tags=None, TagsAll=None, UseEbsOptimizedInstances=None, EbsVolume=None)
