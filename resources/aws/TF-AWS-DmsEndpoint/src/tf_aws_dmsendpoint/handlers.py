...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_dms_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_dms_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_dms_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CertificateArn=None, DatabaseName=None, EndpointArn=None, EndpointId=None, EndpointType=None, EngineName=None, ExtraConnectionAttributes=None, Id=None, KmsKeyArn=None, Password=None, Port=None, ServerName=None, ServiceAccessRole=None, SslMode=None, Tags=None, TagsAll=None, Username=None, ElasticsearchSettings=None, KafkaSettings=None, KinesisSettings=None, MongodbSettings=None, S3Settings=None)
