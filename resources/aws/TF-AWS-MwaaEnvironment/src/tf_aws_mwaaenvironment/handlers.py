...
TYPE_NAME = "TF::AWS::MwaaEnvironment"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_mwaa_environment/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_mwaa_environment',
                'returnValues': ["Arn", "CreatedAt", "Id", "LastUpdated", "ServiceRoleArn", "Status", "WebserverUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_mwaa_environment/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_mwaa_environment/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AirflowConfigurationOptions=None, AirflowVersion=None, Arn=None, CreatedAt=None, DagS3Path=None, EnvironmentClass=None, ExecutionRoleArn=None, Id=None, KmsKey=None, LastUpdated=None, MaxWorkers=None, MinWorkers=None, Name=None, PluginsS3ObjectVersion=None, PluginsS3Path=None, RequirementsS3ObjectVersion=None, RequirementsS3Path=None, ServiceRoleArn=None, SourceBucketArn=None, Status=None, Tags=None, TagsAll=None, WebserverAccessMode=None, WebserverUrl=None, WeeklyMaintenanceWindowStart=None, LoggingConfiguration=None, NetworkConfiguration=None)
