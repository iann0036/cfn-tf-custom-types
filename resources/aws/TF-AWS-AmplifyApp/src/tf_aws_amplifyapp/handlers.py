...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_amplify_app/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_amplify_app',
                'returnValues': ["Arn", "DefaultDomain", "Id", "ProductionBranch"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_amplify_app/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_amplify_app/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessToken=None, Arn=None, AutoBranchCreationPatterns=None, BasicAuthCredentials=None, BuildSpec=None, DefaultDomain=None, Description=None, EnableAutoBranchCreation=None, EnableBasicAuth=None, EnableBranchAutoBuild=None, EnableBranchAutoDeletion=None, EnvironmentVariables=None, IamServiceRoleArn=None, Id=None, Name=None, OauthToken=None, Platform=None, ProductionBranch=None, Repository=None, Tags=None, TagsAll=None, AutoBranchCreationConfig=None, CustomRule=None)
