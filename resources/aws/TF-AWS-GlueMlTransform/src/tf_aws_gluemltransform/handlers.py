...
TYPE_NAME = "TF::AWS::GlueMlTransform"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_glue_ml_transform/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_glue_ml_transform',
                'returnValues': ["Arn", "Id", "LabelCount", "Schema"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_glue_ml_transform/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_glue_ml_transform/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, Description=None, GlueVersion=None, Id=None, LabelCount=None, MaxCapacity=None, MaxRetries=None, Name=None, NumberOfWorkers=None, RoleArn=None, Schema=None, Tags=None, TagsAll=None, Timeout=None, WorkerType=None, InputRecordTables=None, Parameters=None)
