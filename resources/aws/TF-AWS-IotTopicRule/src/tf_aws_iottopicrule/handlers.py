...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_iot_topic_rule/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_iot_topic_rule/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_iot_topic_rule/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, Description=None, Enabled=None, Id=None, Name=None, Sql=None, SqlVersion=None, Tags=None, TagsAll=None, CloudwatchAlarm=None, CloudwatchMetric=None, Dynamodb=None, Dynamodbv2=None, Elasticsearch=None, ErrorAction=None, Firehose=None, IotAnalytics=None, IotEvents=None, Kinesis=None, Lambda=None, Republish=None, S3=None, Sns=None, Sqs=None, StepFunctions=None)
