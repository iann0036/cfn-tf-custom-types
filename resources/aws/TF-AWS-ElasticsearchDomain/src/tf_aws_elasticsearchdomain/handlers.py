...
TYPE_NAME = "TF::AWS::ElasticsearchDomain"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticsearch_domain/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticsearch_domain/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_elasticsearch_domain/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessPolicies=None, AdvancedOptions=None, Arn=None, DomainId=None, DomainName=None, ElasticsearchVersion=None, Endpoint=None, Id=None, KibanaEndpoint=None, Tags=None, TagsAll=None, AdvancedSecurityOptions=None, ClusterConfig=None, CognitoOptions=None, DomainEndpointOptions=None, EbsOptions=None, EncryptAtRest=None, LogPublishingOptions=None, NodeToNodeEncryption=None, SnapshotOptions=None, Timeouts=None, VpcOptions=None)
