...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_docdb_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_docdb_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_docdb_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplyImmediately=None, Arn=None, AvailabilityZones=None, BackupRetentionPeriod=None, ClusterIdentifier=None, ClusterIdentifierPrefix=None, ClusterMembers=None, ClusterResourceId=None, DbClusterParameterGroupName=None, DbSubnetGroupName=None, DeletionProtection=None, EnabledCloudwatchLogsExports=None, Endpoint=None, Engine=None, EngineVersion=None, FinalSnapshotIdentifier=None, HostedZoneId=None, Id=None, KmsKeyId=None, MasterPassword=None, MasterUsername=None, Port=None, PreferredBackupWindow=None, PreferredMaintenanceWindow=None, ReaderEndpoint=None, SkipFinalSnapshot=None, SnapshotIdentifier=None, StorageEncrypted=None, Tags=None, TagsAll=None, VpcSecurityGroupIds=None, Timeouts=None)
