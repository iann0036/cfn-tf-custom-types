...
TYPE_NAME = "TF::AWS::ApprunnerAutoScalingConfigurationVersion"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_apprunner_auto_scaling_configuration_version/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_apprunner_auto_scaling_configuration_version',
                'returnValues': ["Arn", "AutoScalingConfigurationRevision", "Id", "Latest", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_apprunner_auto_scaling_configuration_version/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_apprunner_auto_scaling_configuration_version/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AutoScalingConfigurationName=None, AutoScalingConfigurationRevision=None, Id=None, Latest=None, MaxConcurrency=None, MaxSize=None, MinSize=None, Status=None, Tags=None, TagsAll=None)
