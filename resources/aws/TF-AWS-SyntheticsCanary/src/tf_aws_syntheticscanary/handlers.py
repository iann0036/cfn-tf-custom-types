...
TYPE_NAME = "TF::AWS::SyntheticsCanary"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_synthetics_canary/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_synthetics_canary',
                'returnValues': ["Arn", "EngineArn", "Id", "SourceLocationArn", "Status", "Timeline"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_synthetics_canary/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_synthetics_canary/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, ArtifactS3Location=None, EngineArn=None, ExecutionRoleArn=None, FailureRetentionPeriod=None, Handler=None, Id=None, Name=None, RuntimeVersion=None, S3Bucket=None, S3Key=None, S3Version=None, SourceLocationArn=None, StartCanary=None, Status=None, SuccessRetentionPeriod=None, Tags=None, TagsAll=None, Timeline=None, ZipFile=None, RunConfig=None, Schedule=None, VpcConfig=None)
