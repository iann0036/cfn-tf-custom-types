...
TYPE_NAME = "TF::AWS::StoragegatewayGateway"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_storagegateway_gateway/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "Ec2InstanceId", "EndpointType", "GatewayId", "GatewayNetworkInterface", "HostEnvironment", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_storagegateway_gateway/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_storagegateway_gateway/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActivationKey=None, Arn=None, AverageDownloadRateLimitInBitsPerSec=None, AverageUploadRateLimitInBitsPerSec=None, CloudwatchLogGroupArn=None, Ec2InstanceId=None, EndpointType=None, GatewayId=None, GatewayIpAddress=None, GatewayName=None, GatewayNetworkInterface=None, GatewayTimezone=None, GatewayType=None, GatewayVpcEndpoint=None, HostEnvironment=None, Id=None, MediumChangerType=None, SmbFileShareVisibility=None, SmbGuestPassword=None, SmbSecurityStrategy=None, Tags=None, TagsAll=None, TapeDriveType=None, SmbActiveDirectorySettings=None, Timeouts=None)
