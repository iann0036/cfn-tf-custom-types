...
TYPE_NAME = "TF::AWS::FsxWindowsFileSystem"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_fsx_windows_file_system/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "DnsName", "Id", "NetworkInterfaceIds", "OwnerId", "PreferredFileServerIp", "RemoteAdministrationEndpoint", "VpcId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_fsx_windows_file_system/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_fsx_windows_file_system/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActiveDirectoryId=None, Arn=None, AutomaticBackupRetentionDays=None, CopyTagsToBackups=None, DailyAutomaticBackupStartTime=None, DeploymentType=None, DnsName=None, Id=None, KmsKeyId=None, NetworkInterfaceIds=None, OwnerId=None, PreferredFileServerIp=None, PreferredSubnetId=None, RemoteAdministrationEndpoint=None, SecurityGroupIds=None, SkipFinalBackup=None, StorageCapacity=None, StorageType=None, SubnetIds=None, Tags=None, TagsAll=None, ThroughputCapacity=None, VpcId=None, WeeklyMaintenanceStartTime=None, SelfManagedActiveDirectory=None, Timeouts=None)
