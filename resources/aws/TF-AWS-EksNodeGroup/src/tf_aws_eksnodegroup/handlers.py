...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_eks_node_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_eks_node_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_eks_node_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AmiType=None, Arn=None, CapacityType=None, ClusterName=None, DiskSize=None, ForceUpdateVersion=None, Id=None, InstanceTypes=None, Labels=None, NodeGroupName=None, NodeGroupNamePrefix=None, NodeRoleArn=None, ReleaseVersion=None, Resources=None, Status=None, SubnetIds=None, Tags=None, TagsAll=None, Version=None, LaunchTemplate=None, RemoteAccess=None, ScalingConfig=None, Taint=None, Timeouts=None)
