...
TYPE_NAME = "TF::AWS::Apigatewayv2Stage"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_apigatewayv2_stage/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_apigatewayv2_stage',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_apigatewayv2_stage/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_apigatewayv2_stage/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApiId=None, Arn=None, AutoDeploy=None, ClientCertificateId=None, DeploymentId=None, Description=None, ExecutionArn=None, Id=None, InvokeUrl=None, Name=None, StageVariables=None, Tags=None, TagsAll=None, AccessLogSettings=None, DefaultRouteSettings=None, RouteSettings=None)
