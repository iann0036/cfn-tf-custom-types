...
TYPE_NAME = "TF::AWS::LaunchTemplate"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_launch_template/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "Id", "LatestVersion"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_launch_template/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_launch_template/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, DefaultVersion=None, Description=None, DisableApiTermination=None, EbsOptimized=None, Id=None, ImageId=None, InstanceInitiatedShutdownBehavior=None, InstanceType=None, KernelId=None, KeyName=None, LatestVersion=None, Name=None, NamePrefix=None, RamDiskId=None, SecurityGroupNames=None, Tags=None, TagsAll=None, UpdateDefaultVersion=None, UserData=None, VpcSecurityGroupIds=None, BlockDeviceMappings=None, CapacityReservationSpecification=None, CpuOptions=None, CreditSpecification=None, ElasticGpuSpecifications=None, ElasticInferenceAccelerator=None, EnclaveOptions=None, HibernationOptions=None, IamInstanceProfile=None, InstanceMarketOptions=None, LicenseSpecification=None, MetadataOptions=None, Monitoring=None, NetworkInterfaces=None, Placement=None, TagSpecifications=None)
