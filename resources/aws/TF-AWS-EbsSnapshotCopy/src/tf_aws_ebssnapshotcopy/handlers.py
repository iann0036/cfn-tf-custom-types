...
TYPE_NAME = "TF::AWS::EbsSnapshotCopy"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ebs_snapshot_copy/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "DataEncryptionKeyId", "Id", "OwnerAlias", "OwnerId", "VolumeId", "VolumeSize"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ebs_snapshot_copy/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_ebs_snapshot_copy/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, DataEncryptionKeyId=None, Description=None, Encrypted=None, Id=None, KmsKeyId=None, OwnerAlias=None, OwnerId=None, SourceRegion=None, SourceSnapshotId=None, Tags=None, TagsAll=None, VolumeId=None, VolumeSize=None)
