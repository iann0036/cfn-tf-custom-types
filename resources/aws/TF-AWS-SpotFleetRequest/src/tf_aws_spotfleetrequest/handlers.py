...
TYPE_NAME = "TF::AWS::SpotFleetRequest"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_spot_fleet_request/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_spot_fleet_request/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_spot_fleet_request/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllocationStrategy=None, ClientToken=None, ExcessCapacityTerminationPolicy=None, FleetType=None, IamFleetRole=None, Id=None, InstanceInterruptionBehaviour=None, InstancePoolsToUseCount=None, LoadBalancers=None, OnDemandAllocationStrategy=None, OnDemandMaxTotalPrice=None, OnDemandTargetCapacity=None, ReplaceUnhealthyInstances=None, SpotPrice=None, SpotRequestState=None, Tags=None, TagsAll=None, TargetCapacity=None, TargetGroupArns=None, TerminateInstancesWithExpiration=None, ValidFrom=None, ValidUntil=None, WaitForFulfillment=None, LaunchSpecification=None, LaunchTemplateConfig=None, SpotMaintenanceStrategies=None, Timeouts=None)
