...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_msk_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "BootstrapBrokers", "BootstrapBrokersSaslIam", "BootstrapBrokersSaslScram", "BootstrapBrokersTls", "CurrentVersion", "Id", "ZookeeperConnectString"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_msk_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_msk_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, BootstrapBrokers=None, BootstrapBrokersSaslIam=None, BootstrapBrokersSaslScram=None, BootstrapBrokersTls=None, ClusterName=None, CurrentVersion=None, EnhancedMonitoring=None, Id=None, KafkaVersion=None, NumberOfBrokerNodes=None, Tags=None, TagsAll=None, ZookeeperConnectString=None, BrokerNodeGroupInfo=None, ClientAuthentication=None, ConfigurationInfo=None, EncryptionInfo=None, LoggingInfo=None, OpenMonitoring=None, Timeouts=None)
