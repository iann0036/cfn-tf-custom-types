...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lex_bot/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_lex_bot',
                'returnValues': ["Arn", "Checksum", "CreatedDate", "FailureReason", "Id", "LastUpdatedDate", "Status", "Version"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lex_bot/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_lex_bot/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, Checksum=None, ChildDirected=None, CreateVersion=None, CreatedDate=None, Description=None, DetectSentiment=None, EnableModelImprovements=None, FailureReason=None, Id=None, IdleSessionTtlInSeconds=None, LastUpdatedDate=None, Locale=None, Name=None, NluIntentConfidenceThreshold=None, ProcessBehavior=None, Status=None, Version=None, VoiceId=None, AbortStatement=None, ClarificationPrompt=None, Intent=None, Timeouts=None)
