...
TYPE_NAME = "TF::AWS::GlueDevEndpoint"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_glue_dev_endpoint/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_glue_dev_endpoint',
                'returnValues': ["Arn", "AvailabilityZone", "FailureReason", "Id", "PrivateAddress", "PublicAddress", "Status", "VpcId", "YarnEndpointAddress", "ZeppelinRemoteSparkInterpreterPort"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_glue_dev_endpoint/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_glue_dev_endpoint/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arguments=None, Arn=None, AvailabilityZone=None, ExtraJarsS3Path=None, ExtraPythonLibsS3Path=None, FailureReason=None, GlueVersion=None, Id=None, Name=None, NumberOfNodes=None, NumberOfWorkers=None, PrivateAddress=None, PublicAddress=None, PublicKey=None, PublicKeys=None, RoleArn=None, SecurityConfiguration=None, SecurityGroupIds=None, Status=None, SubnetId=None, Tags=None, TagsAll=None, VpcId=None, WorkerType=None, YarnEndpointAddress=None, ZeppelinRemoteSparkInterpreterPort=None)
