...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_dax_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_dax_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_dax_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, AvailabilityZones=None, ClusterAddress=None, ClusterName=None, ConfigurationEndpoint=None, Description=None, IamRoleArn=None, Id=None, MaintenanceWindow=None, NodeType=None, Nodes=None, NotificationTopicArn=None, ParameterGroupName=None, Port=None, ReplicationFactor=None, SecurityGroupIds=None, SubnetGroupName=None, Tags=None, TagsAll=None, ServerSideEncryption=None, Timeouts=None)
