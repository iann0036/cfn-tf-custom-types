...
TYPE_NAME = "TF::AWS::LambdaEventSourceMapping"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lambda_event_source_mapping/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_lambda_event_source_mapping/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_lambda_event_source_mapping/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BatchSize=None, BisectBatchOnFunctionError=None, Enabled=None, EventSourceArn=None, FunctionArn=None, FunctionName=None, FunctionResponseTypes=None, Id=None, LastModified=None, LastProcessingResult=None, MaximumBatchingWindowInSeconds=None, MaximumRecordAgeInSeconds=None, MaximumRetryAttempts=None, ParallelizationFactor=None, Queues=None, StartingPosition=None, StartingPositionTimestamp=None, State=None, StateTransitionReason=None, Topics=None, TumblingWindowInSeconds=None, Uuid=None, DestinationConfig=None, SelfManagedEventSource=None, SourceAccessConfiguration=None)
