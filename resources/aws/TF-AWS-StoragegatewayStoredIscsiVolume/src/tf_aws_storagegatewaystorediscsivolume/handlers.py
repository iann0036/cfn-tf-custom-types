...
TYPE_NAME = "TF::AWS::StoragegatewayStoredIscsiVolume"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_storagegateway_stored_iscsi_volume/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_storagegateway_stored_iscsi_volume',
                'returnValues': ["Arn", "ChapEnabled", "Id", "LunNumber", "NetworkInterfacePort", "TargetArn", "VolumeAttachmentStatus", "VolumeId", "VolumeSizeInBytes", "VolumeStatus", "VolumeType"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_storagegateway_stored_iscsi_volume/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_storagegateway_stored_iscsi_volume/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, ChapEnabled=None, DiskId=None, GatewayArn=None, Id=None, KmsEncrypted=None, KmsKey=None, LunNumber=None, NetworkInterfaceId=None, NetworkInterfacePort=None, PreserveExistingData=None, SnapshotId=None, Tags=None, TagsAll=None, TargetArn=None, TargetName=None, VolumeAttachmentStatus=None, VolumeId=None, VolumeSizeInBytes=None, VolumeStatus=None, VolumeType=None)
