...
TYPE_NAME = "TF::AWS::RdsClusterInstance"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_rds_cluster_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_rds_cluster_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_rds_cluster_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplyImmediately=None, Arn=None, AutoMinorVersionUpgrade=None, AvailabilityZone=None, CaCertIdentifier=None, ClusterIdentifier=None, CopyTagsToSnapshot=None, DbParameterGroupName=None, DbSubnetGroupName=None, DbiResourceId=None, Endpoint=None, Engine=None, EngineVersion=None, Id=None, Identifier=None, IdentifierPrefix=None, InstanceClass=None, KmsKeyId=None, MonitoringInterval=None, MonitoringRoleArn=None, PerformanceInsightsEnabled=None, PerformanceInsightsKmsKeyId=None, Port=None, PreferredBackupWindow=None, PreferredMaintenanceWindow=None, PromotionTier=None, PubliclyAccessible=None, StorageEncrypted=None, Tags=None, TagsAll=None, Writer=None, Timeouts=None)
