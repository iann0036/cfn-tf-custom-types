...
TYPE_NAME = "TF::AWS::ImagebuilderComponent"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_imagebuilder_component/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_imagebuilder_component',
                'returnValues': ["Arn", "DateCreated", "Encrypted", "Id", "Owner", "Type"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_imagebuilder_component/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_imagebuilder_component/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, ChangeDescription=None, Data=None, DateCreated=None, Description=None, Encrypted=None, Id=None, KmsKeyId=None, Name=None, Owner=None, Platform=None, SupportedOsVersions=None, Tags=None, TagsAll=None, Type=None, Uri=None, Version=None)
