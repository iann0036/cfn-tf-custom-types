...
TYPE_NAME = "TF::AWS::NeptuneCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_neptune_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_neptune_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_neptune_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplyImmediately=None, Arn=None, AvailabilityZones=None, BackupRetentionPeriod=None, ClusterIdentifier=None, ClusterIdentifierPrefix=None, ClusterMembers=None, ClusterResourceId=None, DeletionProtection=None, EnableCloudwatchLogsExports=None, Endpoint=None, Engine=None, EngineVersion=None, FinalSnapshotIdentifier=None, HostedZoneId=None, IamDatabaseAuthenticationEnabled=None, IamRoles=None, Id=None, KmsKeyArn=None, NeptuneClusterParameterGroupName=None, NeptuneSubnetGroupName=None, Port=None, PreferredBackupWindow=None, PreferredMaintenanceWindow=None, ReaderEndpoint=None, ReplicationSourceIdentifier=None, SkipFinalSnapshot=None, SnapshotIdentifier=None, StorageEncrypted=None, Tags=None, TagsAll=None, VpcSecurityGroupIds=None, Timeouts=None)
