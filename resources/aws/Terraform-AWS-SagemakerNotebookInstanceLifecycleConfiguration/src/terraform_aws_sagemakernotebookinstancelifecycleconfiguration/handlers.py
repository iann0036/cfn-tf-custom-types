...
def create_handler(
    ...
    try:
        ...
        tf['resource']['aws_sagemaker_notebook_instance_lifecycle_configuration'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("aws"))['SecretString']
            tf["provider"]["aws"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['aws_sagemaker_notebook_instance_lifecycle_configuration'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
