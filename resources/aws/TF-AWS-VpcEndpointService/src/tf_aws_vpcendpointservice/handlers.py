...
TYPE_NAME = "TF::AWS::VpcEndpointService"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_vpc_endpoint_service/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "AvailabilityZones", "BaseEndpointDnsNames", "Id", "ManagesVpcEndpoints", "PrivateDnsNameConfiguration", "ServiceName", "ServiceType", "State"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_vpc_endpoint_service/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_vpc_endpoint_service/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AcceptanceRequired=None, AllowedPrincipals=None, Arn=None, AvailabilityZones=None, BaseEndpointDnsNames=None, GatewayLoadBalancerArns=None, Id=None, ManagesVpcEndpoints=None, NetworkLoadBalancerArns=None, PrivateDnsName=None, PrivateDnsNameConfiguration=None, ServiceName=None, ServiceType=None, State=None, Tags=None, TagsAll=None)
