...
TYPE_NAME = "TF::AWS::ElasticacheCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticache_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CacheNodes", "ClusterAddress", "ConfigurationEndpoint", "EngineVersionActual", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticache_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_elasticache_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplyImmediately=None, Arn=None, AvailabilityZone=None, AzMode=None, CacheNodes=None, ClusterAddress=None, ClusterId=None, ConfigurationEndpoint=None, Engine=None, EngineVersion=None, EngineVersionActual=None, FinalSnapshotIdentifier=None, Id=None, MaintenanceWindow=None, NodeType=None, NotificationTopicArn=None, NumCacheNodes=None, ParameterGroupName=None, Port=None, PreferredAvailabilityZones=None, ReplicationGroupId=None, SecurityGroupIds=None, SecurityGroupNames=None, SnapshotArns=None, SnapshotName=None, SnapshotRetentionLimit=None, SnapshotWindow=None, SubnetGroupName=None, Tags=None, TagsAll=None)
