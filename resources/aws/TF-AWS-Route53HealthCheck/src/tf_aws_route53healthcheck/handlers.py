...
TYPE_NAME = "TF::AWS::Route53HealthCheck"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_route53_health_check/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_route53_health_check/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_route53_health_check/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ChildHealthThreshold=None, ChildHealthchecks=None, CloudwatchAlarmName=None, CloudwatchAlarmRegion=None, Disabled=None, EnableSni=None, FailureThreshold=None, Fqdn=None, Id=None, InsufficientDataHealthStatus=None, InvertHealthcheck=None, IpAddress=None, MeasureLatency=None, Port=None, ReferenceName=None, Regions=None, RequestInterval=None, ResourcePath=None, SearchString=None, Tags=None, TagsAll=None, Type=None)
