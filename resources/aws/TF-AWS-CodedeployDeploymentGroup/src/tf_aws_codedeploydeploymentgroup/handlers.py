...
TYPE_NAME = "TF::AWS::CodedeployDeploymentGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_codedeploy_deployment_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "ComputePlatform", "DeploymentGroupId", "Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_codedeploy_deployment_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_codedeploy_deployment_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppName=None, Arn=None, AutoscalingGroups=None, ComputePlatform=None, DeploymentConfigName=None, DeploymentGroupId=None, DeploymentGroupName=None, Id=None, ServiceRoleArn=None, Tags=None, TagsAll=None, AlarmConfiguration=None, AutoRollbackConfiguration=None, BlueGreenDeploymentConfig=None, DeploymentStyle=None, Ec2TagFilter=None, Ec2TagSet=None, EcsService=None, LoadBalancerInfo=None, OnPremisesInstanceTagFilter=None, TriggerConfiguration=None)
