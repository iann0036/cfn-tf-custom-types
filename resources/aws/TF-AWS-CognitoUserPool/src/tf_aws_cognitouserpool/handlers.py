...
TYPE_NAME = "TF::AWS::CognitoUserPool"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_cognito_user_pool/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "CreationDate", "CustomDomain", "Domain", "Endpoint", "EstimatedNumberOfUsers", "Id", "LastModifiedDate"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_cognito_user_pool/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_cognito_user_pool/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AliasAttributes=None, Arn=None, AutoVerifiedAttributes=None, CreationDate=None, CustomDomain=None, Domain=None, EmailVerificationMessage=None, EmailVerificationSubject=None, Endpoint=None, EstimatedNumberOfUsers=None, Id=None, LastModifiedDate=None, MfaConfiguration=None, Name=None, SmsAuthenticationMessage=None, SmsVerificationMessage=None, Tags=None, TagsAll=None, UsernameAttributes=None, AccountRecoverySetting=None, AdminCreateUserConfig=None, DeviceConfiguration=None, EmailConfiguration=None, LambdaConfig=None, PasswordPolicy=None, Schema=None, SmsConfiguration=None, SoftwareTokenMfaConfiguration=None, UserPoolAddOns=None, UsernameConfiguration=None, VerificationMessageTemplate=None)
