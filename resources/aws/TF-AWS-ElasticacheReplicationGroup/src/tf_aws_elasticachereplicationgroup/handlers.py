...
TYPE_NAME = "TF::AWS::ElasticacheReplicationGroup"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticache_replication_group/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["Arn", "ClusterEnabled", "ConfigurationEndpointAddress", "EngineVersionActual", "Id", "MemberClusters", "PrimaryEndpointAddress", "ReaderEndpointAddress"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_elasticache_replication_group/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_elasticache_replication_group/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplyImmediately=None, Arn=None, AtRestEncryptionEnabled=None, AuthToken=None, AutoMinorVersionUpgrade=None, AutomaticFailoverEnabled=None, AvailabilityZones=None, ClusterEnabled=None, ConfigurationEndpointAddress=None, Engine=None, EngineVersion=None, EngineVersionActual=None, FinalSnapshotIdentifier=None, GlobalReplicationGroupId=None, Id=None, KmsKeyId=None, MaintenanceWindow=None, MemberClusters=None, MultiAzEnabled=None, NodeType=None, NotificationTopicArn=None, NumberCacheClusters=None, ParameterGroupName=None, Port=None, PrimaryEndpointAddress=None, ReaderEndpointAddress=None, ReplicationGroupDescription=None, ReplicationGroupId=None, SecurityGroupIds=None, SecurityGroupNames=None, SnapshotArns=None, SnapshotName=None, SnapshotRetentionLimit=None, SnapshotWindow=None, SubnetGroupName=None, Tags=None, TagsAll=None, TransitEncryptionEnabled=None, ClusterMode=None, Timeouts=None)
