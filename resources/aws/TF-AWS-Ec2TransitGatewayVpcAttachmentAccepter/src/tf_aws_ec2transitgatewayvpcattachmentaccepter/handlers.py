...
TYPE_NAME = "TF::AWS::Ec2TransitGatewayVpcAttachmentAccepter"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ec2_transit_gateway_vpc_attachment_accepter/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'returnValues': ["ApplianceModeSupport", "DnsSupport", "Id", "Ipv6Support", "SubnetIds", "TransitGatewayId", "VpcId", "VpcOwnerId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_ec2_transit_gateway_vpc_attachment_accepter/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_ec2_transit_gateway_vpc_attachment_accepter/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApplianceModeSupport=None, DnsSupport=None, Id=None, Ipv6Support=None, SubnetIds=None, Tags=None, TagsAll=None, TransitGatewayAttachmentId=None, TransitGatewayDefaultRouteTableAssociation=None, TransitGatewayDefaultRouteTablePropagation=None, TransitGatewayId=None, VpcId=None, VpcOwnerId=None)
