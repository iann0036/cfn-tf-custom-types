...
TYPE_NAME = "TF::AWS::NetworkfirewallFirewall"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_networkfirewall_firewall/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'aws_networkfirewall_firewall',
                'returnValues': ["Arn", "FirewallStatus", "Id", "UpdateToken"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/aws_networkfirewall_firewall/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/aws_networkfirewall_firewall/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Arn=None, DeleteProtection=None, Description=None, FirewallPolicyArn=None, FirewallPolicyChangeProtection=None, FirewallStatus=None, Id=None, Name=None, SubnetChangeProtection=None, Tags=None, TagsAll=None, UpdateToken=None, VpcId=None, SubnetMapping=None)
