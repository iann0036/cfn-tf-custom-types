...
TYPE_NAME = "TF::SignalFx::TimeChart"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/signalfx_time_chart/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'splunk-terraform/signalfx',
                'providerTypeName': 'signalfx',
                'terraformTypeName': 'signalfx_time_chart',
                'returnValues': ["Id", "Url"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/signalfx_time_chart/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/signalfx_time_chart/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AxesIncludeZero=None, AxesPrecision=None, ColorBy=None, Description=None, DisableSampling=None, EndTime=None, Id=None, LegendFieldsToHide=None, MaxDelay=None, MinimumResolution=None, Name=None, OnChartLegendDimension=None, PlotType=None, ProgramText=None, ShowDataMarkers=None, ShowEventLines=None, Stacked=None, StartTime=None, Tags=None, TimeRange=None, Timezone=None, UnitPrefix=None, Url=None, AxisLeft=None, AxisRight=None, EventOptions=None, HistogramOptions=None, LegendOptionsFields=None, VizOptions=None)
