...
def create_handler(
    ...
    try:
        ...
        tf['resource']['runscope_environment'][request.logicalResourceIdentifier] = {}

        if "runscope" == "aws":
            creds = session.credentials
            tf["provider"]["runscope"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("runscope"))['SecretString']
            tf["provider"]["runscope"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['runscope_environment'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
