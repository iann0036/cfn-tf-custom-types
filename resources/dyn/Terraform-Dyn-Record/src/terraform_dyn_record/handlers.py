...
def create_handler(
    ...
    try:
        ...
        tf['resource']['dyn_record'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("dyn"))['SecretString']
            tf["provider"]["dyn"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['dyn_record'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
