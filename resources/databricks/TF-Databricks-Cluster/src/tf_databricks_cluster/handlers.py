...
TYPE_NAME = "TF::Databricks::Cluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/databricks_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'databrickslabs/databricks',
                'providerTypeName': 'databricks',
                'terraformTypeName': 'databricks_cluster',
                'returnValues': ["DefaultTags", "Id", "State", "Url"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/databricks_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/databricks_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoterminationMinutes=None, ClusterId=None, ClusterName=None, CustomTags=None, DefaultTags=None, DriverNodeTypeId=None, EnableElasticDisk=None, EnableLocalDiskEncryption=None, Id=None, IdempotencyToken=None, InstancePoolId=None, IsPinned=None, NodeTypeId=None, NumWorkers=None, PolicyId=None, SingleUserName=None, SparkConf=None, SparkEnvVars=None, SparkVersion=None, SshPublicKeys=None, State=None, Url=None, Autoscale=None, AwsAttributes=None, AzureAttributes=None, ClusterLogConf=None, DockerImage=None, GcpAttributes=None, InitScripts=None, Library=None, Timeouts=None)
