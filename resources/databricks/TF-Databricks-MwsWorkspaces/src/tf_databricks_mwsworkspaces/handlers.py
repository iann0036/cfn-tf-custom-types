...
TYPE_NAME = "TF::Databricks::MwsWorkspaces"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/databricks_mws_workspaces/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'databricks_mws_workspaces',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/databricks_mws_workspaces/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/databricks_mws_workspaces/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountId=None, AwsRegion=None, CreationTime=None, CredentialsId=None, CustomerManagedKeyId=None, DeploymentName=None, Id=None, IsNoPublicIpEnabled=None, ManagedServicesCustomerManagedKeyId=None, NetworkId=None, PricingTier=None, PrivateAccessSettingsId=None, StorageConfigurationId=None, StorageCustomerManagedKeyId=None, WorkspaceId=None, WorkspaceName=None, WorkspaceStatus=None, WorkspaceStatusMessage=None, WorkspaceUrl=None, Timeouts=None)
