...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vultr_instance/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'vultr/vultr',
                'providerTypeName': 'vultr',
                'terraformTypeName': 'vultr_instance',
                'returnValues': ["AllowedBandwidth", "DateCreated", "DefaultPassword", "Disk", "Features", "GatewayV4", "Id", "InternalIp", "Kvm", "MainIp", "NetmaskV4", "Os", "PowerStatus", "Ram", "ServerStatus", "Status", "V6MainIp", "V6Network", "V6NetworkSize", "VcpuCount"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vultr_instance/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vultr_instance/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActivationEmail=None, AllowedBandwidth=None, AppId=None, Backups=None, DateCreated=None, DdosProtection=None, DefaultPassword=None, Disk=None, EnableIpv6=None, EnablePrivateNetwork=None, Features=None, FirewallGroupId=None, GatewayV4=None, Hostname=None, Id=None, InternalIp=None, IsoId=None, Kvm=None, Label=None, MainIp=None, NetmaskV4=None, Os=None, OsId=None, Plan=None, PowerStatus=None, PrivateNetworkIds=None, Ram=None, Region=None, ReservedIpId=None, ScriptId=None, ServerStatus=None, SnapshotId=None, SshKeyIds=None, Status=None, Tag=None, UserData=None, V6MainIp=None, V6Network=None, V6NetworkSize=None, VcpuCount=None, BackupsSchedule=None)
