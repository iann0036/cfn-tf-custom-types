...
TYPE_NAME = "TF::Vultr::BareMetalServer"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vultr_bare_metal_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'vultr_bare_metal_server',
                'returnValues': ["CpuCount", "DateCreated", "DefaultPassword", "Disk", "GatewayV4", "Id", "MacAddress", "MainIp", "NetmaskV4", "Os", "Ram", "Status", "V6MainIp", "V6Network", "V6NetworkSize"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vultr_bare_metal_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vultr_bare_metal_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActivationEmail=None, AppId=None, CpuCount=None, DateCreated=None, DefaultPassword=None, Disk=None, EnableIpv6=None, GatewayV4=None, Hostname=None, Id=None, Label=None, MacAddress=None, MainIp=None, NetmaskV4=None, Os=None, OsId=None, Plan=None, Ram=None, Region=None, ReservedIpv4=None, ScriptId=None, SnapshotId=None, SshKeyIds=None, Status=None, Tag=None, UserData=None, V6MainIp=None, V6Network=None, V6NetworkSize=None)
