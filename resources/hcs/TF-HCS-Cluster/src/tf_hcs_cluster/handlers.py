...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcs_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/hcs',
                'providerTypeName': 'hcs',
                'terraformTypeName': 'hcs_cluster',
                'returnValues': ["BlobContainerName", "ConsulAutomaticUpgrades", "ConsulCaFile", "ConsulClusterId", "ConsulConfigFile", "ConsulConnect", "ConsulExternalEndpointUrl", "ConsulPrivateEndpointUrl", "ConsulRootTokenAccessorId", "ConsulRootTokenSecretId", "ConsulSnapshotInterval", "ConsulSnapshotRetention", "ConsulVersion", "Id", "ManagedApplicationId", "ManagedIdentityName", "State", "StorageAccountName", "StorageAccountResourceGroup", "VnetId", "VnetName", "VnetResourceGroupName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/hcs_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/hcs_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AuditLogStorageContainerUrl=None, AuditLoggingEnabled=None, BlobContainerName=None, ClusterMode=None, ClusterName=None, ConsulAutomaticUpgrades=None, ConsulCaFile=None, ConsulClusterId=None, ConsulConfigFile=None, ConsulConnect=None, ConsulDatacenter=None, ConsulExternalEndpoint=None, ConsulExternalEndpointUrl=None, ConsulFederationToken=None, ConsulPrivateEndpointUrl=None, ConsulRootTokenAccessorId=None, ConsulRootTokenSecretId=None, ConsulSnapshotInterval=None, ConsulSnapshotRetention=None, ConsulVersion=None, Email=None, Id=None, Location=None, ManagedApplicationId=None, ManagedApplicationName=None, ManagedIdentityName=None, ManagedResourceGroupName=None, MinConsulVersion=None, PlanName=None, ResourceGroupName=None, State=None, StorageAccountName=None, StorageAccountResourceGroup=None, Tags=None, VnetCidr=None, VnetId=None, VnetName=None, VnetResourceGroupName=None, Timeouts=None)
