...
def create_handler(
    ...
    try:
        ...
        tf['resource']['docker_config'][request.logicalResourceIdentifier] = {}
        ...
        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['docker_config'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
