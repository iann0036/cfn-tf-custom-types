...
def create_handler(
    ...
    try:
        ...
        tf['resource']['docker_container'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("docker"))['SecretString']
            tf["provider"]["docker"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['docker_container'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
