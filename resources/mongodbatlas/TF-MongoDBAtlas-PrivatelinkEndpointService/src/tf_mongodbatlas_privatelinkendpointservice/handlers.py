...
TYPE_NAME = "TF::MongoDBAtlas::PrivatelinkEndpointService"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_privatelink_endpoint_service/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'mongodbatlas_privatelink_endpoint_service',
                'returnValues': ["AwsConnectionStatus", "AzureStatus", "DeleteRequested", "ErrorMessage", "Id", "InterfaceEndpointId", "PrivateEndpointConnectionName", "PrivateEndpointResourceId"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_privatelink_endpoint_service/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/mongodbatlas_privatelink_endpoint_service/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AwsConnectionStatus=None, AzureStatus=None, DeleteRequested=None, EndpointServiceId=None, ErrorMessage=None, Id=None, InterfaceEndpointId=None, PrivateEndpointConnectionName=None, PrivateEndpointIpAddress=None, PrivateEndpointResourceId=None, PrivateLinkId=None, ProjectId=None, ProviderName=None)
