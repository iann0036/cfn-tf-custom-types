...
TYPE_NAME = "TF::MongoDBAtlas::NetworkPeering"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_network_peering/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'mongodbatlas_network_peering',
                'returnValues': ["AtlasId", "ConnectionId", "ErrorMessage", "ErrorState", "ErrorStateName", "Id", "PeerId", "Status", "StatusName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_network_peering/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/mongodbatlas_network_peering/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccepterRegionName=None, AtlasCidrBlock=None, AtlasGcpProjectId=None, AtlasId=None, AtlasVpcName=None, AwsAccountId=None, AzureDirectoryId=None, AzureSubscriptionId=None, ConnectionId=None, ContainerId=None, ErrorMessage=None, ErrorState=None, ErrorStateName=None, GcpProjectId=None, Id=None, NetworkName=None, PeerId=None, ProjectId=None, ProviderName=None, ResourceGroupName=None, RouteTableCidrBlock=None, Status=None, StatusName=None, VnetName=None, VpcId=None)
