...
TYPE_NAME = "TF::MongoDBAtlas::CloudProviderSnapshotRestoreJob"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_cloud_provider_snapshot_restore_job/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'mongodbatlas_cloud_provider_snapshot_restore_job',
                'returnValues': ["Cancelled", "CreatedAt", "DeliveryUrl", "Expired", "ExpiresAt", "FinishedAt", "Id", "SnapshotRestoreJobId", "Timestamp"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_cloud_provider_snapshot_restore_job/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/mongodbatlas_cloud_provider_snapshot_restore_job/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Cancelled=None, ClusterName=None, CreatedAt=None, DeliveryType=None, DeliveryUrl=None, Expired=None, ExpiresAt=None, FinishedAt=None, Id=None, ProjectId=None, SnapshotId=None, SnapshotRestoreJobId=None, Timestamp=None)
