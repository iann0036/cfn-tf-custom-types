...
TYPE_NAME = "TF::MongoDBAtlas::Cluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'mongodb/mongodbatlas',
                'providerTypeName': 'mongodbatlas',
                'terraformTypeName': 'mongodbatlas_cluster',
                'returnValues': ["ClusterId", "ConnectionStrings", "ContainerId", "Id", "MongoDbVersion", "MongoUri", "MongoUriUpdated", "MongoUriWithOptions", "Paused", "ProviderEncryptEbsVolumeFlag", "SnapshotBackupPolicy", "SrvAddress", "StateName"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/mongodbatlas_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/mongodbatlas_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AutoScalingComputeEnabled=None, AutoScalingComputeScaleDownEnabled=None, AutoScalingDiskGbEnabled=None, BackingProviderName=None, BackupEnabled=None, BiConnector=None, ClusterId=None, ClusterType=None, ConnectionStrings=None, ContainerId=None, DiskSizeGb=None, EncryptionAtRestProvider=None, Id=None, MongoDbMajorVersion=None, MongoDbVersion=None, MongoUri=None, MongoUriUpdated=None, MongoUriWithOptions=None, Name=None, NumShards=None, Paused=None, PitEnabled=None, ProjectId=None, ProviderAutoScalingComputeMaxInstanceSize=None, ProviderAutoScalingComputeMinInstanceSize=None, ProviderBackupEnabled=None, ProviderDiskIops=None, ProviderDiskTypeName=None, ProviderEncryptEbsVolume=None, ProviderEncryptEbsVolumeFlag=None, ProviderInstanceSizeName=None, ProviderName=None, ProviderRegionName=None, ProviderVolumeType=None, ReplicationFactor=None, SnapshotBackupPolicy=None, SrvAddress=None, StateName=None, AdvancedConfiguration=None, BiConnectorConfig=None, Labels=None, ReplicationSpecs=None)
