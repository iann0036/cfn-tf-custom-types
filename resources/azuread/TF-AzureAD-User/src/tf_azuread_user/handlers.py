...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azuread_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/azuread',
                'providerTypeName': 'azuread',
                'terraformTypeName': 'azuread_user',
                'returnValues': ["Id", "Mail", "ObjectId", "OnpremisesSamAccountName", "OnpremisesUserPrincipalName", "UserType"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azuread_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azuread_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountEnabled=None, City=None, CompanyName=None, Country=None, Department=None, DisplayName=None, ForcePasswordChange=None, GivenName=None, Id=None, ImmutableId=None, JobTitle=None, Mail=None, MailNickname=None, Mobile=None, MobilePhone=None, ObjectId=None, OfficeLocation=None, OnpremisesImmutableId=None, OnpremisesSamAccountName=None, OnpremisesUserPrincipalName=None, Password=None, PhysicalDeliveryOfficeName=None, PostalCode=None, State=None, StreetAddress=None, Surname=None, UsageLocation=None, UserPrincipalName=None, UserType=None)
