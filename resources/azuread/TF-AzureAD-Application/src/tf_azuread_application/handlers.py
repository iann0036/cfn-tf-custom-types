...
TYPE_NAME = "TF::AzureAD::Application"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/azuread_application/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/azuread_application/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/azuread_application/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AppRole=None, ApplicationId=None, AvailableToOtherTenants=None, DisplayName=None, FallbackPublicClientEnabled=None, GroupMembershipClaims=None, Homepage=None, Id=None, IdentifierUris=None, LogoutUrl=None, Name=None, Oauth2AllowImplicitFlow=None, Oauth2Permissions=None, ObjectId=None, Owners=None, PreventDuplicateNames=None, PublicClient=None, ReplyUrls=None, SignInAudience=None, Type=None, Api=None, OptionalClaims=None, RequiredResourceAccess=None, Timeouts=None, Web=None)
