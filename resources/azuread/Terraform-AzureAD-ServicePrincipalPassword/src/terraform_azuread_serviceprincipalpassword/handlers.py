...
def create_handler(
    ...
    try:
        ...
        tf['resource']['azuread_service_principal_password'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("azuread"))['SecretString']
            tf["provider"]["azuread"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['azuread_service_principal_password'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
