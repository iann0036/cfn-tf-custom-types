...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ilert_connection/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'ilert_connection',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ilert_connection/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ilert_connection/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CreatedAt=None, Id=None, Name=None, TriggerMode=None, TriggerTypes=None, UpdatedAt=None, AlertSource=None, Autotask=None, AwsLambda=None, AzureFaas=None, Connector=None, Datadog=None, Email=None, Github=None, GoogleFaas=None, Jira=None, Servicenow=None, Slack=None, StatusPageIo=None, Sysdig=None, Topdesk=None, Webhook=None, Zammad=None, Zapier=None, Zendesk=None)
