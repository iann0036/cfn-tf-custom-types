...
TYPE_NAME = "TF::ILert::AlertSource"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ilert_alert_source/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'iLert/ilert',
                'providerTypeName': 'ilert',
                'terraformTypeName': 'ilert_alert_source',
                'returnValues': ["Id", "IntegrationKey", "IntegrationUrl", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ilert_alert_source/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ilert_alert_source/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Active=None, AutoResolutionTimeout=None, Email=None, EmailFiltered=None, EmailResolveFiltered=None, EscalationPolicy=None, FilterOperator=None, Id=None, IncidentCreation=None, IncidentPriorityRule=None, IntegrationKey=None, IntegrationType=None, IntegrationUrl=None, Name=None, ResolveFilterOperator=None, Status=None, Teams=None, AutotaskMetadata=None, EmailPredicate=None, EmailResolvePredicate=None, Heartbeat=None, ResolveKeyExtractor=None, SupportHours=None)
