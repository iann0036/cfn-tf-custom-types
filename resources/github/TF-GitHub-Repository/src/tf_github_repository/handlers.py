...
TYPE_NAME = "TF::GitHub::Repository"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/github_repository/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/github',
                'providerTypeName': 'github',
                'terraformTypeName': 'github_repository',
                'returnValues': ["Etag", "FullName", "GitCloneUrl", "HtmlUrl", "HttpCloneUrl", "Id", "NodeId", "RepoId", "SshCloneUrl", "SvnUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/github_repository/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/github_repository/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowMergeCommit=None, AllowRebaseMerge=None, AllowSquashMerge=None, ArchiveOnDestroy=None, Archived=None, AutoInit=None, DefaultBranch=None, DeleteBranchOnMerge=None, Description=None, Etag=None, FullName=None, GitCloneUrl=None, GitignoreTemplate=None, HasDownloads=None, HasIssues=None, HasProjects=None, HasWiki=None, HomepageUrl=None, HtmlUrl=None, HttpCloneUrl=None, Id=None, IsTemplate=None, LicenseTemplate=None, Name=None, NodeId=None, Private=None, RepoId=None, SshCloneUrl=None, SvnUrl=None, Topics=None, Visibility=None, VulnerabilityAlerts=None, Pages=None, Template=None)
