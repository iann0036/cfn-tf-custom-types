...
def create_handler(
    ...
    try:
        ...
        tf['resource']['github_user_invitation_accepter'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("github"))['SecretString']
            tf["provider"]["github"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['github_user_invitation_accepter'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
