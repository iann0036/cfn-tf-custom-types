...
TYPE_NAME = "TF::ProfitBricks::Server"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/profitbricks_server/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'ionos-cloud/profitbricks',
                'providerTypeName': 'profitbricks',
                'terraformTypeName': 'profitbricks_server',
                'returnValues': ["BootCdrom", "BootVolume", "FirewallruleId", "Id", "PrimaryIp", "PrimaryNic"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/profitbricks_server/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/profitbricks_server/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZone=None, BootCdrom=None, BootImage=None, BootVolume=None, Cores=None, CpuFamily=None, DatacenterId=None, FirewallruleId=None, Id=None, ImageName=None, ImagePassword=None, LicenceType=None, Name=None, PrimaryIp=None, PrimaryNic=None, Ram=None, SshKeyPath=None, Nic=None, Timeouts=None, Volume=None)
