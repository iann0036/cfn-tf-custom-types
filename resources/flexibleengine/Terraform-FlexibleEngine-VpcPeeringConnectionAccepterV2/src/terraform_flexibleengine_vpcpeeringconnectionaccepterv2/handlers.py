...
def create_handler(
    ...
    try:
        ...
        tf['resource']['flexibleengine_vpc_peering_connection_accepter_v2'][request.logicalResourceIdentifier] = {}

        if "flexibleengine" == "aws":
            creds = session.credentials
            tf["provider"]["flexibleengine"] = {
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("flexibleengine"))['SecretString']
            tf["provider"]["flexibleengine"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['flexibleengine_vpc_peering_connection_accepter_v2'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
