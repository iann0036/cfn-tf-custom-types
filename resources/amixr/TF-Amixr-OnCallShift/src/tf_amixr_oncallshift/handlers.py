...
TYPE_NAME = "TF::Amixr::OnCallShift"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/amixr_on_call_shift/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'alertmixer/amixr',
                'providerTypeName': 'amixr',
                'terraformTypeName': 'amixr_on_call_shift',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/amixr_on_call_shift/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/amixr_on_call_shift/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ByDay=None, ByMonth=None, ByMonthday=None, Duration=None, Frequency=None, Id=None, Interval=None, Level=None, Name=None, RollingUsers=None, ScheduleId=None, Start=None, Type=None, Users=None, WeekStart=None)
