...
def create_handler(
    ...
    try:
        ...
        tf['resource']['dns_aaaa_record_set'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("dns"))['SecretString']
            tf["provider"]["dns"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['dns_aaaa_record_set'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
