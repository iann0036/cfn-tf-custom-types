...
def create_handler(
    ...
    try:
        ...
        tf['resource']['gitlab_pipeline_schedule_variable'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("gitlab"))['SecretString']
            tf["provider"]["gitlab"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['gitlab_pipeline_schedule_variable'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
