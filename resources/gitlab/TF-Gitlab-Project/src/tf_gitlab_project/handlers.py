...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gitlab_project/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'gitlabhq/gitlab',
                'providerTypeName': 'gitlab',
                'terraformTypeName': 'gitlab_project',
                'returnValues': ["HttpUrlToRepo", "Id", "PathWithNamespace", "RunnersToken", "SshUrlToRepo", "WebUrl"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gitlab_project/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gitlab_project/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ApprovalsBeforeMerge=None, Archived=None, ContainerRegistryEnabled=None, DefaultBranch=None, Description=None, GroupWithProjectTemplatesId=None, HttpUrlToRepo=None, Id=None, ImportUrl=None, InitializeWithReadme=None, IssuesEnabled=None, LfsEnabled=None, MergeMethod=None, MergeRequestsEnabled=None, Mirror=None, MirrorOverwritesDivergedBranches=None, MirrorTriggerBuilds=None, Name=None, NamespaceId=None, OnlyAllowMergeIfAllDiscussionsAreResolved=None, OnlyAllowMergeIfPipelineSucceeds=None, OnlyMirrorProtectedBranches=None, PackagesEnabled=None, PagesAccessLevel=None, Path=None, PathWithNamespace=None, PipelinesEnabled=None, RemoveSourceBranchAfterMerge=None, RequestAccessEnabled=None, RunnersToken=None, SharedRunnersEnabled=None, SnippetsEnabled=None, SshUrlToRepo=None, Tags=None, TemplateName=None, TemplateProjectId=None, UseCustomTemplate=None, VisibilityLevel=None, WebUrl=None, WikiEnabled=None, PushRules=None)
