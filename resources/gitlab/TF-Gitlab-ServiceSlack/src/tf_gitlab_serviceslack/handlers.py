...
TYPE_NAME = "TF::Gitlab::ServiceSlack"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/gitlab_service_slack/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/gitlab_service_slack/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/gitlab_service_slack/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), BranchesToBeNotified=None, ConfidentialIssueChannel=None, ConfidentialIssuesEvents=None, ConfidentialNoteEvents=None, Id=None, IssueChannel=None, IssuesEvents=None, JobEvents=None, MergeRequestChannel=None, MergeRequestsEvents=None, NoteChannel=None, NoteEvents=None, NotifyOnlyBrokenPipelines=None, NotifyOnlyDefaultBranch=None, PipelineChannel=None, PipelineEvents=None, Project=None, PushChannel=None, PushEvents=None, TagPushChannel=None, TagPushEvents=None, Username=None, Webhook=None, WikiPageChannel=None, WikiPageEvents=None)
