...
TYPE_NAME = "TF::Turbot::LdapDirectory"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/turbot_ldap_directory/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'turbot/turbot',
                'providerTypeName': 'turbot',
                'terraformTypeName': 'turbot_ldap_directory',
                'returnValues': ["Id", "ParentAkas", "Status"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/turbot_ldap_directory/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/turbot_ldap_directory/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Base=None, ConnectivityTestFilter=None, Description=None, DisabledGroupFilter=None, DisabledUserFilter=None, DistinguishedName=None, GroupMemberOfAttribute=None, GroupMembershipAttribute=None, GroupObjectFilter=None, GroupProfileIdTemplate=None, GroupSearchFilter=None, GroupSyncFilter=None, Id=None, Parent=None, ParentAkas=None, Password=None, ProfileIdTemplate=None, RejectUnauthorized=None, Status=None, Tags=None, Title=None, TlsEnabled=None, TlsServerCertificate=None, Url=None, UserCanonicalNameAttribute=None, UserDisplayNameAttribute=None, UserEmailAttribute=None, UserFamilyNameAttribute=None, UserGivenNameAttribute=None, UserMatchFilter=None, UserObjectFilter=None, UserSearchAttributes=None, UserSearchFilter=None)
