...
TYPE_NAME = "TF::Turbot::PolicySetting"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/turbot_policy_setting/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'turbot_policy_setting',
                'returnValues': ["Id", "ResourceAkas", "ValueKeyFingerprint", "ValueSource", "ValueSourceKeyFingerprint", "ValueSourceUsed"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/turbot_policy_setting/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/turbot_policy_setting/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Id=None, Note=None, PgpKey=None, Precedence=None, Resource=None, ResourceAkas=None, Template=None, TemplateInput=None, Type=None, ValidFromTimestamp=None, ValidToTimestamp=None, Value=None, ValueKeyFingerprint=None, ValueSource=None, ValueSourceKeyFingerprint=None, ValueSourceUsed=None)
