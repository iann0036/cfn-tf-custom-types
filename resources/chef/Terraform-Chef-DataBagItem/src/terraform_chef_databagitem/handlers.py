...
def create_handler(
    ...
    try:
        ...
        tf['resource']['chef_data_bag_item'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("chef"))['SecretString']
            tf["provider"]["chef"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['chef_data_bag_item'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
