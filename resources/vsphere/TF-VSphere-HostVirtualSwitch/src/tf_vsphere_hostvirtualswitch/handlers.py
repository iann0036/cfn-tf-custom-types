...
TYPE_NAME = "TF::VSphere::HostVirtualSwitch"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vsphere_host_virtual_switch/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vsphere_host_virtual_switch/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vsphere_host_virtual_switch/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ActiveNics=None, AllowForgedTransmits=None, AllowMacChanges=None, AllowPromiscuous=None, BeaconInterval=None, CheckBeacon=None, Failback=None, HostSystemId=None, Id=None, LinkDiscoveryOperation=None, LinkDiscoveryProtocol=None, Mtu=None, Name=None, NetworkAdapters=None, NotifySwitches=None, NumberOfPorts=None, ShapingAverageBandwidth=None, ShapingBurstSize=None, ShapingEnabled=None, ShapingPeakBandwidth=None, StandbyNics=None, TeamingPolicy=None)
