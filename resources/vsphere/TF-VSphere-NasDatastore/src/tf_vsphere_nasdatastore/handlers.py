...
TYPE_NAME = "TF::VSphere::NasDatastore"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vsphere_nas_datastore/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vsphere_nas_datastore/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vsphere_nas_datastore/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccessMode=None, Accessible=None, Capacity=None, CustomAttributes=None, DatastoreClusterId=None, Folder=None, FreeSpace=None, HostSystemIds=None, Id=None, MaintenanceMode=None, MultipleHostAccess=None, Name=None, ProtocolEndpoint=None, RemoteHosts=None, RemotePath=None, SecurityType=None, Tags=None, Type=None, UncommittedSpace=None, Url=None)
