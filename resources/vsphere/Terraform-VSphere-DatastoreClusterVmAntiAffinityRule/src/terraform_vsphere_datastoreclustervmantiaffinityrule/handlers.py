...
def create_handler(
    ...
    try:
        ...
        tf['resource']['vsphere_datastore_cluster_vm_anti_affinity_rule'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("vsphere"))['SecretString']
            tf["provider"]["vsphere"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['vsphere_datastore_cluster_vm_anti_affinity_rule'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
