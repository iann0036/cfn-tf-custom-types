...
TYPE_NAME = "TF::VSphere::DatastoreCluster"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/vsphere_datastore_cluster/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/vsphere_datastore_cluster/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/vsphere_datastore_cluster/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CustomAttributes=None, DatacenterId=None, Folder=None, Id=None, Name=None, SdrsAdvancedOptions=None, SdrsAutomationLevel=None, SdrsDefaultIntraVmAffinity=None, SdrsEnabled=None, SdrsFreeSpaceThreshold=None, SdrsFreeSpaceThresholdMode=None, SdrsFreeSpaceUtilizationDifference=None, SdrsIoBalanceAutomationLevel=None, SdrsIoLatencyThreshold=None, SdrsIoLoadBalanceEnabled=None, SdrsIoLoadImbalanceThreshold=None, SdrsIoReservableIopsThreshold=None, SdrsIoReservablePercentThreshold=None, SdrsIoReservableThresholdMode=None, SdrsLoadBalanceInterval=None, SdrsPolicyEnforcementAutomationLevel=None, SdrsRuleEnforcementAutomationLevel=None, SdrsSpaceBalanceAutomationLevel=None, SdrsSpaceUtilizationThreshold=None, SdrsVmEvacuationAutomationLevel=None, Tags=None)
