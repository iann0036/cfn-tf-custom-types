...
TYPE_NAME = "TF::NetAppCloudManager::ConnectorAws"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_connector_aws/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'netapp-cloudmanager_connector_aws',
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_connector_aws/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/netapp-cloudmanager_connector_aws/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountId=None, Ami=None, AssociatePublicIpAddress=None, ClientId=None, Company=None, EnableTerminationProtection=None, IamInstanceProfileName=None, Id=None, InstanceType=None, KeyName=None, Name=None, ProxyCertificates=None, ProxyPassword=None, ProxyUrl=None, ProxyUserName=None, Region=None, SecurityGroupId=None, SubnetId=None, AwsTag=None)
