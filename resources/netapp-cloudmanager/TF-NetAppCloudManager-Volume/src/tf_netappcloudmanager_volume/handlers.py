...
TYPE_NAME = "TF::NetAppCloudManager::Volume"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_volume/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'netapp-cloudmanager_volume',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_volume/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/netapp-cloudmanager_volume/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AggregateName=None, CapacityTier=None, ClientId=None, EnableCompression=None, EnableDeduplication=None, EnableThinProvisioning=None, ExportPolicyIp=None, ExportPolicyName=None, ExportPolicyNfsVersion=None, ExportPolicyType=None, Id=None, Igroups=None, Iops=None, Name=None, OsName=None, Permission=None, ProviderVolumeType=None, ShareName=None, Size=None, SnapshotPolicyName=None, SvmName=None, Throughput=None, TieringPolicy=None, Unit=None, Users=None, VolumeProtocol=None, WorkingEnvironmentId=None, WorkingEnvironmentName=None, Initiator=None)
