...
TYPE_NAME = "TF::NetAppCloudManager::Snapmirror"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_snapmirror/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'netapp-cloudmanager_snapmirror',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_snapmirror/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/netapp-cloudmanager_snapmirror/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CapacityTier=None, ClientId=None, DestinationAggregateName=None, DestinationSvmName=None, DestinationVolumeName=None, DestinationWorkingEnvironmentId=None, DestinationWorkingEnvironmentName=None, Id=None, Iops=None, MaxTransferRate=None, Policy=None, ProviderVolumeType=None, Schedule=None, SourceSvmName=None, SourceVolumeName=None, SourceWorkingEnvironmentId=None, SourceWorkingEnvironmentName=None, Throughput=None)
