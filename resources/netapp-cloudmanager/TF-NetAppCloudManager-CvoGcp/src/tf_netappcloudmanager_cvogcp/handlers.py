...
TYPE_NAME = "TF::NetAppCloudManager::CvoGcp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_cvo_gcp/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'NetApp/netapp-cloudmanager',
                'providerTypeName': 'netapp-cloudmanager',
                'terraformTypeName': 'netapp-cloudmanager_cvo_gcp',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_cvo_gcp/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/netapp-cloudmanager_cvo_gcp/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CapacityTier=None, ClientId=None, DataEncryptionType=None, FirewallRule=None, GcpEncryptionParameters=None, GcpServiceAccount=None, GcpVolumeSize=None, GcpVolumeSizeUnit=None, GcpVolumeType=None, Id=None, InstanceType=None, IsHa=None, LicenseType=None, MediatorZone=None, Name=None, NetworkProjectId=None, Node1Zone=None, Node2Zone=None, NssAccount=None, OntapVersion=None, PlatformSerialNumberNode1=None, PlatformSerialNumberNode2=None, ProjectId=None, SerialNumber=None, Subnet0NodeAndDataConnectivity=None, Subnet1ClusterConnectivity=None, Subnet2HaConnectivity=None, Subnet3DataReplication=None, SubnetId=None, SvmPassword=None, TierLevel=None, UseLatestVersion=None, Vpc0FirewallRuleName=None, Vpc0NodeAndDataConnectivity=None, Vpc1ClusterConnectivity=None, Vpc1FirewallRuleName=None, Vpc2FirewallRuleName=None, Vpc2HaConnectivity=None, Vpc3DataReplication=None, Vpc3FirewallRuleName=None, VpcId=None, WorkspaceId=None, WritingSpeedState=None, Zone=None, GcpLabel=None)
