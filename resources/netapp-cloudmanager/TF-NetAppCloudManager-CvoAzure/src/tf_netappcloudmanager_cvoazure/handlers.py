...
TYPE_NAME = "TF::NetAppCloudManager::CvoAzure"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_cvo_azure/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'netapp-cloudmanager_cvo_azure',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_cvo_azure/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/netapp-cloudmanager_cvo_azure/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AllowDeployInExistingRg=None, BackupVolumesToCbs=None, CapacityTier=None, Cidr=None, ClientId=None, CloudProviderAccount=None, DataEncryptionType=None, DiskSize=None, DiskSizeUnit=None, EnableCompliance=None, EnableMonitoring=None, Id=None, InstanceType=None, IsHa=None, LicenseType=None, Location=None, Name=None, NssAccount=None, OntapVersion=None, PlatformSerialNumberNode1=None, PlatformSerialNumberNode2=None, ResourceGroup=None, SecurityGroupId=None, SerialNumber=None, StorageType=None, SubnetId=None, SubscriptionId=None, SvmPassword=None, TierLevel=None, UseLatestVersion=None, VnetId=None, VnetResourceGroup=None, WorkspaceId=None, WritingSpeedState=None, AzureTag=None)
