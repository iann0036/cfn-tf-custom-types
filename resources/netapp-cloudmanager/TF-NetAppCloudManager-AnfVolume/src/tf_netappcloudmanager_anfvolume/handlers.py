...
TYPE_NAME = "TF::NetAppCloudManager::AnfVolume"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_anf_volume/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'netapp-cloudmanager_anf_volume',
                'returnValues': ["Id"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/netapp-cloudmanager_anf_volume/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/netapp-cloudmanager_anf_volume/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), Account=None, CapacityPool=None, ClientId=None, Id=None, Location=None, Name=None, NetappAccount=None, ProtocolTypes=None, ResourceGroups=None, ServiceLevel=None, Size=None, SizeUnit=None, Subnet=None, Subscription=None, VirtualNetwork=None, VolumePath=None, WorkingEnvironmentName=None, ExportPolicy=None)
