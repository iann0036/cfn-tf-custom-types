...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/logzio_alert_v2/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'logzio/logzio',
                'providerTypeName': 'logzio',
                'terraformTypeName': 'logzio_alert_v2',
                'returnValues': ["CreatedAt", "CreatedBy", "Id", "UpdatedAt", "UpdatedBy"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/logzio_alert_v2/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/logzio_alert_v2/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AlertNotificationEndpoints=None, CorrelationOperator=None, CreatedAt=None, CreatedBy=None, Description=None, Id=None, IsEnabled=None, Joins=None, NotificationEmails=None, OutputType=None, SearchTimeframeMinutes=None, SuppressNotificationsMinutes=None, Tags=None, Title=None, UpdatedAt=None, UpdatedBy=None, SubComponents=None)
