...
def create_handler(
    ...
    try:
        ...
        tf['resource']['nomad_quota_specification'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("nomad"))['SecretString']
            tf["provider"]["nomad"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['nomad_quota_specification'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
