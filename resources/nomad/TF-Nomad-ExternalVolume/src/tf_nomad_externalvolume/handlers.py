...
TYPE_NAME = "TF::Nomad::ExternalVolume"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/nomad_external_volume/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'hashicorp/nomad',
                'providerTypeName': 'nomad',
                'terraformTypeName': 'nomad_external_volume',
                'returnValues': ["ControllerRequired", "ControllersExpected", "ControllersHealthy", "Id", "NodesExpected", "NodesHealthy", "PluginProvider", "PluginProviderVersion", "Schedulable"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/nomad_external_volume/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/nomad_external_volume/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CapacityMax=None, CapacityMin=None, CloneId=None, ControllerRequired=None, ControllersExpected=None, ControllersHealthy=None, Id=None, Name=None, Namespace=None, NodesExpected=None, NodesHealthy=None, Parameters=None, PluginId=None, PluginProvider=None, PluginProviderVersion=None, Schedulable=None, Secrets=None, SnapshotId=None, Type=None, VolumeId=None, Capability=None, MountOptions=None)
