...
def create_handler(
    ...
    try:
        ...
        tf['resource']['rundeck_private_key'][request.logicalResourceIdentifier] = {}
        ...
        try:
            secretstr = secretsmanagerclient.get_secret_value(SecretId="terraform/{}".format("rundeck"))['SecretString']
            tf["provider"]["rundeck"].update(json.loads(secretstr))
        except:
            pass

        for prop, value in vars(model).items():
            if value is not None and prop != "tfcfnid":
                tf['resource']['rundeck_private_key'][request.logicalResourceIdentifier][cfn_to_tf_str(prop)] = parse_obj(value)
