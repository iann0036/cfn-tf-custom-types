...
TYPE_NAME = "TF::Dome9::OrganizationalUnit"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/dome9_organizational_unit/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'dome9/dome9',
                'providerTypeName': 'dome9',
                'terraformTypeName': 'dome9_organizational_unit',
                'returnValues': ["AccountId", "AwsAggregateCloudAccountsCount", "AwsCloudAccountsCount", "AzureAggregateCloudAccountsCount", "AzureCloudAccountsCount", "Created", "GoogleAggregateCloudAccountsCount", "GoogleCloudAccountsCount", "Id", "IsParentRoot", "IsRoot", "Path", "PathStr", "SubOrganizationalUnitsCount", "Updated"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/dome9_organizational_unit/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/dome9_organizational_unit/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AccountId=None, AwsAggregateCloudAccountsCount=None, AwsCloudAccountsCount=None, AzureAggregateCloudAccountsCount=None, AzureCloudAccountsCount=None, Created=None, GoogleAggregateCloudAccountsCount=None, GoogleCloudAccountsCount=None, Id=None, IsParentRoot=None, IsRoot=None, Name=None, ParentId=None, Path=None, PathStr=None, SubOrganizationalUnitsCount=None, Updated=None)
