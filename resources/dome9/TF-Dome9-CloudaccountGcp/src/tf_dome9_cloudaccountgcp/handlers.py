...
TYPE_NAME = "TF::Dome9::CloudaccountGcp"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/dome9_cloudaccount_gcp/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'dome9_cloudaccount_gcp',
                'returnValues': ["Id", "OrganizationalUnitName", "OrganizationalUnitPath", "Vendor"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/dome9_cloudaccount_gcp/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/dome9_cloudaccount_gcp/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), ClientEmail=None, ClientId=None, ClientX509CertUrl=None, DomainName=None, GsuiteUser=None, Id=None, Name=None, OrganizationalUnitId=None, OrganizationalUnitName=None, OrganizationalUnitPath=None, PrivateKey=None, PrivateKeyId=None, ProjectId=None, Vendor=None)
