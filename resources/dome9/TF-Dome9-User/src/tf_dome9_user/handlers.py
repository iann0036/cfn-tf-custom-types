...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/dome9_user/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'terraformTypeName': 'dome9_user',
                'returnValues': ["CanSwitchRole", "HasApiKey", "HasApiKeyV1", "HasApiKeyV2", "IamSafe", "Id", "IsAuditor", "IsLocked", "IsMfaEnabled", "IsMobileDevicePaired", "IsSuperUser", "IsSuspended", "LastLogin"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/dome9_user/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/dome9_user/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), CanSwitchRole=None, Create=None, CrossAccountAccess=None, Email=None, FirstName=None, HasApiKey=None, HasApiKeyV1=None, HasApiKeyV2=None, IamSafe=None, Id=None, IsAuditor=None, IsLocked=None, IsMfaEnabled=None, IsMobileDevicePaired=None, IsOwner=None, IsSsoEnabled=None, IsSuperUser=None, IsSuspended=None, LastLogin=None, LastName=None, PermitAlertActions=None, PermitNotifications=None, PermitOnBoarding=None, PermitPolicies=None, PermitRulesets=None, RoleIds=None, Access=None, Manage=None, View=None)
