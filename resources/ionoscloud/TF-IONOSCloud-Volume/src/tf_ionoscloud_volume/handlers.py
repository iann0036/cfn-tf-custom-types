...
TYPE_NAME = "TF::IONOSCloud::Volume"
...
def check_progress(operationid, trackingid, progress, session):
    ...
    try:
        ...
        if result['status'] == 'completed':
            ...
            # retrieve model
            try:
                state_str = s3client.get_object(Bucket=statebucketname, Key="state/ionoscloud_volume/{}.model.json".format(trackingid))['Body'].read()
                if state_str == "":
                    state_str = "{}"
    ...
    try:
        ...
        lambdaclient.invoke(
            ...
            Payload=json.dumps({
                ...
                'providerFullName': 'ionos-cloud/ionoscloud',
                'providerTypeName': 'ionoscloud',
                'terraformTypeName': 'ionoscloud_volume',
                'returnValues': ["Id", "Sshkey"],
            }).encode(),
        )
...
def update_handler(
    ...
    try:
        state_str = s3client.get_object(Bucket=statebucketname, Key="state/ionoscloud_volume/{}.model.json".format(model.tfcfnid))['Body'].read()
        if state_str == "":
            state_str = "{}"
        model_state = json.loads(state_str)
...
def list_handler(
    ...
    try:
        ...
        state_objects = s3client.list_objects_v2(
            ...
            MaxKeys=1000,
            Prefix='state/ionoscloud_volume/'
        )['Contents']

        for state_object in state_objects:
            if state_object['Key'].endswith(".model.json"):
                model = ResourceModel(tfcfnid=state_object['Key'].replace(".model.json", ""), AvailabilityZone=None, BackupUnitId=None, Bus=None, CpuHotPlug=None, DatacenterId=None, DiscVirtioHotPlug=None, DiscVirtioHotUnplug=None, DiskType=None, Id=None, ImageName=None, ImagePassword=None, LicenceType=None, Name=None, NicHotPlug=None, NicHotUnplug=None, RamHotPlug=None, ServerId=None, Size=None, SshKeyPath=None, Sshkey=None, UserData=None, Timeouts=None)
